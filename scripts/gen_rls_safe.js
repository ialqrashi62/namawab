#!/usr/bin/env node
// Wave 3B: Generate e55 RLS migration - safer pattern
'use strict';
const fs = require('fs');

const HAS_TENANT = [
    'appointments', 'patients', 'invoices', 'insurance_claims', 'insurance_authorizations',
    'lab_orders', 'lab_results', 'pharmacy_prescriptions', 'radiology_studies',
    'vital_signs', 'nursing_assessments', 'preop_assessments', 'discharge_summaries',
    ...
    'allergy_assessments', 'anesthesia_assessments', 'audiology_assessments',
    'burn_unit_assessments', 'cardiac_rehab_assessments', 'ccu_assessments',
    'chaplaincy_assessments', 'cicu_assessments', 'ctu_assessments',
    'dermatology_assessments', 'dialysis_assessments', 'epilepsy_assessments',
    'fetal_medicine_assessments', 'genetics_assessments', 'headache_assessments',
    'hematology_assessments', 'icu_assessments', 'immunology_assessments',
    'infection_control_assessments', 'infectious_disease_assessments',
    'ivf_assessments', 'maternal_fetal_assessments', 'memory_clinic_assessments',
    'movement_assessments', 'movement_disorders_assessments', 'multiple_sclerosis_assessments',
    'neonatology_assessments', 'neuro_oncology_assessments', 'neurosurgery_assessments',
    'nicu_assessments', 'nuclear_medicine_assessments', 'nutrition_assessments',
    'occupational_therapy_assessments', 'pain_management_assessments',
    'palliative_care_assessments', 'pathology_assessments', 'physiotherapy_assessments',
    'picu_assessments', 'plastic_surgery_assessments', 'psychiatry_assessments',
    'pulmonary_rehab_assessments', 'radiology_assessments', 'rehabilitation_assessments',
    'sleep_medicine_assessments', 'social_work_assessments', 'speech_therapy_assessments',
    'stroke_unit_assessments', 'thoracic_surgery_assessments', 'transplant_assessments',
    'trauma_surgery_assessments', 'urology_assessments', 'vascular_surgery_assessments',
...
BEGIN
    ...
    LOOP
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = tbl AND table_schema = 'public') THEN
            ...
            EXCEPTION WHEN insufficient_privilege THEN
                RAISE NOTICE 'NO FORCE RLS failed for %', tbl;
            END;
            BEGIN
                EXECUTE format('DROP POLICY IF EXISTS %I ON public.%I', tbl || '_tenant_isolation', tbl);
            EXCEPTION WHEN OTHERS THEN
