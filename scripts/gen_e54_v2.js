// filepath: scripts/gen_e54_v2.js
// Generate e54 migration: only CREATE TABLE IF NOT EXISTS for missing tables
// Skip RLS+triggers for pre-existing tables (we add them in a separate "patch_e54_rls.sql")
'use strict';
const fs = require('fs');
...
console.log(`Generating v2 migration for ${tables.length} dept tables...`);

let up = `-- e54_53_depts_v2_up.sql
-- 53 dept tables (idempotent + defensive)
-- RLS + triggers only added for tables created in this migration
-- For pre-existing tables, see e54_01_backfill_rls.sql
...
`;
let down = `-- e54_53_depts_v2_down.sql
-- Drop all 53 dept tables (created via this migration)
-- Pre-existing tables will NOT be dropped; this is intentional
BEGIN;
...
for (const [dept, { table, cols }] of tables) {
    ...
    up += `-- ${dept}\nDO $$\nBEGIN\n    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='${table}') THEN
        CREATE TABLE ${table} (
...
fs.writeFileSync(path.join(OUT_DIR, 'e54_53_depts_v2_up.sql'), up);
fs.writeFileSync(path.join(OUT_DIR, 'e54_53_depts_v2_down.sql'), down);

console.log(`OK ${tables.length} dept tables, idempotent`);
