// filepath: tier5_derm_ext_103_acne_router.js
'use strict';
const express = require('express');
const router = express.Router();
const engine = require('./tier5_derm_ext_103_acne_engine.js');
const asyncH = (fn) => (req, res, next) => Promise.resolve(fn(req, res, next)).catch(next);
router.post('/cls', asyncH(async (req, res) => res.json(engine.funcs().classify(req.body))));
router.post('/top', asyncH(async (req, res) => res.json(engine.funcs().topicals(req.body))));
router.post('/sys', asyncH(async (req, res) => res.json(engine.funcs().systemic(req.body))));
router.post('/scar', asyncH(async (req, res) => res.json(engine.funcs().scarring(req.body))));
router.post('/ros', asyncH(async (req, res) => res.json(engine.funcs().rosacea(req.body))));
router.post('/mon', asyncH(async (req, res) => res.json(engine.funcs().monitoring(req.body))));
module.exports = router;