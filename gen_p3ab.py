"""Generate SQL up + routes + integration for P3-AB modules"""
import os
...
extras = {
    'sleep': "router.post('/admissions/:id/sleep-study', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
    'bariatric': "router.post('/admissions/:id/bariatric-plan', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
    'geriatrics': "router.post('/admissions/:id/cga-assessment', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
}

for m in ['sleep', 'bariatric', 'geriatrics']:
