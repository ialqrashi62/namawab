...
P2_T0=$(date +%s)
SMK_RC=0
bash "${OPS_DIR}/smoke_test.sh" >"$SMK_LOG" 2>&1 || SMK_RC=$?
P2_DUR=$(( $(date +%s) - P2_T0 ))
...
if [[ "$RUN_BENCHMARK" -eq 1 ]]; then
    ...
    if [[ "$BM_RC" -eq 0 ]]; then
        ...
        P5_RESULT="WARN"
        TOTAL_WARN=$((TOTAL_WARN + 1))
        printf "${YELLOW}WARN${NC} benchmark rc=%s — see log\n" "$BM_RC"
        P5_TESTS="see log"
    fi
    append_summary "5" "PCC benchmark" "$P5_RESULT" "$P5_TESTS" "$P5_DUR"
    rm -f "$BM_LOG"
else
    header "$BLUE" "PHASE 5 — PCC benchmark (skipped)"
    printf "${DIM}  use --with-benchmark to enable (default: skipped)${NC}\n"
    append_summary "5" "PCC benchmark" "SKIP" "no --with-benchmark flag" "-"
fi
...
# =============================================================================
OVERALL_T1=$(date +%s)
OVERALL_DUR=$(( OVERALL_T1 - OVERALL_T0 ))

printf "\n${BOLD}╔══════════════════════════════════════════════════════════════╗${NC}\n"
printf "${BOLD}║                       Summary Table                          ║${NC}\n"
printf "${BOLD}╠══════════════════════════════════════════════════════════════╣${NC}\n"
printf "${BOLD}║  %-26s %-12s %-8s  %-18s${NC}\n" \
    "PHASE" "RESULT" "DURATION" "TESTS"
printf "${BOLD}╠══════════════════════════════════════════════════════════════╣${NC}\n"

for row in "${SUMMARY_ROWS[@]}"; do
    IFS='|' read -r pid label result tests dur <<< "$row"
    # Color-code the result column
    case "$result" in
        OK)        rc_color="${GREEN}" ;;
        WARN|PASS-*-FAIL) rc_color="${YELLOW}" ;;
        FAIL)      rc_color="${RED}" ;;
        SKIP)      rc_color="${DIM}" ;;
        *)         rc_color="${NC}" ;;
    esac
    printf "║  %-26s ${rc_color}%-12s${NC} %-8s  %-18s\n" \
        "$label" "$result" "${dur}s" "$tests"
done

# OVERALL row
OVERALL_RESULT="OK"
if [[ $TOTAL_FAIL -gt 0 ]]; then OVERALL_RESULT="FAIL"
elif [[ $TOTAL_WARN -gt 0 ]]; then OVERALL_RESULT="WARN"
fi

printf "${BOLD}╠══════════════════════════════════════════════════════════════╣${NC}\n"
case "$OVERALL_RESULT" in
    OK)   rc_color="$GREEN" ;;
    WARN) rc_color="$YELLOW" ;;
    FAIL) rc_color="$RED" ;;
esac
OVERALL_TXT=$(printf "${rc_color}%-12s${NC} %-8s  pass=%s warn=%s fail=%s" \
    "$OVERALL_RESULT" "${OVERALL_DUR}s" "$TOTAL_PASS" "$TOTAL_WARN" "$TOTAL_FAIL")
printf "${BOLD}║  %-26s ${OVERALL_TXT}${NC}\n" "OVERALL"
printf "${BOLD}╚══════════════════════════════════════════════════════════════╝${NC}\n"

printf "\n${DIM}Full transcript: %s${NC}\n" "$LOG_FILE"
