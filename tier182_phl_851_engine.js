// filepath: tier182_phl_851_engine.js
...
function phl_lab_assessment(req) {
...
function phl_lab_screening(req) {
  ensureStr(req.tenant_id, 'tid'); ensureStr(req.patient_id, 'pid');
  ensureNum(req.age, 'ag'); ensureEnum(req.type, 'ty', ['type1','type2','type3','other','NA']);
  ensureNum(req.score_1, 's1'); ensureNum(req.score_2, 's2');
  ensureBool(req.flag_1, 'f1'); ensureBool(req.flag_2, 'f2');
...
function phl_lab_procedure(req) {
  ensureStr(req.tenant_id, 'tid'); ensureStr(req.patient_id, 'pid');
  ensureNum(req.age, 'ag'); ensureEnum(req.type, 'ty', ['type1','type2','type3','other','NA']);
  ensureNum(req.score_1, 's1'); ensureNum(req.score_2, 's2');
  ensureBool(req.flag_1, 'f1'); ensureBool(req.flag_2, 'f2');
...
function funcs() { return { phl_lab_assessment, phl_lab_screening, phl_lab_followup, phl_lab_procedure, phl_lab_outcome }; }
