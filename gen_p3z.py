"""Generate SQL up + routes + integration for P3-Z modules"""
import os

base = r'c:\Users\ice\Desktop\NMEDCALVSCODE\pcc'

def make_sql(m, extra_cols=''):
    cols = f'diagnosis TEXT, status TEXT NOT NULL DEFAULT \'active\', cpt_codes TEXT NOT NULL DEFAULT \'[]\',{extra_cols}'
    return f'''
CREATE TABLE IF NOT EXISTS {m}_admission (
  id TEXT PRIMARY KEY, tenant_id TEXT NOT NULL, patient_id INTEGER NOT NULL,
  encounter_id INTEGER NOT NULL, {cols}
...
extras = {
    'ent': "router.post('/admissions/:id/scope', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
    'ophthalmology': "router.post('/admissions/:id/fundoscopy', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
    'urology': "router.post('/admissions/:id/cystoscopy', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
}

for m in ['ent', 'ophthalmology', 'urology']:
