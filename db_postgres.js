...
// ===== TENANT CONTEXT WIRING FOR ROW LEVEL SECURITY =====
// آلية ربط app.tenant_id لكل طلب باستخدام AsyncLocalStorage.
// السبب: سياسات FORCE RLS تعتمد على current_setting('app.tenant_id')، والتطبيق يستخدم
// pool.query مباشرة دون ضبط السياق، ما كان يحجب كل صفوف الجداول المحمية عن مستخدم التطبيق.
// الحل: عند وجود سياق مستأجر للطلب، نحجز اتصالاً واحداً، نضبط app.tenant_id عليه، ننفّذ
// الاستعلام على نفس الاتصال، ثم نعيد ضبط السياق ونحرّر الاتصال (منع تسرّب السياق في الـ pool).
...
pool.query = function (text, params) {
    ...
    // بدون سياق مستأجر (مثل تسجيل الدخول/الصحة/التهيئة) → السلوك الأصلي بلا تغيير.
...
-- P0 modern-modules tenant isolation (medical records, clinical pharmacy, rehab, portal, dietary/nutrition)
ALTER TABLE medical_records_files ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE medical_records_files ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE medical_records_requests ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE medical_records_requests ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE medical_records_coding ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE medical_records_coding ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE clinical_pharmacy_reviews ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE clinical_pharmacy_reviews ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE patient_drug_education ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE patient_drug_education ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE rehab_patients ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE rehab_patients ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE rehab_sessions ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE rehab_sessions ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE rehab_goals ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE rehab_goals ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE rehab_assessments ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE rehab_assessments ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE portal_users ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE portal_users ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE diet_orders ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE diet_orders ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE diet_meals ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE diet_meals ADD COLUMN IF NOT EXISTS facility_id INTEGER;
ALTER TABLE nutrition_assessments ADD COLUMN IF NOT EXISTS tenant_id INTEGER;
ALTER TABLE nutrition_assessments ADD COLUMN IF NOT EXISTS facility_id INTEGER;
CREATE INDEX IF NOT EXISTS idx_medical_records_files_tenant ON medical_records_files (tenant_id);
CREATE INDEX IF NOT EXISTS idx_medical_records_requests_tenant ON medical_records_requests (tenant_id);
CREATE INDEX IF NOT EXISTS idx_medical_records_coding_tenant ON medical_records_coding (tenant_id);
CREATE INDEX IF NOT EXISTS idx_clinical_pharmacy_reviews_tenant ON clinical_pharmacy_reviews (tenant_id);
CREATE INDEX IF NOT EXISTS idx_patient_drug_education_tenant ON patient_drug_education (tenant_id);
CREATE INDEX IF NOT EXISTS idx_rehab_patients_tenant ON rehab_patients (tenant_id);
CREATE INDEX IF NOT EXISTS idx_rehab_sessions_tenant ON rehab_sessions (tenant_id);
CREATE INDEX IF NOT EXISTS idx_rehab_goals_tenant ON rehab_goals (tenant_id);
CREATE INDEX IF NOT EXISTS idx_rehab_assessments_tenant ON rehab_assessments (tenant_id);
CREATE INDEX IF NOT EXISTS idx_portal_users_tenant ON portal_users (tenant_id);
CREATE INDEX IF NOT EXISTS idx_diet_orders_tenant ON diet_orders (tenant_id);
CREATE INDEX IF NOT EXISTS idx_diet_meals_tenant ON diet_meals (tenant_id);
CREATE INDEX IF NOT EXISTS idx_nutrition_assessments_tenant ON nutrition_assessments (tenant_id);
            `);
...
UPDATE medical_records_files SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE medical_records_requests SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE medical_records_coding SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE clinical_pharmacy_reviews SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE patient_drug_education SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE rehab_patients SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE rehab_sessions SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE rehab_goals SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE rehab_assessments SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE portal_users SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE diet_orders SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE diet_meals SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
UPDATE nutrition_assessments SET tenant_id = 1, facility_id = 1 WHERE tenant_id IS NULL;
