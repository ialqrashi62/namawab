...
// AUTO-GEN Tier-3 — geriatric_psych/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class GeriatricPsychAssessment extends Engine {
  constructor() {
    super({
      dept: 'geriatric_psych',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'geriatric_psych');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: geriatric_psych (Tier-3)',
...
module.exports = GeriatricPsychAssessment;
