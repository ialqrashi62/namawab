...
// AUTO-GEN Tier-4 — sport_med/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class SportMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'sport_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'sport_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: sport_med (Tier-4)',
...
module.exports = SportMedAssessment;
