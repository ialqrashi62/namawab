...
// AUTO-GEN Tier-4 — ent_peds/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class EntPedsAssessment extends Engine {
  constructor() {
    super({
      dept: 'ent_peds',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'ent_peds');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: ent_peds (Tier-4)',
...
module.exports = EntPedsAssessment;
