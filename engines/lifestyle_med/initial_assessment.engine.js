...
// AUTO-GEN Tier-3 — lifestyle_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class LifestyleMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'lifestyle_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'lifestyle_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: lifestyle_med (Tier-3)',
...
module.exports = LifestyleMedAssessment;
