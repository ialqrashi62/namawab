...
// AUTO-GEN Tier-4 — ivf_advanced/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class IvfAdvancedAssessment extends Engine {
  constructor() {
    super({
      dept: 'ivf_advanced',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'ivf_advanced');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: ivf_advanced (Tier-4)',
...
module.exports = IvfAdvancedAssessment;
