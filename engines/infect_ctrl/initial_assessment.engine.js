...
// AUTO-GEN T2 — infect_ctrl/initial_assessment.engine.js
...
class InfectCtrlAssessment extends Engine {
  constructor() {
    super({
      dept: 'infect_ctrl',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'infect_ctrl');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: infect_ctrl (Tier-2)',
...
module.exports = InfectCtrlAssessment;
