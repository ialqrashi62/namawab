...
// AUTO-GEN T2 — elective_surgery/initial_assessment.engine.js
...
class ElectiveSurgeryAssessment extends Engine {
  constructor() {
    super({
      dept: 'elective_surgery',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'elective_surgery');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: elective_surgery (Tier-2)',
...
module.exports = ElectiveSurgeryAssessment;
