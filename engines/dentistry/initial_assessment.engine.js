...
// AUTO-GEN Tier-4 — dentistry/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class DentistryAssessment extends Engine {
  constructor() {
    super({
      dept: 'dentistry',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'dentistry');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: dentistry (Tier-4)',
...
module.exports = DentistryAssessment;
