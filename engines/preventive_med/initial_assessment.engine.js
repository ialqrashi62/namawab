...
// AUTO-GEN Tier-3 — preventive_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class PreventiveMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'preventive_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'preventive_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: preventive_med (Tier-3)',
...
module.exports = PreventiveMedAssessment;
