...
// AUTO-GEN Tier-4 — hand_therapy/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class HandTherapyAssessment extends Engine {
  constructor() {
    super({
      dept: 'hand_therapy',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'hand_therapy');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: hand_therapy (Tier-4)',
...
module.exports = HandTherapyAssessment;
