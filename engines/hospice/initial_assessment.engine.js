...
// AUTO-GEN Tier-3 — hospice/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class HospiceAssessment extends Engine {
  constructor() {
    super({
      dept: 'hospice',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'hospice');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: hospice (Tier-3)',
...
module.exports = HospiceAssessment;
