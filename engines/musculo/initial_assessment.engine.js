...
// AUTO-GEN Tier-3 — musculo/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class MusculoAssessment extends Engine {
  constructor() {
    super({
      dept: 'musculo',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'musculo');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: musculo (Tier-3)',
...
module.exports = MusculoAssessment;
