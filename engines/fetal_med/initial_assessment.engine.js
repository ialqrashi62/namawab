...
// AUTO-GEN T2 — fetal_med/initial_assessment.engine.js
...
class FetalMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'fetal_med',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'fetal_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: fetal_med (Tier-2)',
...
module.exports = FetalMedAssessment;
