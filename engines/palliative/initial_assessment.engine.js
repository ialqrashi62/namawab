...
// AUTO-GEN T2 — palliative/initial_assessment.engine.js
...
class PalliativeAssessment extends Engine {
  constructor() {
    super({
      dept: 'palliative',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'palliative');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: palliative (Tier-2)',
