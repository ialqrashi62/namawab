...
// AUTO-GEN T6 procedural — pulm/workflows/asthma_severe/bundle.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class AsthmaSevereBundle extends Engine {
  constructor() {
    super({
      dept: 'pulm/asthma_severe',
      parentDept: 'pulm',
      workflow: 'asthma_severe',
      procedureLabel: 'Severe Asthma GINA Step 4-5',
      scope: 'Pulm',
      redFlags: [],
      drugBlocks: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || 'asthma_severe');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nProcedure bundle: Severe Asthma GINA Step 4-5',
      '\nScope: Pulm',
...
module.exports = AsthmaSevereBundle;
