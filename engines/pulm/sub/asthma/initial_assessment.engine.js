...
// AUTO-GEN T5 sub-unit — pulm/asthma/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class AsthmaAssessment extends Engine {
  constructor() {
    super({
      dept: 'pulm/asthma',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'asthma');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Severe Asthma under pulm',
...
module.exports = AsthmaAssessment;
