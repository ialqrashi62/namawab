...
// AUTO-GEN T5 sub-unit — pulm/cf/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class CfAssessment extends Engine {
  constructor() {
    super({
      dept: 'pulm/cf',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'cf');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Cystic Fibrosis under pulm',
