...
// AUTO-GEN T5 sub-unit — pulm/pulmonary_htn/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class PulmonaryHtnAssessment extends Engine {
  constructor() {
    super({
      dept: 'pulm/pulmonary_htn',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'pulmonary_htn');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Pulmonary Hypertension under pulm',
...
module.exports = PulmonaryHtnAssessment;
