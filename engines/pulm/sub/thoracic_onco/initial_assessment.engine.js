...
// AUTO-GEN T5 sub-unit — pulm/thoracic_onco/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class ThoracicOncoAssessment extends Engine {
  constructor() {
    super({
      dept: 'pulm/thoracic_onco',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'thoracic_onco');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Thoracic Oncology under pulm',
...
module.exports = ThoracicOncoAssessment;
