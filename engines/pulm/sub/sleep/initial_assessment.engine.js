...
// AUTO-GEN T5 sub-unit — pulm/sleep/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class SleepAssessment extends Engine {
  constructor() {
    super({
      dept: 'pulm/sleep',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'sleep');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Sleep Medicine under pulm',
...
module.exports = SleepAssessment;
