...
// AUTO-GEN Tier-3 — biomedical/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class BiomedicalAssessment extends Engine {
  constructor() {
    super({
      dept: 'biomedical',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'biomedical');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: biomedical (Tier-3)',
...
module.exports = BiomedicalAssessment;
