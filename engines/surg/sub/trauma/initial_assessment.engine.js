...
// AUTO-GEN T5 sub-unit — surg/trauma/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class TraumaAssessment extends Engine {
  constructor() {
    super({
      dept: 'surg/trauma',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'trauma');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Trauma Surgery under surg',
