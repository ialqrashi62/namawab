...
// AUTO-GEN T5 sub-unit — surg/bariatric/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class BariatricAssessment extends Engine {
  constructor() {
    super({
      dept: 'surg/bariatric',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'bariatric');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Bariatric Surgery under surg',
...
module.exports = BariatricAssessment;
