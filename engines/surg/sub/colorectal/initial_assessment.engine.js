...
// AUTO-GEN T5 sub-unit — surg/colorectal/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class ColorectalAssessment extends Engine {
  constructor() {
    super({
      dept: 'surg/colorectal',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'colorectal');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Colorectal Surgery under surg',
...
module.exports = ColorectalAssessment;
