...
// AUTO-GEN T5 sub-unit — surg/transplant/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class TransplantAssessment extends Engine {
  constructor() {
    super({
      dept: 'surg/transplant',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'transplant');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Transplant Surgery under surg',
...
module.exports = TransplantAssessment;
