...
// AUTO-GEN Tier-4 — burn_outpatient/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class BurnOutpatientAssessment extends Engine {
  constructor() {
    super({
      dept: 'burn_outpatient',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'burn_outpatient');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: burn_outpatient (Tier-4)',
...
module.exports = BurnOutpatientAssessment;
