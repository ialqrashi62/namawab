...
// AUTO-GEN T5 sub-unit — peds/surgery/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class SurgeryAssessment extends Engine {
  constructor() {
    super({
      dept: 'peds/surgery',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'surgery');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Pediatric Surgery under peds',
...
module.exports = SurgeryAssessment;
