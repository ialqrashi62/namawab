...
// AUTO-GEN T5 sub-unit — peds/cardiology/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class CardiologyAssessment extends Engine {
  constructor() {
    super({
      dept: 'peds/cardiology',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'cardiology');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Pediatric Cardiology under peds',
...
module.exports = CardiologyAssessment;
