...
// AUTO-GEN T5 sub-unit — peds/pulmonology/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class PulmonologyAssessment extends Engine {
  constructor() {
    super({
      dept: 'peds/pulmonology',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'pulmonology');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Pediatric Pulmonology under peds',
...
module.exports = PulmonologyAssessment;
