...
// AUTO-GEN Tier-3 — cardiac_rehab/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class CardiacRehabAssessment extends Engine {
  constructor() {
    super({
      dept: 'cardiac_rehab',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'cardiac_rehab');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: cardiac_rehab (Tier-3)',
...
module.exports = CardiacRehabAssessment;
