...
// AUTO-GEN Tier-3 — interv_pulm/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class IntervPulmAssessment extends Engine {
  constructor() {
    super({
      dept: 'interv_pulm',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'interv_pulm');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: interv_pulm (Tier-3)',
...
module.exports = IntervPulmAssessment;
