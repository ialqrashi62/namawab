...
// AUTO-GEN T2 — perinatal/initial_assessment.engine.js
...
class PerinatalAssessment extends Engine {
  constructor() {
    super({
      dept: 'perinatal',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'perinatal');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: perinatal (Tier-2)',
...
module.exports = PerinatalAssessment;
