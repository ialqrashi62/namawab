...
// AUTO-GEN Tier-3 — pulm_rehab/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class PulmRehabAssessment extends Engine {
  constructor() {
    super({
      dept: 'pulm_rehab',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'pulm_rehab');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: pulm_rehab (Tier-3)',
...
module.exports = PulmRehabAssessment;
