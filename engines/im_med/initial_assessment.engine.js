...
// AUTO-GEN Tier-3 — im_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class ImMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'im_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'im_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: im_med (Tier-3)',
...
module.exports = ImMedAssessment;
