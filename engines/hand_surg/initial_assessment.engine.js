...
// AUTO-GEN T2 — hand_surg/initial_assessment.engine.js
...
class HandSurgAssessment extends Engine {
  constructor() {
    super({
      dept: 'hand_surg',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'hand_surg');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: hand_surg (Tier-2)',
...
module.exports = HandSurgAssessment;
