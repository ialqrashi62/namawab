...
// AUTO-GEN T2 — geriatric/initial_assessment.engine.js
...
class GeriatricAssessment extends Engine {
  constructor() {
    super({
      dept: 'geriatric',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'geriatric');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: geriatric (Tier-2)',
...
module.exports = GeriatricAssessment;
