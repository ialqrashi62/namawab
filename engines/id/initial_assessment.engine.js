...
/**
 * ID-001 Initial Assessment Engine
...
class IDInitialAssessment extends Engine {
  constructor(deps = {}) {
    super(Object.assign({
      redact: new Redactor(),
    ...
    this.id = 'ID-001:initial_assessment';
    this.version = '1.0.0';
    this.deptId = 'ID';
  ...
  async run(input, ctx) {
    ...
    if (this.deps.rag) {
      try {
        chunks = await this.deps.rag.retrieve({
          ...
          corpus: ['cba','sfda','nphies','who','id-guidelines'],
          topK: 8,
          tenantId,
        });
    ...
    const llmReq = {
      promptId: 'PROMPT:ID-001:initial_assessment',
      vars: {
        dept_name: 'Infectious Diseases',
...
module.exports = { IDInitialAssessment };
