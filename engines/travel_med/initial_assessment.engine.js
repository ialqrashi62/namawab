...
// AUTO-GEN Tier-3 — travel_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class TravelMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'travel_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'travel_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: travel_med (Tier-3)',
...
module.exports = TravelMedAssessment;
