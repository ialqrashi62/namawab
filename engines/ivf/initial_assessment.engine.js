...
// AUTO-GEN T2 — ivf/initial_assessment.engine.js
...
class IvfAssessment extends Engine {
  constructor() {
    super({
      dept: 'ivf',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'ivf');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: ivf (Tier-2)',
