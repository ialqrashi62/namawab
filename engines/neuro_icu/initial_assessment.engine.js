...
// AUTO-GEN Tier-3 — neuro_icu/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class NeuroIcuAssessment extends Engine {
  constructor() {
    super({
      dept: 'neuro_icu',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'neuro_icu');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: neuro_icu (Tier-3)',
...
module.exports = NeuroIcuAssessment;
