...
// AUTO-GEN Tier-3 — environmental_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class EnvironmentalMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'environmental_med',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'environmental_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: environmental_med (Tier-3)',
...
module.exports = EnvironmentalMedAssessment;
