...
// AUTO-GEN Tier-4 — forensic_psych/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class ForensicPsychAssessment extends Engine {
  constructor() {
    super({
      dept: 'forensic_psych',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'forensic_psych');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: forensic_psych (Tier-4)',
...
module.exports = ForensicPsychAssessment;
