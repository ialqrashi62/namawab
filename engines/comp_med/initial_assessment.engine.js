...
// AUTO-GEN Tier-4 — comp_med/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class CompMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'comp_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'comp_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: comp_med (Tier-4)',
...
module.exports = CompMedAssessment;
