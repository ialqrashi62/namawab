...
/**
 * GI-001 Initial Assessment Engine
...
class GIInitialAssessment extends Engine {
  constructor(deps = {}) {
    super(Object.assign({
      redact: new Redactor(),
    ...
    this.id = 'GI-001:initial_assessment';
    this.version = '1.0.0';
    this.deptId = 'GI';
  ...
  async run(input, ctx) {
    ...
    if (this.deps.rag) {
      try {
        chunks = await this.deps.rag.retrieve({
          ...
          corpus: ['cba','sfda','nphies','who','gi-guidelines'],
          topK: 8,
          tenantId,
        });
    ...
    const llmReq = {
      promptId: 'PROMPT:GI-001:initial_assessment',
      vars: {
...
module.exports = { GIInitialAssessment };
