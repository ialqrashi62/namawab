...
// AUTO-GEN Tier-3 — molecular_path/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class MolecularPathAssessment extends Engine {
  constructor() {
    super({
      dept: 'molecular_path',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'molecular_path');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: molecular_path (Tier-3)',
...
module.exports = MolecularPathAssessment;
