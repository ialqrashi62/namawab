...
// AUTO-GEN Tier-3 — peds_endo/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class PedsEndoAssessment extends Engine {
  constructor() {
    super({
      dept: 'peds_endo',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'peds_endo');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: peds_endo (Tier-3)',
...
module.exports = PedsEndoAssessment;
