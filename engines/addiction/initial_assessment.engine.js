...
// AUTO-GEN Tier-3 — addiction/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class AddictionAssessment extends Engine {
  constructor() {
    super({
      dept: 'addiction',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'addiction');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: addiction (Tier-3)',
...
module.exports = AddictionAssessment;
