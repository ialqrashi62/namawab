...
// AUTO-GEN T2 — cardiac_surgery/initial_assessment.engine.js
...
class CardiacSurgeryAssessment extends Engine {
  constructor() {
    super({
      dept: 'cardiac_surgery',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'cardiac_surgery');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: cardiac_surgery (Tier-2)',
...
module.exports = CardiacSurgeryAssessment;
