...
// AUTO-GEN T2 — plastic_surg/initial_assessment.engine.js
...
class PlasticSurgAssessment extends Engine {
  constructor() {
    super({
      dept: 'plastic_surg',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'plastic_surg');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: plastic_surg (Tier-2)',
...
module.exports = PlasticSurgAssessment;
