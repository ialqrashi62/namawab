...
// AUTO-GEN T2 — nuclear_med/initial_assessment.engine.js
...
class NuclearMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'nuclear_med',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'nuclear_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: nuclear_med (Tier-2)',
...
module.exports = NuclearMedAssessment;
