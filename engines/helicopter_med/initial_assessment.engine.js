...
// AUTO-GEN Tier-4 — helicopter_med/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class HelicopterMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'helicopter_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'helicopter_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: helicopter_med (Tier-4)',
...
module.exports = HelicopterMedAssessment;
