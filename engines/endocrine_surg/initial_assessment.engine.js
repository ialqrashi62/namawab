...
// AUTO-GEN T2 — endocrine_surg/initial_assessment.engine.js
...
class EndocrineSurgAssessment extends Engine {
  constructor() {
    super({
      dept: 'endocrine_surg',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'endocrine_surg');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: endocrine_surg (Tier-2)',
...
module.exports = EndocrineSurgAssessment;
