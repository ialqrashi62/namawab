...
// AUTO-GEN Tier-3 — community_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class CommunityMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'community_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'community_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: community_med (Tier-3)',
...
module.exports = CommunityMedAssessment;
