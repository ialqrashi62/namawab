...
// AUTO-GEN T2 — renal_tx/initial_assessment.engine.js
...
class RenalTxAssessment extends Engine {
  constructor() {
    super({
      dept: 'renal_tx',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'renal_tx');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: renal_tx (Tier-2)',
...
module.exports = RenalTxAssessment;
