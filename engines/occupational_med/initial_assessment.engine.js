...
// AUTO-GEN T2 — occupational_med/initial_assessment.engine.js
...
class OccupationalMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'occupational_med',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'occupational_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: occupational_med (Tier-2)',
...
module.exports = OccupationalMedAssessment;
