...
// AUTO-GEN T2 — lymphatic/initial_assessment.engine.js
...
class LymphaticAssessment extends Engine {
  constructor() {
    super({
      dept: 'lymphatic',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'lymphatic');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: lymphatic (Tier-2)',
...
module.exports = LymphaticAssessment;
