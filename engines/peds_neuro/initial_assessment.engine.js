...
// AUTO-GEN Tier-3 — peds_neuro/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class PedsNeuroAssessment extends Engine {
  constructor() {
    super({
      dept: 'peds_neuro',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'peds_neuro');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: peds_neuro (Tier-3)',
...
module.exports = PedsNeuroAssessment;
