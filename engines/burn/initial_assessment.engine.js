...
// AUTO-GEN T2 — burn/initial_assessment.engine.js
...
class BurnAssessment extends Engine {
  constructor() {
    super({
      dept: 'burn',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'burn');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: burn (Tier-2)',
