...
// AUTO-GEN Tier-4 — pain_proc/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class PainProcAssessment extends Engine {
  constructor() {
    super({
      dept: 'pain_proc',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'pain_proc');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: pain_proc (Tier-4)',
...
module.exports = PainProcAssessment;
