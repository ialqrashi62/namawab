...
// AUTO-GEN Tier-4 — genetic_test/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class GeneticTestAssessment extends Engine {
  constructor() {
    super({
      dept: 'genetic_test',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'genetic_test');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: genetic_test (Tier-4)',
...
module.exports = GeneticTestAssessment;
