...
// AUTO-GEN Tier-4 — mass_casualty/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class MassCasualtyAssessment extends Engine {
  constructor() {
    super({
      dept: 'mass_casualty',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'mass_casualty');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: mass_casualty (Tier-4)',
...
module.exports = MassCasualtyAssessment;
