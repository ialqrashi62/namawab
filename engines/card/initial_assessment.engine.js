...
/**
 * CARD-001 Initial Assessment Engine
...
class CARDInitialAssessment extends Engine {
  constructor(deps = {}) {
    super(Object.assign({
      redact: new Redactor(),
    ...
    this.id = 'CARD-001:initial_assessment';
    this.version = '1.0.0';
    this.deptId = 'CARD';
  ...
  async run(input, ctx) {
    ...
    if (this.deps.rag) {
      try {
        chunks = await this.deps.rag.retrieve({
          ...
          corpus: ['cba','sfda','nphies','who','card-guidelines'],
          topK: 8,
          tenantId,
        });
    ...
    const llmReq = {
      promptId: 'PROMPT:CARD-001:initial_assessment',
      vars: {
...
module.exports = { CARDInitialAssessment };
