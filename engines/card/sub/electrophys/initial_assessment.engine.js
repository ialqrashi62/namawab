...
// AUTO-GEN T5 sub-unit — card/electrophys/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class ElectrophysAssessment extends Engine {
  constructor() {
    super({
      dept: 'card/electrophys',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'electrophys');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Cardiac Electrophysiology under card',
...
module.exports = ElectrophysAssessment;
