...
// AUTO-GEN T5 sub-unit — card/structural/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class StructuralAssessment extends Engine {
  constructor() {
    super({
      dept: 'card/structural',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'structural');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Structural Heart under card',
...
module.exports = StructuralAssessment;
