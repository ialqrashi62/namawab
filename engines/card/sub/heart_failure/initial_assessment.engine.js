...
// AUTO-GEN T5 sub-unit — card/heart_failure/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class HeartFailureAssessment extends Engine {
  constructor() {
    super({
      dept: 'card/heart_failure',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'heart_failure');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Heart Failure Clinic under card',
...
module.exports = HeartFailureAssessment;
