...
// AUTO-GEN T2 — liver_tx/initial_assessment.engine.js
...
class LiverTxAssessment extends Engine {
  constructor() {
    super({
      dept: 'liver_tx',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'liver_tx');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: liver_tx (Tier-2)',
...
module.exports = LiverTxAssessment;
