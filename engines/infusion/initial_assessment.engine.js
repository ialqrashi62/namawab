...
// AUTO-GEN Tier-4 — infusion/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class InfusionAssessment extends Engine {
  constructor() {
    super({
      dept: 'infusion',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'infusion');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: infusion (Tier-4)',
...
module.exports = InfusionAssessment;
