...
// AUTO-GEN Tier-3 — clinical_ethics/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class ClinicalEthicsAssessment extends Engine {
  constructor() {
    super({
      dept: 'clinical_ethics',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'clinical_ethics');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: clinical_ethics (Tier-3)',
...
module.exports = ClinicalEthicsAssessment;
