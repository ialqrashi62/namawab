...
// AUTO-GEN Tier-3 — neuro_rehab_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class NeuroRehabMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'neuro_rehab_med',
      redFlags: [],
      drugBlocks: [],
      citationMin: 0.7,
      requireRedFlag: true,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'neuro_rehab_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: neuro_rehab_med (Tier-3)',
...
module.exports = NeuroRehabMedAssessment;
