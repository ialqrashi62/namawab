...
// AUTO-GEN Tier-3 — hematology/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class HematologyAssessment extends Engine {
  constructor() {
    super({
      dept: 'hematology',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'hematology');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: hematology (Tier-3)',
...
module.exports = HematologyAssessment;
