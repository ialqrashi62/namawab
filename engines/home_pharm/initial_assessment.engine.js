...
// AUTO-GEN Tier-4 — home_pharm/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class HomePharmAssessment extends Engine {
  constructor() {
    super({
      dept: 'home_pharm',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'home_pharm');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: home_pharm (Tier-4)',
...
module.exports = HomePharmAssessment;
