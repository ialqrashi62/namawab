...
// AUTO-GEN T2 — genetic_counsel/initial_assessment.engine.js
...
class GeneticCounselAssessment extends Engine {
  constructor() {
    super({
      dept: 'genetic_counsel',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'genetic_counsel');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: genetic_counsel (Tier-2)',
...
module.exports = GeneticCounselAssessment;
