...
// AUTO-GEN Tier-3 — bariatric_nutr/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class BariatricNutrAssessment extends Engine {
  constructor() {
    super({
      dept: 'bariatric_nutr',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'bariatric_nutr');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: bariatric_nutr (Tier-3)',
...
module.exports = BariatricNutrAssessment;
