...
// AUTO-GEN Tier-4 — biorepository/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class BiorepositoryAssessment extends Engine {
  constructor() {
    super({
      dept: 'biorepository',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'biorepository');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: biorepository (Tier-4)',
...
module.exports = BiorepositoryAssessment;
