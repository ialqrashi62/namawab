...
// AUTO-GEN Tier-3 — tropical_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class TropicalMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'tropical_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'tropical_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: tropical_med (Tier-3)',
...
module.exports = TropicalMedAssessment;
