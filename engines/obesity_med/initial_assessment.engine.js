...
// AUTO-GEN Tier-3 — obesity_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class ObesityMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'obesity_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'obesity_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: obesity_med (Tier-3)',
...
module.exports = ObesityMedAssessment;
