...
// AUTO-GEN Tier-3 — periop_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class PeriopMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'periop_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'periop_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: periop_med (Tier-3)',
...
module.exports = PeriopMedAssessment;
