...
// AUTO-GEN Tier-3 — andrology/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class AndrologyAssessment extends Engine {
  constructor() {
    super({
      dept: 'andrology',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'andrology');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: andrology (Tier-3)',
...
module.exports = AndrologyAssessment;
