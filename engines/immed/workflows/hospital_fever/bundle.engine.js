...
// AUTO-GEN T6 procedural — immed/workflows/hospital_fever/bundle.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class HospitalFeverBundle extends Engine {
  constructor() {
    super({
      dept: 'immed/hospital_fever',
      parentDept: 'immed',
      workflow: 'hospital_fever',
      procedureLabel: 'Inpatient Fever Workup',
      scope: 'IM',
      redFlags: [],
      drugBlocks: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || 'hospital_fever');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nProcedure bundle: Inpatient Fever Workup',
      '\nScope: IM',
...
module.exports = HospitalFeverBundle;
