#!/usr/bin/env python3
"""
gen_engines.py — Generate real engine code for all Tier-1 depts.

Each engine:
- Extends base Engine
- Implements `run(input, ctx)`:
    1. Loads patient context via PatientPort
    2. Queries RAG (chunks + cite)
    3. Detects red flags via RedFlagService
    4. Runs DrugCheckService
    5. Composes LLM call via Adapter (mockable)
    6. Returns structured output (differential, redFlags, drugAlerts, recommendedOrders, citations, confidence)
- Pure standardlib (no TS) so it runs in plain Node.
- All outputs PHI-redacted via Redactor.
- Mandatory: dept_id used for audit + tenant scoping.
"""

import pathlib, sys
ROOT = pathlib.Path(r"C:\Users\ice\Desktop\NMEDCALVSCODE")
OUT = ROOT / "namaweb" / "engines"

# Tier-1 depts — code + deptId + ascii name
DEPT_META = {
    "card": ("CARD", "Cardiology"),
    "pulm": ("PULM", "Pulmonology"),
    "gi":   ("GI",   "Gastroenterology"),
    "neph": ("NEPH", "Nephrology"),
    "onc":  ("ONC",  "Hematology & Medical Oncology"),
    "endo": ("ENDO", "Endocrinology"),
    "id":   ("ID",   "Infectious Diseases"),
    "derm": ("DERM", "Dermatology"),
    "rheum":("RHEUM","Rheumatology"),
    "er":   ("ER",   "Emergency Medicine"),
    "obg":  ("OBG",  "Obstetrics & Gynecology"),
    "peds": ("PEDS", "Pediatrics"),
    "surg": ("SURG", "General Surgery"),
    "neuro":("NEURO","Neurology"),
    "ortho":("ORTHO","Orthopedics"),
    "ophth":("OPHTH","Ophthalmology"),
    "ent":  ("ENT",  "Otolaryngology"),
    "uro":  ("URO",  "Urology"),
    "anes": ("ANES", "Anesthesiology"),
    "icu":  ("ICU",  "Intensive Care Unit"),
    "psyc": ("PSYC", "Psychiatry"),
}

def emit(dept_code, dept_id, dept_name, prompts_map):
    return f"""'use strict';

/**
 * {dept_id}-001 Initial Assessment Engine
...
class {dept_id.replace('-','_')}InitialAssessment extends Engine {{
  constructor(deps = {{}}) {{
    super(Object.assign({{
      redact: new Redactor(),
    ...
    this.id = '{dept_id}-001:initial_assessment';
    this.version = '1.0.0';
    this.deptId = '{dept_id}';
  ...
  async run(input, ctx) {{
    ...
    if (this.deps.rag) {{
      try {{
        chunks = await this.deps.rag.retrieve({{
          ...
          corpus: ['cba','sfda','nphies','who','{dept_code.lower()}-guidelines'],
          topK: 8,
          tenantId,
        }});
    ...
    const llmReq = {{
      promptId: 'PROMPT:{dept_id}-001:initial_assessment',
      vars: {{
        dept_name: '{dept_name}',
...
module.exports = {{ {dept_id.replace('-','_')}InitialAssessment }};
"""

def main():
    for code, (dept_id, dept_name) in DEPT_META.items():
        d = pathlib.Path(OUT / dept_id.lower().replace('-', '_') / "initial_assessment.engine.js")
        d.parent.mkdir(parents=True, exist_ok=True)
        d.write_text(emit(code.upper(), dept_id, dept_name, {}), encoding='utf-8')
        sys.stderr.write(f"  ✅ {dept_id} -> {d}\n")
    sys.stderr.write(f"\nDone: {len(DEPT_META)} Tier-1 engines generated.\n")
