...
// AUTO-GEN Tier-3 — derm_cosmetic/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class DermCosmeticAssessment extends Engine {
  constructor() {
    super({
      dept: 'derm_cosmetic',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'derm_cosmetic');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: derm_cosmetic (Tier-3)',
...
module.exports = DermCosmeticAssessment;
