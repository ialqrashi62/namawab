...
// AUTO-GEN Tier-3 — gyn_onc/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class GynOncAssessment extends Engine {
  constructor() {
    super({
      dept: 'gyn_onc',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'gyn_onc');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: gyn_onc (Tier-3)',
...
module.exports = GynOncAssessment;
