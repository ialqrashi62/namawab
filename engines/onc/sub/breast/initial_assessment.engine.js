...
// AUTO-GEN T5 sub-unit — onc/breast/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class BreastAssessment extends Engine {
  constructor() {
    super({
      dept: 'onc/breast',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'breast');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Breast Oncology under onc',
...
module.exports = BreastAssessment;
