...
// AUTO-GEN T5 sub-unit — onc/pain/initial_assessment.engine.js
'use strict';

const { Engine } = require('../../../../lib');

class PainAssessment extends Engine {
  constructor() {
    super({
      dept: 'onc/pain',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'pain');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nSub-unit: Cancer Pain under onc',
