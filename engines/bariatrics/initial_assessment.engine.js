...
// AUTO-GEN T2 — bariatrics/initial_assessment.engine.js
...
class BariatricsAssessment extends Engine {
  constructor() {
    super({
      dept: 'bariatrics',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'bariatrics');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: bariatrics (Tier-2)',
...
module.exports = BariatricsAssessment;
