...
// AUTO-GEN T2 — neuro_rehab/initial_assessment.engine.js
...
class NeuroRehabAssessment extends Engine {
  constructor() {
    super({
      dept: 'neuro_rehab',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'neuro_rehab');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: neuro_rehab (Tier-2)',
...
module.exports = NeuroRehabAssessment;
