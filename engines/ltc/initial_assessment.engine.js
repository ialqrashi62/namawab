...
// AUTO-GEN Tier-3 — ltc/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class LtcAssessment extends Engine {
  constructor() {
    super({
      dept: 'ltc',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'ltc');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: ltc (Tier-3)',
