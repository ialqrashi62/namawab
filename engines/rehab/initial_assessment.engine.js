...
// AUTO-GEN T2 — rehab/initial_assessment.engine.js
...
class RehabAssessment extends Engine {
  constructor() {
    super({
      dept: 'rehab',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'rehab');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: rehab (Tier-2)',
