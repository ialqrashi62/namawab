...
// AUTO-GEN T2 — covid/initial_assessment.engine.js
...
class CovidAssessment extends Engine {
  constructor() {
    super({
      dept: 'covid',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'covid');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: covid (Tier-2)',
...
module.exports = CovidAssessment;
