...
// AUTO-GEN T2 — dialysis/initial_assessment.engine.js
...
class DialysisAssessment extends Engine {
  constructor() {
    super({
      dept: 'dialysis',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'dialysis');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: dialysis (Tier-2)',
...
module.exports = DialysisAssessment;
