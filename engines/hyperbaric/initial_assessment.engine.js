...
// AUTO-GEN T2 — hyperbaric/initial_assessment.engine.js
...
class HyperbaricAssessment extends Engine {
  constructor() {
    super({
      dept: 'hyperbaric',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'hyperbaric');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: hyperbaric (Tier-2)',
...
module.exports = HyperbaricAssessment;
