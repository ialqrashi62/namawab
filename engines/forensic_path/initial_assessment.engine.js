...
// AUTO-GEN Tier-4 — forensic_path/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class ForensicPathAssessment extends Engine {
  constructor() {
    super({
      dept: 'forensic_path',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'forensic_path');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: forensic_path (Tier-4)',
...
module.exports = ForensicPathAssessment;
