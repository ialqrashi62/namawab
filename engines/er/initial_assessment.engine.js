...
/**
 * ER-001 Initial Assessment Engine
...
class ERInitialAssessment extends Engine {
  constructor(deps = {}) {
    super(Object.assign({
      redact: new Redactor(),
    ...
    this.id = 'ER-001:initial_assessment';
    this.version = '1.0.0';
    this.deptId = 'ER';
  ...
  async run(input, ctx) {
    ...
    if (this.deps.rag) {
      try {
        chunks = await this.deps.rag.retrieve({
          ...
          corpus: ['cba','sfda','nphies','who','er-guidelines'],
          topK: 8,
          tenantId,
        });
    ...
    const llmReq = {
      promptId: 'PROMPT:ER-001:initial_assessment',
      vars: {
...
module.exports = { ERInitialAssessment };
