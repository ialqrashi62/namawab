...
// AUTO-GEN T2 — maternal_fetal/initial_assessment.engine.js
...
class MaternalFetalAssessment extends Engine {
  constructor() {
    super({
      dept: 'maternal_fetal',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'maternal_fetal');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: maternal_fetal (Tier-2)',
...
module.exports = MaternalFetalAssessment;
