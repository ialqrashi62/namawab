...
// AUTO-GEN Tier-3 — breast_clinic/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class BreastClinicAssessment extends Engine {
  constructor() {
    super({
      dept: 'breast_clinic',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'breast_clinic');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: breast_clinic (Tier-3)',
...
module.exports = BreastClinicAssessment;
