...
// AUTO-GEN Tier-3 — fp_mph/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class FpMphAssessment extends Engine {
  constructor() {
    super({
      dept: 'fp_mph',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'fp_mph');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: fp_mph (Tier-3)',
...
module.exports = FpMphAssessment;
