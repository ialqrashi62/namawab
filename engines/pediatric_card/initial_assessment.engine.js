...
// AUTO-GEN T2 — pediatric_card/initial_assessment.engine.js
...
class PediatricCardAssessment extends Engine {
  constructor() {
    super({
      dept: 'pediatric_card',
      redFlags: RED_FLAGS,
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'pediatric_card');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: pediatric_card (Tier-2)',
...
module.exports = PediatricCardAssessment;
