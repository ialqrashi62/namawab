...
// AUTO-GEN Tier-3 — rural_health/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class RuralHealthAssessment extends Engine {
  constructor() {
    super({
      dept: 'rural_health',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'rural_health');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: rural_health (Tier-3)',
...
module.exports = RuralHealthAssessment;
