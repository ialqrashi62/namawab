...
// AUTO-GEN Tier-3 — adolescent_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class AdolescentMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'adolescent_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'adolescent_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: adolescent_med (Tier-3)',
...
module.exports = AdolescentMedAssessment;
