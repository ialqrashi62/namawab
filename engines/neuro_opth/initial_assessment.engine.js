...
// AUTO-GEN Tier-4 — neuro_opth/initial_assessment.engine.js
// Tier-4 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class NeuroOpthAssessment extends Engine {
  constructor() {
    super({
      dept: 'neuro_opth',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'neuro_opth');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: neuro_opth (Tier-4)',
...
module.exports = NeuroOpthAssessment;
