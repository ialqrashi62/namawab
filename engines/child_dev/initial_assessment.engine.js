...
// AUTO-GEN Tier-3 — child_dev/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class ChildDevAssessment extends Engine {
  constructor() {
    super({
      dept: 'child_dev',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'child_dev');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: child_dev (Tier-3)',
...
module.exports = ChildDevAssessment;
