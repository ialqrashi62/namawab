...
// AUTO-GEN Tier-3 — sleep_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class SleepMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'sleep_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'sleep_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: sleep_med (Tier-3)',
...
module.exports = SleepMedAssessment;
