...
// AUTO-GEN Tier-3 — genetic_med/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class GeneticMedAssessment extends Engine {
  constructor() {
    super({
      dept: 'genetic_med',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'genetic_med');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: genetic_med (Tier-3)',
...
module.exports = GeneticMedAssessment;
