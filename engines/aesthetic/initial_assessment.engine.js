...
// AUTO-GEN Tier-3 — aesthetic/initial_assessment.engine.js
// Tier-3 dept engine. Extends shared Engine base.
'use strict';

const { Engine } = require('../../lib');

class AestheticAssessment extends Engine {
  constructor() {
    super({
      dept: 'aesthetic',
      redFlags: [],
  ...
  async run(input, ctx) {
    ...
    const chunks = await this.rag.search(ctx, text || input.condition || 'aesthetic');

    const promptText = [
      this.registry.compose('SYSTEM_PROMPT_BASE'),
      '\nDepartment: aesthetic (Tier-3)',
...
module.exports = AestheticAssessment;
