...
const r = spawnSync('ssh', [
  ...
  `bash -c '
echo "=== app serving ==="
for p in "api/v4/olap/views" "api/v4/olap/query?view=mv_daily_admissions" "api/health"; do
...
echo ""
echo "=== audit row count after wave 16 ==="
...
echo ""
echo "=== recent audit rows for tenant 1 ==="
PGPASSWORD="$APP_PW" psql -h 127.0.0.1 -U nama_medical_app -d nama_medical_web -c "SET app.tenant_id = '\''1'\''; SELECT id, username, action, module, tenant_id FROM audit_trail ORDER BY id DESC LIMIT 5;" 2>&1 | grep -v "could not change directory"'`
]);
