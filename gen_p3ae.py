"""Generate SQL up + routes + integration for P3-AE modules"""
import os
...
extras = {
    'cardio_ext': "router.post('/admissions/:id/heart-team', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
    'endo_ext': "router.post('/admissions/:id/endocrine-referral', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
    'maternal_fetal': "router.post('/admissions/:id/antenatal-visit', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
}

for m in ['cardio_ext', 'endo_ext', 'maternal_fetal']:
