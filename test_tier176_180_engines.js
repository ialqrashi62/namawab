// filepath: test_tier176_180_engines.js
const fs = require('fs');
const path = require('path');

// Auto-discover engines by scanning filesystem
const dir = 'C:/Users/ice/Desktop/NMEDCALVSCODE/namaweb';
const files = fs.readdirSync(dir).filter(f => /^tier1(7[6-9]|80)_\w+_\d+_engine\.js$/.test(f));
...
for (const e of engines) {
  ...
  const { funcs, ValidationError } = mod;
  const f = funcs();
  for (const fnName of Object.keys(f)) {
    try {
      ...
      if (out && out.patient_id) { console.log('OK', e + '.' + fnName); pass++; }
