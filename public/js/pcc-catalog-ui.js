...
(function () {
    ...
    function normalizeName(name) {
        // strip pcc_ prefix if present (catalog returns names like "pcc_adolescent_ext101")
    ...
    function fetchModuleMeta(moduleName) {
        ...
        var url = API_BASE + "/pcc-" + routeName + "/list";
    ...
    function callFunction(moduleName, fnName, payload) {
        var routeName = normalizeName(moduleName);
        var url = API_BASE + "/pcc-" + routeName + "/call/" + encodeURIComponent(fnName);
        return fetch(url, {
            method: "POST",
    ...
    function humanize(name) {
        return String(name || "").replace(/^pcc[_-]/, "").replace(/_ext\d+$/, "").replace(/_/g, " ");
    }
