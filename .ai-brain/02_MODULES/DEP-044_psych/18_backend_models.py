# filepath: 02_MODULES/DEP-044/18_backend_models.py
# SQLAlchemy models for Psychiatry (DEP-044)
...
class PsychEncounter(Base):
    __tablename__ = "psych_encounters"
...
class PsychOrder(Base):
    __tablename__ = "psych_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("psych_encounters.id"))
...
class PsychResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("psych_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("psych_orders.id"))
...
class PsychNote(Base):
    __tablename__ = "psych_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("psych_encounters.id"))
