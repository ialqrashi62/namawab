// filepath: 02_MODULES/DEP-044/22_frontend_page.tsx
// Main page for Psychiatry (DEP-044)
...
import { PsychAPI } from './24_frontend_api_client';
...
export default function PsychPage({ tenantId, patientId, lang = 'ar' }: PsychPageProps) {
  const { t } = useTranslation('psych');
  ...
  useEffect(() => {
    PsychAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PsychAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PsychAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="PHQ_9" patientId={patientId} />
        </>
      }
      main={
        <>
