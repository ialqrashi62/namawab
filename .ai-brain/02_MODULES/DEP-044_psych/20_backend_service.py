# filepath: 02_MODULES/DEP-044/20_backend_service.py
# Business logic for Psychiatry (DEP-044)
...
class PsychService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PsychEncounter
        q = self.db.query(PsychEncounter).filter(
            PsychEncounter.tenant_id == self.tenant_id,
            PsychEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PsychEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PsychEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PsychEncounter
        e = self.db.query(PsychEncounter).filter(
            PsychEncounter.id == encounter_id,
            PsychEncounter.tenant_id == self.tenant_id
        ).first()
