# filepath: 02_MODULES/DEP-044/21_backend_router.py
# FastAPI router for Psychiatry (DEP-044)
...
from .schemas import *
from .service import PsychService

router = APIRouter(prefix="/api/psych", tags=["Psychiatry"])


@router.get("/list", response_model=List[PsychEncounter])
async def list_encounters(
    ...
    """List Psychiatry encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PsychService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PsychEncounter)
...
@router.post("/", response_model=PsychEncounter, status_code=201)
async def create_encounter(
    payload: PsychEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["psych_specialist", "psych_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PsychService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PsychEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PsychEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["psych_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PsychService(db, tenant_id)
