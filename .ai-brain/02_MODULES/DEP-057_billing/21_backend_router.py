# filepath: 02_MODULES/DEP-057/21_backend_router.py
# FastAPI router for Billing_Coding (DEP-057)
...
router = APIRouter(prefix="/api/billing", tags=["Billing_Coding"])


@router.get("/list", response_model=List[BillingEncounter])
async def list_encounters(
    ...
    """List Billing_Coding encounters for current tenant."""
...
@router.get("/{encounter_id}", response_model=BillingEncounter)
...
@router.post("/", response_model=BillingEncounter, status_code=201)
async def create_encounter(
    payload: BillingEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["billing_specialist", "billing_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = BillingService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=BillingEncounter)
async def update_encounter(
    encounter_id: int,
    payload: BillingEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["billing_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = BillingService(db, tenant_id)
