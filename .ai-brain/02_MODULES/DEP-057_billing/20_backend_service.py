# filepath: 02_MODULES/DEP-057/20_backend_service.py
# Business logic for Billing_Coding (DEP-057)
...
class BillingService:
    """Service layer for Billing_Coding.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import BillingEncounter
        q = self.db.query(BillingEncounter).filter(
            BillingEncounter.tenant_id == self.tenant_id,
            BillingEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(BillingEncounter.patient_id == patient_id)
        if status:
            q = q.filter(BillingEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import BillingEncounter
        e = self.db.query(BillingEncounter).filter(
