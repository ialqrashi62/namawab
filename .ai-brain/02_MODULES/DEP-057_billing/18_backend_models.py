# filepath: 02_MODULES/DEP-057/18_backend_models.py
# SQLAlchemy models for Billing_Coding (DEP-057)
...
class BillingOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("billing_encounters.id"))
...
class BillingResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("billing_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("billing_orders.id"))
