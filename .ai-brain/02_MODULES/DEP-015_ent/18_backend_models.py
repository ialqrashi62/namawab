# filepath: 02_MODULES/DEP-015/18_backend_models.py
# SQLAlchemy models for ENT (DEP-015)
...
class EntEncounter(Base):
    __tablename__ = "ent_encounters"
...
class EntOrder(Base):
    __tablename__ = "ent_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ent_encounters.id"))
...
class EntResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ent_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("ent_orders.id"))
