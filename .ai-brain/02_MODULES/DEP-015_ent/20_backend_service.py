# filepath: 02_MODULES/DEP-015/20_backend_service.py
# Business logic for ENT (DEP-015)
...
class EntService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import EntEncounter
        q = self.db.query(EntEncounter).filter(
            EntEncounter.tenant_id == self.tenant_id,
            EntEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(EntEncounter.patient_id == patient_id)
        if status:
            q = q.filter(EntEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import EntEncounter
        e = self.db.query(EntEncounter).filter(
            EntEncounter.id == encounter_id,
            EntEncounter.tenant_id == self.tenant_id
        ).first()
