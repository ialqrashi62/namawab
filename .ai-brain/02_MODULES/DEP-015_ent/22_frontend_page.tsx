// filepath: 02_MODULES/DEP-015/22_frontend_page.tsx
// Main page for ENT (DEP-015)
...
import { EntAPI } from './24_frontend_api_client';

interface EntPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function EntPage({ tenantId, patientId, lang = 'ar' }: EntPageProps) {
  const { t } = useTranslation('ent');
  ...
  useEffect(() => {
    EntAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    EntAPI.listOrders({ patientId, tenantId }).then(setOrders);
    EntAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="SNOT_22" patientId={patientId} />
        </>
      }
      main={
        <>
