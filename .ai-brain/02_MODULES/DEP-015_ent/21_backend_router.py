# filepath: 02_MODULES/DEP-015/21_backend_router.py
# FastAPI router for ENT (DEP-015)
...
from .schemas import *
from .service import EntService

router = APIRouter(prefix="/api/ent", tags=["ENT"])


@router.get("/list", response_model=List[EntEncounter])
async def list_encounters(
    ...
    """List ENT encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = EntService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=EntEncounter)
...
@router.post("/", response_model=EntEncounter, status_code=201)
async def create_encounter(
    payload: EntEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["ent_specialist", "ent_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = EntService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=EntEncounter)
async def update_encounter(
    encounter_id: int,
    payload: EntEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["ent_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = EntService(db, tenant_id)
