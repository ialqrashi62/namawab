# filepath: 02_MODULES/DEP-010/18_backend_models.py
# SQLAlchemy models for Allergy_Immunology (DEP-010)
...
class AllergyEncounter(Base):
    __tablename__ = "allergy_encounters"
...
class AllergyOrder(Base):
    __tablename__ = "allergy_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("allergy_encounters.id"))
...
class AllergyResult(Base):
    __tablename__ = "allergy_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("allergy_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("allergy_orders.id"))
...
class AllergyNote(Base):
    __tablename__ = "allergy_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("allergy_encounters.id"))
