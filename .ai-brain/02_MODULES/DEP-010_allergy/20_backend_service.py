# filepath: 02_MODULES/DEP-010/20_backend_service.py
# Business logic for Allergy_Immunology (DEP-010)
...
class AllergyService:
    """Service layer for Allergy_Immunology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import AllergyEncounter
        q = self.db.query(AllergyEncounter).filter(
            AllergyEncounter.tenant_id == self.tenant_id,
            AllergyEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(AllergyEncounter.patient_id == patient_id)
        if status:
            q = q.filter(AllergyEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import AllergyEncounter
        e = self.db.query(AllergyEncounter).filter(
            AllergyEncounter.id == encounter_id,
            AllergyEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import AllergyOrder
        o = AllergyOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
