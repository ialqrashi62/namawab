# filepath: 02_MODULES/DEP-010/21_backend_router.py
# FastAPI router for Allergy_Immunology (DEP-010)
...
from .schemas import *
from .service import AllergyService

router = APIRouter(prefix="/api/allergy", tags=["Allergy_Immunology"])


@router.get("/list", response_model=List[AllergyEncounter])
async def list_encounters(
    ...
    """List Allergy_Immunology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = AllergyService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=AllergyEncounter)
...
@router.post("/", response_model=AllergyEncounter, status_code=201)
async def create_encounter(
    payload: AllergyEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["allergy_specialist", "allergy_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = AllergyService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=AllergyEncounter)
async def update_encounter(
    encounter_id: int,
    payload: AllergyEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["allergy_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = AllergyService(db, tenant_id)
