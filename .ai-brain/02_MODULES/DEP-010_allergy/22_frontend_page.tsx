// filepath: 02_MODULES/DEP-010/22_frontend_page.tsx
// Main page for Allergy_Immunology (DEP-010)
...
import { AllergyAPI } from './24_frontend_api_client';
...
export default function AllergyPage({ tenantId, patientId, lang = 'ar' }: AllergyPageProps) {
  ...
  useEffect(() => {
    AllergyAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    AllergyAPI.listOrders({ patientId, tenantId }).then(setOrders);
    AllergyAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="الحساسية والمناعة"
      title_en="Allergy_Immunology"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="ACT" patientId={patientId} />
        </>
      }
      main={
        <>
