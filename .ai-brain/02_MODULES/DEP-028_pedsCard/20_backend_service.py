# filepath: 02_MODULES/DEP-028/20_backend_service.py
# Business logic for Pediatric_Cardiology (DEP-028)
...
class PedscardService:
    """Service layer for Pediatric_Cardiology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PedscardEncounter
        q = self.db.query(PedscardEncounter).filter(
            PedscardEncounter.tenant_id == self.tenant_id,
            PedscardEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PedscardEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PedscardEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PedscardEncounter
        e = self.db.query(PedscardEncounter).filter(
            PedscardEncounter.id == encounter_id,
            PedscardEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import PedscardOrder
        o = PedscardOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
