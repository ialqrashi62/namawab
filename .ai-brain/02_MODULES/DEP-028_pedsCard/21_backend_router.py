# filepath: 02_MODULES/DEP-028/21_backend_router.py
# FastAPI router for Pediatric_Cardiology (DEP-028)
...
from .schemas import *
from .service import PedscardService

router = APIRouter(prefix="/api/pedsCard", tags=["Pediatric_Cardiology"])


@router.get("/list", response_model=List[PedscardEncounter])
async def list_encounters(
    ...
    """List Pediatric_Cardiology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PedscardService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PedscardEncounter)
...
@router.post("/", response_model=PedscardEncounter, status_code=201)
async def create_encounter(
    payload: PedscardEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsCard_specialist", "pedsCard_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedscardService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PedscardEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PedscardEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsCard_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedscardService(db, tenant_id)
