// filepath: 02_MODULES/DEP-031/22_frontend_page.tsx
// Main page for Pediatric_HemOnc (DEP-031)
...
import { PedshemoncAPI } from './24_frontend_api_client';

interface PedshemoncPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function PedshemoncPage({ tenantId, patientId, lang = 'ar' }: PedshemoncPageProps) {
  const { t } = useTranslation('pedsHemOnc');
  ...
  useEffect(() => {
    PedshemoncAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PedshemoncAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PedshemoncAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="دم وأورام الأطفال"
      title_en="Pediatric_HemOnc"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="NCCN_pediatric" patientId={patientId} />
        </>
      }
      main={
        <>
