# filepath: 02_MODULES/DEP-031/21_backend_router.py
# FastAPI router for Pediatric_HemOnc (DEP-031)
...
from .schemas import *
from .service import PedshemoncService

router = APIRouter(prefix="/api/pedsHemOnc", tags=["Pediatric_HemOnc"])


@router.get("/list", response_model=List[PedshemoncEncounter])
async def list_encounters(
    ...
    """List Pediatric_HemOnc encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PedshemoncService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PedshemoncEncounter)
...
@router.post("/", response_model=PedshemoncEncounter, status_code=201)
async def create_encounter(
    payload: PedshemoncEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsHemOnc_specialist", "pedsHemOnc_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedshemoncService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PedshemoncEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PedshemoncEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsHemOnc_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedshemoncService(db, tenant_id)
