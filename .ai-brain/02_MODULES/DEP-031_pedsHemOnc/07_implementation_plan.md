# Implementation Plan — Pediatric_HemOnc (DEP-031)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- pedsHemOnc_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh pedsHemOnc
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-pedsHemOnc-<timestamp>`
