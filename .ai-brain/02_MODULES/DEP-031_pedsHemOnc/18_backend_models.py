# filepath: 02_MODULES/DEP-031/18_backend_models.py
# SQLAlchemy models for Pediatric_HemOnc (DEP-031)
...
class PedshemoncEncounter(Base):
    __tablename__ = "pedsHemOnc_encounters"
...
class PedshemoncOrder(Base):
    __tablename__ = "pedsHemOnc_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsHemOnc_encounters.id"))
...
class PedshemoncResult(Base):
    __tablename__ = "pedsHemOnc_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsHemOnc_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("pedsHemOnc_orders.id"))
...
class PedshemoncNote(Base):
    __tablename__ = "pedsHemOnc_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsHemOnc_encounters.id"))
