# filepath: 02_MODULES/DEP-031/20_backend_service.py
# Business logic for Pediatric_HemOnc (DEP-031)
...
class PedshemoncService:
    """Service layer for Pediatric_HemOnc.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PedshemoncEncounter
        q = self.db.query(PedshemoncEncounter).filter(
            PedshemoncEncounter.tenant_id == self.tenant_id,
            PedshemoncEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PedshemoncEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PedshemoncEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PedshemoncEncounter
        e = self.db.query(PedshemoncEncounter).filter(
            PedshemoncEncounter.id == encounter_id,
            PedshemoncEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import PedshemoncOrder
        o = PedshemoncOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
