# filepath: 02_MODULES/DEP-050/20_backend_service.py
# Business logic for Palliative_Care (DEP-050)
...
class PalliativeService:
    """Service layer for Palliative_Care.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PalliativeEncounter
        q = self.db.query(PalliativeEncounter).filter(
            PalliativeEncounter.tenant_id == self.tenant_id,
            PalliativeEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PalliativeEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PalliativeEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PalliativeEncounter
        e = self.db.query(PalliativeEncounter).filter(
            PalliativeEncounter.id == encounter_id,
            PalliativeEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import PalliativeOrder
        o = PalliativeOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
