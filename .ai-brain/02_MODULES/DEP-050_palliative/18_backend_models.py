# filepath: 02_MODULES/DEP-050/18_backend_models.py
# SQLAlchemy models for Palliative_Care (DEP-050)
...
class PalliativeEncounter(Base):
    __tablename__ = "palliative_encounters"
...
class PalliativeOrder(Base):
    __tablename__ = "palliative_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("palliative_encounters.id"))
...
class PalliativeResult(Base):
    __tablename__ = "palliative_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("palliative_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("palliative_orders.id"))
...
class PalliativeNote(Base):
    __tablename__ = "palliative_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("palliative_encounters.id"))
