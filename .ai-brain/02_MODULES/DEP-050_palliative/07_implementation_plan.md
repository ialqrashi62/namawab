# Implementation Plan — Palliative_Care (DEP-050)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- palliative_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh palliative
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-palliative-<timestamp>`
