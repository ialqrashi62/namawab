// filepath: 02_MODULES/DEP-050/22_frontend_page.tsx
// Main page for Palliative_Care (DEP-050)
...
import { PalliativeAPI } from './24_frontend_api_client';

interface PalliativePageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function PalliativePage({ tenantId, patientId, lang = 'ar' }: PalliativePageProps) {
  ...
  useEffect(() => {
    PalliativeAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PalliativeAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PalliativeAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="الرعاية التلطيفية"
      title_en="Palliative_Care"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="ESAS" patientId={patientId} />
        </>
      }
      main={
        <>
