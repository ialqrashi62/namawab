# Implementation Plan — Reproductive_Medicine_IVF (DEP-036)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- fertility_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh fertility
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-fertility-<timestamp>`
