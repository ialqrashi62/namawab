# filepath: 02_MODULES/DEP-036/21_backend_router.py
# FastAPI router for Reproductive_Medicine_IVF (DEP-036)
...
from .schemas import *
from .service import FertilityService

router = APIRouter(prefix="/api/fertility", tags=["Reproductive_Medicine_IVF"])


@router.get("/list", response_model=List[FertilityEncounter])
async def list_encounters(
    ...
    """List Reproductive_Medicine_IVF encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = FertilityService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=FertilityEncounter)
...
@router.post("/", response_model=FertilityEncounter, status_code=201)
async def create_encounter(
    payload: FertilityEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["fertility_specialist", "fertility_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = FertilityService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=FertilityEncounter)
async def update_encounter(
    encounter_id: int,
    payload: FertilityEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["fertility_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = FertilityService(db, tenant_id)
