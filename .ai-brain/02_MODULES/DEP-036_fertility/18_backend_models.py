# filepath: 02_MODULES/DEP-036/18_backend_models.py
# SQLAlchemy models for Reproductive_Medicine_IVF (DEP-036)
...
class FertilityEncounter(Base):
    __tablename__ = "fertility_encounters"
...
class FertilityOrder(Base):
    __tablename__ = "fertility_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("fertility_encounters.id"))
...
class FertilityResult(Base):
    __tablename__ = "fertility_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("fertility_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("fertility_orders.id"))
...
class FertilityNote(Base):
    __tablename__ = "fertility_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("fertility_encounters.id"))
