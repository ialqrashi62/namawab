// filepath: 02_MODULES/DEP-036/22_frontend_page.tsx
// Main page for Reproductive_Medicine_IVF (DEP-036)
...
import { FertilityAPI } from './24_frontend_api_client';

interface FertilityPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function FertilityPage({ tenantId, patientId, lang = 'ar' }: FertilityPageProps) {
  const { t } = useTranslation('fertility');
  ...
  useEffect(() => {
    FertilityAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    FertilityAPI.listOrders({ patientId, tenantId }).then(setOrders);
    FertilityAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="الإخصاب"
      title_en="Reproductive_Medicine_IVF"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="AMH" patientId={patientId} />
        </>
      }
      main={
        <>
