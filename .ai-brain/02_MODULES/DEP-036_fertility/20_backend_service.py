# filepath: 02_MODULES/DEP-036/20_backend_service.py
# Business logic for Reproductive_Medicine_IVF (DEP-036)
...
class FertilityService:
    """Service layer for Reproductive_Medicine_IVF.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import FertilityEncounter
        q = self.db.query(FertilityEncounter).filter(
            FertilityEncounter.tenant_id == self.tenant_id,
            FertilityEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(FertilityEncounter.patient_id == patient_id)
        if status:
            q = q.filter(FertilityEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import FertilityEncounter
        e = self.db.query(FertilityEncounter).filter(
            FertilityEncounter.id == encounter_id,
            FertilityEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import FertilityOrder
        o = FertilityOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
