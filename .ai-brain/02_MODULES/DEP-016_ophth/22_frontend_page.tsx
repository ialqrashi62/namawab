// filepath: 02_MODULES/DEP-016/22_frontend_page.tsx
// Main page for Ophthalmology (DEP-016)
...
import { OphthAPI } from './24_frontend_api_client';

interface OphthPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function OphthPage({ tenantId, patientId, lang = 'ar' }: OphthPageProps) {
  const { t } = useTranslation('ophth');
  ...
  useEffect(() => {
    OphthAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    OphthAPI.listOrders({ patientId, tenantId }).then(setOrders);
    OphthAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="IOP" patientId={patientId} />
        </>
      }
      main={
        <>
