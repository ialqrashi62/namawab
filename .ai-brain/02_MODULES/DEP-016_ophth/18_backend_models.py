# filepath: 02_MODULES/DEP-016/18_backend_models.py
# SQLAlchemy models for Ophthalmology (DEP-016)
...
class OphthEncounter(Base):
    __tablename__ = "ophth_encounters"
...
class OphthOrder(Base):
    __tablename__ = "ophth_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ophth_encounters.id"))
...
class OphthResult(Base):
    __tablename__ = "ophth_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ophth_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("ophth_orders.id"))
...
class OphthNote(Base):
    __tablename__ = "ophth_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ophth_encounters.id"))
