# filepath: 02_MODULES/DEP-016/21_backend_router.py
# FastAPI router for Ophthalmology (DEP-016)
...
from .schemas import *
from .service import OphthService

router = APIRouter(prefix="/api/ophth", tags=["Ophthalmology"])


@router.get("/list", response_model=List[OphthEncounter])
async def list_encounters(
    ...
    """List Ophthalmology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = OphthService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=OphthEncounter)
...
@router.post("/", response_model=OphthEncounter, status_code=201)
async def create_encounter(
    payload: OphthEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["ophth_specialist", "ophth_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = OphthService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=OphthEncounter)
async def update_encounter(
    encounter_id: int,
    payload: OphthEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["ophth_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = OphthService(db, tenant_id)
