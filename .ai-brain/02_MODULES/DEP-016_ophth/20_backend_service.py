# filepath: 02_MODULES/DEP-016/20_backend_service.py
# Business logic for Ophthalmology (DEP-016)
...
class OphthService:
    """Service layer for Ophthalmology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import OphthEncounter
        q = self.db.query(OphthEncounter).filter(
            OphthEncounter.tenant_id == self.tenant_id,
            OphthEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(OphthEncounter.patient_id == patient_id)
        if status:
            q = q.filter(OphthEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import OphthEncounter
        e = self.db.query(OphthEncounter).filter(
            OphthEncounter.id == encounter_id,
            OphthEncounter.tenant_id == self.tenant_id
        ).first()
