// filepath: 02_MODULES/DEP-045/22_frontend_page.tsx
// Main page for Psychology (DEP-045)
...
import { PsycholAPI } from './24_frontend_api_client';

interface PsycholPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function PsycholPage({ tenantId, patientId, lang = 'ar' }: PsycholPageProps) {
  const { t } = useTranslation('psychol');
  ...
  useEffect(() => {
    PsycholAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PsycholAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PsycholAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="BDI" patientId={patientId} />
        </>
      }
      main={
        <>
