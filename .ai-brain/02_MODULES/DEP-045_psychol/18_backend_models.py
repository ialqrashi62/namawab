# filepath: 02_MODULES/DEP-045/18_backend_models.py
# SQLAlchemy models for Psychology (DEP-045)
...
class PsycholEncounter(Base):
    __tablename__ = "psychol_encounters"
...
class PsycholOrder(Base):
    __tablename__ = "psychol_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("psychol_encounters.id"))
...
class PsycholResult(Base):
    __tablename__ = "psychol_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("psychol_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("psychol_orders.id"))
...
class PsycholNote(Base):
    __tablename__ = "psychol_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("psychol_encounters.id"))
