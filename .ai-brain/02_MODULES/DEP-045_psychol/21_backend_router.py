# filepath: 02_MODULES/DEP-045/21_backend_router.py
# FastAPI router for Psychology (DEP-045)
...
from .schemas import *
from .service import PsycholService

router = APIRouter(prefix="/api/psychol", tags=["Psychology"])


@router.get("/list", response_model=List[PsycholEncounter])
async def list_encounters(
    ...
    """List Psychology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PsycholService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PsycholEncounter)
...
@router.post("/", response_model=PsycholEncounter, status_code=201)
async def create_encounter(
    payload: PsycholEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["psychol_specialist", "psychol_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PsycholService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PsycholEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PsycholEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["psychol_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PsycholService(db, tenant_id)
