# filepath: 02_MODULES/DEP-045/20_backend_service.py
# Business logic for Psychology (DEP-045)
...
class PsycholService:
    """Service layer for Psychology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PsycholEncounter
        q = self.db.query(PsycholEncounter).filter(
            PsycholEncounter.tenant_id == self.tenant_id,
            PsycholEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PsycholEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PsycholEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PsycholEncounter
        e = self.db.query(PsycholEncounter).filter(
            PsycholEncounter.id == encounter_id,
            PsycholEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import PsycholOrder
        o = PsycholOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
