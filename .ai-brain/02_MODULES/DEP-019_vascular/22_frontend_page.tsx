// filepath: 02_MODULES/DEP-019/22_frontend_page.tsx
// Main page for Vascular_Surgery (DEP-019)
...
import { VascularAPI } from './24_frontend_api_client';

interface VascularPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function VascularPage({ tenantId, patientId, lang = 'ar' }: VascularPageProps) {
  const { t } = useTranslation('vascular');
  ...
  useEffect(() => {
    VascularAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    VascularAPI.listOrders({ patientId, tenantId }).then(setOrders);
    VascularAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="جراحة الأوعية"
      title_en="Vascular_Surgery"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="ABI" patientId={patientId} />
        </>
      }
      main={
        <>
