# filepath: 02_MODULES/DEP-019/21_backend_router.py
# FastAPI router for Vascular_Surgery (DEP-019)
...
from .schemas import *
from .service import VascularService

router = APIRouter(prefix="/api/vascular", tags=["Vascular_Surgery"])


@router.get("/list", response_model=List[VascularEncounter])
async def list_encounters(
    ...
    """List Vascular_Surgery encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = VascularService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=VascularEncounter)
...
@router.post("/", response_model=VascularEncounter, status_code=201)
async def create_encounter(
    payload: VascularEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["vascular_specialist", "vascular_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = VascularService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=VascularEncounter)
async def update_encounter(
    encounter_id: int,
    payload: VascularEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["vascular_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = VascularService(db, tenant_id)
