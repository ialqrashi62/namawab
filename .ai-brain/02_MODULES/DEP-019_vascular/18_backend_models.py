# filepath: 02_MODULES/DEP-019/18_backend_models.py
# SQLAlchemy models for Vascular_Surgery (DEP-019)
...
class VascularEncounter(Base):
    __tablename__ = "vascular_encounters"
...
class VascularOrder(Base):
    __tablename__ = "vascular_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("vascular_encounters.id"))
...
class VascularResult(Base):
    __tablename__ = "vascular_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("vascular_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("vascular_orders.id"))
...
class VascularNote(Base):
    __tablename__ = "vascular_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("vascular_encounters.id"))
