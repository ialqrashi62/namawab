# filepath: 02_MODULES/DEP-019/20_backend_service.py
# Business logic for Vascular_Surgery (DEP-019)
...
class VascularService:
    """Service layer for Vascular_Surgery.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import VascularEncounter
        q = self.db.query(VascularEncounter).filter(
            VascularEncounter.tenant_id == self.tenant_id,
            VascularEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(VascularEncounter.patient_id == patient_id)
        if status:
            q = q.filter(VascularEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import VascularEncounter
        e = self.db.query(VascularEncounter).filter(
            VascularEncounter.id == encounter_id,
            VascularEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import VascularOrder
        o = VascularOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
