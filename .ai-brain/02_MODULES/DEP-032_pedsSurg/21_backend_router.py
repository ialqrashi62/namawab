# filepath: 02_MODULES/DEP-032/21_backend_router.py
# FastAPI router for Pediatric_Surgery (DEP-032)
...
from .schemas import *
from .service import PedssurgService

router = APIRouter(prefix="/api/pedsSurg", tags=["Pediatric_Surgery"])


@router.get("/list", response_model=List[PedssurgEncounter])
async def list_encounters(
    ...
    """List Pediatric_Surgery encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PedssurgService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PedssurgEncounter)
...
@router.post("/", response_model=PedssurgEncounter, status_code=201)
async def create_encounter(
    payload: PedssurgEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsSurg_specialist", "pedsSurg_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedssurgService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PedssurgEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PedssurgEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsSurg_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedssurgService(db, tenant_id)
