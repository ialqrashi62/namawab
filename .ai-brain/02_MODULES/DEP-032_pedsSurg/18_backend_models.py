# filepath: 02_MODULES/DEP-032/18_backend_models.py
# SQLAlchemy models for Pediatric_Surgery (DEP-032)
...
class PedssurgEncounter(Base):
    __tablename__ = "pedsSurg_encounters"
...
class PedssurgOrder(Base):
    __tablename__ = "pedsSurg_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsSurg_encounters.id"))
...
class PedssurgResult(Base):
    __tablename__ = "pedsSurg_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsSurg_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("pedsSurg_orders.id"))
...
class PedssurgNote(Base):
    __tablename__ = "pedsSurg_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsSurg_encounters.id"))
