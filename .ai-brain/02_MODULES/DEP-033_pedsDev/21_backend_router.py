# filepath: 02_MODULES/DEP-033/21_backend_router.py
# FastAPI router for Pediatric_Development_Rehab (DEP-033)
...
from .schemas import *
from .service import PedsdevService

router = APIRouter(prefix="/api/pedsDev", tags=["Pediatric_Development_Rehab"])


@router.get("/list", response_model=List[PedsdevEncounter])
async def list_encounters(
    ...
    """List Pediatric_Development_Rehab encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PedsdevService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PedsdevEncounter)
...
@router.post("/", response_model=PedsdevEncounter, status_code=201)
async def create_encounter(
    payload: PedsdevEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsDev_specialist", "pedsDev_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedsdevService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PedsdevEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PedsdevEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsDev_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedsdevService(db, tenant_id)
