# filepath: 02_MODULES/DEP-033/20_backend_service.py
# Business logic for Pediatric_Development_Rehab (DEP-033)
...
class PedsdevService:
    """Service layer for Pediatric_Development_Rehab.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PedsdevEncounter
        q = self.db.query(PedsdevEncounter).filter(
            PedsdevEncounter.tenant_id == self.tenant_id,
            PedsdevEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PedsdevEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PedsdevEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PedsdevEncounter
        e = self.db.query(PedsdevEncounter).filter(
            PedsdevEncounter.id == encounter_id,
            PedsdevEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import PedsdevOrder
        o = PedsdevOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
