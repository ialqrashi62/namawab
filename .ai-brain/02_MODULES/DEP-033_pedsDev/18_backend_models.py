# filepath: 02_MODULES/DEP-033/18_backend_models.py
# SQLAlchemy models for Pediatric_Development_Rehab (DEP-033)
...
class PedsdevEncounter(Base):
    __tablename__ = "pedsDev_encounters"
...
class PedsdevOrder(Base):
    __tablename__ = "pedsDev_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsDev_encounters.id"))
...
class PedsdevResult(Base):
    __tablename__ = "pedsDev_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsDev_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("pedsDev_orders.id"))
...
class PedsdevNote(Base):
    __tablename__ = "pedsDev_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsDev_encounters.id"))
