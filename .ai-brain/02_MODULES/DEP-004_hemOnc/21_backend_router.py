# filepath: 02_MODULES/DEP-004/21_backend_router.py
# FastAPI router for Hematology_Oncology (DEP-004)
...
from .schemas import *
from .service import HemoncService

router = APIRouter(prefix="/api/hemOnc", tags=["Hematology_Oncology"])


@router.get("/list", response_model=List[HemoncEncounter])
async def list_encounters(
    ...
    """List Hematology_Oncology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = HemoncService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=HemoncEncounter)
...
@router.post("/", response_model=HemoncEncounter, status_code=201)
async def create_encounter(
    payload: HemoncEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["hemOnc_specialist", "hemOnc_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = HemoncService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=HemoncEncounter)
async def update_encounter(
    encounter_id: int,
    payload: HemoncEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["hemOnc_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = HemoncService(db, tenant_id)
