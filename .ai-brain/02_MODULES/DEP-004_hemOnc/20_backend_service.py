# filepath: 02_MODULES/DEP-004/20_backend_service.py
# Business logic for Hematology_Oncology (DEP-004)
...
class HemoncService:
    """Service layer for Hematology_Oncology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import HemoncEncounter
        q = self.db.query(HemoncEncounter).filter(
            HemoncEncounter.tenant_id == self.tenant_id,
            HemoncEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(HemoncEncounter.patient_id == patient_id)
        if status:
            q = q.filter(HemoncEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import HemoncEncounter
        e = self.db.query(HemoncEncounter).filter(
            HemoncEncounter.id == encounter_id,
            HemoncEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import HemoncOrder
        o = HemoncOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
