# filepath: 02_MODULES/DEP-004/18_backend_models.py
# SQLAlchemy models for Hematology_Oncology (DEP-004)
...
class HemoncEncounter(Base):
    __tablename__ = "hemOnc_encounters"
...
class HemoncOrder(Base):
    __tablename__ = "hemOnc_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("hemOnc_encounters.id"))
...
class HemoncResult(Base):
    __tablename__ = "hemOnc_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("hemOnc_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("hemOnc_orders.id"))
...
class HemoncNote(Base):
    __tablename__ = "hemOnc_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("hemOnc_encounters.id"))
