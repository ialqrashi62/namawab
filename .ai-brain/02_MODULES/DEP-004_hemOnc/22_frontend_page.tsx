// filepath: 02_MODULES/DEP-004/22_frontend_page.tsx
// Main page for Hematology_Oncology (DEP-004)
...
import { HemoncAPI } from './24_frontend_api_client';

interface HemoncPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function HemoncPage({ tenantId, patientId, lang = 'ar' }: HemoncPageProps) {
  const { t } = useTranslation('hemOnc');
  ...
  useEffect(() => {
    HemoncAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    HemoncAPI.listOrders({ patientId, tenantId }).then(setOrders);
    HemoncAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="أمراض الدم والأورام"
      title_en="Hematology_Oncology"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="NCCN_risk" patientId={patientId} />
        </>
      }
      main={
        <>
