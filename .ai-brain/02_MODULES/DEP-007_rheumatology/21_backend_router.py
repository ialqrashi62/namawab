# filepath: 02_MODULES/DEP-007/21_backend_router.py
# FastAPI router for Rheumatology (DEP-007)
...
from .schemas import *
from .service import RheumatologyService

router = APIRouter(prefix="/api/rheumatology", tags=["Rheumatology"])


@router.get("/list", response_model=List[RheumatologyEncounter])
async def list_encounters(
    ...
    """List Rheumatology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = RheumatologyService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=RheumatologyEncounter)
...
@router.post("/", response_model=RheumatologyEncounter, status_code=201)
async def create_encounter(
    payload: RheumatologyEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["rheumatology_specialist", "rheumatology_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = RheumatologyService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=RheumatologyEncounter)
async def update_encounter(
    encounter_id: int,
    payload: RheumatologyEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["rheumatology_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = RheumatologyService(db, tenant_id)
