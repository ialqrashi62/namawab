# filepath: 02_MODULES/DEP-007/17_rag_pipeline.py
# Rheumatology (DEP-007) RAG Pipeline - Generated 2026-08-08
...
CHROMA_PERSIST_DIR = os.environ.get("CHROMA_PERSIST_DIR", "./chroma_rheumatology")
...
# ============ System Prompt ============
SYSTEM_PROMPT = "الروماتيزم expert at CBAHI/JCI hospital. Use ONLY context. Cite ICD-10/SNOMED. If insufficient: say so. Bilingual (AR primary). Consider Saudi epidemiology."
...
def get_pg_vectordb(tenant_id, collection_prefix=""):
    collection = f"{tenant_id}_rheumatology" if not collection_prefix else f"{collection_prefix}_rheumatology"
...
def build_diagnosis_agent(tenant_id, use_chroma=False):
    ...
    guidelines_tool = create_retriever_tool(retriever, name="rheumatology_guidelines_search", description="Search Rheumatology guidelines.")
...
def ingest_chunks(chunks, tenant_id, batch_size=100):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        metas = [{**c.get('meta', {}), 'dept': 'rheumatology', 'tenant_id': tenant_id} for c in batch]
        vectordb.add_texts(texts=texts, metadatas=metas)
        total += len(batch)
    logger.info(f"[rheumatology] Ingested {total} chunks for tenant {tenant_id}")
