# filepath: 02_MODULES/DEP-007/20_backend_service.py
# Business logic for Rheumatology (DEP-007)
...
class RheumatologyService:
    """Service layer for Rheumatology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import RheumatologyEncounter
        q = self.db.query(RheumatologyEncounter).filter(
            RheumatologyEncounter.tenant_id == self.tenant_id,
            RheumatologyEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(RheumatologyEncounter.patient_id == patient_id)
        if status:
            q = q.filter(RheumatologyEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import RheumatologyEncounter
        e = self.db.query(RheumatologyEncounter).filter(
            RheumatologyEncounter.id == encounter_id,
            RheumatologyEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import RheumatologyOrder
        o = RheumatologyOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
