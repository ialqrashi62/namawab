# filepath: 02_MODULES/DEP-030/17_rag_pipeline.py
# Pediatric_Nephrology (DEP-030) RAG Pipeline - Generated 2026-08-08
...
# ============ Configuration ============
COLLECTION_NAME = "pedsNeph"
PGVECTOR_URL = os.environ.get("PGVECTOR_URL", "postgresql://user:pass@localhost:5432/nama")
CHROMA_PERSIST_DIR = os.environ.get("CHROMA_PERSIST_DIR", "./chroma_pedsNeph")
...
# ============ System Prompt ============
SYSTEM_PROMPT = "كلى الأطفال expert at CBAHI/JCI hospital. Use ONLY context. Cite ICD-10/SNOMED. If insufficient: say so. Bilingual (AR primary). Consider Saudi epidemiology."
...
def get_pg_vectordb(tenant_id, collection_prefix=""):
    collection = f"{tenant_id}_pedsNeph" if not collection_prefix else f"{collection_prefix}_pedsNeph"
...
def build_diagnosis_agent(tenant_id, use_chroma=False):
    ...
    guidelines_tool = create_retriever_tool(retriever, name="pedsNeph_guidelines_search", description="Search Pediatric_Nephrology guidelines.")
...
def ingest_chunks(chunks, tenant_id, batch_size=100):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        metas = [{**c.get('meta', {}), 'dept': 'pedsNeph', 'tenant_id': tenant_id} for c in batch]
        vectordb.add_texts(texts=texts, metadatas=metas)
        total += len(batch)
    logger.info(f"[pedsNeph] Ingested {total} chunks for tenant {tenant_id}")
