# filepath: 02_MODULES/DEP-030/20_backend_service.py
# Business logic for Pediatric_Nephrology (DEP-030)
...
class PedsnephService:
    """Service layer for Pediatric_Nephrology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PedsnephEncounter
        q = self.db.query(PedsnephEncounter).filter(
            PedsnephEncounter.tenant_id == self.tenant_id,
            PedsnephEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PedsnephEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PedsnephEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PedsnephEncounter
        e = self.db.query(PedsnephEncounter).filter(
            PedsnephEncounter.id == encounter_id,
            PedsnephEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import PedsnephOrder
        o = PedsnephOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
