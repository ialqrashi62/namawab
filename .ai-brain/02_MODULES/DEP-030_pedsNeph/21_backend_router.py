# filepath: 02_MODULES/DEP-030/21_backend_router.py
# FastAPI router for Pediatric_Nephrology (DEP-030)
...
from .schemas import *
from .service import PedsnephService

router = APIRouter(prefix="/api/pedsNeph", tags=["Pediatric_Nephrology"])


@router.get("/list", response_model=List[PedsnephEncounter])
async def list_encounters(
    ...
    """List Pediatric_Nephrology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PedsnephService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PedsnephEncounter)
...
@router.post("/", response_model=PedsnephEncounter, status_code=201)
async def create_encounter(
    payload: PedsnephEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsNeph_specialist", "pedsNeph_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedsnephService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PedsnephEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PedsnephEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsNeph_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedsnephService(db, tenant_id)
