# filepath: 02_MODULES/DEP-030/18_backend_models.py
# SQLAlchemy models for Pediatric_Nephrology (DEP-030)
...
class PedsnephEncounter(Base):
    __tablename__ = "pedsNeph_encounters"
...
class PedsnephOrder(Base):
    __tablename__ = "pedsNeph_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsNeph_encounters.id"))
...
class PedsnephResult(Base):
    __tablename__ = "pedsNeph_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsNeph_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("pedsNeph_orders.id"))
...
class PedsnephNote(Base):
    __tablename__ = "pedsNeph_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsNeph_encounters.id"))
