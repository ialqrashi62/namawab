// filepath: 02_MODULES/DEP-030/22_frontend_page.tsx
// Main page for Pediatric_Nephrology (DEP-030)
...
import { PedsnephAPI } from './24_frontend_api_client';

interface PedsnephPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function PedsnephPage({ tenantId, patientId, lang = 'ar' }: PedsnephPageProps) {
  const { t } = useTranslation('pedsNeph');
  ...
  useEffect(() => {
    PedsnephAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PedsnephAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PedsnephAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="كلى الأطفال"
      title_en="Pediatric_Nephrology"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="Schwartz_formula" patientId={patientId} />
        </>
      }
      main={
        <>
