# filepath: 02_MODULES/DEP-026/29_test_integration.py
# Integration tests for General_Pediatrics (DEP-026)
...
class TestPedsAPI:
    ...
    def test_create_requires_role(self, client, db):
        ...
        res = client.post("/api/peds/", json={"patient_id": 1}, headers=headers)
    ...
    def test_create_success(self, client, auth_headers, db):
        ...
        res = client.post("/api/peds/", json={"patient_id": 1, "chief_complaint": "Test"}, headers=auth_headers)
    ...
    def test_ai_diagnose(self, client, auth_headers):
        with patch("rag_pipeline.build_diagnosis_agent") as mock:
            ...
            res = client.post("/api/peds/ai/diagnose", json={"question": "Chest pain"}, headers=auth_headers)
            assert res.status_code == 200