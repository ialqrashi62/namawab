# filepath: 02_MODULES/DEP-026/18_backend_models.py
# SQLAlchemy models for General_Pediatrics (DEP-026)
...
class PedsEncounter(Base):
    __tablename__ = "peds_encounters"
...
class PedsOrder(Base):
    __tablename__ = "peds_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("peds_encounters.id"))
...
class PedsResult(Base):
    __tablename__ = "peds_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("peds_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("peds_orders.id"))
