// filepath: 02_MODULES/DEP-026/22_frontend_page.tsx
// Main page for General_Pediatrics (DEP-026)
...
import { PedsAPI } from './24_frontend_api_client';

interface PedsPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function PedsPage({ tenantId, patientId, lang = 'ar' }: PedsPageProps) {
  const { t } = useTranslation('peds');
  ...
  useEffect(() => {
    PedsAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PedsAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PedsAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="طب الأطفال"
      title_en="General_Pediatrics"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="BMI_percentile" patientId={patientId} />
        </>
      }
      main={
        <>
