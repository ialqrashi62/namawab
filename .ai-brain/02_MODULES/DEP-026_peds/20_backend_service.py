# filepath: 02_MODULES/DEP-026/20_backend_service.py
# Business logic for General_Pediatrics (DEP-026)
...
class PedsService:
    """Service layer for General_Pediatrics.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PedsEncounter
        q = self.db.query(PedsEncounter).filter(
            PedsEncounter.tenant_id == self.tenant_id,
            PedsEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PedsEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PedsEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PedsEncounter
        e = self.db.query(PedsEncounter).filter(
            PedsEncounter.id == encounter_id,
            PedsEncounter.tenant_id == self.tenant_id
        ).first()
