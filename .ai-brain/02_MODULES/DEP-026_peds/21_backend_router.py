# filepath: 02_MODULES/DEP-026/21_backend_router.py
# FastAPI router for General_Pediatrics (DEP-026)
...
from .schemas import *
from .service import PedsService

router = APIRouter(prefix="/api/peds", tags=["General_Pediatrics"])


@router.get("/list", response_model=List[PedsEncounter])
async def list_encounters(
    ...
    """List General_Pediatrics encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PedsService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PedsEncounter)
...
@router.post("/", response_model=PedsEncounter, status_code=201)
async def create_encounter(
    payload: PedsEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["peds_specialist", "peds_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedsService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PedsEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PedsEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["peds_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedsService(db, tenant_id)
