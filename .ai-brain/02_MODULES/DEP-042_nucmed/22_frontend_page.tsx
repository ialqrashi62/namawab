// filepath: 02_MODULES/DEP-042/22_frontend_page.tsx
// Main page for Nuclear_Medicine (DEP-042)
...
import { NucmedAPI } from './24_frontend_api_client';

interface NucmedPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function NucmedPage({ tenantId, patientId, lang = 'ar' }: NucmedPageProps) {
  const { t } = useTranslation('nucmed');
  ...
  useEffect(() => {
    NucmedAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    NucmedAPI.listOrders({ patientId, tenantId }).then(setOrders);
    NucmedAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="الطب النووي"
      title_en="Nuclear_Medicine"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="Deauville_criteria" patientId={patientId} />
        </>
      }
      main={
        <>
