# filepath: 02_MODULES/DEP-042/20_backend_service.py
# Business logic for Nuclear_Medicine (DEP-042)
...
class NucmedService:
    """Service layer for Nuclear_Medicine.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import NucmedEncounter
        q = self.db.query(NucmedEncounter).filter(
            NucmedEncounter.tenant_id == self.tenant_id,
            NucmedEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(NucmedEncounter.patient_id == patient_id)
        if status:
            q = q.filter(NucmedEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import NucmedEncounter
        e = self.db.query(NucmedEncounter).filter(
            NucmedEncounter.id == encounter_id,
            NucmedEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import NucmedOrder
        o = NucmedOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
