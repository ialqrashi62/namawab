# filepath: 02_MODULES/DEP-042/21_backend_router.py
# FastAPI router for Nuclear_Medicine (DEP-042)
...
from .schemas import *
from .service import NucmedService

router = APIRouter(prefix="/api/nucmed", tags=["Nuclear_Medicine"])


@router.get("/list", response_model=List[NucmedEncounter])
async def list_encounters(
    ...
    """List Nuclear_Medicine encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = NucmedService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=NucmedEncounter)
...
@router.post("/", response_model=NucmedEncounter, status_code=201)
async def create_encounter(
    payload: NucmedEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["nucmed_specialist", "nucmed_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = NucmedService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=NucmedEncounter)
async def update_encounter(
    encounter_id: int,
    payload: NucmedEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["nucmed_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = NucmedService(db, tenant_id)
