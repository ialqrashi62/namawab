# filepath: 02_MODULES/DEP-046/17_rag_pipeline.py
# Physical_Therapy_Rehab (DEP-046) RAG Pipeline - Generated 2026-08-08
...
CHROMA_PERSIST_DIR = os.environ.get("CHROMA_PERSIST_DIR", "./chroma_pt")
...
# ============ System Prompt ============
SYSTEM_PROMPT = "العلاج الطبيعي expert at CBAHI/JCI hospital. Use ONLY context. Cite ICD-10/SNOMED. If insufficient: say so. Bilingual (AR primary). Consider Saudi epidemiology."
...
def get_pg_vectordb(tenant_id, collection_prefix=""):
    collection = f"{tenant_id}_pt" if not collection_prefix else f"{collection_prefix}_pt"
...
def build_diagnosis_agent(tenant_id, use_chroma=False):
    ...
    guidelines_tool = create_retriever_tool(retriever, name="pt_guidelines_search", description="Search Physical_Therapy_Rehab guidelines.")
...
def ingest_chunks(chunks, tenant_id, batch_size=100):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        metas = [{**c.get('meta', {}), 'dept': 'pt', 'tenant_id': tenant_id} for c in batch]
        vectordb.add_texts(texts=texts, metadatas=metas)
        total += len(batch)
    logger.info(f"[pt] Ingested {total} chunks for tenant {tenant_id}")
