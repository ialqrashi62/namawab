# filepath: 02_MODULES/DEP-046/21_backend_router.py
# FastAPI router for Physical_Therapy_Rehab (DEP-046)
...
router = APIRouter(prefix="/api/pt", tags=["Physical_Therapy_Rehab"])


@router.get("/list", response_model=List[PtEncounter])
async def list_encounters(
    ...
    """List Physical_Therapy_Rehab encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PtService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PtEncounter)
...
@router.post("/", response_model=PtEncounter, status_code=201)
async def create_encounter(
    payload: PtEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pt_specialist", "pt_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PtService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PtEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PtEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pt_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PtService(db, tenant_id)
