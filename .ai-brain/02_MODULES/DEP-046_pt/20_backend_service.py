# filepath: 02_MODULES/DEP-046/20_backend_service.py
# Business logic for Physical_Therapy_Rehab (DEP-046)
...
class PtService:
    """Service layer for Physical_Therapy_Rehab.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PtEncounter
        q = self.db.query(PtEncounter).filter(
            PtEncounter.tenant_id == self.tenant_id,
            PtEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PtEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PtEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PtEncounter
        e = self.db.query(PtEncounter).filter(
            PtEncounter.id == encounter_id,
            PtEncounter.tenant_id == self.tenant_id
        ).first()
