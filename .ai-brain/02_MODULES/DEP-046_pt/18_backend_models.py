# filepath: 02_MODULES/DEP-046/18_backend_models.py
# SQLAlchemy models for Physical_Therapy_Rehab (DEP-046)
...
class PtEncounter(Base):
    __tablename__ = "pt_encounters"
...
class PtOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pt_encounters.id"))
...
class PtResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pt_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("pt_orders.id"))
