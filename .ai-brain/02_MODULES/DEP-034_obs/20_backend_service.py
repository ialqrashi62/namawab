# filepath: 02_MODULES/DEP-034/20_backend_service.py
# Business logic for Obstetrics (DEP-034)
...
class ObsService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import ObsEncounter
        q = self.db.query(ObsEncounter).filter(
            ObsEncounter.tenant_id == self.tenant_id,
            ObsEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(ObsEncounter.patient_id == patient_id)
        if status:
            q = q.filter(ObsEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import ObsEncounter
        e = self.db.query(ObsEncounter).filter(
            ObsEncounter.id == encounter_id,
            ObsEncounter.tenant_id == self.tenant_id
        ).first()
