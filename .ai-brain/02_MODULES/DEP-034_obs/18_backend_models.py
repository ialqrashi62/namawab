# filepath: 02_MODULES/DEP-034/18_backend_models.py
# SQLAlchemy models for Obstetrics (DEP-034)
...
class ObsEncounter(Base):
    __tablename__ = "obs_encounters"
...
class ObsOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("obs_encounters.id"))
...
class ObsResult(Base):
    __tablename__ = "obs_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("obs_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("obs_orders.id"))
