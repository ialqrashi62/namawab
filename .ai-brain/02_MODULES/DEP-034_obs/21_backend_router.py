# filepath: 02_MODULES/DEP-034/21_backend_router.py
# FastAPI router for Obstetrics (DEP-034)
...
from .schemas import *
from .service import ObsService

router = APIRouter(prefix="/api/obs", tags=["Obstetrics"])


@router.get("/list", response_model=List[ObsEncounter])
async def list_encounters(
    ...
    """List Obstetrics encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = ObsService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=ObsEncounter)
...
@router.post("/", response_model=ObsEncounter, status_code=201)
async def create_encounter(
    payload: ObsEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["obs_specialist", "obs_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = ObsService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=ObsEncounter)
async def update_encounter(
    encounter_id: int,
    payload: ObsEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["obs_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = ObsService(db, tenant_id)
