// filepath: 02_MODULES/DEP-034/22_frontend_page.tsx
// Main page for Obstetrics (DEP-034)
...
import { ObsAPI } from './24_frontend_api_client';
...
export default function ObsPage({ tenantId, patientId, lang = 'ar' }: ObsPageProps) {
  ...
  useEffect(() => {
    ObsAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    ObsAPI.listOrders({ patientId, tenantId }).then(setOrders);
    ObsAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="Bishop" patientId={patientId} />
        </>
      }
      main={
        <>
