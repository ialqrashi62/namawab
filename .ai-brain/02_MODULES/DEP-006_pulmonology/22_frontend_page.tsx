// filepath: 02_MODULES/DEP-006/22_frontend_page.tsx
// Main page for Pulmonology (DEP-006)
...
import { PulmonologyAPI } from './24_frontend_api_client';

interface PulmonologyPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function PulmonologyPage({ tenantId, patientId, lang = 'ar' }: PulmonologyPageProps) {
  const { t } = useTranslation('pulmonology');
  ...
  useEffect(() => {
    PulmonologyAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PulmonologyAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PulmonologyAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="أمراض الصدرية"
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="BODE" patientId={patientId} />
        </>
      }
      main={
        <>
