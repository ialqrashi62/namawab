# filepath: 02_MODULES/DEP-006/20_backend_service.py
# Business logic for Pulmonology (DEP-006)
...
class PulmonologyService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PulmonologyEncounter
        q = self.db.query(PulmonologyEncounter).filter(
            PulmonologyEncounter.tenant_id == self.tenant_id,
            PulmonologyEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PulmonologyEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PulmonologyEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PulmonologyEncounter
        e = self.db.query(PulmonologyEncounter).filter(
            PulmonologyEncounter.id == encounter_id,
            PulmonologyEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import PulmonologyOrder
        o = PulmonologyOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
