# filepath: 02_MODULES/DEP-006/29_test_integration.py
# Integration tests for Pulmonology (DEP-006)
...
@pytest.fixture
def auth_headers():
    token = create_test_token(user_id=1, tenant_id=1, roles=["pulmonology_specialist"])
    return {"Authorization": f"Bearer {token}", "x-tenant-id": "1"}


class TestPulmonologyAPI:
    def test_list_requires_tenant(self, client):
        res = client.get("/api/pulmonology/list")
        assert res.status_code == 400  # Missing tenant header

    def test_list_with_tenant(self, client, auth_headers):
        res = client.get("/api/pulmonology/list", headers=auth_headers)
    ...
    def test_create_requires_role(self, client, db):
        ...
        res = client.post("/api/pulmonology/", json={"patient_id": 1}, headers=headers)
    ...
    def test_create_success(self, client, auth_headers, db):
        ...
        res = client.post("/api/pulmonology/", json={"patient_id": 1, "chief_complaint": "Test"}, headers=auth_headers)
    ...
    def test_ai_diagnose(self, client, auth_headers):
        with patch("rag_pipeline.build_diagnosis_agent") as mock:
            ...
            res = client.post("/api/pulmonology/ai/diagnose", json={"question": "Chest pain"}, headers=auth_headers)
            assert res.status_code == 200