# Implementation Plan — Pulmonology (DEP-006)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- pulmonology_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh pulmonology
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-pulmonology-<timestamp>`
