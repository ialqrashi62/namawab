# filepath: 02_MODULES/DEP-006/18_backend_models.py
# SQLAlchemy models for Pulmonology (DEP-006)
...
class PulmonologyEncounter(Base):
    __tablename__ = "pulmonology_encounters"
...
class PulmonologyOrder(Base):
    __tablename__ = "pulmonology_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pulmonology_encounters.id"))
...
class PulmonologyResult(Base):
    __tablename__ = "pulmonology_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pulmonology_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("pulmonology_orders.id"))
...
class PulmonologyNote(Base):
    __tablename__ = "pulmonology_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pulmonology_encounters.id"))
