# filepath: 02_MODULES/DEP-006/21_backend_router.py
# FastAPI router for Pulmonology (DEP-006)
...
from .schemas import *
from .service import PulmonologyService

router = APIRouter(prefix="/api/pulmonology", tags=["Pulmonology"])


@router.get("/list", response_model=List[PulmonologyEncounter])
async def list_encounters(
    ...
    """List Pulmonology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PulmonologyService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PulmonologyEncounter)
...
@router.post("/", response_model=PulmonologyEncounter, status_code=201)
async def create_encounter(
    payload: PulmonologyEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pulmonology_specialist", "pulmonology_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PulmonologyService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PulmonologyEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PulmonologyEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pulmonology_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PulmonologyService(db, tenant_id)
