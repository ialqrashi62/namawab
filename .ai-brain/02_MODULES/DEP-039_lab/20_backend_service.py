# filepath: 02_MODULES/DEP-039/20_backend_service.py
# Business logic for Laboratory (DEP-039)
...
class LabService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import LabEncounter
        q = self.db.query(LabEncounter).filter(
            LabEncounter.tenant_id == self.tenant_id,
            LabEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(LabEncounter.patient_id == patient_id)
        if status:
            q = q.filter(LabEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import LabEncounter
        e = self.db.query(LabEncounter).filter(
            LabEncounter.id == encounter_id,
            LabEncounter.tenant_id == self.tenant_id
        ).first()
