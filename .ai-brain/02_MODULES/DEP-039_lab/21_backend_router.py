# filepath: 02_MODULES/DEP-039/21_backend_router.py
# FastAPI router for Laboratory (DEP-039)
...
@router.get("/list", response_model=List[LabEncounter])
async def list_encounters(
    ...
    """List Laboratory encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = LabService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=LabEncounter)
...
@router.post("/", response_model=LabEncounter, status_code=201)
async def create_encounter(
    payload: LabEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["lab_specialist", "lab_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = LabService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=LabEncounter)
async def update_encounter(
    encounter_id: int,
    payload: LabEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["lab_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = LabService(db, tenant_id)
