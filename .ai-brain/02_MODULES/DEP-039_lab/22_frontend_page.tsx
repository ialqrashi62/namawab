// filepath: 02_MODULES/DEP-039/22_frontend_page.tsx
// Main page for Laboratory (DEP-039)
...
import { LabAPI } from './24_frontend_api_client';
...
export default function LabPage({ tenantId, patientId, lang = 'ar' }: LabPageProps) {
  ...
  useEffect(() => {
    LabAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    LabAPI.listOrders({ patientId, tenantId }).then(setOrders);
    LabAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="critical_values_protocol" patientId={patientId} />
        </>
      }
      main={
        <>
