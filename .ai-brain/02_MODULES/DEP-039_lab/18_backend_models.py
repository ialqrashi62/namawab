# filepath: 02_MODULES/DEP-039/18_backend_models.py
# SQLAlchemy models for Laboratory (DEP-039)
...
class LabEncounter(Base):
    __tablename__ = "lab_encounters"
...
class LabOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("lab_encounters.id"))
...
class LabResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("lab_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("lab_orders.id"))
