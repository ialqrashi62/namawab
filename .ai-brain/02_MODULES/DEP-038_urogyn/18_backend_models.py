# filepath: 02_MODULES/DEP-038/18_backend_models.py
# SQLAlchemy models for Urogynecology (DEP-038)
...
class UrogynEncounter(Base):
    __tablename__ = "urogyn_encounters"
...
class UrogynOrder(Base):
    __tablename__ = "urogyn_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("urogyn_encounters.id"))
...
class UrogynResult(Base):
    __tablename__ = "urogyn_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("urogyn_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("urogyn_orders.id"))
...
class UrogynNote(Base):
    __tablename__ = "urogyn_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("urogyn_encounters.id"))
