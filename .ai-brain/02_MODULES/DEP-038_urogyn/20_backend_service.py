# filepath: 02_MODULES/DEP-038/20_backend_service.py
# Business logic for Urogynecology (DEP-038)
...
class UrogynService:
    """Service layer for Urogynecology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import UrogynEncounter
        q = self.db.query(UrogynEncounter).filter(
            UrogynEncounter.tenant_id == self.tenant_id,
            UrogynEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(UrogynEncounter.patient_id == patient_id)
        if status:
            q = q.filter(UrogynEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import UrogynEncounter
        e = self.db.query(UrogynEncounter).filter(
            UrogynEncounter.id == encounter_id,
            UrogynEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import UrogynOrder
        o = UrogynOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
