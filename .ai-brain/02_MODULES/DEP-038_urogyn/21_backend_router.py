# filepath: 02_MODULES/DEP-038/21_backend_router.py
# FastAPI router for Urogynecology (DEP-038)
...
from .schemas import *
from .service import UrogynService

router = APIRouter(prefix="/api/urogyn", tags=["Urogynecology"])


@router.get("/list", response_model=List[UrogynEncounter])
async def list_encounters(
    ...
    """List Urogynecology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = UrogynService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=UrogynEncounter)
...
@router.post("/", response_model=UrogynEncounter, status_code=201)
async def create_encounter(
    payload: UrogynEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["urogyn_specialist", "urogyn_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = UrogynService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=UrogynEncounter)
async def update_encounter(
    encounter_id: int,
    payload: UrogynEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["urogyn_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = UrogynService(db, tenant_id)
