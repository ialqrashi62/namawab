# filepath: 02_MODULES/DEP-002/21_backend_router.py
# FastAPI router for Endocrinology (DEP-002)
...
from .schemas import *
from .service import EndocrinologyService

router = APIRouter(prefix="/api/endocrinology", tags=["Endocrinology"])


@router.get("/list", response_model=List[EndocrinologyEncounter])
async def list_encounters(
    ...
    """List Endocrinology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = EndocrinologyService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=EndocrinologyEncounter)
...
@router.post("/", response_model=EndocrinologyEncounter, status_code=201)
async def create_encounter(
    payload: EndocrinologyEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["endocrinology_specialist", "endocrinology_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = EndocrinologyService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=EndocrinologyEncounter)
async def update_encounter(
    encounter_id: int,
    payload: EndocrinologyEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["endocrinology_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = EndocrinologyService(db, tenant_id)
