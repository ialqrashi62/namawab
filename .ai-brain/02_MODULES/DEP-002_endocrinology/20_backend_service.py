# filepath: 02_MODULES/DEP-002/20_backend_service.py
# Business logic for Endocrinology (DEP-002)
...
class EndocrinologyService:
    """Service layer for Endocrinology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import EndocrinologyEncounter
        q = self.db.query(EndocrinologyEncounter).filter(
            EndocrinologyEncounter.tenant_id == self.tenant_id,
            EndocrinologyEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(EndocrinologyEncounter.patient_id == patient_id)
        if status:
            q = q.filter(EndocrinologyEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import EndocrinologyEncounter
        e = self.db.query(EndocrinologyEncounter).filter(
            EndocrinologyEncounter.id == encounter_id,
            EndocrinologyEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import EndocrinologyOrder
        o = EndocrinologyOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
