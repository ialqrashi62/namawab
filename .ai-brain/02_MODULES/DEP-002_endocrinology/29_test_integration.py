# filepath: 02_MODULES/DEP-002/29_test_integration.py
# Integration tests for Endocrinology (DEP-002)
...
@pytest.fixture
def auth_headers():
    token = create_test_token(user_id=1, tenant_id=1, roles=["endocrinology_specialist"])
    return {"Authorization": f"Bearer {token}", "x-tenant-id": "1"}


class TestEndocrinologyAPI:
    def test_list_requires_tenant(self, client):
        res = client.get("/api/endocrinology/list")
        assert res.status_code == 400  # Missing tenant header

    def test_list_with_tenant(self, client, auth_headers):
        res = client.get("/api/endocrinology/list", headers=auth_headers)
    ...
    def test_create_requires_role(self, client, db):
        ...
        res = client.post("/api/endocrinology/", json={"patient_id": 1}, headers=headers)
    ...
    def test_create_success(self, client, auth_headers, db):
        ...
        res = client.post("/api/endocrinology/", json={"patient_id": 1, "chief_complaint": "Test"}, headers=auth_headers)
    ...
    def test_ai_diagnose(self, client, auth_headers):
        with patch("rag_pipeline.build_diagnosis_agent") as mock:
            ...
            res = client.post("/api/endocrinology/ai/diagnose", json={"question": "Chest pain"}, headers=auth_headers)
            assert res.status_code == 200