// filepath: 02_MODULES/DEP-002/22_frontend_page.tsx
// Main page for Endocrinology (DEP-002)
...
import { EndocrinologyAPI } from './24_frontend_api_client';

interface EndocrinologyPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function EndocrinologyPage({ tenantId, patientId, lang = 'ar' }: EndocrinologyPageProps) {
  const { t } = useTranslation('endocrinology');
  ...
  useEffect(() => {
    EndocrinologyAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    EndocrinologyAPI.listOrders({ patientId, tenantId }).then(setOrders);
    EndocrinologyAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="الغدد الصماء"
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="HOMA_IR" patientId={patientId} />
        </>
      }
      main={
        <>
