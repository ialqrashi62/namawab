# Implementation Plan — Nephrology (DEP-005)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- nephrology_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh nephrology
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-nephrology-<timestamp>`
