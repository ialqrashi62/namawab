# filepath: 02_MODULES/DEP-005/18_backend_models.py
# SQLAlchemy models for Nephrology (DEP-005)
...
class NephrologyEncounter(Base):
    __tablename__ = "nephrology_encounters"
...
class NephrologyOrder(Base):
    __tablename__ = "nephrology_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("nephrology_encounters.id"))
...
class NephrologyResult(Base):
    __tablename__ = "nephrology_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("nephrology_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("nephrology_orders.id"))
...
class NephrologyNote(Base):
    __tablename__ = "nephrology_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("nephrology_encounters.id"))
