# filepath: 02_MODULES/DEP-005/17_rag_pipeline.py
# Nephrology (DEP-005) RAG Pipeline - Generated 2026-08-08
...
CHROMA_PERSIST_DIR = os.environ.get("CHROMA_PERSIST_DIR", "./chroma_nephrology")
...
# ============ System Prompt ============
SYSTEM_PROMPT = "أمراض الكلى expert at CBAHI/JCI hospital. Use ONLY context. Cite ICD-10/SNOMED. If insufficient: say so. Bilingual (AR primary). Consider Saudi epidemiology."
...
def get_pg_vectordb(tenant_id, collection_prefix=""):
    collection = f"{tenant_id}_nephrology" if not collection_prefix else f"{collection_prefix}_nephrology"
...
def build_diagnosis_agent(tenant_id, use_chroma=False):
    ...
    guidelines_tool = create_retriever_tool(retriever, name="nephrology_guidelines_search", description="Search Nephrology guidelines.")
...
def ingest_chunks(chunks, tenant_id, batch_size=100):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        metas = [{**c.get('meta', {}), 'dept': 'nephrology', 'tenant_id': tenant_id} for c in batch]
        vectordb.add_texts(texts=texts, metadatas=metas)
        total += len(batch)
    logger.info(f"[nephrology] Ingested {total} chunks for tenant {tenant_id}")
