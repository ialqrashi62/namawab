# filepath: 02_MODULES/DEP-005/20_backend_service.py
# Business logic for Nephrology (DEP-005)
...
class NephrologyService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import NephrologyEncounter
        q = self.db.query(NephrologyEncounter).filter(
            NephrologyEncounter.tenant_id == self.tenant_id,
            NephrologyEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(NephrologyEncounter.patient_id == patient_id)
        if status:
            q = q.filter(NephrologyEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import NephrologyEncounter
        e = self.db.query(NephrologyEncounter).filter(
            NephrologyEncounter.id == encounter_id,
            NephrologyEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import NephrologyOrder
        o = NephrologyOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
