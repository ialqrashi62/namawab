# filepath: 02_MODULES/DEP-005/21_backend_router.py
# FastAPI router for Nephrology (DEP-005)
...
from .schemas import *
from .service import NephrologyService

router = APIRouter(prefix="/api/nephrology", tags=["Nephrology"])


@router.get("/list", response_model=List[NephrologyEncounter])
async def list_encounters(
    ...
    """List Nephrology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = NephrologyService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=NephrologyEncounter)
...
@router.post("/", response_model=NephrologyEncounter, status_code=201)
async def create_encounter(
    payload: NephrologyEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["nephrology_specialist", "nephrology_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = NephrologyService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=NephrologyEncounter)
async def update_encounter(
    encounter_id: int,
    payload: NephrologyEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["nephrology_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = NephrologyService(db, tenant_id)
