# filepath: 02_MODULES/DEP-009/21_backend_router.py
# FastAPI router for Dermatology (DEP-009)
...
from .schemas import *
from .service import DermatologyService

router = APIRouter(prefix="/api/dermatology", tags=["Dermatology"])


@router.get("/list", response_model=List[DermatologyEncounter])
async def list_encounters(
    ...
    """List Dermatology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = DermatologyService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=DermatologyEncounter)
...
@router.post("/", response_model=DermatologyEncounter, status_code=201)
async def create_encounter(
    payload: DermatologyEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["dermatology_specialist", "dermatology_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = DermatologyService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=DermatologyEncounter)
async def update_encounter(
    encounter_id: int,
    payload: DermatologyEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["dermatology_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = DermatologyService(db, tenant_id)
