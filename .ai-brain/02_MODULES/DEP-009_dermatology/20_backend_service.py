# filepath: 02_MODULES/DEP-009/20_backend_service.py
# Business logic for Dermatology (DEP-009)
...
class DermatologyService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import DermatologyEncounter
        q = self.db.query(DermatologyEncounter).filter(
            DermatologyEncounter.tenant_id == self.tenant_id,
            DermatologyEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(DermatologyEncounter.patient_id == patient_id)
        if status:
            q = q.filter(DermatologyEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import DermatologyEncounter
        e = self.db.query(DermatologyEncounter).filter(
            DermatologyEncounter.id == encounter_id,
            DermatologyEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import DermatologyOrder
        o = DermatologyOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
