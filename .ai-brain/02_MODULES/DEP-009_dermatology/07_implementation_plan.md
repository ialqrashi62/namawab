# Implementation Plan — Dermatology (DEP-009)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- dermatology_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh dermatology
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-dermatology-<timestamp>`
