// filepath: 02_MODULES/DEP-009/22_frontend_page.tsx
// Main page for Dermatology (DEP-009)
...
import { DermatologyAPI } from './24_frontend_api_client';
...
export default function DermatologyPage({ tenantId, patientId, lang = 'ar' }: DermatologyPageProps) {
  const { t } = useTranslation('dermatology');
  ...
  useEffect(() => {
    DermatologyAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    DermatologyAPI.listOrders({ patientId, tenantId }).then(setOrders);
    DermatologyAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="PASI" patientId={patientId} />
        </>
      }
      main={
        <>
