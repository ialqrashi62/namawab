# filepath: 02_MODULES/DEP-041/20_backend_service.py
# Business logic for Interventional_Radiology (DEP-041)
...
class IrService:
    """Service layer for Interventional_Radiology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import IrEncounter
        q = self.db.query(IrEncounter).filter(
            IrEncounter.tenant_id == self.tenant_id,
            IrEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(IrEncounter.patient_id == patient_id)
        if status:
            q = q.filter(IrEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import IrEncounter
        e = self.db.query(IrEncounter).filter(
            IrEncounter.id == encounter_id,
            IrEncounter.tenant_id == self.tenant_id
        ).first()
