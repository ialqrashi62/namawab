// filepath: 02_MODULES/DEP-041/22_frontend_page.tsx
// Main page for Interventional_Radiology (DEP-041)
...
import { IrAPI } from './24_frontend_api_client';
...
export default function IrPage({ tenantId, patientId, lang = 'ar' }: IrPageProps) {
  ...
  useEffect(() => {
    IrAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    IrAPI.listOrders({ patientId, tenantId }).then(setOrders);
    IrAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="الأشعة التداخلية"
      title_en="Interventional_Radiology"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="TICI" patientId={patientId} />
        </>
      }
      main={
        <>
