# filepath: 02_MODULES/DEP-041/18_backend_models.py
# SQLAlchemy models for Interventional_Radiology (DEP-041)
...
class IrEncounter(Base):
    __tablename__ = "ir_encounters"
...
class IrOrder(Base):
    __tablename__ = "ir_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ir_encounters.id"))
...
class IrResult(Base):
    __tablename__ = "ir_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ir_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("ir_orders.id"))
