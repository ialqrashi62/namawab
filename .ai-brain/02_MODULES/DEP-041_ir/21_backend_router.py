# filepath: 02_MODULES/DEP-041/21_backend_router.py
# FastAPI router for Interventional_Radiology (DEP-041)
...
router = APIRouter(prefix="/api/ir", tags=["Interventional_Radiology"])


@router.get("/list", response_model=List[IrEncounter])
async def list_encounters(
    ...
    """List Interventional_Radiology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = IrService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=IrEncounter)
...
@router.post("/", response_model=IrEncounter, status_code=201)
async def create_encounter(
    payload: IrEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["ir_specialist", "ir_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = IrService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=IrEncounter)
async def update_encounter(
    encounter_id: int,
    payload: IrEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["ir_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = IrService(db, tenant_id)
