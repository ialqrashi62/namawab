# filepath: 02_MODULES/DEP-003/21_backend_router.py
# FastAPI router for Gastroenterology (DEP-003)
...
from .schemas import *
from .service import GastroenterologyService

router = APIRouter(prefix="/api/gastroenterology", tags=["Gastroenterology"])


@router.get("/list", response_model=List[GastroenterologyEncounter])
async def list_encounters(
    ...
    """List Gastroenterology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = GastroenterologyService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=GastroenterologyEncounter)
...
@router.post("/", response_model=GastroenterologyEncounter, status_code=201)
async def create_encounter(
    payload: GastroenterologyEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["gastroenterology_specialist", "gastroenterology_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = GastroenterologyService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=GastroenterologyEncounter)
async def update_encounter(
    encounter_id: int,
    payload: GastroenterologyEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["gastroenterology_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = GastroenterologyService(db, tenant_id)
