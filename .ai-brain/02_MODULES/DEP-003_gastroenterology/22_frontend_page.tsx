// filepath: 02_MODULES/DEP-003/22_frontend_page.tsx
// Main page for Gastroenterology (DEP-003)
...
import { GastroenterologyAPI } from './24_frontend_api_client';

interface GastroenterologyPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function GastroenterologyPage({ tenantId, patientId, lang = 'ar' }: GastroenterologyPageProps) {
  const { t } = useTranslation('gastroenterology');
  ...
  useEffect(() => {
    GastroenterologyAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    GastroenterologyAPI.listOrders({ patientId, tenantId }).then(setOrders);
    GastroenterologyAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="MELD" patientId={patientId} />
        </>
      }
      main={
        <>
