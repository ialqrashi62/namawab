# filepath: 02_MODULES/DEP-003/28_test_unit.py
# Unit tests for Gastroenterology (DEP-003)
...
class TestGastroenterologyService:
    """Unit tests for service layer."""

    def test_init_requires_tenant(self):
        db = MagicMock()
        with pytest.raises(ValueError, match="tenant_id required"):
            GastroenterologyService(db, tenant_id=None)

    def test_init_sets_tenant(self):
        db = MagicMock()
        svc = GastroenterologyService(db, tenant_id=1)
        assert svc.tenant_id == 1
        db.execute.assert_called_once()

    def test_list_encounters_filters_by_tenant(self):
        db = MagicMock()
        svc = GastroenterologyService(db, tenant_id=42)
