# filepath: 02_MODULES/DEP-003/20_backend_service.py
# Business logic for Gastroenterology (DEP-003)
...
class GastroenterologyService:
    """Service layer for Gastroenterology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import GastroenterologyEncounter
        q = self.db.query(GastroenterologyEncounter).filter(
            GastroenterologyEncounter.tenant_id == self.tenant_id,
            GastroenterologyEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(GastroenterologyEncounter.patient_id == patient_id)
        if status:
            q = q.filter(GastroenterologyEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import GastroenterologyEncounter
        e = self.db.query(GastroenterologyEncounter).filter(
            GastroenterologyEncounter.id == encounter_id,
            GastroenterologyEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import GastroenterologyOrder
        o = GastroenterologyOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
