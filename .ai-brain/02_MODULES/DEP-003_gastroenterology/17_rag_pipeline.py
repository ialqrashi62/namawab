# filepath: 02_MODULES/DEP-003/17_rag_pipeline.py
# Gastroenterology (DEP-003) RAG Pipeline - Generated 2026-08-08
...
CHROMA_PERSIST_DIR = os.environ.get("CHROMA_PERSIST_DIR", "./chroma_gastroenterology")
...
# ============ System Prompt ============
SYSTEM_PROMPT = "الجهاز الهضمي expert at CBAHI/JCI hospital. Use ONLY context. Cite ICD-10/SNOMED. If insufficient: say so. Bilingual (AR primary). Consider Saudi epidemiology."
...
def get_pg_vectordb(tenant_id, collection_prefix=""):
    collection = f"{tenant_id}_gastroenterology" if not collection_prefix else f"{collection_prefix}_gastroenterology"
...
def build_diagnosis_agent(tenant_id, use_chroma=False):
    ...
    guidelines_tool = create_retriever_tool(retriever, name="gastroenterology_guidelines_search", description="Search Gastroenterology guidelines.")
...
def ingest_chunks(chunks, tenant_id, batch_size=100):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        metas = [{**c.get('meta', {}), 'dept': 'gastroenterology', 'tenant_id': tenant_id} for c in batch]
        vectordb.add_texts(texts=texts, metadatas=metas)
        total += len(batch)
    logger.info(f"[gastroenterology] Ingested {total} chunks for tenant {tenant_id}")
