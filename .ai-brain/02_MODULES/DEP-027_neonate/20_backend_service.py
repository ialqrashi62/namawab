# filepath: 02_MODULES/DEP-027/20_backend_service.py
# Business logic for Neonatology (DEP-027)
...
class NeonateService:
    """Service layer for Neonatology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import NeonateEncounter
        q = self.db.query(NeonateEncounter).filter(
            NeonateEncounter.tenant_id == self.tenant_id,
            NeonateEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(NeonateEncounter.patient_id == patient_id)
        if status:
            q = q.filter(NeonateEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import NeonateEncounter
        e = self.db.query(NeonateEncounter).filter(
            NeonateEncounter.id == encounter_id,
            NeonateEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import NeonateOrder
        o = NeonateOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
