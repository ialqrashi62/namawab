# filepath: 02_MODULES/DEP-027/18_backend_models.py
# SQLAlchemy models for Neonatology (DEP-027)
...
class NeonateEncounter(Base):
    __tablename__ = "neonate_encounters"
...
class NeonateOrder(Base):
    __tablename__ = "neonate_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("neonate_encounters.id"))
...
class NeonateResult(Base):
    __tablename__ = "neonate_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("neonate_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("neonate_orders.id"))
...
class NeonateNote(Base):
    __tablename__ = "neonate_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("neonate_encounters.id"))
