# filepath: 02_MODULES/DEP-027/21_backend_router.py
# FastAPI router for Neonatology (DEP-027)
...
from .schemas import *
from .service import NeonateService

router = APIRouter(prefix="/api/neonate", tags=["Neonatology"])


@router.get("/list", response_model=List[NeonateEncounter])
async def list_encounters(
    ...
    """List Neonatology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = NeonateService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=NeonateEncounter)
...
@router.post("/", response_model=NeonateEncounter, status_code=201)
async def create_encounter(
    payload: NeonateEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["neonate_specialist", "neonate_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = NeonateService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=NeonateEncounter)
async def update_encounter(
    encounter_id: int,
    payload: NeonateEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["neonate_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = NeonateService(db, tenant_id)
