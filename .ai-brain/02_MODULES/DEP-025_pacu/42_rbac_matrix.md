# RBAC Permission Matrix — Pacu (DEP-025)
...
A `dep_025` doctor can access a patient only if:
...
```js
requirePermission(`dep_025:read`);
requirePermission(`dep_025:write`);
requirePermission(`dep_025:sign`);
requirePermission(`dep_025:delete`);
```

## Roles that can access Pacu

- `dep_025_doctor` (own specialty)
- `nurse` (read-only)
