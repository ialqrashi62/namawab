# filepath: 02_MODULES/DEP-025/18_backend_models.py
# SQLAlchemy models for PACU (DEP-025)
...
class PacuEncounter(Base):
    __tablename__ = "pacu_encounters"
...
class PacuOrder(Base):
    __tablename__ = "pacu_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pacu_encounters.id"))
...
class PacuResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pacu_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("pacu_orders.id"))
