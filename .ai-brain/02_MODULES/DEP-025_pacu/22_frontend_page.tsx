// filepath: 02_MODULES/DEP-025/22_frontend_page.tsx
// Main page for PACU (DEP-025)
...
import { PacuAPI } from './24_frontend_api_client';

interface PacuPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function PacuPage({ tenantId, patientId, lang = 'ar' }: PacuPageProps) {
  const { t } = useTranslation('pacu');
  ...
  useEffect(() => {
    PacuAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PacuAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PacuAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="إفاقة ما بعد التخدير"
      title_en="PACU"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="Aldrete" patientId={patientId} />
        </>
      }
      main={
        <>
