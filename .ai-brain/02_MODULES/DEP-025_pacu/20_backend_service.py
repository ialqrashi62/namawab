# filepath: 02_MODULES/DEP-025/20_backend_service.py
# Business logic for PACU (DEP-025)
...
class PacuService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PacuEncounter
        q = self.db.query(PacuEncounter).filter(
            PacuEncounter.tenant_id == self.tenant_id,
            PacuEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PacuEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PacuEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PacuEncounter
        e = self.db.query(PacuEncounter).filter(
            PacuEncounter.id == encounter_id,
            PacuEncounter.tenant_id == self.tenant_id
        ).first()
