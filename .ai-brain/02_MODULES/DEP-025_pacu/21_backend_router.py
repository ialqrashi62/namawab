# filepath: 02_MODULES/DEP-025/21_backend_router.py
# FastAPI router for PACU (DEP-025)
...
from .schemas import *
from .service import PacuService

router = APIRouter(prefix="/api/pacu", tags=["PACU"])


@router.get("/list", response_model=List[PacuEncounter])
async def list_encounters(
    ...
    """List PACU encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PacuService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PacuEncounter)
...
@router.post("/", response_model=PacuEncounter, status_code=201)
async def create_encounter(
    payload: PacuEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pacu_specialist", "pacu_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PacuService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PacuEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PacuEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pacu_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PacuService(db, tenant_id)
