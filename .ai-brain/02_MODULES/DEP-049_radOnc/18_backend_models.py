# filepath: 02_MODULES/DEP-049/18_backend_models.py
# SQLAlchemy models for Radiation_Oncology (DEP-049)
...
class RadoncEncounter(Base):
    __tablename__ = "radOnc_encounters"
...
class RadoncOrder(Base):
    __tablename__ = "radOnc_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("radOnc_encounters.id"))
...
class RadoncResult(Base):
    __tablename__ = "radOnc_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("radOnc_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("radOnc_orders.id"))
...
class RadoncNote(Base):
    __tablename__ = "radOnc_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("radOnc_encounters.id"))
