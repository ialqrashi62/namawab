# filepath: 02_MODULES/DEP-049/21_backend_router.py
# FastAPI router for Radiation_Oncology (DEP-049)
...
from .schemas import *
from .service import RadoncService

router = APIRouter(prefix="/api/radOnc", tags=["Radiation_Oncology"])


@router.get("/list", response_model=List[RadoncEncounter])
async def list_encounters(
    ...
    """List Radiation_Oncology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = RadoncService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=RadoncEncounter)
...
@router.post("/", response_model=RadoncEncounter, status_code=201)
async def create_encounter(
    payload: RadoncEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["radOnc_specialist", "radOnc_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = RadoncService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=RadoncEncounter)
async def update_encounter(
    encounter_id: int,
    payload: RadoncEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["radOnc_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = RadoncService(db, tenant_id)
