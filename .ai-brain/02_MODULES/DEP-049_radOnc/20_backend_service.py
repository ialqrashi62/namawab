# filepath: 02_MODULES/DEP-049/20_backend_service.py
# Business logic for Radiation_Oncology (DEP-049)
...
class RadoncService:
    """Service layer for Radiation_Oncology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import RadoncEncounter
        q = self.db.query(RadoncEncounter).filter(
            RadoncEncounter.tenant_id == self.tenant_id,
            RadoncEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(RadoncEncounter.patient_id == patient_id)
        if status:
            q = q.filter(RadoncEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import RadoncEncounter
        e = self.db.query(RadoncEncounter).filter(
            RadoncEncounter.id == encounter_id,
            RadoncEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import RadoncOrder
        o = RadoncOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
