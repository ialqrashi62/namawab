# filepath: 02_MODULES/DEP-056/18_backend_models.py
# SQLAlchemy models for HR_Staffing (DEP-056)
...
class HrEncounter(Base):
    __tablename__ = "hr_encounters"
...
class HrOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("hr_encounters.id"))
...
class HrResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("hr_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("hr_orders.id"))
