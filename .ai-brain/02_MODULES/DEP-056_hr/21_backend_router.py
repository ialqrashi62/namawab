# filepath: 02_MODULES/DEP-056/21_backend_router.py
# FastAPI router for HR_Staffing (DEP-056)
...
router = APIRouter(prefix="/api/hr", tags=["HR_Staffing"])


@router.get("/list", response_model=List[HrEncounter])
async def list_encounters(
    ...
    """List HR_Staffing encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = HrService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=HrEncounter)
...
@router.post("/", response_model=HrEncounter, status_code=201)
async def create_encounter(
    payload: HrEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["hr_specialist", "hr_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = HrService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=HrEncounter)
async def update_encounter(
    encounter_id: int,
    payload: HrEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["hr_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = HrService(db, tenant_id)
