// filepath: 02_MODULES/DEP-056/22_frontend_page.tsx
// Main page for HR_Staffing (DEP-056)
...
import { HrAPI } from './24_frontend_api_client';
...
export default function HrPage({ tenantId, patientId, lang = 'ar' }: HrPageProps) {
  ...
  useEffect(() => {
    HrAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    HrAPI.listOrders({ patientId, tenantId }).then(setOrders);
    HrAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="الموارد البشرية"
      title_en="HR_Staffing"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="employee_turnover_rate" patientId={patientId} />
        </>
      }
      main={
        <>
