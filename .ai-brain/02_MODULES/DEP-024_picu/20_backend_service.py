# filepath: 02_MODULES/DEP-024/20_backend_service.py
# Business logic for PICU (DEP-024)
...
class PicuService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PicuEncounter
        q = self.db.query(PicuEncounter).filter(
            PicuEncounter.tenant_id == self.tenant_id,
            PicuEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PicuEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PicuEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PicuEncounter
        e = self.db.query(PicuEncounter).filter(
            PicuEncounter.id == encounter_id,
            PicuEncounter.tenant_id == self.tenant_id
        ).first()
