// filepath: 02_MODULES/DEP-024/22_frontend_page.tsx
// Main page for PICU (DEP-024)
...
import { PicuAPI } from './24_frontend_api_client';

interface PicuPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function PicuPage({ tenantId, patientId, lang = 'ar' }: PicuPageProps) {
  const { t } = useTranslation('picu');
  ...
  useEffect(() => {
    PicuAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PicuAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PicuAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="العناية المركزة للأطفال"
      title_en="PICU"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="PELOD" patientId={patientId} />
        </>
      }
      main={
        <>
