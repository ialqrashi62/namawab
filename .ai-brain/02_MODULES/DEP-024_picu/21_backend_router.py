# filepath: 02_MODULES/DEP-024/21_backend_router.py
# FastAPI router for PICU (DEP-024)
...
from .schemas import *
from .service import PicuService

router = APIRouter(prefix="/api/picu", tags=["PICU"])


@router.get("/list", response_model=List[PicuEncounter])
async def list_encounters(
    ...
    """List PICU encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PicuService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PicuEncounter)
...
@router.post("/", response_model=PicuEncounter, status_code=201)
async def create_encounter(
    payload: PicuEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["picu_specialist", "picu_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PicuService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PicuEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PicuEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["picu_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PicuService(db, tenant_id)
