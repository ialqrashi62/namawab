# filepath: 02_MODULES/DEP-024/18_backend_models.py
# SQLAlchemy models for PICU (DEP-024)
...
class PicuEncounter(Base):
    __tablename__ = "picu_encounters"
...
class PicuOrder(Base):
    __tablename__ = "picu_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("picu_encounters.id"))
...
class PicuResult(Base):
    __tablename__ = "picu_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("picu_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("picu_orders.id"))
