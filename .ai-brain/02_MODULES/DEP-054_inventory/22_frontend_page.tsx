// filepath: 02_MODULES/DEP-054/22_frontend_page.tsx
// Main page for Inventory_Supply_Chain (DEP-054)
...
import { InventoryAPI } from './24_frontend_api_client';
...
export default function InventoryPage({ tenantId, patientId, lang = 'ar' }: InventoryPageProps) {
  ...
  useEffect(() => {
    InventoryAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    InventoryAPI.listOrders({ patientId, tenantId }).then(setOrders);
    InventoryAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="المخزون وسلسلة الإمداد"
      title_en="Inventory_Supply_Chain"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="stockout_rate" patientId={patientId} />
        </>
      }
      main={
        <>
