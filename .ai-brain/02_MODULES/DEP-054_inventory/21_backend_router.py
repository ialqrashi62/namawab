# filepath: 02_MODULES/DEP-054/21_backend_router.py
# FastAPI router for Inventory_Supply_Chain (DEP-054)
...
router = APIRouter(prefix="/api/inventory", tags=["Inventory_Supply_Chain"])


@router.get("/list", response_model=List[InventoryEncounter])
async def list_encounters(
    ...
    """List Inventory_Supply_Chain encounters for current tenant."""
...
@router.get("/{encounter_id}", response_model=InventoryEncounter)
...
@router.post("/", response_model=InventoryEncounter, status_code=201)
async def create_encounter(
    payload: InventoryEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["inventory_specialist", "inventory_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = InventoryService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=InventoryEncounter)
async def update_encounter(
    encounter_id: int,
    payload: InventoryEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["inventory_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = InventoryService(db, tenant_id)
