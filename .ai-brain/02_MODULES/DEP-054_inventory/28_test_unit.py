# filepath: 02_MODULES/DEP-054/28_test_unit.py
# Unit tests for Inventory_Supply_Chain (DEP-054)
...
class TestInventoryService:
    ...
    def test_init_requires_tenant(self):
        ...
        with pytest.raises(ValueError, match="tenant_id required"):
            InventoryService(db, tenant_id=None)

    def test_init_sets_tenant(self):
        db = MagicMock()
        svc = InventoryService(db, tenant_id=1)
        assert svc.tenant_id == 1
        db.execute.assert_called_once()

    def test_list_encounters_filters_by_tenant(self):
        db = MagicMock()
        svc = InventoryService(db, tenant_id=42)
