# filepath: 02_MODULES/DEP-054/18_backend_models.py
# SQLAlchemy models for Inventory_Supply_Chain (DEP-054)
...
class InventoryEncounter(Base):
    __tablename__ = "inventory_encounters"
...
class InventoryOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("inventory_encounters.id"))
...
class InventoryResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("inventory_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("inventory_orders.id"))
