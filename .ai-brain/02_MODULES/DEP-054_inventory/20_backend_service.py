# filepath: 02_MODULES/DEP-054/20_backend_service.py
# Business logic for Inventory_Supply_Chain (DEP-054)
...
class InventoryService:
    """Service layer for Inventory_Supply_Chain.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import InventoryEncounter
        q = self.db.query(InventoryEncounter).filter(
            InventoryEncounter.tenant_id == self.tenant_id,
            InventoryEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(InventoryEncounter.patient_id == patient_id)
        if status:
            q = q.filter(InventoryEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import InventoryEncounter
        e = self.db.query(InventoryEncounter).filter(
            InventoryEncounter.id == encounter_id,
            InventoryEncounter.tenant_id == self.tenant_id
        ).first()
