// filepath: 02_MODULES/DEP-021/22_frontend_page.tsx
// Main page for Emergency (DEP-021)
...
import { EmergencyAPI } from './24_frontend_api_client';
...
export default function EmergencyPage({ tenantId, patientId, lang = 'ar' }: EmergencyPageProps) {
  ...
  useEffect(() => {
    EmergencyAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    EmergencyAPI.listOrders({ patientId, tenantId }).then(setOrders);
    EmergencyAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="ESI" patientId={patientId} />
        </>
      }
      main={
        <>
