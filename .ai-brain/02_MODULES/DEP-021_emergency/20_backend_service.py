# filepath: 02_MODULES/DEP-021/20_backend_service.py
# Business logic for Emergency (DEP-021)
...
class EmergencyService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        q = self.db.query(EmergencyEncounter).filter(
            EmergencyEncounter.tenant_id == self.tenant_id,
            EmergencyEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(EmergencyEncounter.patient_id == patient_id)
        if status:
            q = q.filter(EmergencyEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import EmergencyEncounter
        e = self.db.query(EmergencyEncounter).filter(
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import EmergencyOrder
        o = EmergencyOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
