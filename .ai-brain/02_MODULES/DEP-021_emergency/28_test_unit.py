# filepath: 02_MODULES/DEP-021/28_test_unit.py
# Unit tests for Emergency (DEP-021)
...
class TestEmergencyService:
    ...
    def test_init_requires_tenant(self):
        ...
        with pytest.raises(ValueError, match="tenant_id required"):
            EmergencyService(db, tenant_id=None)

    def test_init_sets_tenant(self):
        db = MagicMock()
        svc = EmergencyService(db, tenant_id=1)
        assert svc.tenant_id == 1
        db.execute.assert_called_once()

    def test_list_encounters_filters_by_tenant(self):
        db = MagicMock()
        svc = EmergencyService(db, tenant_id=42)
