# filepath: 02_MODULES/DEP-021/21_backend_router.py
# FastAPI router for Emergency (DEP-021)
...
@router.get("/list", response_model=List[EmergencyEncounter])
async def list_encounters(
    ...
    """List Emergency encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = EmergencyService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=EmergencyEncounter)
...
@router.post("/", response_model=EmergencyEncounter, status_code=201)
async def create_encounter(
    ...
    require_tenant_scope(request)
    require_role(user, ["emergency_specialist", "emergency_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = EmergencyService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=EmergencyEncounter)
async def update_encounter(
    ...
    require_tenant_scope(request)
    require_role(user, ["emergency_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = EmergencyService(db, tenant_id)
