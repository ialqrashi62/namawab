# filepath: 02_MODULES/DEP-029/20_backend_service.py
# Business logic for Pediatric_Neurology (DEP-029)
...
class PedsneuroService:
    """Service layer for Pediatric_Neurology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PedsneuroEncounter
        q = self.db.query(PedsneuroEncounter).filter(
            PedsneuroEncounter.tenant_id == self.tenant_id,
            PedsneuroEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PedsneuroEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PedsneuroEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PedsneuroEncounter
        e = self.db.query(PedsneuroEncounter).filter(
            PedsneuroEncounter.id == encounter_id,
            PedsneuroEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import PedsneuroOrder
        o = PedsneuroOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
