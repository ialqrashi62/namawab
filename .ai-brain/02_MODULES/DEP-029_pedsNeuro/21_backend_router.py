# filepath: 02_MODULES/DEP-029/21_backend_router.py
# FastAPI router for Pediatric_Neurology (DEP-029)
...
from .schemas import *
from .service import PedsneuroService

router = APIRouter(prefix="/api/pedsNeuro", tags=["Pediatric_Neurology"])


@router.get("/list", response_model=List[PedsneuroEncounter])
async def list_encounters(
    ...
    """List Pediatric_Neurology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PedsneuroService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PedsneuroEncounter)
...
@router.post("/", response_model=PedsneuroEncounter, status_code=201)
async def create_encounter(
    payload: PedsneuroEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsNeuro_specialist", "pedsNeuro_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedsneuroService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PedsneuroEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PedsneuroEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pedsNeuro_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PedsneuroService(db, tenant_id)
