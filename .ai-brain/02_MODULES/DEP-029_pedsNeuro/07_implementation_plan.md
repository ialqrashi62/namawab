# Implementation Plan — Pediatric_Neurology (DEP-029)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- pedsNeuro_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh pedsNeuro
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-pedsNeuro-<timestamp>`
