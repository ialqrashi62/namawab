# filepath: 02_MODULES/DEP-029/18_backend_models.py
# SQLAlchemy models for Pediatric_Neurology (DEP-029)
...
class PedsneuroEncounter(Base):
    __tablename__ = "pedsNeuro_encounters"
...
class PedsneuroOrder(Base):
    __tablename__ = "pedsNeuro_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsNeuro_encounters.id"))
...
class PedsneuroResult(Base):
    __tablename__ = "pedsNeuro_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsNeuro_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("pedsNeuro_orders.id"))
...
class PedsneuroNote(Base):
    __tablename__ = "pedsNeuro_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pedsNeuro_encounters.id"))
