# filepath: 02_MODULES/DEP-012/20_backend_service.py
# Business logic for Orthopedics (DEP-012)
...
class OrthoService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import OrthoEncounter
        q = self.db.query(OrthoEncounter).filter(
            OrthoEncounter.tenant_id == self.tenant_id,
            OrthoEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(OrthoEncounter.patient_id == patient_id)
        if status:
            q = q.filter(OrthoEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import OrthoEncounter
        e = self.db.query(OrthoEncounter).filter(
            OrthoEncounter.id == encounter_id,
            OrthoEncounter.tenant_id == self.tenant_id
        ).first()
