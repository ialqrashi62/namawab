# filepath: 02_MODULES/DEP-012/21_backend_router.py
# FastAPI router for Orthopedics (DEP-012)
...
router = APIRouter(prefix="/api/ortho", tags=["Orthopedics"])


@router.get("/list", response_model=List[OrthoEncounter])
async def list_encounters(
    ...
    """List Orthopedics encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = OrthoService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=OrthoEncounter)
...
@router.post("/", response_model=OrthoEncounter, status_code=201)
async def create_encounter(
    payload: OrthoEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["ortho_specialist", "ortho_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = OrthoService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=OrthoEncounter)
async def update_encounter(
    encounter_id: int,
    payload: OrthoEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["ortho_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = OrthoService(db, tenant_id)
