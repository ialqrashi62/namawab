# filepath: 02_MODULES/DEP-012/18_backend_models.py
# SQLAlchemy models for Orthopedics (DEP-012)
...
class OrthoEncounter(Base):
    __tablename__ = "ortho_encounters"
...
class OrthoOrder(Base):
    __tablename__ = "ortho_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ortho_encounters.id"))
...
class OrthoResult(Base):
    __tablename__ = "ortho_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ortho_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("ortho_orders.id"))
...
class OrthoNote(Base):
    __tablename__ = "ortho_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ortho_encounters.id"))
