# filepath: 02_MODULES/DEP-012/17_rag_pipeline.py
# Orthopedics (DEP-012) RAG Pipeline - Generated 2026-08-08
...
# ============ Configuration ============
COLLECTION_NAME = "ortho"
PGVECTOR_URL = os.environ.get("PGVECTOR_URL", "postgresql://user:pass@localhost:5432/nama")
CHROMA_PERSIST_DIR = os.environ.get("CHROMA_PERSIST_DIR", "./chroma_ortho")
...
# ============ System Prompt ============
SYSTEM_PROMPT = "العظام expert at CBAHI/JCI hospital. Use ONLY context. Cite ICD-10/SNOMED. If insufficient: say so. Bilingual (AR primary). Consider Saudi epidemiology."
...
def get_pg_vectordb(tenant_id, collection_prefix=""):
    collection = f"{tenant_id}_ortho" if not collection_prefix else f"{collection_prefix}_ortho"
...
def build_diagnosis_agent(tenant_id, use_chroma=False):
    ...
    guidelines_tool = create_retriever_tool(retriever, name="ortho_guidelines_search", description="Search Orthopedics guidelines.")
...
def ingest_chunks(chunks, tenant_id, batch_size=100):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        metas = [{**c.get('meta', {}), 'dept': 'ortho', 'tenant_id': tenant_id} for c in batch]
        vectordb.add_texts(texts=texts, metadatas=metas)
        total += len(batch)
    logger.info(f"[ortho] Ingested {total} chunks for tenant {tenant_id}")
