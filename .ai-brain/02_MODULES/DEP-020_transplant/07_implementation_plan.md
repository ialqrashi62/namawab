# Implementation Plan — Transplant_Surgery (DEP-020)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- transplant_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh transplant
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-transplant-<timestamp>`
