# filepath: 02_MODULES/DEP-020/21_backend_router.py
# FastAPI router for Transplant_Surgery (DEP-020)
...
from .schemas import *
from .service import TransplantService

router = APIRouter(prefix="/api/transplant", tags=["Transplant_Surgery"])


@router.get("/list", response_model=List[TransplantEncounter])
async def list_encounters(
    ...
    """List Transplant_Surgery encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = TransplantService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=TransplantEncounter)
...
@router.post("/", response_model=TransplantEncounter, status_code=201)
async def create_encounter(
    payload: TransplantEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["transplant_specialist", "transplant_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = TransplantService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=TransplantEncounter)
async def update_encounter(
    encounter_id: int,
    payload: TransplantEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["transplant_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = TransplantService(db, tenant_id)
