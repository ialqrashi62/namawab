# filepath: 02_MODULES/DEP-020/20_backend_service.py
# Business logic for Transplant_Surgery (DEP-020)
...
class TransplantService:
    """Service layer for Transplant_Surgery.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import TransplantEncounter
        q = self.db.query(TransplantEncounter).filter(
            TransplantEncounter.tenant_id == self.tenant_id,
            TransplantEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(TransplantEncounter.patient_id == patient_id)
        if status:
            q = q.filter(TransplantEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import TransplantEncounter
        e = self.db.query(TransplantEncounter).filter(
            TransplantEncounter.id == encounter_id,
            TransplantEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import TransplantOrder
        o = TransplantOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
