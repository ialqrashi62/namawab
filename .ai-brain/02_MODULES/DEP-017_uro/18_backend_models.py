# filepath: 02_MODULES/DEP-017/18_backend_models.py
# SQLAlchemy models for Urology (DEP-017)
...
class UroEncounter(Base):
    __tablename__ = "uro_encounters"
...
class UroOrder(Base):
    __tablename__ = "uro_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("uro_encounters.id"))
...
class UroResult(Base):
    __tablename__ = "uro_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("uro_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("uro_orders.id"))
