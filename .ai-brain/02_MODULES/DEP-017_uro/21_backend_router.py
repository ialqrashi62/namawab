# filepath: 02_MODULES/DEP-017/21_backend_router.py
# FastAPI router for Urology (DEP-017)
...
from .schemas import *
from .service import UroService

router = APIRouter(prefix="/api/uro", tags=["Urology"])


@router.get("/list", response_model=List[UroEncounter])
async def list_encounters(
    ...
    """List Urology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = UroService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=UroEncounter)
...
@router.post("/", response_model=UroEncounter, status_code=201)
async def create_encounter(
    payload: UroEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["uro_specialist", "uro_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = UroService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=UroEncounter)
async def update_encounter(
    encounter_id: int,
    payload: UroEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["uro_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = UroService(db, tenant_id)
