# filepath: 02_MODULES/DEP-017/20_backend_service.py
# Business logic for Urology (DEP-017)
...
class UroService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import UroEncounter
        q = self.db.query(UroEncounter).filter(
            UroEncounter.tenant_id == self.tenant_id,
            UroEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(UroEncounter.patient_id == patient_id)
        if status:
            q = q.filter(UroEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import UroEncounter
        e = self.db.query(UroEncounter).filter(
            UroEncounter.id == encounter_id,
            UroEncounter.tenant_id == self.tenant_id
        ).first()
