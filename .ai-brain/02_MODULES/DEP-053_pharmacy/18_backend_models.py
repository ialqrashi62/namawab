# filepath: 02_MODULES/DEP-053/18_backend_models.py
# SQLAlchemy models for Pharmacy (DEP-053)
...
class PharmacyEncounter(Base):
    __tablename__ = "pharmacy_encounters"
...
class PharmacyOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pharmacy_encounters.id"))
...
class PharmacyResult(Base):
    __tablename__ = "pharmacy_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pharmacy_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("pharmacy_orders.id"))
...
class PharmacyNote(Base):
    __tablename__ = "pharmacy_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pharmacy_encounters.id"))
