# filepath: 02_MODULES/DEP-053/21_backend_router.py
# FastAPI router for Pharmacy (DEP-053)
...
@router.get("/list", response_model=List[PharmacyEncounter])
async def list_encounters(
    ...
    """List Pharmacy encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PharmacyService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PharmacyEncounter)
...
@router.post("/", response_model=PharmacyEncounter, status_code=201)
async def create_encounter(
    payload: PharmacyEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pharmacy_specialist", "pharmacy_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PharmacyService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PharmacyEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PharmacyEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pharmacy_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PharmacyService(db, tenant_id)
