# filepath: 02_MODULES/DEP-053/17_rag_pipeline.py
# Pharmacy (DEP-053) RAG Pipeline - Generated 2026-08-08
...
CHROMA_PERSIST_DIR = os.environ.get("CHROMA_PERSIST_DIR", "./chroma_pharmacy")
...
# ============ System Prompt ============
SYSTEM_PROMPT = "الصيدلية expert at CBAHI/JCI hospital. Use ONLY context. Cite ICD-10/SNOMED. If insufficient: say so. Bilingual (AR primary). Consider Saudi epidemiology."
...
def get_pg_vectordb(tenant_id, collection_prefix=""):
    collection = f"{tenant_id}_pharmacy" if not collection_prefix else f"{collection_prefix}_pharmacy"
...
def build_diagnosis_agent(tenant_id, use_chroma=False):
    ...
    guidelines_tool = create_retriever_tool(retriever, name="pharmacy_guidelines_search", description="Search Pharmacy guidelines.")
...
def ingest_chunks(chunks, tenant_id, batch_size=100):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        metas = [{**c.get('meta', {}), 'dept': 'pharmacy', 'tenant_id': tenant_id} for c in batch]
        vectordb.add_texts(texts=texts, metadatas=metas)
        total += len(batch)
    logger.info(f"[pharmacy] Ingested {total} chunks for tenant {tenant_id}")
