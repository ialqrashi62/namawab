// filepath: 02_MODULES/DEP-053/22_frontend_page.tsx
// Main page for Pharmacy (DEP-053)
...
import { PharmacyAPI } from './24_frontend_api_client';
...
export default function PharmacyPage({ tenantId, patientId, lang = 'ar' }: PharmacyPageProps) {
  ...
  useEffect(() => {
    PharmacyAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PharmacyAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PharmacyAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="medication_adherence_rate" patientId={patientId} />
        </>
      }
      main={
        <>
