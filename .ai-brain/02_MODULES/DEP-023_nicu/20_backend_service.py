# filepath: 02_MODULES/DEP-023/20_backend_service.py
# Business logic for NICU (DEP-023)
...
class NicuService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import NicuEncounter
        q = self.db.query(NicuEncounter).filter(
            NicuEncounter.tenant_id == self.tenant_id,
            NicuEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(NicuEncounter.patient_id == patient_id)
        if status:
            q = q.filter(NicuEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import NicuEncounter
        e = self.db.query(NicuEncounter).filter(
            NicuEncounter.id == encounter_id,
            NicuEncounter.tenant_id == self.tenant_id
        ).first()
