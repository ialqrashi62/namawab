# filepath: 02_MODULES/DEP-023/21_backend_router.py
# FastAPI router for NICU (DEP-023)
...
from .schemas import *
from .service import NicuService

router = APIRouter(prefix="/api/nicu", tags=["NICU"])


@router.get("/list", response_model=List[NicuEncounter])
async def list_encounters(
    ...
    """List NICU encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = NicuService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=NicuEncounter)
...
@router.post("/", response_model=NicuEncounter, status_code=201)
async def create_encounter(
    payload: NicuEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["nicu_specialist", "nicu_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = NicuService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=NicuEncounter)
async def update_encounter(
    encounter_id: int,
    payload: NicuEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["nicu_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = NicuService(db, tenant_id)
