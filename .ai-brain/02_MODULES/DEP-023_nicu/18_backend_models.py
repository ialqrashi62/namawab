# filepath: 02_MODULES/DEP-023/18_backend_models.py
# SQLAlchemy models for NICU (DEP-023)
...
class NicuEncounter(Base):
    __tablename__ = "nicu_encounters"
...
class NicuOrder(Base):
    __tablename__ = "nicu_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("nicu_encounters.id"))
...
class NicuResult(Base):
    __tablename__ = "nicu_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("nicu_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("nicu_orders.id"))
