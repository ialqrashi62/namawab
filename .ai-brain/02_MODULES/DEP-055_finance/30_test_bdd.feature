# filepath: 02_MODULES/DEP-055/30_test_bdd.feature
# BDD scenarios for Finance_Accounting (DEP-055)
# Language: Gherkin (English for international tooling)

Feature: Finance_Accounting clinical workflow

  Background:
    Given a CBAHI/JCI-accredited hospital
    And a tenant with id 1
    And a finance specialist user
    And a patient with MRN 100045

  Scenario: List encounters for patient
    Given the patient has 3 active encounters
    When the specialist requests GET /api/finance/list?patient_id=100045
  ...
  Scenario: Create new encounter with valid data
    ...
    When the specialist POSTs to /api/finance/ with patient_id=100045
    Then the response status is 201
    And the encounter has status "active"

  Scenario: Unauthorized access is rejected
    Given no auth token
    When the user requests GET /api/finance/list
  ...
  Scenario: AI diagnosis returns differential
    ...
    When the specialist requests POST /api/finance/ai/diagnose
