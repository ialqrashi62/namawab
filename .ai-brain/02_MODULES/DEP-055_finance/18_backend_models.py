# filepath: 02_MODULES/DEP-055/18_backend_models.py
# SQLAlchemy models for Finance_Accounting (DEP-055)
...
class FinanceEncounter(Base):
    __tablename__ = "finance_encounters"
...
class FinanceOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("finance_encounters.id"))
...
class FinanceResult(Base):
    __tablename__ = "finance_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("finance_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("finance_orders.id"))
