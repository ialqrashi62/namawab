# filepath: 02_MODULES/DEP-055/20_backend_service.py
# Business logic for Finance_Accounting (DEP-055)
...
class FinanceService:
    """Service layer for Finance_Accounting.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import FinanceEncounter
        q = self.db.query(FinanceEncounter).filter(
            FinanceEncounter.tenant_id == self.tenant_id,
            FinanceEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(FinanceEncounter.patient_id == patient_id)
        if status:
            q = q.filter(FinanceEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import FinanceEncounter
        e = self.db.query(FinanceEncounter).filter(
            FinanceEncounter.id == encounter_id,
            FinanceEncounter.tenant_id == self.tenant_id
        ).first()
