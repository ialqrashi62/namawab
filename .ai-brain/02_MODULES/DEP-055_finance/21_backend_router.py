# filepath: 02_MODULES/DEP-055/21_backend_router.py
# FastAPI router for Finance_Accounting (DEP-055)
...
router = APIRouter(prefix="/api/finance", tags=["Finance_Accounting"])


@router.get("/list", response_model=List[FinanceEncounter])
async def list_encounters(
    ...
    """List Finance_Accounting encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = FinanceService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=FinanceEncounter)
...
@router.post("/", response_model=FinanceEncounter, status_code=201)
async def create_encounter(
    payload: FinanceEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["finance_specialist", "finance_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = FinanceService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=FinanceEncounter)
async def update_encounter(
    encounter_id: int,
    payload: FinanceEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["finance_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = FinanceService(db, tenant_id)
