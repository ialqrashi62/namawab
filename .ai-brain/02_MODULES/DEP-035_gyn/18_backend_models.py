# filepath: 02_MODULES/DEP-035/18_backend_models.py
# SQLAlchemy models for Gynecology (DEP-035)
...
class GynEncounter(Base):
    __tablename__ = "gyn_encounters"
...
class GynOrder(Base):
    __tablename__ = "gyn_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("gyn_encounters.id"))
...
class GynResult(Base):
    __tablename__ = "gyn_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("gyn_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("gyn_orders.id"))
