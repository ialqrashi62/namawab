// filepath: 02_MODULES/DEP-035/22_frontend_page.tsx
// Main page for Gynecology (DEP-035)
...
import { GynAPI } from './24_frontend_api_client';

interface GynPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function GynPage({ tenantId, patientId, lang = 'ar' }: GynPageProps) {
  const { t } = useTranslation('gyn');
  ...
  useEffect(() => {
    GynAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    GynAPI.listOrders({ patientId, tenantId }).then(setOrders);
    GynAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="أمراض النساء"
      title_en="Gynecology"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="FIGO_stage" patientId={patientId} />
        </>
      }
      main={
        <>
