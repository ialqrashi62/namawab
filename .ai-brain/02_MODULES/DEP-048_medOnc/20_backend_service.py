# filepath: 02_MODULES/DEP-048/20_backend_service.py
# Business logic for Medical_Oncology (DEP-048)
...
class MedoncService:
    """Service layer for Medical_Oncology.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import MedoncEncounter
        q = self.db.query(MedoncEncounter).filter(
            MedoncEncounter.tenant_id == self.tenant_id,
            MedoncEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(MedoncEncounter.patient_id == patient_id)
        if status:
            q = q.filter(MedoncEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import MedoncEncounter
        e = self.db.query(MedoncEncounter).filter(
            MedoncEncounter.id == encounter_id,
            MedoncEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import MedoncOrder
        o = MedoncOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
