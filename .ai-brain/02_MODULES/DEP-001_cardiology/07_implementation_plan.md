# Implementation Plan — Cardiology (DEP-001)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- cardiology_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh cardiology
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-cardiology-<timestamp>`
