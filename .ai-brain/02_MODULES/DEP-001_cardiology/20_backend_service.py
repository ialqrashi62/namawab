# filepath: 02_MODULES/DEP-001/20_backend_service.py
# Business logic for Cardiology (DEP-001)
...
class CardiologyService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import CardiologyEncounter
        q = self.db.query(CardiologyEncounter).filter(
            CardiologyEncounter.tenant_id == self.tenant_id,
            CardiologyEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(CardiologyEncounter.patient_id == patient_id)
        if status:
            q = q.filter(CardiologyEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import CardiologyEncounter
        e = self.db.query(CardiologyEncounter).filter(
            CardiologyEncounter.id == encounter_id,
            CardiologyEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import CardiologyOrder
        o = CardiologyOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
