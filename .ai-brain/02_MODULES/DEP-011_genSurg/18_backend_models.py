# filepath: 02_MODULES/DEP-011/18_backend_models.py
# SQLAlchemy models for General_Surgery (DEP-011)
...
class GensurgEncounter(Base):
    __tablename__ = "genSurg_encounters"
...
class GensurgOrder(Base):
    __tablename__ = "genSurg_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("genSurg_encounters.id"))
...
class GensurgResult(Base):
    __tablename__ = "genSurg_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("genSurg_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("genSurg_orders.id"))
...
class GensurgNote(Base):
    __tablename__ = "genSurg_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("genSurg_encounters.id"))
