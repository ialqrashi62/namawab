# filepath: 02_MODULES/DEP-011/17_rag_pipeline.py
# General_Surgery (DEP-011) RAG Pipeline - Generated 2026-08-08
...
# ============ Configuration ============
COLLECTION_NAME = "genSurg"
PGVECTOR_URL = os.environ.get("PGVECTOR_URL", "postgresql://user:pass@localhost:5432/nama")
CHROMA_PERSIST_DIR = os.environ.get("CHROMA_PERSIST_DIR", "./chroma_genSurg")
...
# ============ System Prompt ============
SYSTEM_PROMPT = "الجراحة العامة expert at CBAHI/JCI hospital. Use ONLY context. Cite ICD-10/SNOMED. If insufficient: say so. Bilingual (AR primary). Consider Saudi epidemiology."
...
def get_pg_vectordb(tenant_id, collection_prefix=""):
    collection = f"{tenant_id}_genSurg" if not collection_prefix else f"{collection_prefix}_genSurg"
...
def build_diagnosis_agent(tenant_id, use_chroma=False):
    ...
    guidelines_tool = create_retriever_tool(retriever, name="genSurg_guidelines_search", description="Search General_Surgery guidelines.")
...
def ingest_chunks(chunks, tenant_id, batch_size=100):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        metas = [{**c.get('meta', {}), 'dept': 'genSurg', 'tenant_id': tenant_id} for c in batch]
        vectordb.add_texts(texts=texts, metadatas=metas)
        total += len(batch)
    logger.info(f"[genSurg] Ingested {total} chunks for tenant {tenant_id}")
