// filepath: 02_MODULES/DEP-011/22_frontend_page.tsx
// Main page for General_Surgery (DEP-011)
...
import { GensurgAPI } from './24_frontend_api_client';

interface GensurgPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function GensurgPage({ tenantId, patientId, lang = 'ar' }: GensurgPageProps) {
  const { t } = useTranslation('genSurg');
  ...
  useEffect(() => {
    GensurgAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    GensurgAPI.listOrders({ patientId, tenantId }).then(setOrders);
    GensurgAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="الجراحة العامة"
      title_en="General_Surgery"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="ASA" patientId={patientId} />
        </>
      }
      main={
        <>
