# filepath: 02_MODULES/DEP-011/20_backend_service.py
# Business logic for General_Surgery (DEP-011)
...
class GensurgService:
    """Service layer for General_Surgery.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import GensurgEncounter
        q = self.db.query(GensurgEncounter).filter(
            GensurgEncounter.tenant_id == self.tenant_id,
            GensurgEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(GensurgEncounter.patient_id == patient_id)
        if status:
            q = q.filter(GensurgEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import GensurgEncounter
        e = self.db.query(GensurgEncounter).filter(
            GensurgEncounter.id == encounter_id,
            GensurgEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import GensurgOrder
        o = GensurgOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
