// filepath: 02_MODULES/DEP-018/22_frontend_page.tsx
// Main page for Plastic_Burns (DEP-018)
...
import { PlasticAPI } from './24_frontend_api_client';

interface PlasticPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function PlasticPage({ tenantId, patientId, lang = 'ar' }: PlasticPageProps) {
  const { t } = useTranslation('plastic');
  ...
  useEffect(() => {
    PlasticAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PlasticAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PlasticAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="التجميل والحروق"
      title_en="Plastic_Burns"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="TBSA" patientId={patientId} />
        </>
      }
      main={
        <>
