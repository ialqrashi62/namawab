# filepath: 02_MODULES/DEP-018/20_backend_service.py
# Business logic for Plastic_Burns (DEP-018)
...
class PlasticService:
    """Service layer for Plastic_Burns.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PlasticEncounter
        q = self.db.query(PlasticEncounter).filter(
            PlasticEncounter.tenant_id == self.tenant_id,
            PlasticEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PlasticEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PlasticEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PlasticEncounter
        e = self.db.query(PlasticEncounter).filter(
            PlasticEncounter.id == encounter_id,
            PlasticEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import PlasticOrder
        o = PlasticOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
