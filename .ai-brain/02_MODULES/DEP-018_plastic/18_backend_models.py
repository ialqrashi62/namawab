# filepath: 02_MODULES/DEP-018/18_backend_models.py
# SQLAlchemy models for Plastic_Burns (DEP-018)
...
class PlasticEncounter(Base):
    __tablename__ = "plastic_encounters"
...
class PlasticOrder(Base):
    __tablename__ = "plastic_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("plastic_encounters.id"))
...
class PlasticResult(Base):
    __tablename__ = "plastic_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("plastic_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("plastic_orders.id"))
...
class PlasticNote(Base):
    __tablename__ = "plastic_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("plastic_encounters.id"))
