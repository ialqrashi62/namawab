# filepath: 02_MODULES/DEP-018/21_backend_router.py
# FastAPI router for Plastic_Burns (DEP-018)
...
from .schemas import *
from .service import PlasticService

router = APIRouter(prefix="/api/plastic", tags=["Plastic_Burns"])


@router.get("/list", response_model=List[PlasticEncounter])
async def list_encounters(
    ...
    """List Plastic_Burns encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PlasticService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PlasticEncounter)
...
@router.post("/", response_model=PlasticEncounter, status_code=201)
async def create_encounter(
    payload: PlasticEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["plastic_specialist", "plastic_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PlasticService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PlasticEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PlasticEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["plastic_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PlasticService(db, tenant_id)
