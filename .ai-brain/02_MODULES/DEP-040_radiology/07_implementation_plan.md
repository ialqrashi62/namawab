# Implementation Plan — Radiology (DEP-040)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- radiology_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh radiology
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-radiology-<timestamp>`
