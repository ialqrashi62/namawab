# filepath: 02_MODULES/DEP-040/18_backend_models.py
# SQLAlchemy models for Radiology (DEP-040)
...
class RadiologyEncounter(Base):
    __tablename__ = "radiology_encounters"
...
class RadiologyOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("radiology_encounters.id"))
...
class RadiologyResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("radiology_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("radiology_orders.id"))
