# filepath: 02_MODULES/DEP-040/21_backend_router.py
# FastAPI router for Radiology (DEP-040)
...
@router.get("/list", response_model=List[RadiologyEncounter])
async def list_encounters(
    ...
    """List Radiology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = RadiologyService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=RadiologyEncounter)
...
@router.post("/", response_model=RadiologyEncounter, status_code=201)
async def create_encounter(
    payload: RadiologyEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["radiology_specialist", "radiology_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = RadiologyService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=RadiologyEncounter)
async def update_encounter(
    encounter_id: int,
    payload: RadiologyEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["radiology_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = RadiologyService(db, tenant_id)
