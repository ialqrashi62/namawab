// filepath: 02_MODULES/DEP-040/22_frontend_page.tsx
// Main page for Radiology (DEP-040)
...
import { RadiologyAPI } from './24_frontend_api_client';
...
export default function RadiologyPage({ tenantId, patientId, lang = 'ar' }: RadiologyPageProps) {
  ...
  useEffect(() => {
    RadiologyAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    RadiologyAPI.listOrders({ patientId, tenantId }).then(setOrders);
    RadiologyAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="BI_RADS" patientId={patientId} />
        </>
      }
      main={
        <>
