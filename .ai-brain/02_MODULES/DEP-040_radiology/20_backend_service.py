# filepath: 02_MODULES/DEP-040/20_backend_service.py
# Business logic for Radiology (DEP-040)
...
class RadiologyService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import RadiologyEncounter
        q = self.db.query(RadiologyEncounter).filter(
            RadiologyEncounter.tenant_id == self.tenant_id,
            RadiologyEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(RadiologyEncounter.patient_id == patient_id)
        if status:
            q = q.filter(RadiologyEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import RadiologyEncounter
        e = self.db.query(RadiologyEncounter).filter(
            RadiologyEncounter.id == encounter_id,
            RadiologyEncounter.tenant_id == self.tenant_id
        ).first()
