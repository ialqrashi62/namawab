# filepath: 02_MODULES/DEP-040/28_test_unit.py
# Unit tests for Radiology (DEP-040)
...
class TestRadiologyService:
    ...
    def test_init_requires_tenant(self):
        ...
        with pytest.raises(ValueError, match="tenant_id required"):
            RadiologyService(db, tenant_id=None)

    def test_init_sets_tenant(self):
        db = MagicMock()
        svc = RadiologyService(db, tenant_id=1)
        assert svc.tenant_id == 1
        db.execute.assert_called_once()

    def test_list_encounters_filters_by_tenant(self):
        db = MagicMock()
        svc = RadiologyService(db, tenant_id=42)
