// filepath: 02_MODULES/DEP-052/22_frontend_page.tsx
// Main page for Pain_Management (DEP-052)
...
import { PainAPI } from './24_frontend_api_client';
...
export default function PainPage({ tenantId, patientId, lang = 'ar' }: PainPageProps) {
  const { t } = useTranslation('pain');
  ...
  useEffect(() => {
    PainAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PainAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PainAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="علاج الألم"
      title_en="Pain_Management"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="VAS" patientId={patientId} />
        </>
      }
      main={
        <>
