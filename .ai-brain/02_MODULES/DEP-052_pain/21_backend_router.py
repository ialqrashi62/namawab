# filepath: 02_MODULES/DEP-052/21_backend_router.py
# FastAPI router for Pain_Management (DEP-052)
...
from .schemas import *
from .service import PainService

router = APIRouter(prefix="/api/pain", tags=["Pain_Management"])


@router.get("/list", response_model=List[PainEncounter])
async def list_encounters(
    ...
    """List Pain_Management encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PainService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PainEncounter)
...
@router.post("/", response_model=PainEncounter, status_code=201)
async def create_encounter(
    payload: PainEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pain_specialist", "pain_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PainService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PainEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PainEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["pain_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PainService(db, tenant_id)
