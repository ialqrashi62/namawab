# filepath: 02_MODULES/DEP-052/20_backend_service.py
# Business logic for Pain_Management (DEP-052)
...
class PainService:
    """Service layer for Pain_Management.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PainEncounter
        q = self.db.query(PainEncounter).filter(
            PainEncounter.tenant_id == self.tenant_id,
            PainEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PainEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PainEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PainEncounter
        e = self.db.query(PainEncounter).filter(
            PainEncounter.id == encounter_id,
            PainEncounter.tenant_id == self.tenant_id
        ).first()
