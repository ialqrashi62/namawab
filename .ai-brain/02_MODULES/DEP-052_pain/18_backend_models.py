# filepath: 02_MODULES/DEP-052/18_backend_models.py
# SQLAlchemy models for Pain_Management (DEP-052)
...
class PainEncounter(Base):
    __tablename__ = "pain_encounters"
...
class PainOrder(Base):
    __tablename__ = "pain_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pain_encounters.id"))
...
class PainResult(Base):
    __tablename__ = "pain_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("pain_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("pain_orders.id"))
