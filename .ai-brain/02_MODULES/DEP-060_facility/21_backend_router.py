# filepath: 02_MODULES/DEP-060/21_backend_router.py
# FastAPI router for Facility_Biomedical_Management (DEP-060)
...
router = APIRouter(prefix="/api/facility", tags=["Facility_Biomedical_Management"])


@router.get("/list", response_model=List[FacilityEncounter])
async def list_encounters(
    ...
    """List Facility_Biomedical_Management encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = FacilityService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=FacilityEncounter)
...
@router.post("/", response_model=FacilityEncounter, status_code=201)
async def create_encounter(
    payload: FacilityEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["facility_specialist", "facility_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = FacilityService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=FacilityEncounter)
async def update_encounter(
    encounter_id: int,
    payload: FacilityEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["facility_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = FacilityService(db, tenant_id)
