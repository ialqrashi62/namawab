# filepath: 02_MODULES/DEP-060/18_backend_models.py
# SQLAlchemy models for Facility_Biomedical_Management (DEP-060)
...
class FacilityEncounter(Base):
    __tablename__ = "facility_encounters"
...
class FacilityOrder(Base):
    __tablename__ = "facility_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("facility_encounters.id"))
...
class FacilityResult(Base):
    __tablename__ = "facility_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("facility_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("facility_orders.id"))
...
class FacilityNote(Base):
    __tablename__ = "facility_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("facility_encounters.id"))
