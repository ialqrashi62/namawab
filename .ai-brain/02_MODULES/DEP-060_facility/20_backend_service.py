# filepath: 02_MODULES/DEP-060/20_backend_service.py
# Business logic for Facility_Biomedical_Management (DEP-060)
...
class FacilityService:
    """Service layer for Facility_Biomedical_Management.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import FacilityEncounter
        q = self.db.query(FacilityEncounter).filter(
            FacilityEncounter.tenant_id == self.tenant_id,
            FacilityEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(FacilityEncounter.patient_id == patient_id)
        if status:
            q = q.filter(FacilityEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import FacilityEncounter
        e = self.db.query(FacilityEncounter).filter(
            FacilityEncounter.id == encounter_id,
            FacilityEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import FacilityOrder
        o = FacilityOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
