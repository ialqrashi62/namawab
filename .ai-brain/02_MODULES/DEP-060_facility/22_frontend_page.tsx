// filepath: 02_MODULES/DEP-060/22_frontend_page.tsx
// Main page for Facility_Biomedical_Management (DEP-060)
...
import { FacilityAPI } from './24_frontend_api_client';
...
export default function FacilityPage({ tenantId, patientId, lang = 'ar' }: FacilityPageProps) {
  ...
  useEffect(() => {
    FacilityAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    FacilityAPI.listOrders({ patientId, tenantId }).then(setOrders);
    FacilityAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="إدارة المنشأة"
      title_en="Facility_Biomedical_Management"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="equipment_uptime" patientId={patientId} />
        </>
      }
      main={
        <>
