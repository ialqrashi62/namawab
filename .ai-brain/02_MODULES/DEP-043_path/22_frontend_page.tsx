// filepath: 02_MODULES/DEP-043/22_frontend_page.tsx
// Main page for Pathology (DEP-043)
...
import { PathAPI } from './24_frontend_api_client';
...
export default function PathPage({ tenantId, patientId, lang = 'ar' }: PathPageProps) {
  ...
  useEffect(() => {
    PathAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    PathAPI.listOrders({ patientId, tenantId }).then(setOrders);
    PathAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="TNM_staging" patientId={patientId} />
        </>
      }
      main={
        <>
