# filepath: 02_MODULES/DEP-043/18_backend_models.py
# SQLAlchemy models for Pathology (DEP-043)
...
class PathEncounter(Base):
    __tablename__ = "path_encounters"
...
class PathOrder(Base):
    __tablename__ = "path_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("path_encounters.id"))
...
class PathResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("path_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("path_orders.id"))
