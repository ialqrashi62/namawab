# filepath: 02_MODULES/DEP-043/20_backend_service.py
# Business logic for Pathology (DEP-043)
...
class PathService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import PathEncounter
        q = self.db.query(PathEncounter).filter(
            PathEncounter.tenant_id == self.tenant_id,
            PathEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(PathEncounter.patient_id == patient_id)
        if status:
            q = q.filter(PathEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import PathEncounter
        e = self.db.query(PathEncounter).filter(
            PathEncounter.id == encounter_id,
            PathEncounter.tenant_id == self.tenant_id
        ).first()
