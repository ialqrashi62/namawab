# filepath: 02_MODULES/DEP-043/21_backend_router.py
# FastAPI router for Pathology (DEP-043)
...
router = APIRouter(prefix="/api/path", tags=["Pathology"])


@router.get("/list", response_model=List[PathEncounter])
async def list_encounters(
    ...
    """List Pathology encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = PathService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=PathEncounter)
...
@router.post("/", response_model=PathEncounter, status_code=201)
async def create_encounter(
    payload: PathEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["path_specialist", "path_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = PathService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=PathEncounter)
async def update_encounter(
    encounter_id: int,
    payload: PathEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["path_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = PathService(db, tenant_id)
