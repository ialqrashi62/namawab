...
Feature: Pathology clinical workflow
  ...
  Scenario: Create new encounter with valid data
    ...
    When the specialist POSTs to /api/path/ with patient_id=100045
    Then the response status is 201
    And the encounter has status "active"

  Scenario: Unauthorized access is rejected
    Given no auth token
    When the user requests GET /api/path/list
  ...
  Scenario: AI diagnosis returns differential
    ...
    When the specialist requests POST /api/path/ai/diagnose
