# filepath: 02_MODULES/DEP-008/21_backend_router.py
# FastAPI router for Infectious_Diseases (DEP-008)
...
from .schemas import *
from .service import InfectiousService

router = APIRouter(prefix="/api/infectious", tags=["Infectious_Diseases"])


@router.get("/list", response_model=List[InfectiousEncounter])
async def list_encounters(
    ...
    """List Infectious_Diseases encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = InfectiousService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=InfectiousEncounter)
...
@router.post("/", response_model=InfectiousEncounter, status_code=201)
async def create_encounter(
    payload: InfectiousEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["infectious_specialist", "infectious_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = InfectiousService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=InfectiousEncounter)
async def update_encounter(
    encounter_id: int,
    payload: InfectiousEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["infectious_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = InfectiousService(db, tenant_id)
