# filepath: 02_MODULES/DEP-008/18_backend_models.py
# SQLAlchemy models for Infectious_Diseases (DEP-008)
...
class InfectiousEncounter(Base):
    __tablename__ = "infectious_encounters"
...
class InfectiousOrder(Base):
    __tablename__ = "infectious_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("infectious_encounters.id"))
...
class InfectiousResult(Base):
    __tablename__ = "infectious_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("infectious_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("infectious_orders.id"))
...
class InfectiousNote(Base):
    __tablename__ = "infectious_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("infectious_encounters.id"))
