# Implementation Plan — Infectious_Diseases (DEP-008)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- infectious_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh infectious
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-infectious-<timestamp>`
