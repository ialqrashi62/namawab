# filepath: 02_MODULES/DEP-008/20_backend_service.py
# Business logic for Infectious_Diseases (DEP-008)
...
class InfectiousService:
    """Service layer for Infectious_Diseases.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import InfectiousEncounter
        q = self.db.query(InfectiousEncounter).filter(
            InfectiousEncounter.tenant_id == self.tenant_id,
            InfectiousEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(InfectiousEncounter.patient_id == patient_id)
        if status:
            q = q.filter(InfectiousEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import InfectiousEncounter
        e = self.db.query(InfectiousEncounter).filter(
            InfectiousEncounter.id == encounter_id,
            InfectiousEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import InfectiousOrder
        o = InfectiousOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
