# Implementation Plan — Insurance_Claims_NPHIES (DEP-058)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- insurance_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh insurance
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-insurance-<timestamp>`
