# filepath: 02_MODULES/DEP-058/20_backend_service.py
# Business logic for Insurance_Claims_NPHIES (DEP-058)
...
class InsuranceService:
    """Service layer for Insurance_Claims_NPHIES.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import InsuranceEncounter
        q = self.db.query(InsuranceEncounter).filter(
            InsuranceEncounter.tenant_id == self.tenant_id,
            InsuranceEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(InsuranceEncounter.patient_id == patient_id)
        if status:
            q = q.filter(InsuranceEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import InsuranceEncounter
        e = self.db.query(InsuranceEncounter).filter(
            InsuranceEncounter.id == encounter_id,
            InsuranceEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import InsuranceOrder
        o = InsuranceOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
