# filepath: 02_MODULES/DEP-058/28_test_unit.py
# Unit tests for Insurance_Claims_NPHIES (DEP-058)
...
class TestInsuranceService:
    ...
    def test_init_requires_tenant(self):
        ...
        with pytest.raises(ValueError, match="tenant_id required"):
            InsuranceService(db, tenant_id=None)

    def test_init_sets_tenant(self):
        db = MagicMock()
        svc = InsuranceService(db, tenant_id=1)
        assert svc.tenant_id == 1
        db.execute.assert_called_once()

    def test_list_encounters_filters_by_tenant(self):
        db = MagicMock()
        svc = InsuranceService(db, tenant_id=42)
