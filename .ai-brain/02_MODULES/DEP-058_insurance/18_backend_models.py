# filepath: 02_MODULES/DEP-058/18_backend_models.py
# SQLAlchemy models for Insurance_Claims_NPHIES (DEP-058)
...
class InsuranceEncounter(Base):
    __tablename__ = "insurance_encounters"
...
class InsuranceOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("insurance_encounters.id"))
...
class InsuranceResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("insurance_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("insurance_orders.id"))
...
class InsuranceNote(Base):
    __tablename__ = "insurance_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("insurance_encounters.id"))
