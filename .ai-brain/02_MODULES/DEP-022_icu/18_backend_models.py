# filepath: 02_MODULES/DEP-022/18_backend_models.py
# SQLAlchemy models for ICU_Adult (DEP-022)
...
class IcuEncounter(Base):
    __tablename__ = "icu_encounters"
...
class IcuOrder(Base):
    __tablename__ = "icu_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("icu_encounters.id"))
...
class IcuResult(Base):
    __tablename__ = "icu_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("icu_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("icu_orders.id"))
