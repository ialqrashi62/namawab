# filepath: 02_MODULES/DEP-022/20_backend_service.py
# Business logic for ICU_Adult (DEP-022)
...
class IcuService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import IcuEncounter
        q = self.db.query(IcuEncounter).filter(
            IcuEncounter.tenant_id == self.tenant_id,
            IcuEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(IcuEncounter.patient_id == patient_id)
        if status:
            q = q.filter(IcuEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import IcuEncounter
        e = self.db.query(IcuEncounter).filter(
            IcuEncounter.id == encounter_id,
            IcuEncounter.tenant_id == self.tenant_id
        ).first()
