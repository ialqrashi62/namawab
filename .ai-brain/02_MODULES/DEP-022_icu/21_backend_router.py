# filepath: 02_MODULES/DEP-022/21_backend_router.py
# FastAPI router for ICU_Adult (DEP-022)
...
from .schemas import *
from .service import IcuService

router = APIRouter(prefix="/api/icu", tags=["ICU_Adult"])


@router.get("/list", response_model=List[IcuEncounter])
async def list_encounters(
    ...
    """List ICU_Adult encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = IcuService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=IcuEncounter)
...
@router.post("/", response_model=IcuEncounter, status_code=201)
async def create_encounter(
    payload: IcuEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["icu_specialist", "icu_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = IcuService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=IcuEncounter)
async def update_encounter(
    encounter_id: int,
    payload: IcuEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["icu_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = IcuService(db, tenant_id)
