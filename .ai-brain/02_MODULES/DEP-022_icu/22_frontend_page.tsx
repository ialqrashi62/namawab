// filepath: 02_MODULES/DEP-022/22_frontend_page.tsx
// Main page for ICU_Adult (DEP-022)
...
import { IcuAPI } from './24_frontend_api_client';

interface IcuPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function IcuPage({ tenantId, patientId, lang = 'ar' }: IcuPageProps) {
  const { t } = useTranslation('icu');
  ...
  useEffect(() => {
    IcuAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    IcuAPI.listOrders({ patientId, tenantId }).then(setOrders);
    IcuAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
    ...
    <DeptPage
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="APACHE_II" patientId={patientId} />
        </>
      }
      main={
        <>
