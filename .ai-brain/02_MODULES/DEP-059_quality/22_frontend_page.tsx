// filepath: 02_MODULES/DEP-059/22_frontend_page.tsx
// Main page for Quality_Safety (DEP-059)
...
import { QualityAPI } from './24_frontend_api_client';
...
export default function QualityPage({ tenantId, patientId, lang = 'ar' }: QualityPageProps) {
  ...
  useEffect(() => {
    QualityAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    QualityAPI.listOrders({ patientId, tenantId }).then(setOrders);
    QualityAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="الجودة وسلامة المرضى"
      title_en="Quality_Safety"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="HSMR" patientId={patientId} />
        </>
      }
      main={
        <>
