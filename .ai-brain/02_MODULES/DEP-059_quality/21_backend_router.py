# filepath: 02_MODULES/DEP-059/21_backend_router.py
# FastAPI router for Quality_Safety (DEP-059)
...
router = APIRouter(prefix="/api/quality", tags=["Quality_Safety"])


@router.get("/list", response_model=List[QualityEncounter])
async def list_encounters(
    ...
    """List Quality_Safety encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = QualityService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=QualityEncounter)
...
@router.post("/", response_model=QualityEncounter, status_code=201)
async def create_encounter(
    payload: QualityEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["quality_specialist", "quality_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = QualityService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=QualityEncounter)
async def update_encounter(
    encounter_id: int,
    payload: QualityEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["quality_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = QualityService(db, tenant_id)
