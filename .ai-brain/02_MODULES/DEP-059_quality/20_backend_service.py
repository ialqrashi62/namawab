# filepath: 02_MODULES/DEP-059/20_backend_service.py
# Business logic for Quality_Safety (DEP-059)
...
class QualityService:
    """Service layer for Quality_Safety.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import QualityEncounter
        q = self.db.query(QualityEncounter).filter(
            QualityEncounter.tenant_id == self.tenant_id,
            QualityEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(QualityEncounter.patient_id == patient_id)
        if status:
            q = q.filter(QualityEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import QualityEncounter
        e = self.db.query(QualityEncounter).filter(
            QualityEncounter.id == encounter_id,
            QualityEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import QualityOrder
        o = QualityOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
