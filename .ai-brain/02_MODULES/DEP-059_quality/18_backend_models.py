# filepath: 02_MODULES/DEP-059/18_backend_models.py
# SQLAlchemy models for Quality_Safety (DEP-059)
...
class QualityEncounter(Base):
    __tablename__ = "quality_encounters"
...
class QualityOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("quality_encounters.id"))
...
class QualityResult(Base):
    ...
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("quality_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("quality_orders.id"))
