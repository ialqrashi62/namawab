# filepath: 02_MODULES/DEP-014/21_backend_router.py
# FastAPI router for Cardiothoracic_Surgery (DEP-014)
...
from .schemas import *
from .service import CardiothService

router = APIRouter(prefix="/api/cardioTh", tags=["Cardiothoracic_Surgery"])


@router.get("/list", response_model=List[CardiothEncounter])
async def list_encounters(
    ...
    """List Cardiothoracic_Surgery encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = CardiothService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=CardiothEncounter)
...
@router.post("/", response_model=CardiothEncounter, status_code=201)
async def create_encounter(
    payload: CardiothEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["cardioTh_specialist", "cardioTh_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = CardiothService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=CardiothEncounter)
async def update_encounter(
    encounter_id: int,
    payload: CardiothEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["cardioTh_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = CardiothService(db, tenant_id)
