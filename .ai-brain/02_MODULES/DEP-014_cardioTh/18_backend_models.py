# filepath: 02_MODULES/DEP-014/18_backend_models.py
# SQLAlchemy models for Cardiothoracic_Surgery (DEP-014)
...
class CardiothEncounter(Base):
    __tablename__ = "cardioTh_encounters"
...
class CardiothOrder(Base):
    __tablename__ = "cardioTh_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("cardioTh_encounters.id"))
...
class CardiothResult(Base):
    __tablename__ = "cardioTh_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("cardioTh_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("cardioTh_orders.id"))
...
class CardiothNote(Base):
    __tablename__ = "cardioTh_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("cardioTh_encounters.id"))
