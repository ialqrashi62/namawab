# filepath: 02_MODULES/DEP-014/20_backend_service.py
# Business logic for Cardiothoracic_Surgery (DEP-014)
...
class CardiothService:
    """Service layer for Cardiothoracic_Surgery.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import CardiothEncounter
        q = self.db.query(CardiothEncounter).filter(
            CardiothEncounter.tenant_id == self.tenant_id,
            CardiothEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(CardiothEncounter.patient_id == patient_id)
        if status:
            q = q.filter(CardiothEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import CardiothEncounter
        e = self.db.query(CardiothEncounter).filter(
            CardiothEncounter.id == encounter_id,
            CardiothEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import CardiothOrder
        o = CardiothOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
