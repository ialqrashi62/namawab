# filepath: 02_MODULES/DEP-037/21_backend_router.py
# FastAPI router for Maternal_Fetal_Medicine (DEP-037)
...
from .schemas import *
from .service import MfmService

router = APIRouter(prefix="/api/mfm", tags=["Maternal_Fetal_Medicine"])


@router.get("/list", response_model=List[MfmEncounter])
async def list_encounters(
    ...
    """List Maternal_Fetal_Medicine encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = MfmService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=MfmEncounter)
...
@router.post("/", response_model=MfmEncounter, status_code=201)
async def create_encounter(
    payload: MfmEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["mfm_specialist", "mfm_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = MfmService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=MfmEncounter)
async def update_encounter(
    encounter_id: int,
    payload: MfmEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["mfm_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = MfmService(db, tenant_id)
