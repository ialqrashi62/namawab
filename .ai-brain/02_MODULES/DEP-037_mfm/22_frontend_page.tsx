// filepath: 02_MODULES/DEP-037/22_frontend_page.tsx
// Main page for Maternal_Fetal_Medicine (DEP-037)
...
import { MfmAPI } from './24_frontend_api_client';

interface MfmPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function MfmPage({ tenantId, patientId, lang = 'ar' }: MfmPageProps) {
  const { t } = useTranslation('mfm');
  ...
  useEffect(() => {
    MfmAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    MfmAPI.listOrders({ patientId, tenantId }).then(setOrders);
    MfmAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="طب الأم والجنين"
      title_en="Maternal_Fetal_Medicine"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="Bishops" patientId={patientId} />
        </>
      }
      main={
        <>
