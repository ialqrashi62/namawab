# filepath: 02_MODULES/DEP-037/18_backend_models.py
# SQLAlchemy models for Maternal_Fetal_Medicine (DEP-037)
...
class MfmEncounter(Base):
    __tablename__ = "mfm_encounters"
...
class MfmOrder(Base):
    __tablename__ = "mfm_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("mfm_encounters.id"))
...
class MfmResult(Base):
    __tablename__ = "mfm_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("mfm_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("mfm_orders.id"))
