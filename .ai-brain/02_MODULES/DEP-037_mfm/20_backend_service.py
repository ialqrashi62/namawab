# filepath: 02_MODULES/DEP-037/20_backend_service.py
# Business logic for Maternal_Fetal_Medicine (DEP-037)
...
class MfmService:
    """Service layer for Maternal_Fetal_Medicine.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import MfmEncounter
        q = self.db.query(MfmEncounter).filter(
            MfmEncounter.tenant_id == self.tenant_id,
            MfmEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(MfmEncounter.patient_id == patient_id)
        if status:
            q = q.filter(MfmEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import MfmEncounter
        e = self.db.query(MfmEncounter).filter(
            MfmEncounter.id == encounter_id,
            MfmEncounter.tenant_id == self.tenant_id
        ).first()
