# filepath: 02_MODULES/DEP-051/21_backend_router.py
# FastAPI router for Anesthesia (DEP-051)
...
from .schemas import *
from .service import AnesthService

router = APIRouter(prefix="/api/anesth", tags=["Anesthesia"])


@router.get("/list", response_model=List[AnesthEncounter])
async def list_encounters(
    ...
    """List Anesthesia encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = AnesthService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=AnesthEncounter)
...
@router.post("/", response_model=AnesthEncounter, status_code=201)
async def create_encounter(
    payload: AnesthEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["anesth_specialist", "anesth_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = AnesthService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=AnesthEncounter)
async def update_encounter(
    encounter_id: int,
    payload: AnesthEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["anesth_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = AnesthService(db, tenant_id)
