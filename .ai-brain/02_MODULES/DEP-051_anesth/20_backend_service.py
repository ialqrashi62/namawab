# filepath: 02_MODULES/DEP-051/20_backend_service.py
# Business logic for Anesthesia (DEP-051)
...
class AnesthService:
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import AnesthEncounter
        q = self.db.query(AnesthEncounter).filter(
            AnesthEncounter.tenant_id == self.tenant_id,
            AnesthEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(AnesthEncounter.patient_id == patient_id)
        if status:
            q = q.filter(AnesthEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import AnesthEncounter
        e = self.db.query(AnesthEncounter).filter(
            AnesthEncounter.id == encounter_id,
            AnesthEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import AnesthOrder
        o = AnesthOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
