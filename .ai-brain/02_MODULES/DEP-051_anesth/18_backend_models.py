# filepath: 02_MODULES/DEP-051/18_backend_models.py
# SQLAlchemy models for Anesthesia (DEP-051)
...
class AnesthEncounter(Base):
    __tablename__ = "anesth_encounters"
...
class AnesthOrder(Base):
    __tablename__ = "anesth_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("anesth_encounters.id"))
...
class AnesthResult(Base):
    __tablename__ = "anesth_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("anesth_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("anesth_orders.id"))
...
class AnesthNote(Base):
    __tablename__ = "anesth_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("anesth_encounters.id"))
