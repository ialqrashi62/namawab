// filepath: 02_MODULES/DEP-051/22_frontend_page.tsx
// Main page for Anesthesia (DEP-051)
...
import { AnesthAPI } from './24_frontend_api_client';

interface AnesthPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function AnesthPage({ tenantId, patientId, lang = 'ar' }: AnesthPageProps) {
  const { t } = useTranslation('anesth');
  ...
  useEffect(() => {
    AnesthAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    AnesthAPI.listOrders({ patientId, tenantId }).then(setOrders);
    AnesthAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);
