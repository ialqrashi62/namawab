// filepath: 02_MODULES/DEP-047/22_frontend_page.tsx
// Main page for Occupational_Therapy (DEP-047)
...
import { OtAPI } from './24_frontend_api_client';
...
export default function OtPage({ tenantId, patientId, lang = 'ar' }: OtPageProps) {
  ...
  useEffect(() => {
    OtAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    OtAPI.listOrders({ patientId, tenantId }).then(setOrders);
    OtAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="العلاج الوظيفي"
      title_en="Occupational_Therapy"
      tenantId={tenantId}
      lang={lang}
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="A_One" patientId={patientId} />
        </>
      }
      main={
        <>
