# filepath: 02_MODULES/DEP-047/20_backend_service.py
# Business logic for Occupational_Therapy (DEP-047)
...
class OtService:
    """Service layer for Occupational_Therapy.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import OtEncounter
        q = self.db.query(OtEncounter).filter(
            OtEncounter.tenant_id == self.tenant_id,
            OtEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(OtEncounter.patient_id == patient_id)
        if status:
            q = q.filter(OtEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import OtEncounter
        e = self.db.query(OtEncounter).filter(
            OtEncounter.id == encounter_id,
            OtEncounter.tenant_id == self.tenant_id
        ).first()
