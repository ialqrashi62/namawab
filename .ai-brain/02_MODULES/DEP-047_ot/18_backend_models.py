# filepath: 02_MODULES/DEP-047/18_backend_models.py
# SQLAlchemy models for Occupational_Therapy (DEP-047)
...
class OtEncounter(Base):
    __tablename__ = "ot_encounters"
...
class OtOrder(Base):
    ...
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ot_encounters.id"))
...
class OtResult(Base):
    __tablename__ = "ot_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("ot_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("ot_orders.id"))
