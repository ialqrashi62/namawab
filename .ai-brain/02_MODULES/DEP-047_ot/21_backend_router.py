# filepath: 02_MODULES/DEP-047/21_backend_router.py
# FastAPI router for Occupational_Therapy (DEP-047)
...
router = APIRouter(prefix="/api/ot", tags=["Occupational_Therapy"])


@router.get("/list", response_model=List[OtEncounter])
async def list_encounters(
    ...
    """List Occupational_Therapy encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = OtService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=OtEncounter)
...
@router.post("/", response_model=OtEncounter, status_code=201)
async def create_encounter(
    payload: OtEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["ot_specialist", "ot_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = OtService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=OtEncounter)
async def update_encounter(
    encounter_id: int,
    payload: OtEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["ot_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = OtService(db, tenant_id)
