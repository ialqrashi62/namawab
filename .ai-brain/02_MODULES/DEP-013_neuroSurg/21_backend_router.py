# filepath: 02_MODULES/DEP-013/21_backend_router.py
# FastAPI router for Neurosurgery (DEP-013)
...
from .schemas import *
from .service import NeurosurgService

router = APIRouter(prefix="/api/neuroSurg", tags=["Neurosurgery"])


@router.get("/list", response_model=List[NeurosurgEncounter])
async def list_encounters(
    ...
    """List Neurosurgery encounters for current tenant."""
    require_tenant_scope(request)
    tenant_id = request.state.tenant_id
    svc = NeurosurgService(db, tenant_id)
    return svc.list_encounters(patient_id, status, limit, offset)


@router.get("/{encounter_id}", response_model=NeurosurgEncounter)
...
@router.post("/", response_model=NeurosurgEncounter, status_code=201)
async def create_encounter(
    payload: NeurosurgEncounterCreate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["neuroSurg_specialist", "neuroSurg_nurse", "admin"])
    tenant_id = request.state.tenant_id
    svc = NeurosurgService(db, tenant_id)
    return svc.create_encounter(payload.model_dump(), actor_id=user["id"])


@router.put("/{encounter_id}", response_model=NeurosurgEncounter)
async def update_encounter(
    encounter_id: int,
    payload: NeurosurgEncounterUpdate,
    request: Request,
    db=Depends(get_db),
    user=Depends(verify_jwt)
):
    require_tenant_scope(request)
    require_role(user, ["neuroSurg_specialist", "admin"])
    tenant_id = request.state.tenant_id
    svc = NeurosurgService(db, tenant_id)
