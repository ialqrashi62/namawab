# Implementation Plan — Neurosurgery (DEP-013)
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test -- neuroSurg_test.js
      - run: npm run lint
  deploy-staging:
    ...
    runs-on: ubuntu-latest
    steps:
      - run: ./scripts/deploy_staging.sh neuroSurg
```

## 3. Rollback Plan
1. Tag current production: `git tag prod-neuroSurg-<timestamp>`
