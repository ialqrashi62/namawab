# filepath: 02_MODULES/DEP-013/20_backend_service.py
# Business logic for Neurosurgery (DEP-013)
...
class NeurosurgService:
    """Service layer for Neurosurgery.
    ...
    def list_encounters(self, patient_id: Optional[int] = None, status: Optional[str] = None,
        ...
        from .models import NeurosurgEncounter
        q = self.db.query(NeurosurgEncounter).filter(
            NeurosurgEncounter.tenant_id == self.tenant_id,
            NeurosurgEncounter.deleted_at.is_(None)
        )
        if patient_id:
            q = q.filter(NeurosurgEncounter.patient_id == patient_id)
        if status:
            q = q.filter(NeurosurgEncounter.status == status)
        return [e.to_dict() for e in q.limit(limit).offset(offset).all()]

    def get_encounter(self, encounter_id: int) -> Optional[dict]:
        from .models import NeurosurgEncounter
        e = self.db.query(NeurosurgEncounter).filter(
            NeurosurgEncounter.id == encounter_id,
            NeurosurgEncounter.tenant_id == self.tenant_id
        ).first()
    ...
    def create_order(self, payload: dict, actor_id: int) -> dict:
        from .models import NeurosurgOrder
        o = NeurosurgOrder(
            tenant_id=self.tenant_id,
            patient_id=payload["patient_id"],
