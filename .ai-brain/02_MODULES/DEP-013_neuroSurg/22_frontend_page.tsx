// filepath: 02_MODULES/DEP-013/22_frontend_page.tsx
// Main page for Neurosurgery (DEP-013)
...
import { NeurosurgAPI } from './24_frontend_api_client';

interface NeurosurgPageProps {
  tenantId: number;
  patientId: number;
  lang?: 'ar' | 'en';
}

export default function NeurosurgPage({ tenantId, patientId, lang = 'ar' }: NeurosurgPageProps) {
  const { t } = useTranslation('neuroSurg');
  ...
  useEffect(() => {
    NeurosurgAPI.listEncounters({ patientId, tenantId }).then(setEncounter);
    NeurosurgAPI.listOrders({ patientId, tenantId }).then(setOrders);
    NeurosurgAPI.listResults({ patientId, tenantId }).then(setResults);
  }, [patientId, tenantId]);

  return (
    <DeptPage
      title_ar="جراحة المخ والأعصاب"
      ...
      sidebar={
        <>
          ...
          <RiskStratifier scoreType="GCS" patientId={patientId} />
        </>
      }
      main={
        <>
