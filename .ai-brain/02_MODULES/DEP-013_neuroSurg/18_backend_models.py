# filepath: 02_MODULES/DEP-013/18_backend_models.py
# SQLAlchemy models for Neurosurgery (DEP-013)
...
class NeurosurgEncounter(Base):
    __tablename__ = "neuroSurg_encounters"
...
class NeurosurgOrder(Base):
    __tablename__ = "neuroSurg_orders"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("neuroSurg_encounters.id"))
...
class NeurosurgResult(Base):
    __tablename__ = "neuroSurg_results"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("neuroSurg_encounters.id"))
    order_id = Column(BigInteger, ForeignKey("neuroSurg_orders.id"))
...
class NeurosurgNote(Base):
    __tablename__ = "neuroSurg_notes"
    id = Column(BigInteger, primary_key=True)
    tenant_id = Column(BigInteger, nullable=False, index=True)
    patient_id = Column(BigInteger, ForeignKey("patients.id"), nullable=False, index=True)
    encounter_id = Column(BigInteger, ForeignKey("neuroSurg_encounters.id"))
