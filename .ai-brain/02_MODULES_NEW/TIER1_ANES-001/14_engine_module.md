...
export class ANES_001_Engine
  implements Engine<ANES_001_Input, ANES_001_Output>
{
  id = 'ANES-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: ANES_001_Input, ctx: ExecutionContext): Promise<ANES_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_anes-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:ANES-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('ANES-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new ANES_001_Engine(
