...
export class card_101_Engine implements Engine<card_101_Input, card_101_Output> {
  ...
  async execute(input: card_101_Input, ctx: ExecutionContext): Promise<card_101_Output> {
    ...
    const chunks = await this.rag.retrieve({ query: 'specialty context', corpus: ['cba','card'], topK: 5, tenantId });
    const redFlags = [] as RedFlag[];

    const llm = await this.langchain.build('PROMPT:CARD-101:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('CARD-101'), tenantId });
