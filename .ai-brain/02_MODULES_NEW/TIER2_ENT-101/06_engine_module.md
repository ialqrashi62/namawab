...
export class ent_101_Engine implements Engine<ent_101_Input, ent_101_Output> {
  ...
  async execute(input: ent_101_Input, ctx: ExecutionContext): Promise<ent_101_Output> {
    ...
    const chunks = await this.rag.retrieve({ query: 'specialty context', corpus: ['cba','ent'], topK: 5, tenantId });
    const redFlags = [] as RedFlag[];

    const llm = await this.langchain.build('PROMPT:ENT-101:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('ENT-101'), tenantId });
