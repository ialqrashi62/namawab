...
export class fct_101_Engine implements Engine<fct_101_Input, fct_101_Output> {
  ...
  async execute(input: fct_101_Input, ctx: ExecutionContext): Promise<fct_101_Output> {
    ...
    const chunks = await this.rag.retrieve({ query: 'specialty context', corpus: ['cba','fct'], topK: 5, tenantId });
    const redFlags = [] as RedFlag[];

    const llm = await this.langchain.build('PROMPT:FCT-101:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('FCT-101'), tenantId });
