...
export class rare_103_Engine implements Engine<rare_103_Input, rare_103_Output> {
  ...
  async execute(input: rare_103_Input, ctx: ExecutionContext): Promise<rare_103_Output> {
    ...
    const llm = await this.langchain.build('PROMPT:RARE-103:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('RARE-103'), tenantId });
