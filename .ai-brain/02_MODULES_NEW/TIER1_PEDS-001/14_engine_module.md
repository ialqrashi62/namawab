...
export class PEDS_001_Engine
  implements Engine<PEDS_001_Input, PEDS_001_Output>
{
  id = 'PEDS-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: PEDS_001_Input, ctx: ExecutionContext): Promise<PEDS_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_peds-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:PEDS-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('PEDS-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new PEDS_001_Engine(
