...
export class rehab_103_Engine implements Engine<rehab_103_Input, rehab_103_Output> {
  ...
  async execute(input: rehab_103_Input, ctx: ExecutionContext): Promise<rehab_103_Output> {
    ...
    const chunks = await this.rag.retrieve({ query: 'specialty context', corpus: ['cba','rehab'], topK: 5, tenantId });
    const redFlags = [] as RedFlag[];

    const llm = await this.langchain.build('PROMPT:REHAB-103:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('REHAB-103'), tenantId });
