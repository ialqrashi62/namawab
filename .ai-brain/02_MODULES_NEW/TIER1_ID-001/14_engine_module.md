...
export class ID_001_Engine
  implements Engine<ID_001_Input, ID_001_Output>
{
  id = 'ID-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: ID_001_Input, ctx: ExecutionContext): Promise<ID_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_id-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:ID-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('ID-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new ID_001_Engine(
