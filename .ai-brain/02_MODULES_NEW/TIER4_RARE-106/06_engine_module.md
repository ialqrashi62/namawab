...
export class rare_106_Engine implements Engine<rare_106_Input, rare_106_Output> {
  ...
  async execute(input: rare_106_Input, ctx: ExecutionContext): Promise<rare_106_Output> {
    ...
    const llm = await this.langchain.build('PROMPT:RARE-106:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('RARE-106'), tenantId });
