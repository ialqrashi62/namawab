# ORTHO-001 — Prompt Engineering

## Prompt Registry Entry

```yaml
- id: PROMPT:ORTHO-001:initial_assessment
  version: 1.0.0
  status: draft
  owner: CMO
  dept: ORTHO-001
  ...
  input_schema_ref: SCHEMA:ortho_001_encounter_input
  output_schema_ref: SCHEMA:ortho_001_assessment_output
  guardrail_set_id: GRD:ORTHO-001:v1
  citations_required: 3
  max_tokens: 1500
  temperature: 0.1
  model_target: gpt-4o
  eval_id: EVAL:ORTHO-001:initial_assessment
  few_shot_count: 5
```

## Guardrails (per GRD:ORTHO-001:v1)
...
```
npm run prompt:eval -- --id=PROMPT:ORTHO-001:initial_assessment
```
