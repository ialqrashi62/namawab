# ORTHO-001 — Engine Module

```ts
// namaweb/engines/ortho_001/initial_assessment.engine.ts
import { Engine, ExecutionContext, PatientPort, GuidelinePort, AuditPort, CalculatorPort, RAGPort } from '../../shared';

interface ORTHO_001_Input {
...
export class ORTHO_001_Engine
  implements Engine<ORTHO_001_Input, ORTHO_001_Output>
{
  id = 'ORTHO-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: ORTHO_001_Input, ctx: ExecutionContext): Promise<ORTHO_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_ortho-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:ORTHO-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('ORTHO-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new ORTHO_001_Engine(
