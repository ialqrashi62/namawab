...
export class pulm_103_Engine implements Engine<pulm_103_Input, pulm_103_Output> {
  ...
  async execute(input: pulm_103_Input, ctx: ExecutionContext): Promise<pulm_103_Output> {
    ...
    const llm = await this.langchain.build('PROMPT:PULM-103:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('PULM-103'), tenantId });
