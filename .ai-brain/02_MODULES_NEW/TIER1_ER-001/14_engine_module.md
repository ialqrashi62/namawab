...
export class ER_001_Engine
  implements Engine<ER_001_Input, ER_001_Output>
{
  id = 'ER-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: ER_001_Input, ctx: ExecutionContext): Promise<ER_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_er-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:ER-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('ER-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new ER_001_Engine(
