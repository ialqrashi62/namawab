...
export class gi_101_Engine implements Engine<gi_101_Input, gi_101_Output> {
  ...
  async execute(input: gi_101_Input, ctx: ExecutionContext): Promise<gi_101_Output> {
    ...
    const chunks = await this.rag.retrieve({ query: 'specialty context', corpus: ['cba','gi'], topK: 5, tenantId });
    const redFlags = [] as RedFlag[];

    const llm = await this.langchain.build('PROMPT:GI-101:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('GI-101'), tenantId });
