...
export class rare_111_Engine implements Engine<rare_111_Input, rare_111_Output> {
  ...
  async execute(input: rare_111_Input, ctx: ExecutionContext): Promise<rare_111_Output> {
    ...
    const llm = await this.langchain.build('PROMPT:RARE-111:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('RARE-111'), tenantId });
