...
export class ENT_001_Engine
  implements Engine<ENT_001_Input, ENT_001_Output>
{
  id = 'ENT-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: ENT_001_Input, ctx: ExecutionContext): Promise<ENT_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_ent-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:ENT-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('ENT-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new ENT_001_Engine(
