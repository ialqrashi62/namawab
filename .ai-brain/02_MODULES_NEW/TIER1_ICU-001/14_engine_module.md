...
export class ICU_001_Engine
  implements Engine<ICU_001_Input, ICU_001_Output>
{
  id = 'ICU-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: ICU_001_Input, ctx: ExecutionContext): Promise<ICU_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_icu-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:ICU-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('ICU-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new ICU_001_Engine(
