...
export class neuro_101_Engine implements Engine<neuro_101_Input, neuro_101_Output> {
  ...
  async execute(input: neuro_101_Input, ctx: ExecutionContext): Promise<neuro_101_Output> {
    ...
    const chunks = await this.rag.retrieve({ query: 'specialty context', corpus: ['cba','neuro'], topK: 5, tenantId });
    const redFlags = [] as RedFlag[];

    const llm = await this.langchain.build('PROMPT:NEURO-101:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('NEURO-101'), tenantId });
