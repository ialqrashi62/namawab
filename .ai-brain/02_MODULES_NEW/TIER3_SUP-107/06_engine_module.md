...
export class sup_107_Engine implements Engine<sup_107_Input, sup_107_Output> {
  ...
  async execute(input: sup_107_Input, ctx: ExecutionContext): Promise<sup_107_Output> {
    ...
    const llm = await this.langchain.build('PROMPT:SUP-107:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('SUP-107'), tenantId });
