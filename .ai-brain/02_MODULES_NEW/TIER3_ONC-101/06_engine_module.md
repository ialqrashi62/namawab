...
export class onc_101_Engine implements Engine<onc_101_Input, onc_101_Output> {
  ...
  async execute(input: onc_101_Input, ctx: ExecutionContext): Promise<onc_101_Output> {
    ...
    const chunks = await this.rag.retrieve({ query: 'specialty context', corpus: ['cba','onc'], topK: 5, tenantId });
    const redFlags = [] as RedFlag[];

    const llm = await this.langchain.build('PROMPT:ONC-101:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('ONC-101'), tenantId });
