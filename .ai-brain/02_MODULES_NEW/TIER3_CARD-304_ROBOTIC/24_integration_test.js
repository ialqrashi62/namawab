/**
 * Robotic CV Surgery — Integration Tests
 */
...
(async () => {
  console.log('--- Health ---');
  const h = await get('/api/rcv/health');
  ...
  console.log('\n--- Cases ---');
  const c = await get('/api/rcv/cases');
  assert(c.status === 401 || c.status === 200, 'GET /cases auth-gated');

  console.log('\n--- Direct DB ---');
  const r = await db.query("SELECT COUNT(*) AS c FROM robotic_cv_cases WHERE tenant_id = 1");
  assert(r.rows[0].c >= 0, 'robotic_cv_cases table queryable');
