# RHEUM-001 — Engine Module

```ts
// namaweb/engines/rheum_001/initial_assessment.engine.ts
import { Engine, ExecutionContext, PatientPort, GuidelinePort, AuditPort, CalculatorPort, RAGPort } from '../../shared';

interface RHEUM_001_Input {
...
export class RHEUM_001_Engine
  implements Engine<RHEUM_001_Input, RHEUM_001_Output>
{
  id = 'RHEUM-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: RHEUM_001_Input, ctx: ExecutionContext): Promise<RHEUM_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_rheum-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:RHEUM-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('RHEUM-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new RHEUM_001_Engine(
