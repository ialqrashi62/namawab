# NEURO-001 — Engine Module

```ts
// namaweb/engines/neuro_001/initial_assessment.engine.ts
import { Engine, ExecutionContext, PatientPort, GuidelinePort, AuditPort, CalculatorPort, RAGPort } from '../../shared';

interface NEURO_001_Input {
...
export class NEURO_001_Engine
  implements Engine<NEURO_001_Input, NEURO_001_Output>
{
  id = 'NEURO-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: NEURO_001_Input, ctx: ExecutionContext): Promise<NEURO_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_neuro-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:NEURO-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('NEURO-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new NEURO_001_Engine(
