...
export class PSYC_001_Engine
  implements Engine<PSYC_001_Input, PSYC_001_Output>
{
  id = 'PSYC-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: PSYC_001_Input, ctx: ExecutionContext): Promise<PSYC_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_psyc-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:PSYC-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('PSYC-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new PSYC_001_Engine(
