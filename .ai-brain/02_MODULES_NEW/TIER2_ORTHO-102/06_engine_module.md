...
export class ortho_102_Engine implements Engine<ortho_102_Input, ortho_102_Output> {
  ...
  async execute(input: ortho_102_Input, ctx: ExecutionContext): Promise<ortho_102_Output> {
    ...
    const chunks = await this.rag.retrieve({ query: 'specialty context', corpus: ['cba','ortho'], topK: 5, tenantId });
    const redFlags = [] as RedFlag[];

    const llm = await this.langchain.build('PROMPT:ORTHO-102:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('ORTHO-102'), tenantId });
