...
export class sup_105_Engine implements Engine<sup_105_Input, sup_105_Output> {
  ...
  async execute(input: sup_105_Input, ctx: ExecutionContext): Promise<sup_105_Output> {
    ...
    const llm = await this.langchain.build('PROMPT:SUP-105:main_assessment', { input, ctxBundle, chunks, redFlags, availableCalcs: this.calc.listForDept('SUP-105'), tenantId });
