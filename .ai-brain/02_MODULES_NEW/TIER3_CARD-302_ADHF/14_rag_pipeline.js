...
class HFRAG {
  ...
  async retrieve(query, queryEmbedding, topK = 10, topic = null, language = 'en') {
    ...
    let sql = `SELECT id, content_chunk, embedding, metadata, source, topic, citation_class
               FROM hf_clinical_knowledge WHERE tenant_id = $1 AND language = $2`;
  ...
  safetyCheck(ragResponse) {
    ...
    if (ragResponse.context.includes('lvad') && !ragResponse.context.includes('evaluation')) {
      blocked.push('LVAD_RECOMMENDATION_WITHOUT_EVALUATION');
    }
    return {
      blocked,
