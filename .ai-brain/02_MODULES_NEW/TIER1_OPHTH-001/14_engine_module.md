# OPHTH-001 — Engine Module

```ts
// namaweb/engines/ophth_001/initial_assessment.engine.ts
import { Engine, ExecutionContext, PatientPort, GuidelinePort, AuditPort, CalculatorPort, RAGPort } from '../../shared';

interface OPHTH_001_Input {
...
export class OPHTH_001_Engine
  implements Engine<OPHTH_001_Input, OPHTH_001_Output>
{
  id = 'OPHTH-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: OPHTH_001_Input, ctx: ExecutionContext): Promise<OPHTH_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_ophth-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:OPHTH-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('OPHTH-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new OPHTH_001_Engine(
