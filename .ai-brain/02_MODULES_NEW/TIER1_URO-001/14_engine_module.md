...
export class URO_001_Engine
  implements Engine<URO_001_Input, URO_001_Output>
{
  id = 'URO-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: URO_001_Input, ctx: ExecutionContext): Promise<URO_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_uro-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:URO-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('URO-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new URO_001_Engine(
