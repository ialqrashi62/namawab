...
export class OBG_001_Engine
  implements Engine<OBG_001_Input, OBG_001_Output>
{
  id = 'OBG-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: OBG_001_Input, ctx: ExecutionContext): Promise<OBG_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_obg-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:OBG-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('OBG-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new OBG_001_Engine(
