...
export class DERM_001_Engine
  implements Engine<DERM_001_Input, DERM_001_Output>
{
  id = 'DERM-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: DERM_001_Input, ctx: ExecutionContext): Promise<DERM_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_derm-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:DERM-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('DERM-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new DERM_001_Engine(
