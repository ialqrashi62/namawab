...
export class NEPH_001_Engine
  implements Engine<NEPH_001_Input, NEPH_001_Output>
{
  id = 'NEPH-001:initial_assessment';
  version = '1.0.0';
  ...
  async execute(input: NEPH_001_Input, ctx: ExecutionContext): Promise<NEPH_001_Output> {
    ...
    const chunks = await this.rag.retrieve({
      ...
      corpus: ['cba', 'guidelines', 'kb_neph-001'],
      topK: 8, tenantId,
    });
    ...
    // 5. LLM call
    const llm = await this.langchain.build('PROMPT:NEPH-001:initial_assessment',
      { input, ctxBundle, chunks, redFlags, drugAlerts, availableCalcs: this.calc.listForDept('NEPH-001'), tenantId });
...
```ts
// registry/engines.ts
engineRegistry.register(new NEPH_001_Engine(
