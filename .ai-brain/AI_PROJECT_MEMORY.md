...
* **المرحلة التالية الموصى بها**: الانتقال إلى المرحلة الخامسة (التحقق النهائي من بيئة Staging وبوابة جودة E47).

### Phase 199: Jumanasoft SaaS Staging Verification & e47 Gate (JUMANASOFT_SAAS_STAGING_VERIFIED_AND_E47_COMPLETED)
* **تاريخ المرحلة**: 2026-07-02 | الحالة: `JUMANASOFT_SAAS_STAGING_VERIFIED_AND_E47_COMPLETED` | التحقق من بيئة Staging وتشغيل هجرات الفوترة e47 بنجاح.
* **التفاصيل**: تطبيق كامل مخرجات المرحلة الخامسة وبوابة جودة E47:
  1. التحقق من سلامة وعزل قاعدة بيانات الاختبار المحلية `jumanasoft_staging` وربطها الآمن على خادم التطوير المحلي.
  2. تطبيق ملفات هجرة جداول الفوترة الصاعدة `e47_billing_tables_candidate_up.sql` والتحقق الهيكلي وصمامات الأمان عبر `e47_billing_tables_candidate_validate.sql` بنجاح كامل وحصولها على النتيجة `all_ok = true`.
  3. تنفيذ واختبار خطة التراجع السريع عبر `e47_billing_tables_candidate_down.sql` للتأكد من خلوها من أي تعليق أو cascade مدمر.
  4. استبدال تقرير التحقق المعلق وحل المخاطر الأمنية وإصدار التقرير النهائي المعتمد بنجاح `JUMANASOFT_VERIFY_STAGING_AND_E47_GATE_FINAL_REPORT_AR.md`.
* **نتائج بوابات الجودة**: اجتياز 10/10 فحوصات ساكنة لأمان جداول e47، اجتياز اختبارات جاهزية البيئة والفشل الآمن، ومطابقة ترميز UTF-8 وخلو التتبع من الأسرار.
* **المرحلة التالية الموصى بها**: البدء في المرحلة السادسة (الربط الفعلي والتكامل التشفيري للربط مع هيئة الزكاة والضريبة والجمارك ZATCA Phase 2).

### Phase 200: Jumanasoft SaaS Clinical Safety & Care Quality (Phase F1 - COMPLETED)
* **تاريخ المرحلة**: 2026-07-03 | الحالة: `CLINICAL_SAFETY_F1_COMPLETED` | بناء وتفعيل ميزات السلامة السريرية للمواليد وغرف العمليات وتقييمات التمريض.
* **التفاصيل**: تطبيق كامل مخرجات المرحلة F1:
  1. فرض مطابقة جرد الأدوات الجراحية والشاش والإبر قبل وبعد الجراحة مع حظر الحفظ في حال عدم التطابق، ما لم يتم إرفاق تبرير تجاوز (`override_reason`).
  2. احتساب تقييمات مخاطر السقوط وقرح الفراش للتمريض (Braden & Morse) وتفعيل شارة "مريض عالي الخطورة" تلقائياً.
  3. التحقق من صحة مدخلات تقييم حديثي الولادة (Neonatal APGAR) وتوليد تنبيه سريري حرج في حال تدني النقاط عن 7.
  4. إصلاح حراس الهوية ومجموعات الصلاحيات الموحدة للوصول السريري.
* **الملفات الجديدة والمُعدَّلة**:
  - `namaweb/server.js` (تعديل مسارات التوثيق وإدراج منطق المطابقة والتحقق والاحتساب وصلاحيات `'patients'`)
  - `namaweb/seed_clinical_specialties.js` (بذر مرن لقوالب التقييمات الأربعة لبيئتي التطوير والإنتاج)
  - `namaweb/clinical_safety_f1_test.js` (ملف اختبارات تكاملية شامل للسلامة السريرية - 6/6 PASS)
* **النتائج والنشر**: اجتياز 100% من اختبارات النظام (168 اختباراً) بنجاح كامل، والنشر الفعلي على السيرفر الرئيسي المستضيف `jumanasoft.com`.

### Phase 201: Jumanasoft SaaS Clinical Notes & Smart Templates (Phase F2 - COMPLETED)
* **تاريخ المرحلة**: 2026-07-03 | الحالة: `CLINICAL_NOTES_F2_COMPLETED` | بناء وتفعيل مسارات SOAP وتكامل التوقيع وقوالب الاختصارات السريعة (Dot Phrases).
* **التفاصيل**: تطبيق كامل مخرجات المرحلة F2:
  1. إنشاء جدول `clinical_smart_templates` لحفظ اختصارات النصوص (Dot Phrases) الخاصة بكل طبيب مع تفعيل عزل RLS وحماية البيانات.
  2. إدراج مسارات SOAP الطبية (`GET/POST /api/clinical/notes` و `POST /api/clinical/notes/:id/lock`) لدعم التوثيق المنظم، مع حظر تعديل السجلات الطبية المقفلة رقمياً.
  3. تفعيل مسارات إدارة الاختصارات السريعة وتوسيع النصوص (`GET/POST/DELETE /api/clinical/smart-templates`).
* **الملفات الجديدة والمُعدَّلة**:
  - `namaweb/migrations/e21_02_smart_templates_up.sql` (إنشاء الجدول وتفعيل RLS ومنح الصلاحيات)
  - `namaweb/server.js` (إدراج مسارات SOAP وقوالب الاختصارات وتنسيق التحققات الرقمية)
  - `namaweb/clinical_notes_f2_test.js` (اختبارات تكاملية شاملة للملاحظات الطبية والاختصارات - 9/9 PASS)
* **النتائج والنشر**: تشغيل الهجرات بنجاح، واجتياز كافة الـ 169 اختباراً بنجاح 100%، والنشر الفعلي والمزامنة على السيرفر الرئيسي وإعادة تشغيل PM2.
* **المرحلة التالية الموصى بها**: `ICU Prevention Bundles & Infection Control (Phase F3)`

### Phase 202: Jumanasoft SaaS ICU Prevention Bundles & Infection Control (Phase F3 - COMPLETED)
* **تاريخ المرحلة**: 2026-07-03 | الحالة: `ICU_PREVENTION_BUNDLES_F3_COMPLETED` | بناء وتفعيل مسارات التدقيق اليومي التفاعلي لمنع العدوى المكتسبة (VAP, CLABSI, CAUTI).
* **التفاصيل**: تطبيق كامل مخرجات المرحلة F3:
  1. إنشاء جدول `icu_prevention_bundles` وتفعيل RLS وعزل المستأجرين مع مفتاح تفرد يمنع تكرار التدقيق اليومي.
  2. برمجة مسارات التدقيق اليومي ومحرك احتساب نسبة الالتزام تلقائياً بناءً على البنود المطابقة المنجزة.
  3. فرض شرط التبرير المكتوب (`non_compliance_reason`) عند عدم المطابقة التامة لحماية سلامة المرضى وتغذية مكافحة العدوى.
* **الملفات الجديدة والمُعدَّلة**:
  - `namaweb/migrations/e21_03_icu_bundles_up.sql` (إنشاء الجدول وتفعيل RLS ومنح الصلاحيات)
  - `namaweb/server.js` (إدراج مسارات الوقاية والتدقيق واحتساب نسب المطابقة والتحقق من المبررات)
  - `namaweb/icu_bundles_f3_test.js` (اختبارات تكاملية شاملة لحزم الوقاية والتدقيق - 4/4 PASS)
* **النتائج والنشر**: تشغيل الهجرات بنجاح، واجتياز كافة الـ 170 اختباراً بنجاح 100%، والنشر الفعلي والمزامنة على السيرفر الرئيسي وإعادة تشغيل PM2.
* **المرحلة التالية الموصى بها**: `WHO Growth Charts & Pediatrics Integration (Phase F4)`

### Phase 203: ICU Prevention Bundles & WHO Growth Charts UI Integration (Phase F3/F4 UI - COMPLETED)
* **تاريخ المرحلة**: 2026-07-03 | الحالة: `ICU_BUNDLES_GROWTH_CHARTS_UI_COMPLETED` | بناء وتفعيل واجهة حزم الوقاية ومخططات نمو الأطفال السريرية.
* **التفاصيل**: تطبيق كامل مخرجات المرحلة الثالثة والواجهة الرابعة من مقترح العمل البرمجي:
  1. دمج واجهة مستخدم حزم الوقاية ومكافحة العدوى بالعناية المركزة (`icuTab === 'bundles'`) في `public/js/app.js`؛ تشمل نموذجاً تفاعلياً لاختيار المريض ونوع الحزمة (VAP, CLABSI, CAUTI)، واحتساب نسبة الالتزام ديناميكياً، وفرض حظر إجباري على الحفظ دون إدخال سبب عدم المطابقة إذا قلت النسبة عن 100%، واسترجاع سجل عمليات التدقيق التاريخية.
  2. بناء وتكامل محرك تحديث مخططات نمو الأطفال حسب معايير منظمة الصحة العالمية (WHO Growth Curves) عبر `window.e1UpdateGrowthChart` وربطه بمستدعي تحميل السجل `e1LoadPedHistory` لتخزين البيانات السريرية مؤقتاً بالذاكرة وتحديث مسارات الرسم البياني SVG والتحويل المناسب للإحداثيات بشكل تلقائي.
* **الملفات الجديدة والمُعدَّلة**:
  - `namaweb/public/js/app.js` (تفعيل تبويب حزم الوقاية، دمج حاسبة ونموذج الوقاية، بناء محرك رسم المخطط البياني للنمو)
* **النتائج والنشر**: اجتياز 170 اختباراً بنجاح 100% محلياً، ورفع التغييرات إلى الفرع المستضاف `integration/all-epics` ونشر الكود وتحديث البناء على خادم الإنتاج الفعلي `jumanasoft.com` وإعادة تشغيل PM2 والتحقق من رابط الصحة العامة بنجاح كامل.
* **المرحلة التالية الموصى بها**: البدء في مراجعة مخرجات المرحلة السريرية ودورة المطابقة الشاملة.

### Phase 204: EMR Clinical Signatures, Action RBAC, & Department Owners (Phase F5 - COMPLETED)
* **تاريخ المرحلة**: 2026-07-03 | الحالة: `CLINICAL_SIGNATURES_F5_COMPLETED` | بناء وتفعيل حظر التوقيع والوصول السريري المتقدم ومصفوفة ملاك الأقسام.
* **التفاصيل**: تطبيق مخرجات المرحلة F5:
  1. توحيد ودمج مسار قفل السجل الطبي المزدوج في نهاية خدمة موحدة `POST /api/clinical/records/:id/lock`.
  2. فرض سياسات التوقيع السريري: منع أدوار التمريض أو الفئات الطبية المساعدة من توقيع سجلات الأطباء (403) مع السماح لهم بتوقيع التقييمات التمريضية الخاصة بهم.
  3. تطبيق حارس الصلاحيات التفصيلية `requirePermission` على عمليات الإلغاء الحساسة: إلغاء العمليات الجراحية (`or:cancel`)، إلغاء الفواتير المالية (`invoices:cancel`)، وحذف الرسائل الداخلية (`messages:delete`).
  4. ترحيل وتفعيل عمود مالك القسم `owner_role` بجدول الأقسام الطبية والتشغيلية `clinical_departments`.
* **الملفات الجديدة والمُعدَّلة**:
  - `namaweb/server.js` (تحديث المسارات، تأسيس حارس requirePermission في الأعلى، وتفعيل حدود التوقيع السريري)
  - `namaweb/db_postgres.js` (إضافة العمود للهيكل البرمجي)
  - `namaweb/migrations/p1_03_department_owners_up.sql`, `down.sql`, `validate.sql` (ملفات هجرات قاعدة البيانات والفحص)
  - `namaweb/cross_tenant_clinical_signatures_test.js` (اختبار الجودة والأمان التلقائي الجديد - 12/12 PASS)
* **النتائج والنشر**: تشغيل الهجرات بنجاح، واجتياز كافة الـ 173 اختباراً بنجاح 100%، والنشر الفعلي على السيرفر الرئيسي وإعادة تشغيل PM2 والتحقق من رابط الصحة.

### Phase 205: DR Hardening & Integration Readiness (Phase F6/B1-B2 - COMPLETED)
* **تاريخ المرحلة**: 2026-07-03 | الحالة: `DR_HARDENING_INTEGRATION_READINESS_COMPLETED` | إعداد متطلبات الأمان التشغيلي وجاهزية التكامليات الرقمية مع الأنظمة العالمية.
* **التفاصيل**: تطبيق مخرجات المرحلة F6/B1-B2:
  1. صياغة سجل استجابة متكامل للحوادث الأمنية والتشغيلية يغطي تعطلب خادم Redis أو Docker daemon أو كشف الـ KEK أو محاولات الوصول غير المصرح عبر المستأجرين بالتوافق مع لوائح PDPL و NCA.
  2. توثيق إجراءات حفظ واستعادة مفتاح التشفير (KEK Escrow) باستخدام أداة PowerShell المعتمدة `nama_kek_escrow.ps1` وخطوات الـ Recovery خارج الموقع.
  3. إعداد مستند القرار المعماري (ADR) لتكامل قنوات Mirth Connect لمعالجة رسائل HL7 و FHIR كخيار وسيط معزول لضمان عدم تأثر أداء المونوليث الأساسي.
  4. رسم خريطة مطابقة حقول البيانات السريرية مع موارد FHIR R4 القياسية وتخطيط تشغيل Sandbox محلي تجريبي خالي تماماً من أي بيانات حية (No PHI).
* **الملفات الجديدة**:
  - `docs/runbooks/nama_security_incident_runbook_ar.md` (دليل الاستجابة للحوادث)
  - `docs/runbooks/nama_kek_escrow_dr_plan_ar.md` (دليل الكوارث واستعادة المفاتيح)
  - `docs/runbooks/nama_mirth_integration_adr_ar.md` (القرار المعماري لـ Mirth Connect)
  - `docs/runbooks/nama_fhir_sandbox_blueprint_ar.md` (مخطط تحويل ومعايرة FHIR)
* **النتائج والنشر**: نجاح جميع الفحوصات واختبارات النظام الـ 173 التراكمية بنسبة 100%، مزامنة الأدلة الجديدة مع مجلدات خادم الإنتاج الحقيقي، والتحقق من سلامة واستقرار الخدمة 200 OK.

### Phase 206: Doctor Station Layout Redesign (Stitch Design System - COMPLETED)
* **تاريخ المرحلة**: 2026-07-05 | الحالة: `DOCTOR_STATION_LAYOUT_REDESIGN_COMPLETED` | تصحيح وتحديث واجهة محطة الطبيب وإصلاح تداخل التبويبات التخصصية E1.
* **التفاصيل**: تطبيق مخرجات إعادة هيكلة التصميم وتوافقه مع معايير Stitch:
  1. حل مشكلة تداخل العناصر والتفاف الأزرار عمودياً عبر فئة تخطيط مخصصة `.doctor-split-layout` تعطي مساحة العمل الكاملة 1fr (على اليسار في وضع العربية RTL) وجدول السجلات 380px ثابت (على اليمين).
  2. تنسيق أزرار التبويبات التخصصية الـ 23 بفئة `.e1-tab` الأنيقة المحددة الحدود لمنع امتدادها المفرط وتفادي الفراغات البيضاء المزعجة بالمنتصف.
  3. تنفيذ كسر كاش المتصفح (Cache Busting) لملفات `app.js` و `styles.css` بتحديث قيم الإصدارات في `index.html` إلى `v=20260705_4` لضمان تحميل التصميم الجديد فوراً لدى كافة الزوار.
* **الملفات المُعدَّلة**:
  - `namaweb/public/css/styles.css` (إدخال فئات التنسيق المخصصة `.doctor-split-layout` و `.e1-tab`)
  - `namaweb/public/js/app.js` (استبدال `.split-layout` بـ `.doctor-split-layout` في دالة renderDoctor للهيكل والواجهة الفعلية)
...
### Phase 210: HOS_PRO Phase 2 Implementation - Centralized Beds, NPHIES Eligibility & OPD Doctor Queue (COMPLETED)
* **تاريخ المرحلة**: 2026-07-06 | الحالة: `HOS_PRO_PHASE_2_COMPLETED` | تطوير وترقية موديولات إدارة الأسرة وبوابة NPHIES ومحطة عيادات OPD الخارجية.
* **التفاصيل**: تطبيق كامل المرحلة الثانية (Phase 2):
  1. **سير عمل العيادات الخارجية (OPD Doctor Queue & Encounter)**: برمجة مسارات `/api/opd/doctor/queue` لجلب قائمة الانتظار للمواعيد اليومية للمريض ودمج أحدث علامات حيوية لليوم، ومسار `/api/opd/encounter/start` لبدء الاستشارة وتحديث حالة الموعد إلى `In-Consultation`. وتحديث واجهة الطبيب `renderDoctor(el)` لتعرض جدولاً تفاعلياً لقائمة الانتظار مع العلامات الحيوية وسير العمل الكامل.
  2. **إدارة الأسرة والتحقق من الأهلية**: ربط وتفعيل لوحات خريطة الأسرة البصرية (Bed Board) بشكل معزول كلياً للمستأجرين، وتفعيل لوحات استعلام الأهلية الفوري (Eligibility) لـ NPHIES.
  3. **الاختبارات والتراكمي**: إنشاء ملف اختبارات الجودة وعزل المستأجرين الجديد `opd_beds_nphies_test.js` (10/10 PASS) والتحقق بنجاح كامل من حزمة الاختبارات التراكمية الـ 177 للمشروع 100%.
* **الملفات الجديدة والمُعدَّلة**:
  - `namaweb/server.js` (إدراج مسارات OPD Doctor Queue وبدء الجلسات وعزل المستأجرين)
  - `namaweb/public/js/app.js` (إعادة هيكلة واجهة الطبيب وإدراج قائمة انتظار المواعيد ودوال الربط)
  - `namaweb/opd_beds_nphies_test.js` (ملف اختبارات السلامة والخصوصية لـ Phase 2 - 10/10 PASS)
  - `namaweb/task.md` & `namaweb/walkthrough.md` (تحديث قائمة المهام وتقرير التسليم للمرحلة الثانية)
* **النتائج والنشر**: اجتياز كامل الـ **177** اختباراً بنجاح 100%، رفع ودفع التغييرات للمستودعات، وتفعيل عملية النشر المستمر للإنتاج الفعلي وإعادة تشغيل PM2 والتحقق من استقرار رابط الصحة 200 OK.

### Phase 211: HOS_PRO Phase 3 Implementation - Special Nursing, CSSD BI Gate & Calibrations (COMPLETED)
* **تاريخ المرحلة**: 2026-07-06 | الحالة: `HOS_PRO_PHASE_3_COMPLETED` | تطوير وترقية موديولات التمريض التخصصي وصمامات أمان التعقيم ومعايرة الأجهزة وتتبع النفايات الطبية الخطرة.
* **التفاصيل**: تطبيق كامل المرحلة الثالثة (Phase 3):
  1. **صمامات أمان التعقيم (CSSD BI Gate & Issue)**: تفعيل صمام الأمان fail-closed في مسار `cssd_trays/:id/issue` لحظر صرف الحزم الجراحية والأدوات الطبية ما لم تجتز فحص المؤشر الحيوي للتعقيم (BI Passed).
  2. **العد الجراحي الثنائي (Surgical Counts)**: تفعيل ومطابقة قائمة العد الجراحي (الشاش، الإبر، الأدوات) ومطابقتها التامة قبل إقفال العملية وتوقيع ممرض التعقيم والتدوير ثنائياً.
  3. **معايرة الأجهزة والنفايات الطبية**: إنشاء جداول `device_calibrations` و `medical_waste_logs` في `db_postgres.js` وتفعيل RLS، وبرمجة مسارات الاستعلام والتسجيل الخاصة بها في `server.js` للتوافق مع متطلبات سباهي (CBAHI).
  4. **الاختبارات والتراكمي**: إنشاء ملف اختبارات الجودة وعزل المستأجرين الجديد `cssd_nursing_safety_test.js` (13/13 PASS) والتحقق بنجاح كامل من حزمة الاختبارات التراكمية الـ 178 للمشروع 100%.
* **الملفات الجديدة والمُعدَّلة**:
  - `namaweb/db_postgres.js` (إدراج جداول `device_calibrations` و `medical_waste_logs` وتفعيل RLS)
  - `namaweb/server.js` (إدراج مسارات معايرة الأجهزة وتتبع النفايات الطبية والتحقق من RLS)
  - `namaweb/cssd_nursing_safety_test.js` (ملف اختبارات السلامة والخصوصية لـ Phase 3 - 13/13 PASS)
