...
-- =====================================================================
-- File:        DEP-023-vascular-surgery_up.sql
-- Department:  DEP-023 — Vascular Surgery (جراحة الأوعية الدموية)
...
-- Main entity table for Vascular Surgery (جراحة الأوعية الدموية)
CREATE TABLE IF NOT EXISTS blueprint_v2.vascular_case (
...
-- Indexes
CREATE INDEX idx_vascular_case_tenant ON blueprint_v2.vascular_case(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.vascular_case ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.vascular_case FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY vascular_case_tenant_isolation ON blueprint_v2.vascular_case
    USING (
...
CREATE TRIGGER vascular_case_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.vascular_case
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.vascular_case FROM PUBLIC;
