...
-- =====================================================================
-- File:        DEP-021-neurosurgery_up.sql
-- Department:  DEP-021 — Neurosurgery (جراحة المخ والأعصاب)
...
-- Main entity table for Neurosurgery (جراحة المخ والأعصاب)
CREATE TABLE IF NOT EXISTS blueprint_v2.neurosurg_case (
...
-- Indexes
CREATE INDEX idx_neurosurg_case_tenant ON blueprint_v2.neurosurg_case(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.neurosurg_case ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.neurosurg_case FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY neurosurg_case_tenant_isolation ON blueprint_v2.neurosurg_case
    USING (
...
CREATE TRIGGER neurosurg_case_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.neurosurg_case
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.neurosurg_case FROM PUBLIC;
