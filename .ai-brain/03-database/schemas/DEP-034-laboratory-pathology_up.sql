...
-- =====================================================================
-- File:        DEP-034-laboratory-pathology_up.sql
-- Department:  DEP-034 — Laboratory & Pathology (المختبر وعلم الأمراض)
...
-- Main entity table for Laboratory & Pathology (المختبر وعلم الأمراض)
CREATE TABLE IF NOT EXISTS blueprint_v2.lab_result (
...
-- Indexes
CREATE INDEX idx_lab_result_tenant ON blueprint_v2.lab_result(tenant_id);
CREATE INDEX idx_lab_result_loinc ON blueprint_v2.lab_result(test_loinc);
CREATE INDEX idx_lab_result_abnormal ON blueprint_v2.lab_result(abnormal_flag) WHERE abnormal_flag <> '';

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.lab_result ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.lab_result FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY lab_result_tenant_isolation ON blueprint_v2.lab_result
    USING (
...
CREATE TRIGGER lab_result_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.lab_result
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.lab_result FROM PUBLIC;
