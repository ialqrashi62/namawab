...
-- =====================================================================
-- File:        DEP-032-well-baby-lactation_up.sql
-- Department:  DEP-032 — Well-Baby & Lactation Clinic (عيادة الطفل السليم والرضاعة)
...
-- Main entity table for Well-Baby & Lactation Clinic (عيادة الطفل السليم والرضاعة)
CREATE TABLE IF NOT EXISTS blueprint_v2.well_baby_visit (
...
-- Indexes
CREATE INDEX idx_well_baby_visit_tenant ON blueprint_v2.well_baby_visit(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.well_baby_visit ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.well_baby_visit FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY well_baby_visit_tenant_isolation ON blueprint_v2.well_baby_visit
    USING (
...
CREATE TRIGGER well_baby_visit_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.well_baby_visit
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.well_baby_visit FROM PUBLIC;
