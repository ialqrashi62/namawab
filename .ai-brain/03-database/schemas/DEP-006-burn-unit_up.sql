...
-- =====================================================================
-- File:        DEP-006-burn-unit_up.sql
-- Department:  DEP-006 — Burn Unit (وحدة الحروق)
...
-- Main entity table for Burn Unit (وحدة الحروق)
CREATE TABLE IF NOT EXISTS blueprint_v2.burn_assessment (
    ...
    fluid_resuscitation_protocol TEXT  -- PHI-ENVELOPE,
...
-- Indexes
CREATE INDEX idx_burn_assessment_tenant ON blueprint_v2.burn_assessment(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.burn_assessment ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.burn_assessment FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY burn_assessment_tenant_isolation ON blueprint_v2.burn_assessment
    USING (
...
CREATE TRIGGER burn_assessment_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.burn_assessment
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.burn_assessment FROM PUBLIC;
