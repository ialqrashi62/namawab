...
-- =====================================================================
-- File:        DEP-015-infectious-diseases_up.sql
-- Department:  DEP-015 — Infectious Diseases (الأمراض المعدية)
...
-- Main entity table for Infectious Diseases (الأمراض المعدية)
CREATE TABLE IF NOT EXISTS blueprint_v2.id_consult (
    ...
    culture_result TEXT  -- PHI-ENVELOPE,
...
-- Indexes
CREATE INDEX idx_id_consult_tenant ON blueprint_v2.id_consult(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.id_consult ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.id_consult FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY id_consult_tenant_isolation ON blueprint_v2.id_consult
    USING (
...
CREATE TRIGGER id_consult_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.id_consult
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.id_consult FROM PUBLIC;
