...
-- =====================================================================
-- File:        DEP-024-urology_up.sql
-- Department:  DEP-024 — Urology (جراحة المسالك البولية)
...
-- Main entity table for Urology (جراحة المسالك البولية)
CREATE TABLE IF NOT EXISTS blueprint_v2.urology_procedure (
...
-- Indexes
CREATE INDEX idx_urology_procedure_tenant ON blueprint_v2.urology_procedure(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.urology_procedure ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.urology_procedure FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY urology_procedure_tenant_isolation ON blueprint_v2.urology_procedure
    USING (
...
CREATE TRIGGER urology_procedure_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.urology_procedure
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.urology_procedure FROM PUBLIC;
