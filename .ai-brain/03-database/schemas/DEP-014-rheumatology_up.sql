...
-- =====================================================================
-- File:        DEP-014-rheumatology_up.sql
-- Department:  DEP-014 — Rheumatology (أمراض الروماتيزم)
...
-- Main entity table for Rheumatology (أمراض الروماتيزم)
CREATE TABLE IF NOT EXISTS blueprint_v2.rheum_assessment (
...
-- Indexes
CREATE INDEX idx_rheum_assessment_tenant ON blueprint_v2.rheum_assessment(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.rheum_assessment ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.rheum_assessment FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY rheum_assessment_tenant_isolation ON blueprint_v2.rheum_assessment
    USING (
...
CREATE TRIGGER rheum_assessment_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.rheum_assessment
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.rheum_assessment FROM PUBLIC;
