...
-- =====================================================================
-- File:        DEP-027-plastic-reconstructive_up.sql
-- Department:  DEP-027 — Plastic & Reconstructive Surgery (جراحة التجميل والترميمية)
...
-- Main entity table for Plastic & Reconstructive Surgery (جراحة التجميل والترميمية)
CREATE TABLE IF NOT EXISTS blueprint_v2.plastic_surgery_case (
...
-- Indexes
CREATE INDEX idx_plastic_case_tenant ON blueprint_v2.plastic_surgery_case(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.plastic_surgery_case ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.plastic_surgery_case FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY plastic_surgery_case_tenant_isolation ON blueprint_v2.plastic_surgery_case
    USING (
...
CREATE TRIGGER plastic_surgery_case_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.plastic_surgery_case
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.plastic_surgery_case FROM PUBLIC;
