...
-- =====================================================================
-- File:        DEP-003-ccu_up.sql
-- Department:  DEP-003 — Coronary Care Unit (CCU) (وحدة العناية القلبية)
...
-- Main entity table for Coronary Care Unit (CCU) (وحدة العناية القلبية)
CREATE TABLE IF NOT EXISTS blueprint_v2.ccu_admission (
...
-- Indexes
CREATE INDEX idx_ccu_admission_tenant ON blueprint_v2.ccu_admission(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.ccu_admission ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.ccu_admission FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY ccu_admission_tenant_isolation ON blueprint_v2.ccu_admission
    USING (
...
CREATE TRIGGER ccu_admission_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.ccu_admission
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.ccu_admission FROM PUBLIC;
