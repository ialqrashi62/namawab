...
-- =====================================================================
-- File:        DEP-029-obgyn_up.sql
-- Department:  DEP-029 — Obstetrics & Gynecology (OB/GYN) (النساء والتوليد)
...
-- Main entity table for Obstetrics & Gynecology (OB/GYN) (النساء والتوليد)
CREATE TABLE IF NOT EXISTS blueprint_v2.obgyn_visit (
...
-- Indexes
CREATE INDEX idx_obgyn_visit_tenant ON blueprint_v2.obgyn_visit(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.obgyn_visit ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.obgyn_visit FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY obgyn_visit_tenant_isolation ON blueprint_v2.obgyn_visit
    USING (
...
CREATE TRIGGER obgyn_visit_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.obgyn_visit
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.obgyn_visit FROM PUBLIC;
