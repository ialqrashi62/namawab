...
-- =====================================================================
-- File:        DEP-019-general-surgery_up.sql
-- Department:  DEP-019 — General Surgery (الجراحة العامة)
...
-- Main entity table for General Surgery (الجراحة العامة)
CREATE TABLE IF NOT EXISTS blueprint_v2.surgical_case (
...
-- Indexes
CREATE INDEX idx_surgical_case_tenant ON blueprint_v2.surgical_case(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.surgical_case ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.surgical_case FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY surgical_case_tenant_isolation ON blueprint_v2.surgical_case
    USING (
...
CREATE TRIGGER surgical_case_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.surgical_case
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.surgical_case FROM PUBLIC;
