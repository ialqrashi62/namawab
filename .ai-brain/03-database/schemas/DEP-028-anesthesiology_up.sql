...
-- =====================================================================
-- File:        DEP-028-anesthesiology_up.sql
-- Department:  DEP-028 — Anesthesiology & Pain Management (التخدير وإدارة الألم)
...
-- Main entity table for Anesthesiology & Pain Management (التخدير وإدارة الألم)
CREATE TABLE IF NOT EXISTS blueprint_v2.anesthesia_record (
...
-- Indexes
CREATE INDEX idx_anesthesia_record_tenant ON blueprint_v2.anesthesia_record(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.anesthesia_record ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.anesthesia_record FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY anesthesia_record_tenant_isolation ON blueprint_v2.anesthesia_record
    USING (
...
CREATE TRIGGER anesthesia_record_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.anesthesia_record
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.anesthesia_record FROM PUBLIC;
