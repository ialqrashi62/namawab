...
-- =====================================================================
-- File:        DEP-035-pharmacy_up.sql
-- Department:  DEP-035 — Pharmacy & Therapeutics (الصيدلية والعلاج الدوائي)
...
-- Main entity table for Pharmacy & Therapeutics (الصيدلية والعلاج الدوائي)
CREATE TABLE IF NOT EXISTS blueprint_v2.medication_dispense (
...
-- Indexes
CREATE INDEX idx_dispense_tenant ON blueprint_v2.medication_dispense(tenant_id);
CREATE INDEX idx_dispense_drug ON blueprint_v2.medication_dispense(drug_atc);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.medication_dispense ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.medication_dispense FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY medication_dispense_tenant_isolation ON blueprint_v2.medication_dispense
    USING (
...
CREATE TRIGGER medication_dispense_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.medication_dispense
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.medication_dispense FROM PUBLIC;
