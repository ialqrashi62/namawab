...
-- =====================================================================
-- File:        DEP-008-pulmonology_up.sql
-- Department:  DEP-008 — Pulmonology (أمراض الجهاز التنفسي)
...
-- Main entity table for Pulmonology (أمراض الجهاز التنفسي)
CREATE TABLE IF NOT EXISTS blueprint_v2.pulmonary_function_test (
    ...
    interpretation TEXT  -- PHI-ENVELOPE,
...
-- Indexes
CREATE INDEX idx_pft_tenant ON blueprint_v2.pulmonary_function_test(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.pulmonary_function_test ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.pulmonary_function_test FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY pulmonary_function_test_tenant_isolation ON blueprint_v2.pulmonary_function_test
    USING (
...
CREATE TRIGGER pulmonary_function_test_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.pulmonary_function_test
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.pulmonary_function_test FROM PUBLIC;
