...
-- =====================================================================
-- File:        DEP-031-reproductive-ivf_up.sql
-- Department:  DEP-031 — Reproductive Medicine & IVF (طب الإنجاب والإخصاب)
...
-- Main entity table for Reproductive Medicine & IVF (طب الإنجاب والإخصاب)
CREATE TABLE IF NOT EXISTS blueprint_v2.ivf_cycle (
...
-- Indexes
CREATE INDEX idx_ivf_cycle_tenant ON blueprint_v2.ivf_cycle(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.ivf_cycle ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.ivf_cycle FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY ivf_cycle_tenant_isolation ON blueprint_v2.ivf_cycle
    USING (
...
CREATE TRIGGER ivf_cycle_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.ivf_cycle
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.ivf_cycle FROM PUBLIC;
