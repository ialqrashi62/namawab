...
-- =====================================================================
-- File:        DEP-017-neurology_up.sql
-- Department:  DEP-017 — Neurology (أمراض الأعصاب)
...
-- Main entity table for Neurology (أمراض الأعصاب)
CREATE TABLE IF NOT EXISTS blueprint_v2.neuro_exam (
...
-- Indexes
CREATE INDEX idx_neuro_exam_tenant ON blueprint_v2.neuro_exam(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.neuro_exam ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.neuro_exam FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY neuro_exam_tenant_isolation ON blueprint_v2.neuro_exam
    USING (
...
CREATE TRIGGER neuro_exam_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.neuro_exam
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.neuro_exam FROM PUBLIC;
