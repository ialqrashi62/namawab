...
-- =====================================================================
-- File:        DEP-016-dermatology_up.sql
-- Department:  DEP-016 — Dermatology (الأمراض الجلدية)
...
-- Main entity table for Dermatology (الأمراض الجلدية)
CREATE TABLE IF NOT EXISTS blueprint_v2.derm_lesion_record (
...
-- Indexes
CREATE INDEX idx_derm_lesion_tenant ON blueprint_v2.derm_lesion_record(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.derm_lesion_record ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.derm_lesion_record FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY derm_lesion_record_tenant_isolation ON blueprint_v2.derm_lesion_record
    USING (
...
CREATE TRIGGER derm_lesion_record_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.derm_lesion_record
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.derm_lesion_record FROM PUBLIC;
