...
-- =====================================================================
-- File:        DEP-012-hematology_up.sql
-- Department:  DEP-012 — Hematology (أمراض الدم)
...
-- Main entity table for Hematology (أمراض الدم)
CREATE TABLE IF NOT EXISTS blueprint_v2.hematology_panel (
...
-- Indexes
CREATE INDEX idx_hematology_panel_tenant ON blueprint_v2.hematology_panel(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.hematology_panel ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.hematology_panel FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY hematology_panel_tenant_isolation ON blueprint_v2.hematology_panel
    USING (
...
CREATE TRIGGER hematology_panel_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.hematology_panel
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.hematology_panel FROM PUBLIC;
