...
-- =====================================================================
-- File:        DEP-018-psychiatry_up.sql
-- Department:  DEP-018 — Psychiatry & Mental Health (الطب النفسي والصحة النفسية)
...
-- Main entity table for Psychiatry & Mental Health (الطب النفسي والصحة النفسية)
CREATE TABLE IF NOT EXISTS blueprint_v2.psych_assessment (
...
-- Indexes
CREATE INDEX idx_psych_assessment_tenant ON blueprint_v2.psych_assessment(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.psych_assessment ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.psych_assessment FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY psych_assessment_tenant_isolation ON blueprint_v2.psych_assessment
    USING (
...
CREATE TRIGGER psych_assessment_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.psych_assessment
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.psych_assessment FROM PUBLIC;
