...
-- =====================================================================
-- File:        DEP-007-cardiology_up.sql
-- Department:  DEP-007 — Cardiology (أمراض القلب)
...
-- Main entity table for Cardiology (أمراض القلب)
CREATE TABLE IF NOT EXISTS blueprint_v2.cardiology_consult (
    ...
    indication TEXT NOT NULL  -- PHI-ENVELOPE,
    recommendation TEXT  -- PHI-ENVELOPE,
...
-- Indexes
CREATE INDEX idx_cardiology_consult_tenant ON blueprint_v2.cardiology_consult(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.cardiology_consult ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.cardiology_consult FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY cardiology_consult_tenant_isolation ON blueprint_v2.cardiology_consult
    USING (
...
CREATE TRIGGER cardiology_consult_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.cardiology_consult
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.cardiology_consult FROM PUBLIC;
