...
-- =====================================================================
-- File:        DEP-002-icu-adult_up.sql
-- Department:  DEP-002 — Adult Intensive Care Unit (ICU) (وحدة العناية المركزة للبالغين)
...
-- Main entity table for Adult Intensive Care Unit (ICU) (وحدة العناية المركزة للبالغين)
CREATE TABLE IF NOT EXISTS blueprint_v2.icu_admission (
...
-- Indexes
CREATE INDEX idx_icu_admission_tenant ON blueprint_v2.icu_admission(tenant_id);
CREATE INDEX idx_icu_admission_admit ON blueprint_v2.icu_admission(admitted_at_utc DESC);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.icu_admission ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.icu_admission FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY icu_admission_tenant_isolation ON blueprint_v2.icu_admission
    USING (
...
CREATE TRIGGER icu_admission_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.icu_admission
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.icu_admission FROM PUBLIC;
