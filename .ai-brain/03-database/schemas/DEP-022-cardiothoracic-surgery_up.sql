...
-- =====================================================================
-- File:        DEP-022-cardiothoracic-surgery_up.sql
-- Department:  DEP-022 — Cardiothoracic Surgery (جراحة القلب والصدر)
...
-- Main entity table for Cardiothoracic Surgery (جراحة القلب والصدر)
CREATE TABLE IF NOT EXISTS blueprint_v2.cts_case (
...
-- Indexes
CREATE INDEX idx_cts_case_tenant ON blueprint_v2.cts_case(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.cts_case ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.cts_case FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY cts_case_tenant_isolation ON blueprint_v2.cts_case
    USING (
...
CREATE TRIGGER cts_case_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.cts_case
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.cts_case FROM PUBLIC;
