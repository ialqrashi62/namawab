...
-- =====================================================================
-- File:        DEP-009-gastroenterology_up.sql
-- Department:  DEP-009 — Gastroenterology (أمراض الجهاز الهضمي)
...
-- Main entity table for Gastroenterology (أمراض الجهاز الهضمي)
CREATE TABLE IF NOT EXISTS blueprint_v2.gi_endoscopy (
    ...
    findings_snomed TEXT  -- PHI-ENVELOPE,
    complications TEXT  -- PHI-ENVELOPE,
...
-- Indexes
CREATE INDEX idx_gi_endoscopy_tenant ON blueprint_v2.gi_endoscopy(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.gi_endoscopy ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.gi_endoscopy FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY gi_endoscopy_tenant_isolation ON blueprint_v2.gi_endoscopy
    USING (
...
CREATE TRIGGER gi_endoscopy_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.gi_endoscopy
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.gi_endoscopy FROM PUBLIC;
