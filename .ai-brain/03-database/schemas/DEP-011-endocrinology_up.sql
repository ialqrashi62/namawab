...
-- =====================================================================
-- File:        DEP-011-endocrinology_up.sql
-- Department:  DEP-011 — Endocrinology & Diabetes (أمراض الغدد الصماء والسكري)
...
-- Main entity table for Endocrinology & Diabetes (أمراض الغدد الصماء والسكري)
CREATE TABLE IF NOT EXISTS blueprint_v2.endocrine_visit (
...
-- Indexes
CREATE INDEX idx_endocrine_visit_tenant ON blueprint_v2.endocrine_visit(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.endocrine_visit ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.endocrine_visit FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY endocrine_visit_tenant_isolation ON blueprint_v2.endocrine_visit
    USING (
...
CREATE TRIGGER endocrine_visit_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.endocrine_visit
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.endocrine_visit FROM PUBLIC;
