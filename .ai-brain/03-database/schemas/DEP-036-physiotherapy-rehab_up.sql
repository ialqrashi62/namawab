...
-- =====================================================================
-- File:        DEP-036-physiotherapy-rehab_up.sql
-- Department:  DEP-036 — Physiotherapy & Rehabilitation (العلاج الطبيعي وإعادة التأهيل)
...
-- Main entity table for Physiotherapy & Rehabilitation (العلاج الطبيعي وإعادة التأهيل)
CREATE TABLE IF NOT EXISTS blueprint_v2.rehab_session (
...
-- Indexes
CREATE INDEX idx_rehab_session_tenant ON blueprint_v2.rehab_session(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.rehab_session ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.rehab_session FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY rehab_session_tenant_isolation ON blueprint_v2.rehab_session
    USING (
...
CREATE TRIGGER rehab_session_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.rehab_session
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.rehab_session FROM PUBLIC;
