...
-- =====================================================================
-- File:        DEP-001-emergency-dept_up.sql
-- Department:  DEP-001 — Emergency Department (قسم الطوارئ)
-- Group:       emergency_and_critical_care
...
-- Main entity table for Emergency Department (قسم الطوارئ)
CREATE TABLE IF NOT EXISTS blueprint_v2.ed_encounter (
    ...
    chief_complaint TEXT NOT NULL  -- PHI-ENVELOPE,
    ...
    early_warning_score NUMERIC(4,2),
...
-- Indexes
CREATE INDEX idx_ed_encounter_tenant ON blueprint_v2.ed_encounter(tenant_id);
CREATE INDEX idx_ed_encounter_arrival ON blueprint_v2.ed_encounter(arrival_at_utc DESC);
CREATE INDEX idx_ed_encounter_triage ON blueprint_v2.ed_encounter(triage_level) WHERE deleted_at_utc IS NULL;

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.ed_encounter ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.ed_encounter FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY ed_encounter_tenant_isolation ON blueprint_v2.ed_encounter
    USING (
...
CREATE TRIGGER ed_encounter_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.ed_encounter
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.ed_encounter FROM PUBLIC;
