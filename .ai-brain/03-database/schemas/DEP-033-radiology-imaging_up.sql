...
-- =====================================================================
-- File:        DEP-033-radiology-imaging_up.sql
-- Department:  DEP-033 — Radiology & Medical Imaging (الأشعة والتصوير الطبي)
-- Group:       diagnostics_and_therapeutics
...
-- Main entity table for Radiology & Medical Imaging (الأشعة والتصوير الطبي)
CREATE TABLE IF NOT EXISTS blueprint_v2.imaging_study (
    ...
    dicom_study_uid TEXT UNIQUE  -- PHI-ENVELOPE,
    report_text TEXT  -- PHI-ENVELOPE,
...
-- Indexes
CREATE INDEX idx_imaging_study_tenant ON blueprint_v2.imaging_study(tenant_id);
CREATE INDEX idx_imaging_study_modality ON blueprint_v2.imaging_study(modality) WHERE deleted_at_utc IS NULL;

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.imaging_study ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.imaging_study FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY imaging_study_tenant_isolation ON blueprint_v2.imaging_study
    USING (
...
CREATE TRIGGER imaging_study_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.imaging_study
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.imaging_study FROM PUBLIC;
