...
-- =====================================================================
-- File:        DEP-020-orthopedics-trauma_up.sql
-- Department:  DEP-020 — Orthopedics & Trauma (جراحة العظام والكسور)
...
-- Main entity table for Orthopedics & Trauma (جراحة العظام والكسور)
CREATE TABLE IF NOT EXISTS blueprint_v2.ortho_fracture (
    ...
    treatment_plan TEXT  -- PHI-ENVELOPE,
...
-- Indexes
CREATE INDEX idx_ortho_fracture_tenant ON blueprint_v2.ortho_fracture(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.ortho_fracture ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.ortho_fracture FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY ortho_fracture_tenant_isolation ON blueprint_v2.ortho_fracture
    USING (
...
CREATE TRIGGER ortho_fracture_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.ortho_fracture
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.ortho_fracture FROM PUBLIC;
