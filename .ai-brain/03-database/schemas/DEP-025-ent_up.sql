...
-- =====================================================================
-- File:        DEP-025-ent_up.sql
-- Department:  DEP-025 — Otorhinolaryngology (ENT) (الأنف والأذن والحنجرة)
...
-- Main entity table for Otorhinolaryngology (ENT) (الأنف والأذن والحنجرة)
CREATE TABLE IF NOT EXISTS blueprint_v2.ent_exam (
...
-- Indexes
CREATE INDEX idx_ent_exam_tenant ON blueprint_v2.ent_exam(tenant_id);

-- Row-Level Security (FORCE = enforce even for table owner)
ALTER TABLE blueprint_v2.ent_exam ENABLE ROW LEVEL SECURITY;
ALTER TABLE blueprint_v2.ent_exam FORCE ROW LEVEL SECURITY;

-- Tenant isolation policy
CREATE POLICY ent_exam_tenant_isolation ON blueprint_v2.ent_exam
    USING (
...
CREATE TRIGGER ent_exam_set_updated_at_utc
BEFORE UPDATE ON blueprint_v2.ent_exam
FOR EACH ROW EXECUTE FUNCTION blueprint_v2.trg_set_updated_at_utc();

-- Revoke public access; app role is granted explicitly.
REVOKE ALL ON blueprint_v2.ent_exam FROM PUBLIC;
