...
## Cognitive Core: Ultra-Specialized Perioperative Precision
...
- **Context Window Management:** Current surgery + pre-op assessment + allergies + prior anesthesia complications + intra-op vitals + PACU plan.
...
- **API Specifications:**
  ...
  - `POST /api/anesthesia/drug`
  - `PUT /api/anesthesia/complete`
- **Data Model:** `anesthesia_records`, `anesthesia_drug_logs`, `airway_assessments`, `asa_assessments`.
- **Business Logic:** Block surgery start until anesthesia consent + ASA documented; alert on drug dose thresholds; mandatory PACU handoff.

### 3. Frontend / UI-UX (The Interface)
- **Stitch Tokens:** `AnesthesiaTimeline`, `ASAPicker`, `MallampatiSelector`, `DrugDoseAlert`, `PACUHandoffCard`.
