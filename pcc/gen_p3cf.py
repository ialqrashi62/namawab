#!/usr/bin/env python3
"""P3-CF generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_billing', 'PCC Billing', 'bill'),
    ('pcc_scheduling', 'PCC Scheduling', 'sch'),
    ('pcc_telemed', 'PCC Telemed', 'tm'),
]
PKG = 'p3cf_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CF package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CF module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_billing': ['Billing', 'Charge', 'Insurance', 'Discount', 'Payment', 'Refund', 'Statement', 'Denial', 'Reclaim', 'Tax'],
    'pcc_scheduling': ['Schedule', 'Slot', 'Waitlist', 'Reminder', 'Booking', 'Cancel', 'Reschedule', 'Capacity', 'Resource', 'Template'],
    'pcc_telemed': ['Visit', 'Consent', 'Connection', 'Prescribe', 'Charting', 'Triage', 'Reimburse', 'Platform', 'FollowUp', 'Audit'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CF {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CF {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cf_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cf_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CF: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
