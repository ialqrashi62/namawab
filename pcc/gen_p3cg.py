#!/usr/bin/env python3
"""P3-CG generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_pharmacy', 'PCC Pharmacy', 'rx'),
    ('pcc_dialysis', 'PCC Dialysis', 'dial'),
    ('pcc_oncology_ext', 'PCC Oncology Ext', 'onx'),
]
PKG = 'p3cg_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CG package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CG module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_pharmacy': ['Dispense', 'Interaction', 'Allergy', 'DoseCheck', 'Refill', 'Compounding', 'Narcotic', 'IVAdmixture', 'Formulary', 'Counseling'],
    'pcc_dialysis': ['Access', 'Treatment', 'Clearance', 'DryWeight', 'Ultrafiltration', 'Heparin', 'Sodium', 'Bicarbonate', 'Reuse', 'KtV'],
    'pcc_oncology_ext': ['Regimen', 'Cycle', 'Toxicity', 'Response', 'DoseReduction', 'HoldReason', 'Biomarker', 'Survivorship', 'TumorBoard', 'Palliative'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CG {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CG {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cg_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cg_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CG: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
