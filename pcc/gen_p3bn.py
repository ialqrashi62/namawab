"""
P3-BN generator: gastro_ext, rheum_ext2, id_ext
Same in-memory shim pattern as P3-BM.
"""
import os
...
VER = "3.26.0"
MODULES = [
    ("gastro_ext", "Gastro-Ext", "Gastro-Advanced"),
    ("rheum_ext2", "Rheum-Ext-2", "Rheum-Advanced"),
    ("id_ext", "ID-Ext", "Infectious-Disease"),
]

GA_FNS = ["IBS","IBD","LiverLesion","Cirrhosis","HepB","HepC","PUD","GERD","Pancreatitis","ColonCancer"]
RH_FNS = ["RA","SLE","PsoriaticArthritis","AnkylosingSpondyl","Gout","Vasculitis","Sjogren","Scleroderma","Polymyalgia","PediatricRheum"]
ID_FNS = ["SepsisBundle","EmpiricAbx","HIVInitiation","TB","Travel","Fungal","Viral","OPAT","ProstheticJoint","HIVPrep"]
FNS_MAP = {"gastro_ext": GA_FNS, "rheum_ext2": RH_FNS, "id_ext": ID_FNS}
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bn_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bn_up.sql"), "w", encoding="utf-8") as f:
