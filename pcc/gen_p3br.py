#!/usr/bin/env python3
"""P3-BR generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('anesthesia2', 'Anesthesia 2', 'anes'),
    ('radiology2', 'Radiology 2', 'rad'),
    ('pathology_ext', 'Pathology Extended', 'path'),
]
PKG = 'p3br_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-BR package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-BR module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'anesthesia2':
        cap = ['ASAClass', 'Airway', 'Regional', 'General', 'Monitoring', 'Pain', 'Complications', 'Fluids', 'Emergence', 'RegionalBlock']
    elif mod == 'radiology2':
        cap = ['CT', 'MRI', 'US', 'Xray', 'Nuclear', 'Interventional', 'Mammo', 'Fluoro', 'PE', 'Biopsy']
    else:
        cap = ['Biopsy', 'Frozen', 'ImmunoHisto', 'Molecular', 'Cyto', 'HematoPath', 'Surgical', 'Autopsy', 'Consult', 'MolecularDx']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-BR {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'anesthesia2':
        engine_funcs = ['ASAClass', 'Airway', 'Regional', 'General', 'Monitoring', 'Pain', 'Complications', 'Fluids', 'Emergence', 'RegionalBlock']
    elif mod == 'radiology2':
        engine_funcs = ['CT', 'MRI', 'US', 'Xray', 'Nuclear', 'Interventional', 'Mammo', 'Fluoro', 'PE', 'Biopsy']
    else:
        engine_funcs = ['Biopsy', 'Frozen', 'ImmunoHisto', 'Molecular', 'Cyto', 'HematoPath', 'Surgical', 'Autopsy', 'Consult', 'MolecularDx']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-BR {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3br_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3br_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-BR: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
