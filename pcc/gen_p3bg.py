"""
P3-BG generator: transplant_immunology, mens_health_ext, palliative_ext2
Same in-memory shim pattern as P3-BF.
"""
import os
...
VER = "3.19.0"
MODULES = [
    ("transplant_immunology", "Transplant-Immunology", "Transplant-Imm"),
    ("mens_health_ext", "Mens-Health-Ext", "Mens-Health"),
    ("palliative_ext2", "Palliative-Ext-2", "Palliative"),
]

TI_FNS = ["ABOMatch","Crossmatch","DSAPanel","InductionProtocol","MaintenanceIS","RejectionAcute","RejectionChronic","DSAmonitor","InfectionProphylaxis","VaccinationSchedule"]
MH_FNS = ["WellMan","Testosterone","ErectileDysfunction","ProstateScreen","BPH","Hypogonadism","InfertilityMale","STI","HairLoss","MentalHealthMale"]
PE_FNS = ["SymptomBurden","PrognosisEst","AdvanceDirective","HospiceEval","PainRefractory","DyspneaMgmt","DeliriumTerminal","NutritionHydration","GriefBereavement","CaregiverBurnout"]
FNS_MAP = {"transplant_immunology": TI_FNS, "mens_health_ext": MH_FNS, "palliative_ext2": PE_FNS}
...
def routes(mod, label, fns_list_str):
    return (
        ...
        f"  res.json({{ module: '{mod}', label: '{label}', functions: fns.length, version: '{VER}' }});\n"
        f"}});\n\n"
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bg_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bg_up.sql"), "w", encoding="utf-8") as f:
