#!/usr/bin/env python3
"""P3-CL generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_cardio_ext4', 'PCC Cardio Ext4', 'cardx4'),
    ('pcc_ortho_ext3', 'PCC Ortho Ext3', 'ortx3'),
    ('pcc_derma_ext3', 'PCC Derma Ext3', 'drmx3'),
]
PKG = 'p3cl_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CL package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CL module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_cardio_ext4': ['RiskStratification', 'ACS', 'HeartFailure', 'Arrhythmia', 'Valvular', 'Hypertension', 'Lipid', 'Anticoag', 'Cardioversion', 'Echo'],
    'pcc_ortho_ext3': ['Fx', 'Joint', 'Spine', 'Sports', 'Pediatric', 'Tumor', 'Hand', 'Foot', 'Postop', 'Rehab'],
    'pcc_derma_ext3': ['Lesion', 'Rash', 'Burn', 'Melanoma', 'Psoriasis', 'Acne', 'Ulcer', 'Mohs', 'Dermoscopy', 'Patch'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CL {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CL {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cl_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cl_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CL: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
