#!/usr/bin/env python3
"""P3-DD generator: 3 modules = engines + tests + integration tests + routes + SQL."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_sports_science', 'PCC Sports Science', 'spt'),
    ('pcc_performance_medicine', 'PCC Performance Medicine', 'prf'),
    ('pcc_occupational_rehab', 'PCC Occupational Rehab', 'ocr'),
]
PKG = 'p3dd_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
ENGINE_FUNCS = {
    'pcc_sports_science': ['Biomechanics', 'LoadMonitoring', 'InjuryRisk', 'ReturnToPlay', 'NutritionPeriodization', 'HydrationStrategy', 'RecoveryOptimization', 'YouthAthlete', 'TeamHealth', 'AltitudeTraining'],
    'pcc_performance_medicine': ['VO2Max', 'LactateThreshold', 'MovementScreen', 'CognitivePerformance', 'HRVMonitoring', 'SleepForPerformance', 'MentalSkills', 'EquipmentOptimization', 'PeriodizationPlan', 'Overtraining'],
    'pcc_occupational_rehab': ['WorkCapacity', 'ErgonomicAssessment', 'FunctionalRestoration', 'ReturnToWork', 'VocationalRetraining', 'WorkHardening', 'PainAtWork', 'CognitiveDemands', 'SafetyClearance', 'DisabilityEvaluation'],
}
