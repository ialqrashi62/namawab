// P3-AP integration: transfusion_med
...
console.log('transfusion_med integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO transfusion_med (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); db.prepare('INSERT INTO transfusion_med (tenant_id, data) VALUES (?, ?)').run('t2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM transfusion_med WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO transfusion_med (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM transfusion_med').get(); assert.strictEqual(r.c, 1); });
