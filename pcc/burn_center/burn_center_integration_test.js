...
console.log('burn_center integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO burn_center (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); db.prepare('INSERT INTO burn_center (tenant_id, data) VALUES (?, ?)').run('t2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM burn_center WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO burn_center (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM burn_center').get(); assert.strictEqual(r.c, 1); });
