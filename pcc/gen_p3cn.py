#!/usr/bin/env python3
"""P3-CN generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_hem_ext3', 'PCC Hem Ext3', 'hemx3'),
    ('pcc_id_ext3', 'PCC ID Ext3', 'idx3'),
    ('pcc_pulm_ext3', 'PCC Pulm Ext3', 'pulx3'),
]
PKG = 'p3cn_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CN package schema v{VER}\n',
             'CREATE SCHEMA IF NOT EXISTS p3cn;']
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CN module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_hem_ext3': ['Anemia', 'Transfusion', 'Coag', 'Marrow', 'Mds', 'Mpn', 'Lymphoma', 'Leukemia', 'Transplant', 'Iron'],
    'pcc_id_ext3': ['Cdiff', 'Mrsa', 'Vre', 'Esbl', 'Tbflu', 'Malaria', 'Tb', 'Hiv', 'Hep', 'Travel'],
    'pcc_pulm_ext3': ['Asthma', 'Copd', 'Pna', 'Tb', 'Pe', 'Ca', 'Mesothelioma', 'Sarcoid', 'Ipf', 'Sleep'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CN {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CN {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cn_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cn_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CN: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
