#!/usr/bin/env python3
"""P3-EL generator v3.102.0 — pcc_neuro_ext3, pcc_pediatric_behavior, pcc_pediatric_imaging"""
import os
...
MODULES = [
    ('pcc_neuro_ext3', 'PCC Neuro Ext3', 'pcc-neuro-ext3'),
    ('pcc_pediatric_behavior', 'PCC Pediatric Behavior', 'pcc-pediatric-behavior'),
    ('pcc_pediatric_imaging', 'PCC Pediatric Imaging', 'pcc-pediatric-imaging'),
]

ENGINE_FUNCS = {
    'pcc_neuro_ext3': [
        'NeuroSarcoidosisEval', 'NeuroBehcetEval', 'NeurosyphilisProtocol',
        'NeuroLymeDisease', 'NeuromyelitisOptica', 'ProgressiveMS',
        'MOGAntibodyDisease', 'CLIPPERSProtocol', 'AutoimmuneEncephalitisExtended',
        'CNSVasculitis',
    ],
    'pcc_pediatric_behavior': [
        'AutismSpectrumEval', 'ADHDAssessment', 'PediatricAnxiety',
        'PediatricDepression', 'PediatricOCD', 'PediatricBipolarEval',
        'PediatricConductDisorder', 'PediatricOppositionalDefiant',
        'PediatricTicDisorders', 'PediatricSelectiveMutism',
    ],
    'pcc_pediatric_imaging': [
        'PediatricBrainMRI', 'PediatricCTHead', 'PediatricChestImaging',
        'PediatricAbdomenImaging', 'PediatricSpineImaging', 'PediatricMusculoskeletalImaging',
        'PediatricCardiacImaging', 'PediatricFetalImaging', 'PediatricUltrasound',
        'PediatricNuclearMedicine',
    ],
}
...
def write_routes(module, label, funcs):
    lines = [f"// {PHASE} {module}_routes v{VER}", "// P3-EL: authenticate via requireAuth middleware (sandbox: helmet/CSP enforced at app level)", "'use strict';", "const express = require('express');", f"const Engine = require('./{module}_engine.js');", "const VER = '%s';" % VER, "const router = express.Router();", ""]
