"""
P3-AY generator: hippotherapy, aquatic_therapy, child_life
Same in-memory shim pattern as P3-AV/AW/AX.
"""
import os
...
P3 = "P3-AY"
MODULES = [
    ("hippotherapy", "Hippotherapy", "Animal-Assisted-PT"),
    ("aquatic_therapy", "Aquatic-Therapy", "Aquatic-PT"),
    ("child_life", "Child-Life", "Pediatric-Support"),
]

HIPPO_FNS = ["HippotherapyEval","HorseSelection","GaitOnHorse","PediatricHippotherapy","AdultHippotherapy","HippotherapyContra","SessionStructure","ProgressMeasure","SafetyProtocol","HippotherapyDischarge"]
AQUATIC_FNS = ["AquaticAssessment","PoolSelection","AquaticExercise","BadelogicJoint","RheumatologyAquatic","NeuroAquatic","AquaticSafety","Hallwick","AquaticCardiac","AquaticDosing"]
CL_FNS = ["ChildLifeAssessment","ProceduralPreparation","MedicalPlay","DistractionToolbox","PainCoping","HospitalSchool","SiblingSupport","EndOfLifeChild","ChildLifeDosing","ChildLifeDischarge"]
FNS_MAP = dict(zip([m[0] for m in MODULES], [HIPPO_FNS, AQUATIC_FNS, CL_FNS]))
...
def routes(mod, label, fns_list_str):
    return (
        ...
        f"  res.json({{ module: '{mod}', label: '{label}', functions: fns.length, version: '3.11.0' }});\n"
        f"}});\n\n"
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3ay_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3ay_up.sql"), "w", encoding="utf-8") as f:
