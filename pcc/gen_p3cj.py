#!/usr/bin/env python3
"""P3-CJ generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_icu_ext3', 'PCC ICU Ext3', 'icux3'),
    ('pcc_ed_ext2', 'PCC ED Ext2', 'edx2'),
    ('pcc_ob_ext2', 'PCC OB Ext2', 'obx2'),
]
PKG = 'p3cj_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CJ package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CJ module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_icu_ext3': ['Ventilation', 'Sedation', 'Drivers', 'Nutrition', 'Transport', 'Braden', 'HandHygiene', 'Discharge', 'DailyGoals', 'Requiring'],
    'pcc_ed_ext2': ['Triage', 'TraumaTeam', 'FastTrack', 'PatientFlow', 'Complaint', 'RSI', 'PainProtocol', 'Discharge', 'Admit', 'Briefing'],
    'pcc_ob_ext2': ['GADobstetric', 'Labor', 'Mode', 'FHR', 'Filter', 'PostnatalCare', 'Bleeding', 'Screening', 'Antenatal', 'Risk'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CJ {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CJ {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cj_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cj_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CJ: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
