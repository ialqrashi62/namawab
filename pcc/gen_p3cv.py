#!/usr/bin/env python3
"""P3-CV generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_palliative', 'PCC Palliative', 'pal'),
    ('pcc_pain_mgmt', 'PCC Pain Management', 'pmn'),
    ('pcc_sports_med', 'PCC Sports Medicine', 'smd'),
]
PKG = 'p3cv_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CV package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CV module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_palliative': ['Symptom', 'Performance', 'Prognosis', 'Goals', 'AdvanceCare', 'FamilyMeeting', 'Hospice', 'Medication', 'Breakthrough', 'Spiritual'],
    'pcc_pain_mgmt': ['Nrs', 'OpioidRisk', 'Adjuvant', 'Breakthrough', 'Bowel', 'Sedation', 'Nausea', 'Itch', 'Respiratory', 'Urinary'],
    'pcc_sports_med': ['Injury', 'ReturnToPlay', 'Concussion', 'CardiacScreen', 'Hydration', 'Heat', 'Overuse', 'Doping', 'Nutrition', 'Imaging'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CV {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CV {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cv_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cv_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CV: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
