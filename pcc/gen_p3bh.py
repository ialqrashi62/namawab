"""
P3-BH generator: transplant_pharmacy, neuro_ext2, cv_ext2
Same in-memory shim pattern as P3-BG.
"""
import os
...
VER = "3.20.0"
MODULES = [
    ("transplant_pharmacy", "Transplant-Pharmacy", "Transplant-Pharm"),
    ("neuro_ext2", "Neuro-Ext-2", "Neuro"),
    ("cv_ext2", "CV-Ext-2", "Cardiology"),
]

TP_FNS = ["TacLevel","MMFDose","SteroidTaper","ValcyteCMV","BactrimPCP","Antifungal","DrugInteraction","AdherenceMonitor","SideEffectMgmt","TherapeuticDrugMonitor"]
NE_FNS = ["StrokeTriage","Migraine","Seizure","Parkinsons","MSRelapse","DementiaEval","GBS","Myasthenia","Neuropathy","BrainTumor"]
CV_FNS = ["ACS","HeartFailure","Arrhythmia","ValveDisease","Hypertension","Anticoagulation","LipidMgmt","PAD","Cardiomyopathy","Pericardial"]
FNS_MAP = {"transplant_pharmacy": TP_FNS, "neuro_ext2": NE_FNS, "cv_ext2": CV_FNS}
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bh_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bh_up.sql"), "w", encoding="utf-8") as f:
