#!/usr/bin/env python3
"""P3-CU generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_immunizations', 'PCC Immunizations', 'imm'),
    ('pcc_cancer_screen', 'PCC Cancer Screen', 'csr'),
    ('pcc_womens_health', 'PCC Womens Health', 'wmh'),
]
PKG = 'p3cu_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CU package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CU module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_immunizations': ['Immunization', 'Schedule', 'Catchup', 'AllergyToVaccine', 'Consent', 'LotNumber', 'Site', 'Adrs', 'Pregnancy', 'Titer'],
    'pcc_cancer_screen': ['Breast', 'Colon', 'Cervix', 'Prostate', 'Lung', 'Skin', 'Ovarian', 'Hpv', 'Smear', 'Recall'],
    'pcc_womens_health': ['Pregnancy', 'Contraception', 'Menopause', 'Pcos', 'Endometriosis', 'Infertility', 'Postpartum', 'Sti', 'Domestic', 'Vaginitis'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CU {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CU {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cu_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cu_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CU: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
