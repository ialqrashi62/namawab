// P3-BB integration: falls_prevention (Falls-Prevention)
...
console.log('falls_prevention integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO falls_prevention (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO falls_prevention (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM falls_prevention WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO falls_prevention (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM falls_prevention').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.FallsRiskAssessment({}); assert.strictEqual(typeof r, 'object'); });
