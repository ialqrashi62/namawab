#!/usr/bin/env python3
"""P3-DC generator: 3 modules = engines + tests + integration tests + routes + SQL."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_lifestyle_medicine', 'PCC Lifestyle Medicine', 'lfs'),
    ('pcc_environmental_medicine', 'PCC Environmental Medicine', 'env'),
    ('pcc_space_medicine', 'PCC Space Medicine', 'spc'),
]
PKG = 'p3dc_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
ENGINE_FUNCS = {
    'pcc_lifestyle_medicine': ['PhysicalActivity', 'NutritionHabits', 'SleepHygiene', 'StressManagement', 'SocialConnection', 'SubstanceUse', 'Mindfulness', 'WorkLifeBalance', 'NatureExposure', 'PurposeAndMeaning'],
    'pcc_environmental_medicine': ['AirQuality', 'WaterSafety', 'ToxinExposure', 'AllergenMapping', 'ClimateHealth', 'BuiltEnvironment', 'OccupationalEnv', 'FoodEnvironment', 'VectorRisk', 'RadiationSafety'],
    'pcc_space_medicine': ['MicrogravityPhysiology', 'RadiationProtection', 'IsolationPsychology', 'EVAMedical', 'Countermeasures', 'SpaceNutrition', 'TelemedicineSpace', 'ReentryCare', 'AstronautSelection', 'LongDurationHealth'],
}
