#!/usr/bin/env python3
"""P3-BY generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('allergy_ext2', 'Allergy Extended 2', 'allergy2'),
    ('cv_ext3', 'Cerebrovascular Extended 3', 'cv3'),
    ('sleep_ext2', 'Sleep Extended 2', 'sleep2'),
]
PKG = 'p3by_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-BY package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-BY module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'allergy_ext2':
        cap = ['Rhinitis', 'Asthma', 'Food', 'Drug', 'Urticaria', 'Anaphylaxis', 'Sting', 'Eczema', 'Contact', 'AIT']
    elif mod == 'cv_ext3':
        cap = ['Stroke', 'TIA', 'SAH', 'Aneurysm', 'AVM', 'Carotid', 'ICP', 'Seizure', 'MS', 'Park']
    else:
        cap = ['Insomnia', 'OSA', 'RLS', 'Narcolepsy', 'Parasomnia', 'Circadian', 'CPAP', 'Daytime', 'Pediatric', 'SleepStudy']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-BY {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'allergy_ext2':
        engine_funcs = ['Rhinitis', 'Asthma', 'Food', 'Drug', 'Urticaria', 'Anaphylaxis', 'Sting', 'Eczema', 'Contact', 'AIT']
    elif mod == 'cv_ext3':
        engine_funcs = ['Stroke', 'TIA', 'SAH', 'Aneurysm', 'AVM', 'Carotid', 'ICP', 'Seizure', 'MS', 'Park']
    else:
        engine_funcs = ['Insomnia', 'OSA', 'RLS', 'Narcolepsy', 'Parasomnia', 'Circadian', 'CPAP', 'Daytime', 'Pediatric', 'SleepStudy']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-BY {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3by_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3by_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-BY: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
