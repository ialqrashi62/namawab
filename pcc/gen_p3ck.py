#!/usr/bin/env python3
"""P3-CK generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_neuro_ext2', 'PCC Neuro Ext2', 'nxx2'),
    ('pcc_psych_ext3', 'PCC Psych Ext3', 'psx3'),
    ('pcc_neonatal_ext2', 'PCC Neonatal Ext2', 'nnx2'),
]
PKG = 'p3ck_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CK package schema v{VER}\n',
             'CREATE SCHEMA IF NOT EXISTS p3ck;']
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CK module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_neuro_ext2': ['StrokeScale', 'Seizure', 'Headache', 'GCS', 'Neuropathy', 'Movement', 'Dementia', 'Ms', 'Gbs', 'Myasthenia'],
    'pcc_psych_ext3': ['Screening', 'Risk', 'Depression', 'Anxiety', 'Substance', 'Psychosis', 'Bipolar', 'MedMgmt', 'Therapy', 'Restraint'],
    'pcc_neonatal_ext2': ['GestationAge', 'APGAR', 'BirthWeight', 'NewbornScreen', 'Breastfeed', 'Hyperbilirubin', 'Feeding', 'DischargeChecklist', 'SepsisEval', 'CordCare'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CK {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CK {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3ck_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3ck_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CK: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
