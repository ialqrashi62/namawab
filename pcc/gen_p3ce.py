#!/usr/bin/env python3
"""P3-CE generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_quality', 'PCC Quality', 'qual'),
    ('pcc_research', 'PCC Research', 'res'),
    ('pcc_education', 'PCC Education', 'edu'),
]
PKG = 'p3ce_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CE package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CE module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_quality': ['Quality', 'Indicator', 'Audit', 'Safety', 'Performance', 'Improvement', 'Peer', 'Credentialing', 'Satisfaction', 'Report'],
    'pcc_research': ['Protocol', 'Consent', 'IRB', 'Enrollment', 'Adverse', 'Randomization', 'Biostats', 'Publication', 'Funding', 'Dataset'],
    'pcc_education': ['Curriculum', 'Rotation', 'Simulation', 'Eval', 'Lecture', 'Bedside', 'Cert', 'Fellow', 'CEU', 'Exam'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CE {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CE {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3ce_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3ce_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CE: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
