// P3-AM integration: palliative_ext (self-running)
...
console.log('palliative_ext integration tests:');
it('multi-tenant isolation', () => {
  const db = makeDb();
  db.prepare('INSERT INTO palliative_ext (tenant_id, data) VALUES (?, ?)').run('t1', JSON.stringify({x:1}));
  db.prepare('INSERT INTO palliative_ext (tenant_id, data) VALUES (?, ?)').run('t2', JSON.stringify({x:2}));
  const r = db.prepare('SELECT COUNT(*) c FROM palliative_ext WHERE tenant_id=?').get('t1');
  assert.strictEqual(r.c, 1);
});
it('CRUD persistence', () => {
  const db = makeDb();
  db.prepare('INSERT INTO palliative_ext (tenant_id, data) VALUES (?, ?)').run('t1', '{}');
  const r = db.prepare('SELECT COUNT(*) c FROM palliative_ext').get();
  assert.strictEqual(r.c, 1);
});
