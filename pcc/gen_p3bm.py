"""
P3-BM generator: ophth_ext, hem_ext, onco_ext2
Same in-memory shim pattern as P3-BL.
"""
import os
...
VER = "3.25.0"
MODULES = [
    ("ophth_ext", "Ophth-Ext", "Ophthalmology"),
    ("hem_ext", "Hem-Ext", "Hematology"),
    ("onco_ext2", "Onco-Ext-2", "Oncology-Advanced"),
]

OP_FNS = ["Glaucoma","DiabeticRetinopathy","MacularDegeneration","Cataract","Conjunctivitis","Uveitis","RetinalDetach","CornealAbrasion","Strabismus","EyeTrauma"]
HE_FNS = ["AnemiaWorkup","SickleCell","DVT","AnticoagClinic","Thrombocytopenia","Neutropenia","Transfusion","Hypercoagulable","Myeloma","BleedingDiath"]
ON_FNS = ["TNMStaging","ChemoRegimen","TumorResponse","FebrileNeutropenia","TumorLysis","OncEmergency","Survivorship","ClinicalTrial","Immunotherapy","TargetedTx"]
FNS_MAP = {"ophth_ext": OP_FNS, "hem_ext": HE_FNS, "onco_ext2": ON_FNS}
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bm_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bm_up.sql"), "w", encoding="utf-8") as f:
