#!/usr/bin/env python3
"""P3-BZ generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('id_ext2', 'Infectious Disease Extended 2', 'id2'),
    ('rheum_ext3', 'Rheumatology Extended 3', 'rheum3'),
    ('neph_ext3', 'Nephrology Extended 3', 'neph3'),
]
PKG = 'p3bz_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-BZ package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-BZ module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'id_ext2':
        cap = ['UTI', 'Pneumonia', 'SSTI', 'Cdiff', 'Sepsis', 'HIV', 'TB', 'HepB', 'HepC', 'Influenza']
    elif mod == 'rheum_ext3':
        cap = ['RA', 'SLE', 'SSc', 'Vasculitis', 'Gout', 'OA', 'SpA', 'PMR', 'Sjogren', 'Myositis']
    else:
        cap = ['CKD', 'AKI', 'GN', 'Dialysis', 'Rhabdo', 'Electrolyte', 'HTN', 'Stone', 'Txp', 'PKD']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-BZ {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'id_ext2':
        engine_funcs = ['UTI', 'Pneumonia', 'SSTI', 'Cdiff', 'Sepsis', 'HIV', 'TB', 'HepB', 'HepC', 'Influenza']
    elif mod == 'rheum_ext3':
        engine_funcs = ['RA', 'SLE', 'SSc', 'Vasculitis', 'Gout', 'OA', 'SpA', 'PMR', 'Sjogren', 'Myositis']
    else:
        engine_funcs = ['CKD', 'AKI', 'GN', 'Dialysis', 'Rhabdo', 'Electrolyte', 'HTN', 'Stone', 'Txp', 'PKD']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-BZ {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3bz_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3bz_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-BZ: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
