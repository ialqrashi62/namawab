"""
P3-BK generator: sports_med_ext, pain_ext2, psych_ext
Same in-memory shim pattern as P3-BJ.
"""
import os
...
VER = "3.23.0"
MODULES = [
    ("sports_med_ext", "Sports-Med-Ext", "Sports-Medicine"),
    ("pain_ext2", "Pain-Ext-2", "Pain-Advanced"),
    ("psych_ext", "Psych-Ext", "Psychiatry"),
]

SP_FNS = ["Concussion","ACL","RotatorCuff","Tendinopathy","StressFracture","Overtraining","DopingScreen","SuddenCardiac","Preparticipation","RecoveryProtocol"]
PA_FNS = ["OpioidRotation","NerveBlock","CancerPain","SpinalCordStim","IntrathecalPump","MigraineAcute","CRPS","PediatricPain","Tapering","Multimodal"]
PS_FNS = ["Depression","Anxiety","Bipolar","PTSD","OCD","Eating","Substance","Suicide","ADHD","Psychosis"]
FNS_MAP = {"sports_med_ext": SP_FNS, "pain_ext2": PA_FNS, "psych_ext": PS_FNS}
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bk_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bk_up.sql"), "w", encoding="utf-8") as f:
