#!/usr/bin/env python3
"""P3-CS generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_sepsis', 'PCC Sepsis', 'sep'),
    ('pcc_code_blue', 'PCC Code Blue', 'cb'),
    ('pcc_stroke_path', 'PCC Stroke Path', 'str'),
]
PKG = 'p3cs_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CS package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CS module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_sepsis': ['Screening', 'Lactate', 'Abx', 'Fluid', 'Vasopressor', 'Culture', 'SourceCtl', 'DeEscalate', 'Procalcitonin', 'SepsisShock'],
    'pcc_code_blue': ['Confirm', 'Cpr', 'Defib', 'Epi', 'Amio', 'Airway', 'Rhythm', 'Rosc', 'Etiology', 'Termination'],
    'pcc_stroke_path': ['Nihss', 'Imaging', 'Tpa', 'Thrombectomy', 'Consent', 'BpTarget', 'NihssFollowup', 'Hemorrhage', 'Swallow', 'Transfer'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CS {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CS {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cs_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cs_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CS: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
