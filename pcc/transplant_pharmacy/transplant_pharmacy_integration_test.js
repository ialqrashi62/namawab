// P3-BH integration: transplant_pharmacy (Transplant-Pharmacy)
...
console.log('transplant_pharmacy integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO transplant_pharmacy (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO transplant_pharmacy (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM transplant_pharmacy WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO transplant_pharmacy (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM transplant_pharmacy').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.TacLevel({}); assert.strictEqual(typeof r, 'object'); });
