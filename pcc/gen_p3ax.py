"""
P3-AX generator: music_therapy, art_therapy, recreational_therapy
Same in-memory shim pattern as P3-AV/AW.
"""
import os
...
P3 = "P3-AX"
MODULES = [
    ("music_therapy", "Music-Therapy", "Expressive-Arts"),
    ("art_therapy", "Art-Therapy", "Expressive-Arts"),
    ("recreational_therapy", "Recreational-Therapy", "RT"),
]

MUSIC_FNS = ["MusicTherapyAssessment","RhythmicEntrainment","NeuroMusicTherapy","MusicPain","PediatricMusic","MusicPalliative","MusicInpatient","MusicPsychiatric","MusicDosing","MusicOutcome"]
ART_FNS = ["ArtTherapyAssessment","ArtMedium","ArtTrauma","ArtGroup","ArtPediatric","ArtGeriatric","ArtDosing","ArtInpatient","ArtOutcome","ArtDigital"]
REC_FNS = ["RTAssessment","LeisureBarriers","CommunityReintegration","AdaptedSports","RTPediatric","RTGeriatric","RTDosing","RTInpatient","RTWellness","RTDischarge"]
FNS_MAP = dict(zip([m[0] for m in MODULES], [MUSIC_FNS, ART_FNS, REC_FNS]))
...
def routes(mod, label, fns_list_str):
    return (
        ...
        f"  res.json({{ module: '{mod}', label: '{label}', functions: fns.length, version: '3.10.0' }});\n"
        f"}});\n\n"
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3ax_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3ax_up.sql"), "w", encoding="utf-8") as f:
