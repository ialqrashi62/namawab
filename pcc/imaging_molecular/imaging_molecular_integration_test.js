// P3-AT integration: imaging_molecular
...
console.log('imaging_molecular integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO imaging_molecular (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); db.prepare('INSERT INTO imaging_molecular (tenant_id, data) VALUES (?, ?)').run('t2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM imaging_molecular WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO imaging_molecular (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM imaging_molecular').get(); assert.strictEqual(r.c, 1); });
