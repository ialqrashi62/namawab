#!/usr/bin/env python3
"""P3-DA generator: 3 modules = engines + tests + integration tests + routes + SQL."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_integrative_medicine', 'PCC Integrative Medicine', 'int'),
    ('pcc_functional_medicine', 'PCC Functional Medicine', 'fnc'),
    ('pcc_longevity_medicine', 'PCC Longevity Medicine', 'lng'),
]
PKG = 'p3da_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-DA package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-DA module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_integrative_medicine': ['HolisticAssessment', 'MindBody', 'Acupuncture', 'HerbalMedicine', 'NutritionTherapy', 'YogaTherapy', 'StressReduction', 'SleepOptimization', 'DetoxProtocol', 'IntegrativeOncology'],
    'pcc_functional_medicine': ['RootCause', 'Timeline', 'EliminationDiet', 'GutHealing', 'HormoneBalance', 'Toxicity', 'Inflammation', 'MitochondrialSupport', 'ImmuneModulation', 'PersonalizedPlan'],
    'pcc_longevity_medicine': ['BiologicalAge', 'Senolytics', 'HormoneOptimization', 'MetabolicHealth', 'CognitivePreservation', 'MuscleMass', 'CardiovascularFitness', 'Nutraceuticals', 'LifestyleScore', 'MortalityRisk'],
}
...
def make_engine(mod):
  ...
  return {{ plan, t }};
    ...
    return f"""// P3-DA {mod}_engine v{VER}
'use strict';
{chr(10).join(funcs)}
module.exports = {{
  {', '.join(cap)}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-DA {mod} integration test v{VER}
const Engine = require('./{mod}_engine.js');
const assert = require('assert');
let passed = 0, failed = 0;
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-DA {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3da_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3da_{VER.replace(".","")}_{mod}_up.sql'
        with open(mod_sql, 'w', encoding='utf-8') as f:
            f.write(make_module_sql(mod))
        engine_path = f'{mod}/{mod}_engine.js'
        with open(engine_path, 'w', encoding='utf-8') as f:
            f.write(make_engine(mod))
        test_path = f'{mod}/{mod}_test.js'
    ...
    print(f'Generated P3-DA: engines + tests + integ tests + routes + SQL (v{VER})')
