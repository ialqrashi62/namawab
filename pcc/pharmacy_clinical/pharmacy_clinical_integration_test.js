// P3-AO integration: pharmacy_clinical
...
console.log('pharmacy_clinical integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO pharmacy_clinical (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); db.prepare('INSERT INTO pharmacy_clinical (tenant_id, data) VALUES (?, ?)').run('t2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM pharmacy_clinical WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO pharmacy_clinical (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM pharmacy_clinical').get(); assert.strictEqual(r.c, 1); });
