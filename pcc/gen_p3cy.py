#!/usr/bin/env python3
"""P3-CY generator: 3 modules = engines + tests + integration tests + routes + SQL."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_travel_med', 'PCC Travel Medicine', 'trv'),
    ('pcc_genetic_counseling', 'PCC Genetic Counseling', 'gnc'),
    ('pcc_wound_care_ext', 'PCC Wound Care Extended', 'wce'),
]
PKG = 'p3cy_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CY package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CY module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_travel_med': ['DestinationRisk', 'VaccinationNeed', 'MalariaProphylaxis', 'TravelersDiarrhea', 'JetLag', 'DvtRisk', 'Altitude', 'DivingFitness', 'PregnancyTravel', 'ReturnEvaluation'],
    'pcc_genetic_counseling': ['RiskAssessment', 'Pedigree', 'CarrierScreen', 'PrenatalTesting', 'CancerGenetics', 'Pharmacogenomics', 'VariantInterpretation', 'Consent', 'FamilyCommunication', 'Referral'],
    'pcc_wound_care_ext': ['WoundAssessment', 'Debridement', 'InfectionControl', 'Dressing', 'PressureInjury', 'DiabeticFoot', 'VacTherapy', 'HealingScore', 'NutritionWound', 'ScarManagement'],
}
...
def make_engine(mod):
  ...
  return {{ plan, t }};
    ...
    return f"""// P3-CY {mod}_engine v{VER}
'use strict';
{chr(10).join(funcs)}
module.exports = {{
  {', '.join(cap)}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CY {mod} integration test v{VER}
const Engine = require('./{mod}_engine.js');
const assert = require('assert');
let passed = 0, failed = 0;
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CY {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cy_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cy_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CY: engines + tests + integ tests + routes + SQL (v{VER})')
