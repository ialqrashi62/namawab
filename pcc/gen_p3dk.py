#!/usr/bin/env python3
"""P3-DK generator: 3 modules = engines + tests + integration tests + routes + SQL."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_pulmonary_rehabilitation', 'PCC Pulmonary Rehabilitation', 'prr'),
    ('pcc_thoracic_surgery', 'PCC Thoracic Surgery', 'ths'),
    ('pcc_respiratory_therapy', 'PCC Respiratory Therapy', 'rpt'),
]
PKG = 'p3dk_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
ENGINE_FUNCS = {
    'pcc_pulmonary_rehabilitation': ['ExerciseCapacity', 'DyspneaIndex', 'SixMinuteWalk', 'RehabAdherence', 'InhalerTechnique', 'AirwayClearance', 'PulmonaryEducation', 'SmokingCessation', 'NutritionPulmonary', 'PsychosocialScreen'],
    'pcc_thoracic_surgery': ['ThoracotomyRisk', 'VatsEligibility', 'LobectomyAssessment', 'ChestTubeProtocol', 'PneumothoraxManagement', 'PleuralEffusionPlan', 'MediastinalMassWorkup', 'ThoracicTraumaTriage', 'EsophagealSurgeryPrep', 'PostThoracotomyCare'],
    'pcc_respiratory_therapy': ['AerosolTherapy', 'MechanicalVentilationWean', 'NonInvasiveVentilation', 'HighFlowNasalCannula', 'ArterialBloodGasInterpret', 'BronchoscopyPrep', 'SputumInduction', 'PulmonaryFunctionTestPrep', 'OxygenConservingDevice', 'RespiratoryEmergencyBag'],
}
