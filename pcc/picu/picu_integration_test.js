...
const SCHEMA = `CREATE TABLE picu_admission (
...
function writeAuditLog(c, { tenantId, actorId, action, entityType, entityId, payload }) {
  const prev = c.exec(`SELECT entry_hash FROM picu_audit_log WHERE tenant_id = ? ORDER BY id DESC LIMIT 1`, [tenantId]);
  ...
  c.run(`INSERT INTO picu_audit_log (tenant_id, actor_id, action, entity_type, entity_id, payload, prev_hash, entry_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ...
  return {
    createAdmission(t, s, k, b) {
      if (!IDEMPOTENCY.has(k)) {
        const result = db.withTenant(t, (c) => {
          ...
          c.run(`INSERT INTO picu_admission (id, tenant_id, patient_id, encounter_id, admission_type, cpt_codes) VALUES (?, ?, ?, ?, ?, ?)`,
            [id, t, b.patientId, b.encounterId, b.admissionType, JSON.stringify(b.cptCodes || [])]);
          writeAuditLog(c, { tenantId: t, actorId: s.userId, action: 'CREATE', entityType: 'picu_admission', entityId: id, payload: b });
          return c.exec(`SELECT * FROM picu_admission WHERE id = ?`, [id])[0];
        });
    ...
    addVital(t, s, admissionId, b) {
      return db.withTenant(t, (c) => {
        const adm = c.exec(`SELECT id FROM picu_admission WHERE id = ? AND tenant_id = ?`, [admissionId, t]);
        if (adm.length === 0) { const e = new Error('Not found'); e.statusCode = 404; throw e; }
        const id = crypto.randomUUID();
        c.run(`INSERT INTO picu_vital_sign (id, tenant_id, admission_id, measured_at, heart_rate, sbp_mmhg, spo2_pct) VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [id, t, admissionId, b.measuredAt || new Date().toISOString(), b.heartRate, b.sbpMmhg, b.spo2Pct]);
        writeAuditLog(c, { tenantId: t, actorId: s.userId, action: 'CREATE', entityType: 'picu_vital_sign', entityId: id, payload: b });
        return { id, admissionId };
      });
    },
    list(t, lim = 50) {
      return db.withTenant(t, (c) => c.exec(`SELECT id, tenant_id, patient_id, admission_type, status FROM picu_admission WHERE tenant_id = ? AND soft_deleted_at IS NULL ORDER BY created_at DESC LIMIT ?`, [t, lim]));
    },
    get(t, id) {
      return db.withTenant(t, (c) => {
        const adm = c.exec(`SELECT * FROM picu_admission WHERE id = ? AND tenant_id = ?`, [id, t]);
        if (adm.length === 0) { const e = new Error('Not found'); e.statusCode = 404; throw e; }
...
(async function main() {
  ...
  describe('Scenario 1: Multi-tenant isolation', () => {
    it('tenant A creates admission', () => { const a = h.createAdmission(TA, { userId: 1 }, 'picu-iso-1', { patientId: 100, encounterId: 200, admissionType: 'critical_care' }); assert(a.id); assertEq(a.tenant_id, TA); });
  ...
  describe('Scenario 2: CRUD round-trip', () => {
    it('create', () => { const a = h.createAdmission(TA, { userId: 2 }, 'picu-crud-1', { patientId: 300, encounterId: 400, admissionType: 'critical_care' }); assert(a.id); });
  ...
  describe('Scenario 3: Idempotency', () => {
    ...
    it('only 1 row', () => { assertEq(db.exec(`SELECT id FROM picu_admission WHERE patient_id = ? AND encounter_id = ?`, [500, 600]).length, 1); });
  });

  describe('Scenario 4: Vitals chain', () => {
    let admId;
    it('create admission', () => { const a = h.createAdmission(TA, { userId: 4 }, 'picu-vital-1', { patientId: 700, encounterId: 800, admissionType: 'critical_care' }); admId = a.id; });
  ...
  describe('Scenario 5: Audit hash chain', () => {
    it('count >= 5', () => { assert(db.exec(`SELECT COUNT(*) AS n FROM picu_audit_log WHERE tenant_id = ?`, [TA])[0].n >= 5); });
    it('first prev_hash null', () => { assertEq(db.exec(`SELECT prev_hash FROM picu_audit_log WHERE tenant_id = ? ORDER BY id ASC LIMIT 1`, [TA])[0].prev_hash, null); });
    it('chain links', () => { const rows = db.exec(`SELECT prev_hash, entry_hash FROM picu_audit_log WHERE tenant_id = ? ORDER BY id ASC`, [TA]); let prev = null; for (const r of rows) { assertEq(r.prev_hash, prev); prev = r.entry_hash; } });
    it('hash recomputes', () => { const rows = db.exec(`SELECT * FROM picu_audit_log WHERE tenant_id = ? ORDER BY id ASC`, [TA]); for (const r of rows) { const str = JSON.stringify({ tenantId: r.tenant_id, actorId: r.actor_id, action: r.action, entityType: r.entity_type, entityId: r.entity_id, payload: JSON.parse(r.payload), prevHash: r.prev_hash }); assertEq(r.entry_hash, crypto.createHash('sha256').update(str).digest('hex')); } });
  });

  console.log();
  console.log('picu integration tests: ' + passed + ' passed, ' + failed + ' failed');
  console.log('='.repeat(50));
  if (failed > 0) { process.stderr.write('FAILED\n'); process.exit(1); }
  process.stdout.write('PASS: 17/17 picu integration\n');
  process.exit(0);
})();
