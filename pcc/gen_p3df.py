#!/usr/bin/env python3
"""P3-DF generator: 3 modules = engines + tests + integration tests + routes + SQL."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_immune_health', 'PCC Immune Health', 'imm'),
    ('pcc_allergy_precision', 'PCC Allergy Precision', 'alr'),
    ('pcc_inflammation', 'PCC Inflammation', 'inf'),
]
PKG = 'p3df_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
ENGINE_FUNCS = {
    'pcc_immune_health': ['ImmunePanel', 'VaccineResponse', 'AutoimmuneRisk', 'Immunodeficiency', 'AllergyImmune', 'InfectionSusceptibility', 'ImmuneAging', 'Th1Th2Balance', 'CytokineProfile', 'ImmuneSupportPlan'],
    'pcc_allergy_precision': ['AllergenComponent', 'CrossReactivity', 'OralAllergy', 'DrugAllergyGenetics', 'VenomAllergy', 'AtopicDermatitis', 'AllergicRhinitis', 'AsthmaAllergy', 'FoodChallenge', 'Desensitization'],
    'pcc_inflammation': ['CRPTrend', 'ESRPattern', 'CytokineStorm', 'ChronicInflammation', 'Neuroinflammation', 'CardiovascularInflammation', 'GutInflammation', 'AutoimmuneFlare', 'AntiInflammatoryDiet', 'InflammationResolution'],
}
