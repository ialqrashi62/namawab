#!/usr/bin/env python3
"""P3-CB generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_workflow', 'PCC Workflow', 'wf'),
    ('pcc_analytics', 'PCC Analytics', 'an'),
    ('pcc_compliance', 'PCC Compliance', 'comp'),
]
PKG = 'p3cb_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CB package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CB module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'pcc_workflow':
        cap = ['State', 'Transition', 'Assignment', 'Escalation', 'Notify', 'Approval', 'Schedule', 'Queue', 'Timeout', 'Batch']
    elif mod == 'pcc_analytics':
        cap = ['Aggregate', 'Group', 'Trend', 'Anomaly', 'Cohort', 'Funnel', 'Retention', 'Conversion', 'KPI', 'Report']
    else:
        cap = ['HIPAA', 'NPHIES', 'ZATCA', 'PDPL', 'CBAHI', 'Audit', 'Consent', 'Breach', 'Access', 'Retention']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-CB {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'pcc_workflow':
        engine_funcs = ['State', 'Transition', 'Assignment', 'Escalation', 'Notify', 'Approval', 'Schedule', 'Queue', 'Timeout', 'Batch']
    elif mod == 'pcc_analytics':
        engine_funcs = ['Aggregate', 'Group', 'Trend', 'Anomaly', 'Cohort', 'Funnel', 'Retention', 'Conversion', 'KPI', 'Report']
    else:
        engine_funcs = ['HIPAA', 'NPHIES', 'ZATCA', 'PDPL', 'CBAHI', 'Audit', 'Consent', 'Breach', 'Access', 'Retention']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-CB {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cb_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cb_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CB: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
