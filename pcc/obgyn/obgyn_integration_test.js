...
function writeAuditLog(c, { tenantId, actorId, action, entityType, entityId, payload }) {
  const prev = c.exec(`SELECT entry_hash FROM obgyn_audit_log WHERE tenant_id = ? ORDER BY id DESC LIMIT 1`, [tenantId]);
  ...
  c.run(`INSERT INTO obgyn_audit_log (tenant_id, actor_id, action, entity_type, entity_id, payload, prev_hash, entry_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
function makeHandlers(db) {
  return {
    create(t, s, k, b) {
      if (!IDEMPOTENCY.has(k)) {
        const result = db.withTenant(t, (c) => {
          ...
          c.run(`INSERT INTO obgyn_pregnancy (id, tenant_id, patient_id, encounter_id, gestational_age_weeks, cpt_codes) VALUES (?, ?, ?, ?, ?, ?)`,
            [id, t, b.patientId, b.encounterId, b.gestationalAgeWeeks || 'unspecified', JSON.stringify(b.cptCodes || [])]);
          writeAuditLog(c, { tenantId: t, actorId: s.userId, action: 'CREATE', entityType: 'obgyn_pregnancy', entityId: id, payload: b });
          return c.exec(`SELECT * FROM obgyn_pregnancy WHERE id = ?`, [id])[0];
        });
    ...
    list(t, lim = 50) {
      return db.withTenant(t, (c) => c.exec(`SELECT id, tenant_id, patient_id, gestational_age_weeks, status FROM obgyn_pregnancy WHERE tenant_id = ? AND soft_deleted_at IS NULL ORDER BY created_at DESC LIMIT ?`, [t, lim]));
    },
    get(t, id) {
      return db.withTenant(t, (c) => {
        const row = c.exec(`SELECT * FROM obgyn_pregnancy WHERE id = ? AND tenant_id = ?`, [id, t]);
...
(async function main() {
  ...
  describe('Scenario 1: Multi-tenant isolation', () => {
    it('tenant A creates record', () => { const a = h.create(TA, { userId: 1 }, 'obgyn-iso-1', { patientId: 100, encounterId: 200, gestationalAgeWeeks: 'routine' }); assert(a.id); assertEq(a.tenant_id, TA); });
  ...
  describe('Scenario 2: CRUD round-trip', () => {
    it('create', () => { const a = h.create(TA, { userId: 2 }, 'obgyn-crud-1', { patientId: 300, encounterId: 400, gestationalAgeWeeks: 'routine' }); assert(a.id); });
  ...
  describe('Scenario 3: Idempotency', () => {
    const KEY = 'obgyn-idem-key-001';
    const BODY = { patientId: 500, encounterId: 600, gestationalAgeWeeks: 'routine' };
    ...
    it('only 1 row', () => { assertEq(db.exec(`SELECT id FROM obgyn_pregnancy WHERE patient_id = ? AND encounter_id = ?`, [500, 600]).length, 1); });
  });

  describe('Scenario 4: Record chain', () => {
    let recId;
    it('create record', () => { const a = h.create(TA, { userId: 4 }, 'obgyn-chain-1', { patientId: 700, encounterId: 800, gestationalAgeWeeks: 'routine' }); recId = a.id; assert(a.id); });
    it('create second', () => { const a = h.create(TA, { userId: 4 }, 'obgyn-chain-2', { patientId: 750, encounterId: 850, gestationalAgeWeeks: 'routine' }); assert(a.id); });
    it('read after write', () => { const d = h.get(TA, recId); assert(d); });
  });

  describe('Scenario 5: Audit hash chain', () => {
    it('count >= 5', () => { assert(db.exec(`SELECT COUNT(*) AS n FROM obgyn_audit_log WHERE tenant_id = ?`, [TA])[0].n >= 5); });
    it('first prev_hash null', () => { assertEq(db.exec(`SELECT prev_hash FROM obgyn_audit_log WHERE tenant_id = ? ORDER BY id ASC LIMIT 1`, [TA])[0].prev_hash, null); });
    it('chain links', () => { const rows = db.exec(`SELECT prev_hash, entry_hash FROM obgyn_audit_log WHERE tenant_id = ? ORDER BY id ASC`, [TA]); let prev = null; for (const r of rows) { assertEq(r.prev_hash, prev); prev = r.entry_hash; } });
    it('hash recomputes', () => { const rows = db.exec(`SELECT * FROM obgyn_audit_log WHERE tenant_id = ? ORDER BY id ASC`, [TA]); for (const r of rows) { const str = JSON.stringify({ tenantId: r.tenant_id, actorId: r.actor_id, action: r.action, entityType: r.entity_type, entityId: r.entity_id, payload: JSON.parse(r.payload), prevHash: r.prev_hash }); assertEq(r.entry_hash, crypto.createHash('sha256').update(str).digest('hex')); } });
  });

  console.log();
  console.log('obgyn integration tests: ' + passed + ' passed, ' + failed + ' failed');
  console.log('='.repeat(50));
  if (failed > 0) { process.stderr.write('FAILED\n'); process.exit(1); }
  process.stdout.write('PASS: 17/17 obgyn integration\n');
  process.exit(0);
})();
