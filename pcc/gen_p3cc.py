#!/usr/bin/env python3
"""P3-CC generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_decision', 'PCC Decision', 'dec'),
    ('pcc_clinical_dx', 'PCC Clinical DX', 'cdx'),
    ('pcc_drug', 'PCC Drug', 'drug'),
]
PKG = 'p3cc_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CC package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CC module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'pcc_decision':
        cap = ['Triage', 'Risk', 'Recommendation', 'Differential', 'Path', 'Severity', 'Outcome', 'FollowUp', 'Test', 'Therapy']
    elif mod == 'pcc_clinical_dx':
        cap = ['Differential', 'Workup', 'Imaging', 'Lab', 'Consult', 'Spec', 'FollowUp', 'Disposition', 'Pathway', 'Alert']
    else:
        cap = ['Dose', 'Interaction', 'Allergy', 'Renal', 'Hepatic', 'Level', 'Pregnancy', 'Route', 'Frequency', 'Duration']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-CC {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'pcc_decision':
        engine_funcs = ['Triage', 'Risk', 'Recommendation', 'Differential', 'Path', 'Severity', 'Outcome', 'FollowUp', 'Test', 'Therapy']
    elif mod == 'pcc_clinical_dx':
        engine_funcs = ['Differential', 'Workup', 'Imaging', 'Lab', 'Consult', 'Spec', 'FollowUp', 'Disposition', 'Pathway', 'Alert']
    else:
        engine_funcs = ['Dose', 'Interaction', 'Allergy', 'Renal', 'Hepatic', 'Level', 'Pregnancy', 'Route', 'Frequency', 'Duration']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-CC {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cc_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cc_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CC: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
