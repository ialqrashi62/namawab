"""
P3-BC generator: home_health, community_health, med_psych
Same in-memory shim pattern as P3-AV/AW/AX/AY/AZ/BA/BB.
"""
import os
...
MODULES = [
    ...
    ("community_health", "Community-Health", "Public-Health"),
    ("med_psych", "Med-Psych", "Consultation-Liaison"),
]

HH_FNS = ["HomeHealthEligibility","OASISAssessment","HomePT","HomeHealthNursing","MedicationReconciliation","HomeHealthDischarge","HomeHealthOutcome","HomeHealthWound","HomeHealthCardiac","HomeHealthPedi"]
CH_FNS = ["CommunityRiskAssessment","HealthDisparities","SDOH","HealthLiteracy","VaccinationOutreach","CommunityMaternal","CommunityMental","CommunityScreening","CommunityOutbreak","CommunityEval"]
MP_FNS = ["DepressionScreen","AnxietyScreen","SuicideScreen","DeliriumScreen","SubstanceUse","PsychMed","MedPsychConsult","SeriousMentalIllness","MedPsychPed","PsychOutcome"]
FNS_MAP = dict(zip([m[0] for m in MODULES], [HH_FNS, CH_FNS, MP_FNS]))
...
def routes(mod, label, fns_list_str):
    return (
        ...
        f"  res.json({{ module: '{mod}', label: '{label}', functions: fns.length, version: '3.15.0' }});\n"
        f"}});\n\n"
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bc_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bc_up.sql"), "w", encoding="utf-8") as f:
