#!/usr/bin/env python3
"""P3-EM generator v3.103.0 — pcc_neuro_ext4, pcc_pediatric_surg_ext, pcc_pediatric_rehab"""
import os
...
MODULES = [
    ('pcc_neuro_ext4', 'PCC Neuro Ext4', 'pcc-neuro-ext4'),
    ('pcc_pediatric_surg_ext', 'PCC Pediatric Surg Ext', 'pcc-pediatric-surg-ext'),
    ('pcc_pediatric_rehab', 'PCC Pediatric Rehab', 'pcc-pediatric-rehab'),
]

ENGINE_FUNCS = {
    'pcc_neuro_ext4': [
        'MitochondrialDiseaseNeuro', 'LeukodystrophyEval', 'NeurocutaneousSyndromes',
        'CharcotMarieTooth', 'MyastheniaGravisCrisis', 'GuillainBarreSyndrome',
        'CIDPEval', 'ALSProtocol', 'PolymyositisDermatomyositis',
        'MyotonicDystrophy',
    ],
    'pcc_pediatric_surg_ext': [
        'PediatricLaparoscopic', 'PediatricRoboticSurg', 'PediatricEndoscopic',
        'PediatricFetalSurg', 'PediatricMinimallyInvasive', 'PediatricDaySurg',
        'PediatricAmbulatorySurg', 'PediatricSameDayDischarge', 'PediatricPreOpEval',
        'PediatricPostOpCare',
    ],
    'pcc_pediatric_rehab': [
        'PediatricRehabAssessment', 'PediatricPT', 'PediatricOT',
        'PediatricSpeechRehab', 'PediatricCognitiveRehab', 'PediatricAquaticTherapy',
        'PediatricConstraintTherapy', 'PediatricRoboticRehab', 'PediatricGaitTraining',
        'PediatricSportsRehab',
    ],
}
...
def write_routes(module, label, funcs):
    lines = [f"// {PHASE} {module}_routes v{VER}", "// P3-EM: authenticate via requireAuth middleware (sandbox: helmet/CSP enforced at app level)", "'use strict';", "const express = require('express');", f"const Engine = require('./{module}_engine.js');", "const VER = '%s';" % VER, "const router = express.Router();", ""]
