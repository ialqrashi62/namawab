#!/usr/bin/env python3
"""P3-CA generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_utility', 'PCC Utility', 'util'),
    ('pcc_audit', 'PCC Audit', 'audit'),
    ('pcc_admin', 'PCC Admin', 'admin'),
]
PKG = 'p3ca_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CA package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CA module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'pcc_utility':
        cap = ['Validate', 'Hash', 'Format', 'Audit', 'Tenant', 'Role', 'Date', 'Pagination', 'Error', 'Cache']
    elif mod == 'pcc_audit':
        cap = ['Log', 'Compliance', 'Retention', 'Hash', 'Search', 'Filter', 'Range', 'Export', 'Alert', 'Quota']
    else:
        cap = ['Facility', 'User', 'Module', 'Config', 'Branches', 'Resource', 'Backup', 'Restore', 'Migration', 'Health']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-CA {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'pcc_utility':
        engine_funcs = ['Validate', 'Hash', 'Format', 'Audit', 'Tenant', 'Role', 'Date', 'Pagination', 'Error', 'Cache']
    elif mod == 'pcc_audit':
        engine_funcs = ['Log', 'Compliance', 'Retention', 'Hash', 'Search', 'Filter', 'Range', 'Export', 'Alert', 'Quota']
    else:
        engine_funcs = ['Facility', 'User', 'Module', 'Config', 'Branches', 'Resource', 'Backup', 'Restore', 'Migration', 'Health']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-CA {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3ca_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3ca_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CA: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
