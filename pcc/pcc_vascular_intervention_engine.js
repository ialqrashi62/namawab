...
/**
 * CarotidStentPlacement — CAS eligibility + risk per CREST + ACST-2
 */
function CarotidStentPlacement(input) {
  ...
  const anatomyFavorable = Boolean(i.anatomyFavorable); // high bifurcation, severe tortuosity

  // Per ACC/AHA + SVS
  let indication, preferredApproach, perioperativeRisk;
  if (symptomatic && stenosis >= 50) {
    indication = 'indicated';
    preferredApproach = age >= 70 ? 'CEA-preferred' : 'CEA-or-CAS-acceptable';
  } else if (!symptomatic && stenosis >= 60) {
    indication = 'indicated';
    preferredApproach = 'CEA-preferred';
  } else if (symptomatic && stenosis < 50 || !symptomatic && stenosis < 60) {
    indication = 'medical-management';
    preferredApproach = 'optimal-medical-therapy';
  } else {
    indication = 'review-required';
    preferredApproach = 'review';
  }

  // CAS-favorable scenarios
  if (age < 70 && (priorNeckSurgery || priorNeckRadiation || contralateralOcclusion)) {
    preferredApproach = 'CAS-preferred';
  }

  // Perioperative risk
  if (recentMi || egfr < 30 || ef < 30) {
    perioperativeRisk = 'high';
  } else if (age >= 80) {
    perioperativeRisk = 'high';
  } else if (!anatomyFavorable) {
    perioperativeRisk = 'high';
  } else {
    perioperativeRisk = age >= 70 ? 'moderate' : 'low';
  }
...
/**
 * AAAEndovascularRepair — EVAR eligibility per ESVS 2024
 */
function AAAEndovascularRepair(input) {
  ...
  const neckAngulation = Number(i.neckAngulation || 0); // degrees
  const iliacAnatomy = String(i.iliacAnatomy || 'favorable');
  const age = Number(i.age || 70);
  const surgicalRisk = String(i.surgicalRisk || 'low'); // low/moderate/high
  ...
  if (rupture) {
    eligible = true;
    recommendation = 'emergent-EVAR-if-anatomy-permits-else-open';
  } else if (aneurysmSize < 55) {
    eligible = false;
    recommendation = 'surveillance-anatomy-not-yet-relevant';
  } else if (aneurysmSize >= 55 || symptomatic) {
    if (anatomySuitability === 'unfavorable') {
      eligible = false;
      recommendation = 'open-repair-or-fenestrated-branched-EVAR';
    } else if (anatomySuitability === 'borderline' && surgicalRisk === 'low') {
      eligible = false;
      recommendation = 'open-repair-preferred';
    } else if (age >= 80 || surgicalRisk === 'high' || egfr < 30) {
      eligible = true;
      recommendation = 'EVAR-recommended-lower-perioperative-risk';
    } else {
      eligible = true;
      recommendation = 'EVAR-vs-open-shared-decision-making';
    }
  } else {
    eligible = false;
  ...
  // Pre-op workup
  const workup = [
    'thin-cut-CTA-chest-abdomen-pelvis',
    'cardiac-clearance',
    'pulmonary-function-if-needed',
    'ankle-brachial-index',
    'type-and-screen',
    'informed-consent-with-endoleak-and-reintervention',
  ];
...
/**
 * PeripheralAngioplasty — femoropopliteal + tibial disease per SVS
 */
function PeripheralAngioplasty(input) {
  const i = input || {};
  const lesionLocation = String(i.lesionLocation || 'fem-pop');
  const lesionLength = Number(i.lesionLength || 0); // cm
  ...
  if (cli) {
    ...
    technique = lesionLength >= 25 ? 'subintimal-angioplasty-or-bypass' : 'POBA-or-DCB-or-stent';
    restenosisRisk = lesionLength >= 15 ? 'high' : 'moderate';
  } else if (claudication && abi < 0.9) {
    if (lesionLength <= 15) {
      recommendation = 'structured-exercise-first-then-consider-revascularization';
      technique = 'supervised-exercise-therapy-12-weeks';
      restenosisRisk = lesionLength >= 10 ? 'high' : 'moderate';
    } else {
      recommendation = 'structured-exercise-then-revascularization-if-fails';
      technique = 'exercise-first-then-DCB-or-DES';
      restenosisRisk = 'high';
    }
  } else {
    recommendation = 'medical-management';
    technique = 'risk-factor-control-and-exercise';
    restenosisRisk = 'low';
  }

  if (chronicTotalOcclusion) {
    technique = `${technique} - CTO-crossing-wire-or-reentry-device`;
  }
...
/**
 * DVTThrombolysis — catheter-directed thrombolysis per ATTRACT trial
 */
function DVTThrombolysis(input) {
  ...
  // Inclusion/exclusion per ATTRACT
  ...
  if (exclusions.length > 0) {
    eligible = false;
    recommendation = 'anticoagulation-alone';
    technique = 'LMWH-or-DOAC';
  } else if (phlegmasia || iliacVein) {
    eligible = true;
    recommendation = 'catheter-directed-thrombolysis-consider';
    technique = 'CDT-with-tPA-or-mechanical-thrombectomy';
  } else if (acuityDays <= 14 && dvtLocation === 'iliofemoral' && symptomSeverity === 'severe') {
    eligible = true;
    recommendation = 'CDT-consider-if-low-bleed-risk';
    technique = 'infusion-catheter-tPA';
  } else if (acuityDays <= 14 && symptomSeverity === 'severe') {
    eligible = false;
    recommendation = 'anticoagulation-with-close-monitoring';
    technique = 'LMWH-or-DOAC-then-reimage';
  } else {
    eligible = false;
    recommendation = 'anticoagulation-alone-per-standard';
    technique = 'LMWH-or-DOAC';
  }
...
/**
 * VaricoseVeinAblation — endovenous ablation per AVF/SVS
 */
function VaricoseVeinAblation(input) {
  const i = input || {};
  const veinInvolved = String(i.veinInvolved || 'great-saphenous');
  const veinDiameter = Number(i.veinDiameter || 0); // mm
  const refluxDuration = Number(i.refluxDuration || 0); // ms
  ...
  if (pregnancy) {
    recommendation = 'compression-only';
    technique = 'graduated-compression-30-40mmHg';
    followUp = 'postpartum-reassess';
  } else if (ceapClass === 'C1' || ceapClass === 'C0') {
    recommendation = 'no-treatment-needed';
    technique = 'observation';
    followUp = 'as-needed';
  } else if (veinDiameter >= 3 && refluxDuration >= 500) {
    recommendation = 'endovenous-ablation';
    technique = 'radiofrequency-or-laser-ablation';
    followUp = '1-week-ultrasound-6-weeks-clinic';
    ...
    recommendation = 'compression-or-sclerotherapy';
    technique = 'foam-sclerotherapy-or-compression';
    followUp = '6-weeks-clinic';
  } else {
    recommendation = 'shared-decision';
    technique = 'compression-with-monitoring';
    followUp = '3-months-clinic';
  }
...
/**
 * AVMEmbolization — arteriovenous malformation per SVS
 */
function AVMEmbolization(input) {
  const i = input || {};
  const location = String(i.location || 'peripheral');
  const schobinger = String(i.schobinger || 'I'); // I-IV
  ...
  if (schobinger === 'I' && symptoms === 'cosmetic') {
    recommendation = 'observation-or-compression';
    technique = 'serial-monitoring';
    stages = 0;
  } else if (schobinger === 'II' || schobinger === 'III') {
    recommendation = 'endovascular-embolization';
    technique = 'liquid-embolization-Onyx-or-glue';
    stages = feedingArteries >= 3 ? 3 : 2;
  } else if (schobinger === 'IV' || highFlow) {
    recommendation = 'urgent-multimodal-treatment';
    technique = 'combination-embolization-then-surgical-resection';
    stages = 4;
  } else {
    recommendation = 'individualized-treatment';
    technique = 'multidisciplinary-team-review';
    stages = 1;
  }
...
/**
 * RenalArteryStenting — RAS intervention per ASTRAL + CORAL trials
 */
function RenalArteryStenting(input) {
  const i = input || {};
  const stenosis = Number(i.stenosis || 0);  // %
  const bpUncontrolled = Boolean(i.bpUncontrolled);
  const bpOnMeds = Number(i.bpOnMeds || 0);  // number of antihypertensives
  const egfr = Number(i.egfr || 60);
  const egfrDecline = Number(i.egfrDecline || 0); // ml/min/yr
  const flashPulmonaryEdema = Boolean(i.flashPulmonaryEdema);
  const rapidHypertension = Boolean(i.rapidHypertension);
  const smallKidney = Boolean(i.smallKidney); // <7cm
  const resistiveIndex = Number(i.resistiveIndex || 0.7);

  // ASTRAL/CORAL: stenting does NOT benefit most patients
  let recommendation, indication;
  const flashIndication = flashPulmonaryEdema || rapidHypertension;
  if (flashIndication && stenosis >= 70) {
    indication = 'flash-pulmonary-edema-or-rapid-hypertension';
    recommendation = 'renal-artery-stenting-indicated';
  } else if (smallKidney || resistiveIndex >= 0.8) {
    indication = 'kidney-likely-non-salvageable';
    recommendation = 'medical-management-only';
  } else if (egfrDecline >= 5 && stenosis >= 70) {
    indication = 'progressive-renal-dysfunction';
    recommendation = 'renal-artery-stenting-consider';
  } else if (bpUncontrolled && bpOnMeds >= 3 && stenosis >= 70) {
    indication = 'resistant-hypertension';
    recommendation = 'renal-artery-stenting-consider-after-shared-decision';
  } else {
    indication = 'medical-management-preferred';
    recommendation = 'medical-management-with-close-followup';
  }
...
/**
 * MesentericIschemiaIntervention — acute vs chronic mesenteric ischemia
 */
function MesentericIschemiaIntervention(input) {
  const i = input || {};
  const acuity = String(i.acuity || 'acute');  // acute/chronic
  ...
  const symptomDuration = Number(i.symptionDuration || 0); // hours
  const wtLoss = Number(i.wtLoss || 0);  // kg
  const foodFear = Boolean(i.foodFear);

  let diagnosis, recommendation, technique;
  if (acuity === 'acute') {
    if (painOutOfProportion && lacticAcid >= 2 && ctFindings === 'consistent') {
      diagnosis = 'acute-mesenteric-ischemia';
      recommendation = 'emergent-endovascular-or-open-revascularization';
      technique = 'hybrid-endovascular-then-laparotomy-if-peritonitis';
    } else if (lacticAcid < 2 && symptomDuration < 6) {
      diagnosis = 'early-mesenteric-ischemia';
      recommendation = 'urgent-endovascular-revascularization';
      technique = 'aspiration-thrombectomy-or-thrombolysis';
    } else {
      diagnosis = 'mesenteric-ischemia-suspected';
      recommendation = 'CT-angiography-immediate';
      technique = 'diagnostic-imaging';
    }
  } else {  // chronic
    if (wtLoss >= 5 && foodFear && vesselInvolvement >= 1) {
      diagnosis = 'chronic-mesenteric-ischemia';
      recommendation = 'mesenteric-revascularization';
      technique = vesselInvolvement >= 3 ? 'multi-vessel-bypass' : 'endovascular-angioplasty-stent';
    } else {
      diagnosis = 'chronic-mesenteric-ischemia-suspected';
      recommendation = 'cta-with-mesenteric-protocol';
      technique = 'diagnostic-imaging-then-shared-decision';
    }
  }
...
/**
 * ClaudicationRevascularization — lifestyle vs intervention per CLEVER trial
 */
function ClaudicationRevascularization(input) {
  ...
  if (exerciseTrial) {
    recommendation = 'structured-exercise-3-months-then-revascularize-if-fails';
    technique = 'supervised-exercise-then-consider-endovascular';
  } else if (lifeLimiting || occupation === 'active') {
    if (tasc === 'A' || tasc === 'B') {
      recommendation = 'endovascular-first';
      technique = 'angioplasty-with-DCB-or-stent';
    } else if (tasc === 'C') {
      recommendation = 'endovascular-or-bypass-shared-decision';
      technique = lesionLocation === 'fem-pop' && tasc === 'C' ? 'endovascular-or-bypass' : 'endovascular';
    } else {
      recommendation = 'open-bypass-preferred';
      technique = 'surgical-bypass';
    }
  } else {
    recommendation = 'medical-management-with-exercise';
    technique = 'risk-factor-control-statins-supervised-exercise';
  }
...
/**
 * VascularTraumaControl — endovascular management of vascular trauma
 */
function VascularTraumaControl(input) {
  ...
  const hardSigns = Boolean(i.hardSigns);  // expanding hematoma, active bleeding, ischemia
  ...
  const timeSinceInjury = Number(i.timeSinceInjury || 0); // hours

  let recommendation, technique;
  if (!hemodynamicallyStable || hardSigns) {
    recommendation = 'emergent-vascular-control';
    technique = mskInjury ? 'REBOA-or-tourniquet-then-OR' : 'OR-for-vascular-control-and-repair';
  } else if (distalIschemia && timeSinceInjury < 6) {
    recommendation = 'urgent-revascularization';
    technique = 'endovascular-stent-graft-or-bypass';
  } else if (hemodynamicallyStable && !hardSigns && !distalIschemia) {
    recommendation = 'CT-angiography-then-vascular-consult';
    technique = 'non-operative-management-with-close-monitoring-or-endovascular';
  } else {
    recommendation = 'multidisciplinary-team';
    technique = 'vascular-and-trauma-surgery';
  }
