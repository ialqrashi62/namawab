#!/usr/bin/env python3
"""P3-DU generator v3.85.0 — pcc_neonatal_icu, pcc_pain_procedure_suite, pcc_ortho_sports_surgery"""
import os
...
MODULES = [
    ('pcc_neonatal_icu', 'PCC Neonatal ICU', 'pcc-neonatal-icu'),
    ('pcc_pain_procedure_suite', 'PCC Pain Procedure Suite', 'pcc-pain-procedure-suite'),
    ('pcc_ortho_sports_surgery', 'PCC Ortho Sports Surgery', 'pcc-ortho-sports-surgery'),
]

ENGINE_FUNCS = {
    'pcc_neonatal_icu': [
        'NICUAdmissionCriteria', 'ThermoregulationProtocol', 'NeonatalVentilation',
        'TPNNeonatal', 'NeonatalSepsisKaiser', 'BronchopulmonaryDysplasia',
        'IVHPremature', 'ROPExamSchedule', 'NeonatalSeizureWorkup',
        'CongenitalHeartDuctus',
    ],
    'pcc_pain_procedure_suite': [
        'ProceduralSedation', 'EpiduralBlock', 'FacetJointInjection',
        'RadiofrequencyAblation', 'SpinalCordStimulator', 'IntrathecalPump',
        'NerveBlockPeripheral', 'TriggerPointInjection', 'JointAspiration',
        'PainProcedureConsciousSedation',
    ],
    'pcc_ortho_sports_surgery': [
        'ACLRRepair', 'RotatorCuffRepair', 'MeniscusRepair',
        'HipArthroscopy', 'AchillesTendonRepair', 'ShoulderInstability',
        'TennisElbowRelease', 'HipReplacementIndication', 'KneeReplacementIndication',
        'SportInjuryReturnToPlay',
    ],
}
...
def write_routes(module, label, funcs):
    lines = [f"// {PHASE} {module}_routes v{VER}", "// P3-DU: authenticate via requireAuth middleware (sandbox: helmet/CSP enforced at app level)", "'use strict';", "const express = require('express');", f"const Engine = require('./{module}_engine.js');", "const VER = '%s';" % VER, "const router = express.Router();", ""]
