"""
P3-BO generator: allergy_ext, endocrine_ext, derm_ext2
Same in-memory shim pattern as P3-BN.
"""
import os
...
VER = "3.27.0"
MODULES = [
    ("allergy_ext", "Allergy-Ext", "Allergy-Immunology"),
    ("endocrine_ext", "Endocrine-Ext", "Endocrine"),
    ("derm_ext2", "Derm-Ext-2", "Derm-Advanced"),
]

AL_FNS = ["Anaphylaxis","FoodAllergy","DrugAllergy","Urticaria","Angioedema","AllergicRhinitis","Asthma","Atopic","Venom","PrimaryImmuno"]
EN_FNS = ["DiabetesT2","Hypothyroid","Hyperthyroid","AdrenalInsufficient","Cushings","Pheo","Calcium","Pituitary","AdrenalIncidental","GenderAffirming"]
DR_FNS = ["Acne","Psoriasis","Eczema","SkinCancer","DrugRash","Bullous","Autoimmune","Hair","PediatricDerm","Ulcer"]
FNS_MAP = {"allergy_ext": AL_FNS, "endocrine_ext": EN_FNS, "derm_ext2": DR_FNS}
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bo_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bo_up.sql"), "w", encoding="utf-8") as f:
