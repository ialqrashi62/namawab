// P3-BC integration: community_health (Community-Health)
...
console.log('community_health integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO community_health (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO community_health (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM community_health WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO community_health (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM community_health').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.CommunityRiskAssessment({}); assert.strictEqual(typeof r, 'object'); });
