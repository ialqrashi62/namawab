...
console.log('diving integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO diving (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); db.prepare('INSERT INTO diving (tenant_id, data) VALUES (?, ?)').run('t2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM diving WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO diving (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM diving').get(); assert.strictEqual(r.c, 1); });
