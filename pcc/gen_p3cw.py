#!/usr/bin/env python3
"""P3-CW generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_occupational_health', 'PCC Occupational Health', 'occ'),
    ('pcc_sleep_med', 'PCC Sleep Medicine', 'slp'),
    ('pcc_allergy_immunology', 'PCC Allergy Immunology', 'alg'),
]
PKG = 'p3cw_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CW package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CW module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_occupational_health': ['Fitness', 'Exposure', 'Vaccination', 'Injury', 'ReturnToWork', 'Hearing', 'Vision', 'Respiratory', 'Chemical', 'Ergonomics'],
    'pcc_sleep_med': ['Ahi', 'Insomnia', 'Cpap', 'Daytime', 'Apnea', 'Oxygen', 'Restless', 'Narcolepsy', 'Parasomnia', 'Hypopnea'],
    'pcc_allergy_immunology': ['Ige', 'SkinTest', 'Anaphylaxis', 'Desensitization', 'FoodAllergy', 'DrugAllergy', 'InsectAllergy', 'AsthmaAllergy', 'Immunodeficiency', 'Biologic'],
}
...
def make_engine(mod):
  ...
  return {{ plan, t }};
    ...
    return f"""// P3-CW {mod}_engine v{VER}
'use strict';
{chr(10).join(funcs)}
module.exports = {{
  {', '.join(cap)}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CW {mod} integration test v{VER}
const Engine = require('./{mod}_engine.js');
const assert = require('assert');
let passed = 0, failed = 0;
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CW {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cw_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cw_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CW: engines + tests + integ tests + routes + SQL (v{VER})')
