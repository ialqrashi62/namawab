"""
P3-BE generator: transplant_living, neonatal_ext, perinatal_ext
Same in-memory shim pattern as P3-AV..BD.
"""
import os
...
P3 = "P3-BE"
MODULES = [
    ("transplant_living", "Transplant-Living", "Transplant"),
    ("neonatal_ext", "Neonatal-Ext", "NICU"),
    ("perinatal_ext", "Perinatal-Ext", "Maternal-Fetal"),
]

TL_FNS = ["LivingDonorWorkup","DonorNephrectomy","PairedExchange","LivingDonorFollowup","ABOiTransplant","Desensitization","LDRecipient","LDOutcomes","LDRecipientDose","LDComplications"]
NN_FNS = ["APHARScore","TherapeuticHypothermia","NEC","RDS","BPD","ROP","NEOScore","SEPSISScreen","NeuroOutcomes","Dehydration"]
PN_FNS = ["PrenatalCare","HighRiskPregnancy","Preeclampsia","PrenatalScreen","FGR","LaborMgmt","IntrapartumMonitor","PostpartumHemorrhage","PerinatalMental","PerinatalOutcome"]
FNS_MAP = dict(zip([m[0] for m in MODULES], [TL_FNS, NN_FNS, PN_FNS]))
...
def routes(mod, label, fns_list_str):
    return (
        ...
        f"  res.json({{ module: '{mod}', label: '{label}', functions: fns.length, version: '3.17.0' }});\n"
        f"}});\n\n"
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3be_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3be_up.sql"), "w", encoding="utf-8") as f:
