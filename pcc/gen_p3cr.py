#!/usr/bin/env python3
"""P3-CR generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_surgical_checklist', 'PCC Surgical Checklist', 'scl'),
    ('pcc_handoff', 'PCC Handoff', 'hnd'),
    ('pcc_safety', 'PCC Safety', 'sfy'),
]
PKG = 'p3cr_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CR package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CR module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_surgical_checklist': ['SignIn', 'TimeOut', 'SignOut', 'SiteMark', 'AllergyCheck', 'AntibioConfirm', 'ImplantConfirm', 'CountsFinal', 'SpecimenConfirm', 'Recovery'],
    'pcc_handoff': ['Ipass', 'Sbar', 'Shift', 'Discharge', 'Icu', 'Or', 'Er', 'Anesthesia', 'Primary', 'Receiving'],
    'pcc_safety': ['Fall', 'Restraint', 'Suicide', 'Elopement', 'Mislabel', 'WrongPt', 'Fire', 'Radiation', 'Sharps', 'Hazard'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CR {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CR {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cr_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cr_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CR: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
