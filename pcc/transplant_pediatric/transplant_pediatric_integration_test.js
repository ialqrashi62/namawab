// P3-BF integration: transplant_pediatric (Transplant-Pediatric)
...
console.log('transplant_pediatric integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO transplant_pediatric (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO transplant_pediatric (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM transplant_pediatric WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO transplant_pediatric (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM transplant_pediatric').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.PediatricEval({}); assert.strictEqual(typeof r, 'object'); });
