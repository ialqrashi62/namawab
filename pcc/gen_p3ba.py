"""
P3-BA generator: wound_ostomy, chronic_pain_rehab, telerehab
Same in-memory shim pattern as P3-AV/AW/AX/AY/AZ.
"""
import os
...
P3 = "P3-BA"
MODULES = [
    ("wound_ostomy", "Wound-Ostomy", "WOCN"),
    ("chronic_pain_rehab", "Chronic-Pain-Rehab", "Pain-Rehab"),
    ("telerehab", "Telerehab", "Digital-Health"),
]

WO_FNS = ["WoundAssessment","PressureInjuryStage","WoundDressing","NPWT","OstomySite","StomaComplication","WoundInfection","Continence","WoundOutcome","WoundDosing"]
CP_FNS = ["PainBiopsychosocial","OpioidStewardship","PainMedication","FunctionalRestoration","PainEducation","PainInterventional","PainPedi","PainAndSleep","PainDosing","PainOutcome"]
TR_FNS = ["TelerehabEligibility","TelerehabModality","TelerehabSafety","TelerehabExercise","TelerehabEval","TelerehabAdherence","TelerehabBilling","TelerehabTechSupport","TelerehabProgress","TelerehabDosing"]
FNS_MAP = dict(zip([m[0] for m in MODULES], [WO_FNS, CP_FNS, TR_FNS]))
...
def routes(mod, label, fns_list_str):
    return (
        ...
        f"  res.json({{ module: '{mod}', label: '{label}', functions: fns.length, version: '3.13.0' }});\n"
        f"}});\n\n"
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3ba_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3ba_up.sql"), "w", encoding="utf-8") as f:
