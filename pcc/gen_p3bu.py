#!/usr/bin/env python3
"""P3-BU generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('neonatal_ext3', 'Neonatal Extended 3', 'neo3'),
    ('perinatal_ext3', 'Perinatal Extended 3', 'per3'),
    ('hem_ext2', 'Hematology Extended 2', 'hem2'),
]
PKG = 'p3bu_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-BU package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-BU module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'neonatal_ext3':
        cap = ['Apnea', 'Jaundice', 'SepsisScreen', 'NEC', 'BPD', 'IVH', 'ROP', 'Cooling', 'Feed', 'Discharge']
    elif mod == 'perinatal_ext3':
        cap = ['Anomaly', 'Triploidy', 'Twins', 'Previa', 'Accreta', 'Preterm', 'ROM', 'Induction', 'Postdates', 'Postpartum']
    else:
        cap = ['Anemia', 'Thrombocyt', 'Coag', 'DVT', 'Anticoag', 'Bleed', 'TTP', 'DIC', 'Sickle', 'Lymphoma']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-BU {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'neonatal_ext3':
        engine_funcs = ['Apnea', 'Jaundice', 'SepsisScreen', 'NEC', 'BPD', 'IVH', 'ROP', 'Cooling', 'Feed', 'Discharge']
    elif mod == 'perinatal_ext3':
        engine_funcs = ['Anomaly', 'Triploidy', 'Twins', 'Previa', 'Accreta', 'Preterm', 'ROM', 'Induction', 'Postdates', 'Postpartum']
    else:
        engine_funcs = ['Anemia', 'Thrombocyt', 'Coag', 'DVT', 'Anticoag', 'Bleed', 'TTP', 'DIC', 'Sickle', 'Lymphoma']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-BU {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3bu_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3bu_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-BU: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
