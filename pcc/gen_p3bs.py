#!/usr/bin/env python3
"""P3-BS generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('geri_ext', 'Geriatrics Extended', 'geri'),
    ('gen_med_ext', 'General Medicine Extended', 'gmed'),
    ('trauma_ext', 'Trauma Extended', 'trauma'),
]
PKG = 'p3bs_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-BS package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-BS module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'geri_ext':
        cap = ['Frailty', 'Polypharm', 'Delirium', 'Falls', 'Dementia', 'Nutrition', 'PressureUlcer', 'Depression', 'Advance', 'Sarcopenia']
    elif mod == 'gen_med_ext':
        cap = ['Triage', 'Sepsis', 'ChestPain', 'ShortBreath', 'AbdPain', 'Fever', 'Syncope', 'BackPain', 'Headache', 'Dizzy']
    else:
        cap = ['Triage', 'Primary', 'Secondary', 'FAST', 'Head', 'Chest', 'Abdomen', 'Pelvis', 'Spine', 'MTP']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-BS {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'geri_ext':
        engine_funcs = ['Frailty', 'Polypharm', 'Delirium', 'Falls', 'Dementia', 'Nutrition', 'PressureUlcer', 'Depression', 'Advance', 'Sarcopenia']
    elif mod == 'gen_med_ext':
        engine_funcs = ['Triage', 'Sepsis', 'ChestPain', 'ShortBreath', 'AbdPain', 'Fever', 'Syncope', 'BackPain', 'Headache', 'Dizzy']
    else:
        engine_funcs = ['Triage', 'Primary', 'Secondary', 'FAST', 'Head', 'Chest', 'Abdomen', 'Pelvis', 'Spine', 'MTP']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-BS {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3bs_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3bs_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-BS: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
