#!/usr/bin/env python3
"""P3-BX generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('endo_ext2', 'Endocrinology Extended 2', 'endo2'),
    ('ortho_ext2', 'Orthopedics Extended 2', 'ortho2'),
    ('cardio_ext3', 'Cardiology Extended 3', 'cardio3'),
]
PKG = 'p3bx_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-BX package schema v{VER}\n',
             'CREATE SCHEMA IF NOT EXISTS p3bx;']
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-BX module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'endo_ext2':
        cap = ['Diabetes', 'Thyroid', 'Adrenal', 'Pituitary', 'Calcium', 'Bone', 'AdrenalMass', 'Obesity', 'Lipid', 'ReproEndo']
    elif mod == 'ortho_ext2':
        cap = ['Fracture', 'Joint', 'Spine', 'Sports', 'Trauma', 'Tumor', 'Hand', 'Foot', 'Pediatric', 'Recon']
    else:
        cap = ['ACS', 'HF', 'AF', 'Valve', 'HTN', 'Lipid', 'Anticoag', 'EP', 'Pericardial', 'PAD']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-BX {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'endo_ext2':
        engine_funcs = ['Diabetes', 'Thyroid', 'Adrenal', 'Pituitary', 'Calcium', 'Bone', 'AdrenalMass', 'Obesity', 'Lipid', 'ReproEndo']
    elif mod == 'ortho_ext2':
        engine_funcs = ['Fracture', 'Joint', 'Spine', 'Sports', 'Trauma', 'Tumor', 'Hand', 'Foot', 'Pediatric', 'Recon']
    else:
        engine_funcs = ['ACS', 'HF', 'AF', 'Valve', 'HTN', 'Lipid', 'Anticoag', 'EP', 'Pericardial', 'PAD']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-BX {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3bx_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3bx_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-BX: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
