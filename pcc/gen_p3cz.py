#!/usr/bin/env python3
"""P3-CZ generator: 3 modules = engines + tests + integration tests + routes + SQL."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_rehab_medicine', 'PCC Rehabilitation Medicine', 'rhm'),
    ('pcc_pain_rehab', 'PCC Pain Rehabilitation', 'prh'),
    ('pcc_geriatric_surgery', 'PCC Geriatric Surgery', 'gcs'),
]
PKG = 'p3cz_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CZ package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CZ module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_rehab_medicine': ['FunctionalStatus', 'Impairment', 'GoalSetting', 'TherapyPlan', 'OutcomeMeasure', 'DischargePlan', 'Equipment', 'Caregiver', 'CommunityReintegration', 'QualityOfLife'],
    'pcc_pain_rehab': ['PainAdmission', 'Multidisciplinary', 'PhysicalTherapy', 'OccupationalTherapy', 'Psychology', 'Interventional', 'MedicationTaper', 'FunctionalRestoration', 'Discharge', 'RelapsePrevention'],
    'pcc_geriatric_surgery': ['FrailtyIndex', 'Prehabilitation', 'DeliriumRisk', 'NutritionScreen', 'Polypharmacy', 'MobilityPlan', 'DischargeDestination', 'ComplicationRisk', 'PalliativeTalk', 'FollowUp'],
}
...
def make_engine(mod):
  ...
  return {{ plan, t }};
    ...
    return f"""// P3-CZ {mod}_engine v{VER}
'use strict';
{chr(10).join(funcs)}
module.exports = {{
  {', '.join(cap)}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CZ {mod} integration test v{VER}
const Engine = require('./{mod}_engine.js');
const assert = require('assert');
let passed = 0, failed = 0;
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CZ {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cz_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cz_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CZ: engines + tests + integ tests + routes + SQL (v{VER})')
