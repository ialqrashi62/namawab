#!/usr/bin/env python3
"""P3-CO generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_ent_ext3', 'PCC ENT Ext3', 'entx3'),
    ('pcc_uro_ext2', 'PCC Uro Ext2', 'urox2'),
    ('pcc_ophth_ext2', 'PCC Ophth Ext2', 'ophx2'),
]
PKG = 'p3co_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CO package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CO module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_ent_ext3': ['Hearing', 'Ottis', 'Sinusitis', 'Tonsil', 'Hoarseness', 'Epistaxis', 'Vertigo', 'Tinnitus', 'Allergic', 'Vertigo2'],
    'pcc_uro_ext2': ['Bph', 'Pca', 'Renal', 'Stone', 'Bladder', 'Incontinence', 'Erectile', 'Urethritis', 'Prostatitis', 'Hematuria'],
    'pcc_ophth_ext2': ['Visual', 'Cataract', 'Glaucoma', 'Retina', 'Uveitis', 'Conjunctivitis', 'Keratitis', 'Macular', 'Strab', 'Trauma'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CO {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CO {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3co_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3co_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CO: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
