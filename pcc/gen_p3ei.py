#!/usr/bin/env python3
"""P3-EI generator v3.99.0 — pcc_neuropsych_ext2, pcc_pediatric_oncology, pcc_pediatric_neurosurg"""
import os
...
MODULES = [
    ('pcc_neuropsych_ext2', 'PCC Neuropsych Ext2', 'pcc-neuropsych-ext2'),
    ('pcc_pediatric_oncology', 'PCC Pediatric Oncology', 'pcc-pediatric-oncology'),
    ('pcc_pediatric_neurosurg', 'PCC Pediatric Neurosurg', 'pcc-pediatric-neurosurg'),
]

ENGINE_FUNCS = {
    'pcc_neuropsych_ext2': [
        'NeurocognitiveDisorderMajor', 'FrontotemporalDementia', 'LewyBodyDementia',
        'VascularDementia', 'MildCognitiveImpairment', 'WernickeKorsakoff',
        'TraumaticBrainInjuryCognitive', 'PostConcussionSyndrome',
        'ChemotherapyRelatedCognitive', 'AutoimmuneEncephalitisCognitive',
    ],
    'pcc_pediatric_oncology': [
        'PediatricLeukemiaALL', 'PediatricLeukemiaAML', 'PediatricBrainTumor',
        'NeuroblastomaManagement', 'WilmsTumorProtocol', 'PediatricLymphoma',
        'PediatricBoneTumor', 'PediatricRetinoblastoma', 'PediatricHepaticTumor',
        'PediatricOncologicEmergency',
    ],
    'pcc_pediatric_neurosurg': [
        'PediatricHydrocephalus', 'ChiariMalformation', 'Craniosynostosis',
        'SpinalDysraphism', 'PediatricBrainTumorSurg', 'PediatricEpilepsySurg',
        'PediatricTBI', 'PediatricSpineTrauma', 'PediatricVascularNeurosurg',
...
def write_routes(module, label, funcs):
    lines = [f"// {PHASE} {module}_routes v{VER}", "// P3-EI: authenticate via requireAuth middleware (sandbox: helmet/CSP enforced at app level)", "'use strict';", "const express = require('express');", f"const Engine = require('./{module}_engine.js');", "const VER = '%s';" % VER, "const router = express.Router();", ""]
