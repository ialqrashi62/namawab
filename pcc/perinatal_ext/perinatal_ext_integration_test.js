// P3-BE integration: perinatal_ext (Perinatal-Ext)
...
console.log('perinatal_ext integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO perinatal_ext (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO perinatal_ext (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM perinatal_ext WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO perinatal_ext (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM perinatal_ext').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.PrenatalCare({}); assert.strictEqual(typeof r, 'object'); });
