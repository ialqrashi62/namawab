// P3-AS integration: pharmacy_compounding
...
console.log('pharmacy_compounding integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO pharmacy_compounding (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); db.prepare('INSERT INTO pharmacy_compounding (tenant_id, data) VALUES (?, ?)').run('t2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM pharmacy_compounding WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO pharmacy_compounding (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM pharmacy_compounding').get(); assert.strictEqual(r.c, 1); });
