#!/usr/bin/env python3
"""P3-BT generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('breast_ext', 'Breast Extended', 'breast'),
    ('icu_ext2', 'ICU Extended 2', 'icu2'),
    ('obgyn_ext2', 'OB/GYN Extended 2', 'obgyn2'),
]
PKG = 'p3bt_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-BT package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-BT module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'breast_ext':
        cap = ['Screen', 'Mass', 'Nipple', 'Cancer', 'BRCA', 'Mastectomy', 'Reconstruction', 'Lactation', 'Gynecomastia', 'Survivorship']
    elif mod == 'icu_ext2':
        cap = ['Ventilator', 'Sedation', 'Shock', 'DVT', 'Glucose', 'Electrolyte', 'Transfusion', 'CRRT', 'ICP', 'Nutrition']
    else:
        cap = ['Pregnancy', 'PreEclampsia', 'GDM', 'PPROM', 'PPH', 'Ectopic', 'Induction', 'GynCancer', 'Infertility', 'Menopause']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-BT {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'breast_ext':
        engine_funcs = ['Screen', 'Mass', 'Nipple', 'Cancer', 'BRCA', 'Mastectomy', 'Reconstruction', 'Lactation', 'Gynecomastia', 'Survivorship']
    elif mod == 'icu_ext2':
        engine_funcs = ['Ventilator', 'Sedation', 'Shock', 'DVT', 'Glucose', 'Electrolyte', 'Transfusion', 'CRRT', 'ICP', 'Nutrition']
    else:
        engine_funcs = ['Pregnancy', 'PreEclampsia', 'GDM', 'PPROM', 'PPH', 'Ectopic', 'Induction', 'GynCancer', 'Infertility', 'Menopause']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-BT {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3bt_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3bt_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-BT: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
