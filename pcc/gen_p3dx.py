#!/usr/bin/env python3
"""P3-DX generator v3.88.0 — pcc_minimally_invasive_surgery, pcc_sleep_clinic, pcc_vascular_intervention"""
import os
...
MODULES = [
    ('pcc_minimally_invasive_surgery', 'PCC Minimally Invasive Surgery', 'pcc-minimally-invasive-surgery'),
    ('pcc_sleep_clinic', 'PCC Sleep Clinic', 'pcc-sleep-clinic'),
    ('pcc_vascular_intervention', 'PCC Vascular Intervention', 'pcc-vascular-intervention'),
]

ENGINE_FUNCS = {
    'pcc_minimally_invasive_surgery': [
        'LaparoscopicCholecystectomy', 'RoboticProstatectomyIndication', 'LaparoscopicHerniaRepair',
        'ThoracoscopicLobectomy', 'EndoscopicSinusSurgery', 'LaparoscopicColonResection',
        'RoboticHysterectomy', 'NOTESProcedureSelection', 'LaparoscopicNephrectomy',
        'MISPatientSelectionCriteria',
    ],
    'pcc_sleep_clinic': [
        'PolysomnographyInterpretation', 'OSAHSeverityStratification', 'CPAPTitrationProtocol',
        'BiPAPIndication', 'InsomniaCBTProtocol', 'RestlessLegSyndrome',
        'NarcolepsyDiagnosis', 'CircadianRhythmDisorder', 'ParasomniaEvaluation',
        'SleepHygieneEducation',
    ],
    'pcc_vascular_intervention': [
        'CarotidStentPlacement', 'AAAEndovascularRepair', 'PeripheralAngioplasty',
        'DVTThrombolysis', 'VaricoseVeinAblation', 'AVMEmbolization',
        'RenalArteryStenting', 'MesentericIschemiaIntervention', 'ClaudicationRevascularization',
...
def write_routes(module, label, funcs):
    lines = [f"// {PHASE} {module}_routes v{VER}", "// P3-DX: authenticate via requireAuth middleware (sandbox: helmet/CSP enforced at app level)", "'use strict';", "const express = require('express');", f"const Engine = require('./{module}_engine.js');", "const VER = '%s';" % VER, "const router = express.Router();", ""]
