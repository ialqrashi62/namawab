"""
P3-AZ generator: low_vision, voice_therapy, prosthetics_orthotics
Same in-memory shim pattern as P3-AV/AW/AX/AY.
"""
import os
...
P3 = "P3-AZ"
MODULES = [
    ("low_vision", "Low-Vision", "Sensory-Rehab"),
    ("voice_therapy", "Voice-Therapy", "Speech-Swallowing"),
    ("prosthetics_orthotics", "Prosthetics-Orthotics", "Devices"),
]

LV_FNS = ["LowVisionAssessment","MagnificationRx","VisualField","ContrastSensitivity","AssistiveTech","ADL","MobilityOM","LowVisionDriving","PedLowVision","LowVisionOutcome"]
VT_FNS = ["VoiceEval","VocalHygiene","VoiceDisorder","SLPResonantVoice","PediatricVoice","VoiceForSinger","TransgenderVoice","VoiceLaryngectomy","VoiceDosing","VoiceOutcome"]
PO_FNS = ["ProstheticPrescription","SocketFit","OrthoticPrescription","OrthoticScoliosis","ProstheticGait","ProstheticTraining","OrthoticComplications","ActivityKLevel","PediatricProsthetic","DeviceFollowup"]
FNS_MAP = dict(zip([m[0] for m in MODULES], [LV_FNS, VT_FNS, PO_FNS]))
...
def routes(mod, label, fns_list_str):
    return (
        ...
        f"  res.json({{ module: '{mod}', label: '{label}', functions: fns.length, version: '3.12.0' }});\n"
        f"}});\n\n"
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3az_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3az_up.sql"), "w", encoding="utf-8") as f:
