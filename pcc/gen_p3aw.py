"""
P3-AW generator: vestibular_rehab, lymphedema, driving_rehab
Same in-memory shim pattern as P3-AV.
"""
import os
...
P3 = "P3-AW"
MODULES = [
    ("vestibular_rehab", "Vestibular-Rehab", "Neuro-Rehab"),
    ("lymphedema", "Lymphedema", "Rehab"),
    ("driving_rehab", "Driving-Rehab", "Driver-Rehab"),
]

VESTIBULAR_FNS = ["DixHallpike","HeadImpulse","RombergTest","DynamicVisualAcuity","GazeStability","VestibularMigraine","MeniereAttack","BalanceAssessment","VORAdaptation","PPPD"]
LYMPH_FNS = ["LymphedemaStage","LimbVolume","LymphedemaExcess","CompressionClass","MLDTechnique","ExerciseLymphedema","SkinCare","LymphedemaRisk","PneumaticCompression","PediatricLymphedema"]
DRIVING_FNS = ["FitnessToDrive","VisionDrive","CognitiveDrive","MotorDrive","SeizureDrive","AdaptiveEquipment","OnRoadAssessment","DriverRehabPlan","SeniorDriving","DVMSubmission"]
FNS_MAP = dict(zip([m[0] for m in MODULES], [VESTIBULAR_FNS, LYMPH_FNS, DRIVING_FNS]))
...
def routes(mod, label, fns_list_str):
    return (
        ...
        f"  res.json({{ module: '{mod}', label: '{label}', functions: fns.length, version: '3.9.0' }});\n"
        f"}});\n\n"
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3aw_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3aw_up.sql"), "w", encoding="utf-8") as f:
