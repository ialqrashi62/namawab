#!/usr/bin/env python3
"""P3-CT generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_ambulatory', 'PCC Ambulatory', 'amb'),
    ('pcc_specialty_clinic', 'PCC Specialty Clinic', 'spc'),
    ('pcc_urgent_care', 'PCC Urgent Care', 'uc'),
]
PKG = 'p3ct_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CT package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CT module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_ambulatory': ['VisitType', 'Refill', 'Wellness', 'ChronicCare', 'Preventive', 'Immunization', 'HgbA1c', 'BpCheck', 'Smoking', 'DrVisit'],
    'pcc_specialty_clinic': ['Referral', 'Consult', 'SecondOpinion', 'FollowUp', 'Procedure', 'Triage', 'NextStep', 'Interval', 'Coord', 'Transition'],
    'pcc_urgent_care': ['WalkIn', 'InjuryType', 'Illness', 'Stitches', 'Splint', 'Neb', 'EkgUrgent', 'XrayOnsite', 'LabRapid', 'DcUrgent'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CT {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CT {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3ct_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3ct_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CT: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
