// P3-BB integration: geriatric_assessment (Geriatric-Assessment)
...
console.log('geriatric_assessment integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO geriatric_assessment (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO geriatric_assessment (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM geriatric_assessment WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO geriatric_assessment (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM geriatric_assessment').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.ComprehensiveGeriatric({}); assert.strictEqual(typeof r, 'object'); });
