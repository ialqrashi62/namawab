#!/usr/bin/env python3
"""P3-CP generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_rehab_ext3', 'PCC Rehab Ext3', 'rehx3'),
    ('pcc_pall_ext3', 'PCC Pall Ext3', 'palx3'),
    ('pcc_home_health', 'PCC Home Health', 'hha'),
]
PKG = 'p3cp_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CP package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CP module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_rehab_ext3': ['PhysTherapy', 'OccTherapy', 'SpeechLang', 'PostStroke', 'Sci', 'Tbi', 'Amp', 'Burnr', 'PreOp', 'Back'],
    'pcc_pall_ext3': ['PainMng', 'Dyspnea', 'Nausea', 'Constipation', 'Delirium', 'Anxietyp', 'Hospice', 'Advance', 'Family', 'Grief'],
    'pcc_home_health': ['Intake', 'Wound', 'IvTherapy', 'Therapy', 'MedAdmin', 'Tele', 'Falls', 'Caregiver', 'Discharge', 'AdmitHome'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CP {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CP {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cp_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cp_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CP: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
