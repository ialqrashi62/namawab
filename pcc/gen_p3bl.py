"""
P3-BL generator: occupational_ext, rehab_ext2, ent_ext
Same in-memory shim pattern as P3-BK.
"""
import os
...
VER = "3.24.0"
MODULES = [
    ("occupational_ext", "Occupational-Ext", "Occupational-Health"),
    ("rehab_ext2", "Rehab-Ext-2", "Rehab-Advanced"),
    ("ent_ext", "ENT-Ext", "ENT"),
]

OC_FNS = ["WorkInjury","FunctionalCapacity","ReturnToWork","Ergonomic","CumulativeTrauma","HearingLoss","VisionScreen","RespiratorFit","DrugTest","DisabilityRating"]
RH_FNS = ["StrokeRehab","TBIRehab","SPInjury","Amputee","CardiacRehab","PulmonaryRehab","BurnRehab","JointRepl","PainRehab","ProstheticUse"]
EN_FNS = ["HearingLoss","Vertigo","Tinnitus","Sinusitis","OSA","Hoarseness","Epistaxis","Dysphagia","Thyroid","Otitis"]
FNS_MAP = {"occupational_ext": OC_FNS, "rehab_ext2": RH_FNS, "ent_ext": EN_FNS}
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bl_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bl_up.sql"), "w", encoding="utf-8") as f:
