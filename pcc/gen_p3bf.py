"""
P3-BF generator: fertility, transplant_pediatric, womens_health_ext
Same in-memory shim pattern as P3-BE.
"""
import os
...
VER = "3.18.0"
MODULES = [
    ("fertility", "Fertility", "Reproductive-Endo"),
    ("transplant_pediatric", "Transplant-Pediatric", "Pedi-Surgery"),
    ("womens_health_ext", "Womens-Health-Ext", "OB-GYN"),
]

FT_FNS = ["FertilityEval","IVFProtocol","OHSS","Endometriosis","PCOS","Malefactor","Miscarriage","PGT","FertilityPreservation","FertilityOutcome"]
TP_FNS = ["PediatricEval","PediatricLD","PediatricImmuno","PediatricGrowth","PediatricAdherence","PediatricSchool","PediatricVaccines","PediatricPTLD","PediatricTransition","PediatricOutcome"]
WH_FNS = ["WellWoman","Menopause","PCOSWH","EndometriosisWH","UTI","STI","CervicalScreen","Urogyn","PelvicFloorWH","IVFandGyn"]
FNS_MAP = {"fertility": FT_FNS, "transplant_pediatric": TP_FNS, "womens_health_ext": WH_FNS}
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bf_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bf_up.sql"), "w", encoding="utf-8") as f:
