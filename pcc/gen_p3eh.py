#!/usr/bin/env python3
"""P3-EH generator v3.98.0 — pcc_voice_swallowing, pcc_pediatric_rheum, pcc_pediatric_gi_ext"""
import os
...
MODULES = [
    ('pcc_voice_swallowing', 'PCC Voice Swallowing', 'pcc-voice-swallowing'),
    ('pcc_pediatric_rheum', 'PCC Pediatric Rheum', 'pcc-pediatric-rheum'),
    ('pcc_pediatric_gi_ext', 'PCC Pediatric GI Ext', 'pcc-pediatric-gi-ext'),
]

ENGINE_FUNCS = {
    'pcc_voice_swallowing': [
        'VocalCordNoduleEvaluation', 'LaryngopharyngealReflux', 'MuscleTensionDysphonia',
        'SpasmodicDysphonia', 'VocalCordParalysis', 'SubglotticStenosis',
        'TracheoesophagealFistula', 'ZenkerDiverticulum', 'DysphagiaSwallowEval',
        'VocalCordPolyps', 'VoiceTherapyProtocol',
    ],
    'pcc_pediatric_rheum': [
        'JuvenileIdiopathicArthritis', 'KawasakiDisease', 'HenochSchonleinPurpura',
        'PediatricSLE', 'JuvenileDermatomyositis', 'PediatricVasculitis',
        'PeriodicFeverSyndromes', 'PediatricScleroderma', 'PediatricBehcet',
        'GrowingPainsEvaluation',
    ],
    'pcc_pediatric_gi_ext': [
        'PediatricGERDEvaluation', 'CeliacDisease', 'PediatricIBD',
        'HirschsprungDisease', 'PyloricStenosis', 'Intussusception',
        'PediatricHepatology', 'PediatricPancreatitis', 'NeonatalCholestasis',
        'PediatricLiverTransplant',
    ],
}
...
def write_routes(module, label, funcs):
    lines = [f"// {PHASE} {module}_routes v{VER}", "// P3-EH: authenticate via requireAuth middleware (sandbox: helmet/CSP enforced at app level)", "'use strict';", "const express = require('express');", f"const Engine = require('./{module}_engine.js');", "const VER = '%s';" % VER, "const router = express.Router();", ""]
