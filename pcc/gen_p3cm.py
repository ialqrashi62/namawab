#!/usr/bin/env python3
"""P3-CM generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_gi_ext3', 'PCC GI Ext3', 'gix3'),
    ('pcc_endo_ext3', 'PCC Endo Ext3', 'endx3'),
    ('pcc_rheum_ext4', 'PCC Rheum Ext4', 'rhex4'),
]
PKG = 'p3cm_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CM package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CM module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_gi_ext3': ['Dysphagia', 'GERD', 'IBS', 'IBD', 'Celiac', 'HepB', 'HepC', 'Cirr', 'Ppi', 'Scope'],
    'pcc_endo_ext3': ['DmType', 'A1c', 'Thyroid', 'Calcium', 'Adrenal', 'Pituitary', 'Osteo', 'Pcos', 'Dka', 'Lipid'],
    'pcc_rheum_ext4': ['Ra', 'Sle', 'Spa', 'Vasculitis', 'Gout', 'Osteo', 'Sjogren', 'Scleroderma', 'Myositis', 'Biologic'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CM {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CM {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cm_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cm_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CM: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
