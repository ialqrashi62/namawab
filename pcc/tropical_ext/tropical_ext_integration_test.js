// P3-AU integration: tropical_ext
...
console.log('tropical_ext integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO tropical_ext (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); db.prepare('INSERT INTO tropical_ext (tenant_id, data) VALUES (?, ?)').run('t2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM tropical_ext WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO tropical_ext (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM tropical_ext').get(); assert.strictEqual(r.c, 1); });
