// P3-AZ integration: prosthetics_orthotics (Prosthetics-Orthotics)
...
console.log('prosthetics_orthotics integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO prosthetics_orthotics (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO prosthetics_orthotics (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM prosthetics_orthotics WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO prosthetics_orthotics (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM prosthetics_orthotics').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.ProstheticPrescription({}); assert.strictEqual(typeof r, 'object'); });
