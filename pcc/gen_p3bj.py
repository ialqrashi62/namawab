"""
P3-BJ generator: perinatal_ext2, pharmacy_ext, dental_ext
Same in-memory shim pattern as P3-BI.
"""
import os
...
VER = "3.22.0"
MODULES = [
    ("perinatal_ext2", "Perinatal-Ext-2", "Maternal-Fetal-Adv"),
    ("pharmacy_ext", "Pharmacy-Ext", "Pharmacy-Adv"),
    ("dental_ext", "Dental-Ext", "Dental"),
]

PN_FNS = ["PreeclampsiaSevere","FGR","GDM","PretermLabor","PPROM","MultipleGestation","AnemiaPregnancy","PostpartumHemorrhage","CervicalInsufficiency","RHisoimmunization"]
PH_FNS = ["RenalDosing","HepaticDosing","AnticoagReversal","AKIvancomycin","Aminoglycoside","PharmacokineticConsult","IVtoPO","TherapeuticSubstitution","Polypharmacy","AllergyReconcile"]
DN_FNS = ["CariesRisk","Periodontitis","Endocarditis","OralCancerScreen","TMJ","Trauma","Ortho","Pediatric","MedComplex","DentalAbscess"]
FNS_MAP = {"perinatal_ext2": PN_FNS, "pharmacy_ext": PH_FNS, "dental_ext": DN_FNS}
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bj_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bj_up.sql"), "w", encoding="utf-8") as f:
