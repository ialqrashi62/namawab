// P3-AQ integration: ecmo_service
...
console.log('ecmo_service integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO ecmo_service (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); db.prepare('INSERT INTO ecmo_service (tenant_id, data) VALUES (?, ?)').run('t2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM ecmo_service WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO ecmo_service (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM ecmo_service').get(); assert.strictEqual(r.c, 1); });
