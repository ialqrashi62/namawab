#!/usr/bin/env python3
"""P3-CI generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_lab_ext2', 'PCC Lab Ext2', 'lab2'),
    ('pcc_path_ext', 'PCC Path Ext', 'pax'),
    ('pcc_rad_ext2', 'PCC Rad Ext2', 'rad2'),
]
PKG = 'p3ci_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CI package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CI module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_lab_ext2': ['Comprehensive', 'Toxicology', 'Molecular', 'Banked', 'Convenience', 'Reference', 'PointOfCare', 'Quality', 'Turnaround', 'Critical'],
    'pcc_path_ext': ['SpecimenType', 'Grossing', 'Embedding', 'Stain', 'Diagnosis', 'Margin', 'Stage', 'Grade', 'Tnm', 'Molecular'],
    'pcc_rad_ext2': ['Modality', 'BodyPart', 'Indication', 'Contrast', 'Urgency', 'Comparison', 'Dose', 'Pregnancy', 'Pediatric', 'Report'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CI {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CI {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3ci_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3ci_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CI: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
