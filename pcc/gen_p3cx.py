#!/usr/bin/env python3
"""P3-CX generator: 3 modules = engines + tests + integration tests + routes + SQL."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_weight_mgmt', 'PCC Weight Management', 'wmg'),
    ('pcc_smoking_cessation', 'PCC Smoking Cessation', 'smk'),
    ('pcc_addiction_med', 'PCC Addiction Medicine', 'add'),
]
PKG = 'p3cx_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CX package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CX module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_weight_mgmt': ['Bmi', 'ObesityClass', 'BariatricReferral', 'DietPlan', 'Exercise', 'Comorbidity', 'Medication', 'FollowUp', 'Goal', 'SurgeryRisk'],
    'pcc_smoking_cessation': ['Readiness', 'PackYears', 'Fagerstrom', 'QuitPlan', 'Nrt', 'Varenicline', 'Bupropion', 'Counseling', 'Relapse', 'CarbonMonoxide'],
    'pcc_addiction_med': ['Audit', 'Dast', 'Cage', 'Motivation', 'Withdrawal', 'MatOpioid', 'MatAlcohol', 'Overdose', 'HarmReduction', 'RelapsePlan'],
}
...
def make_engine(mod):
  ...
  return {{ plan, t }};
    ...
    return f"""// P3-CX {mod}_engine v{VER}
'use strict';
{chr(10).join(funcs)}
module.exports = {{
  {', '.join(cap)}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CX {mod} integration test v{VER}
const Engine = require('./{mod}_engine.js');
const assert = require('assert');
let passed = 0, failed = 0;
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CX {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cx_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cx_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CX: engines + tests + integ tests + routes + SQL (v{VER})')
