// P3-AY integration: hippotherapy (Hippotherapy)
...
console.log('hippotherapy integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO hippotherapy (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO hippotherapy (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM hippotherapy WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO hippotherapy (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM hippotherapy').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.HippotherapyEval({}); assert.strictEqual(typeof r, 'object'); });
