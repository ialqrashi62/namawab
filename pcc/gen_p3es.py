#!/usr/bin/env python3
"""P3-ES generator v3.109.0 — pcc_neuro_ext10, pcc_pediatric_cardio_ext2, pcc_pediatric_oncology_ext"""
import os
...
MODULES = [
    ('pcc_neuro_ext10', 'PCC Neuro Ext10', 'pcc-neuro-ext10'),
    ('pcc_pediatric_cardio_ext2', 'PCC Pediatric Cardio Ext2', 'pcc-pediatric-cardio-ext2'),
    ('pcc_pediatric_oncology_ext', 'PCC Pediatric Oncology Ext', 'pcc-pediatric-oncology-ext'),
]

ENGINE_FUNCS = {
    'pcc_neuro_ext10': [
        'CerebellarAtaxiaEval', 'SpinocerebellarDegeneration', 'OlivopontocerebellarAtrophy',
        'DentatorubralPallidoluysianAtrophy', 'FriedreichAtaxiaExt',
        'AtaxiaTelangiectasiaExt', 'CerebrotendinousXanthomatosis',
        'NiemannPickDisease', 'GaucherDiseaseType2', 'MetachromaticLeukodystrophyExt',
    ],
    'pcc_pediatric_cardio_ext2': [
        'PediatricASDEval', 'PediatricVSDPostRepair', 'PediatricAVCanal',
        'PediatricTOFRepair', 'PediatricTranspositionGreatArteries',
        'PediatricTruncusArteriosus', 'PediatricTAPVR', 'PediatricHLHS',
        'PediatricCoarctationAorta', 'PediatricEbsteinAnomaly',
    ],
    'pcc_pediatric_oncology_ext': [
        'PediatricALLRelapse', 'PediatricAMLExt', 'PediatricCML',
        'PediatricMDS', 'PediatricJMML', 'PediatricBurkittLymphoma',
        'PediatricHodgkinLymphoma', 'PediatricNHL', 'PediatricBrainstemGlioma',
...
def write_routes(module, label, funcs):
    lines = [f"// {PHASE} {module}_routes v{VER}", "// P3-ES: authenticate via requireAuth middleware (sandbox: helmet/CSP enforced at app level)", "'use strict';", "const express = require('express');", f"const Engine = require('./{module}_engine.js');", "const VER = '%s';" % VER, "const router = express.Router();", ""]
