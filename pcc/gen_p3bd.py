"""
P3-BD generator: comprehensive_rehab, sleep_medicine_ext, transplant_extended
Same in-memory shim pattern as P3-AV/AW/AX/AY/AZ/BA/BB/BC.
"""
import os
...
P3 = "P3-BD"
MODULES = [
    ("comprehensive_rehab", "Comprehensive-Rehab", "Rehab-IRF"),
    ("sleep_medicine_ext", "Sleep-Medicine-Ext", "Sleep"),
    ("transplant_extended", "Transplant-Extended", "Transplant"),
]

CR_FNS = ["ComprehensiveRehab","FIMScore","WeissFIMGain","PTIntensity","OT","SLP","RehabTeam","RehabDischarge","RehabOutcome","RehabPayment"]
SM_FNS = ["Polysomnography","OSATreatment","CPAPAdherence","InsomniaCBTI","CircadianDisorder","PediatricSleep","Narcolepsy","RestlessLegs","SleepAndMed","SleepOutcome"]
TX_FNS = ["TransplantEval","LivingDonor","MMFMismatch","Waitlist","PostTransplant","TransplantImmuno","TransplantInfect","TransplantRejection","TransplantSurgery","TransplantOutcome"]
FNS_MAP = dict(zip([m[0] for m in MODULES], [CR_FNS, SM_FNS, TX_FNS]))
...
def routes(mod, label, fns_list_str):
    return (
        ...
        f"  res.json({{ module: '{mod}', label: '{label}', functions: fns.length, version: '3.16.0' }});\n"
        f"}});\n\n"
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bd_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bd_up.sql"), "w", encoding="utf-8") as f:
