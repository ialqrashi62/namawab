// P3-AW integration: vestibular_rehab (Vestibular-Rehab)
...
console.log('vestibular_rehab integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO vestibular_rehab (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO vestibular_rehab (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM vestibular_rehab WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO vestibular_rehab (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM vestibular_rehab').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.DixHallpike({}); assert.strictEqual(typeof r, 'object'); });
