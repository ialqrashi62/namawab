"""
P3-BI generator: neonatal_ext2, rad_ext, lab_ext
Same in-memory shim pattern as P3-BH.
"""
import os
...
VER = "3.21.0"
MODULES = [
    ("neonatal_ext2", "Neonatal-Ext-2", "NICU-Advanced"),
    ("rad_ext", "Rad-Ext", "Radiology-Advanced"),
    ("lab_ext", "Lab-Ext", "Lab-Advanced"),
]

NN_FNS = ["TherapeuticHypothermia","NEOScore","SepsisScreen","Ventilation","Feeding","BPD","ROP","IVH","NEC","DischargeReadiness"]
RD_FNS = ["CTHead","MRIProtocol","Ultrasound","ContrastReaction","RadiationDose","Biopsy","PediatricDose","IVContrastRenal","ImageQuality","CriticalFinding"]
LB_FNS = ["BloodCulture","ABG","Troponin","BNP","Coags","LFT","Renal","CBC","HbA1c","MicroSensitivity"]
FNS_MAP = {"neonatal_ext2": NN_FNS, "rad_ext": RD_FNS, "lab_ext": LB_FNS}
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bi_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bi_up.sql"), "w", encoding="utf-8") as f:
