// P3-AR integration: cardiology_ext2
...
console.log('cardiology_ext2 integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO cardiology_ext2 (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); db.prepare('INSERT INTO cardiology_ext2 (tenant_id, data) VALUES (?, ?)').run('t2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM cardiology_ext2 WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO cardiology_ext2 (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM cardiology_ext2').get(); assert.strictEqual(r.c, 1); });
