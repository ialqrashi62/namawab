// P3-AX integration: recreational_therapy (Recreational-Therapy)
...
console.log('recreational_therapy integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO recreational_therapy (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO recreational_therapy (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM recreational_therapy WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO recreational_therapy (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM recreational_therapy').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.RTAssessment({}); assert.strictEqual(typeof r, 'object'); });
