/**
 * pcc/sicu/sicu_routes.js
 * 5 Express endpoints for sicu.
 */
'use strict';
...
function authenticate(req, res, next) {
  ...
  const role = req.header('x-pcc-role') || 'SICU';
  if (!userId || !tenantId) return res.status(401).json({ error: 'missing auth' });
  if (role !== 'SICU') return res.status(403).json({ error: 'SICU role required' });
...
router.get('/decision/apache', authenticate, async (req, res) => {
  ...
  res.json({ decision: 'apache', score: 0, module: 'sicu' });
});
