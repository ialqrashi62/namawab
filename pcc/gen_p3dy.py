#!/usr/bin/env python3
"""P3-DY generator v3.89.0 — pcc_bariatric_medicine, pcc_hepato_pancreatic_surgery, pcc_colorectal_surgery"""
import os
...
MODULES = [
    ('pcc_bariatric_medicine', 'PCC Bariatric Medicine', 'pcc-bariatric-medicine'),
    ('pcc_hepato_pancreatic_surgery', 'PCC Hepato Pancreatic Surgery', 'pcc-hepato-pancreatic-surgery'),
    ('pcc_colorectal_surgery', 'PCC Colorectal Surgery', 'pcc-colorectal-surgery'),
]

ENGINE_FUNCS = {
    'pcc_bariatric_medicine': [
        'BMIClassification', 'BariatricSurgeryEligibility', 'RouxEnYIndication',
        'SleeveGastrectomySelection', 'GastricBypassRevision', 'PostBariatricNutrition',
        'BariatricPsychEval', 'WeightRegainManagement', 'BariatricComplications',
        'MetabolicSurgeryOutcomes',
    ],
    'pcc_hepato_pancreatic_surgery': [
        'WhippleIndication', 'LiverResectionHCC', 'PancreaticCancerStaging',
        'CholangiocarcinomaSurgery', 'BiliaryReconstruction', 'LiverTransplantHCC',
        'PancreaticNecrosectomy', 'DistalPancreatectomy', 'HepaticCystFenestration',
        'PortalHypertensionShunt',
    ],
    'pcc_colorectal_surgery': [
        'ColonCancerResection', 'RectalCancerTME', 'LowAnteriorResection',
        'HartmannProcedure', 'DiverticulitisSurgery', 'IBDColectomy',
        'ColostomyReversal', 'AnalFissureSurgery', 'HemorrhoidectomyIndication',
        'RectalProlapseRepair',
    ],
}
...
def write_routes(module, label, funcs):
    lines = [f"// {PHASE} {module}_routes v{VER}", "// P3-DY: authenticate via requireAuth middleware (sandbox: helmet/CSP enforced at app level)", "'use strict';", "const express = require('express');", f"const Engine = require('./{module}_engine.js');", "const VER = '%s';" % VER, "const router = express.Router();", ""]
