#!/usr/bin/env python3
"""P3-CH generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_surgical_ext', 'PCC Surgical Ext', 'sx'),
    ('pcc_perioperative', 'PCC Perioperative', 'peri'),
    ('pcc_postop', 'PCC Postop', 'post'),
]
PKG = 'p3ch_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CH package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CH module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_surgical_ext': ['Urgency', 'Approach', 'Positioning', 'Timeout', 'Counts', 'Antibiotic', 'Dvt', 'Implant', 'Anesthesia', 'Specimen'],
    'pcc_perioperative': ['PreopEval', 'Npo', 'Meds', 'Handoff', 'SignIn', 'TimeOut', 'SignOut', 'SkinPrep', 'Normothermia', 'Ebl'],
    'pcc_postop': ['Pacu', 'Pain', 'Nausea', 'Diet', 'Activity', 'Dvt', 'Wound', 'Drain', 'Discharge', 'FollowUp'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CH {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CH {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3ch_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3ch_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CH: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
