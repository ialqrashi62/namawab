"""
P3-BB generator: falls_prevention, frailty, geriatric_assessment
Same in-memory shim pattern as P3-AV/AW/AX/AY/AZ/BA.
"""
import os
...
P3 = "P3-BB"
MODULES = [
    ("falls_prevention", "Falls-Prevention", "Geriatric-Rehab"),
    ("frailty", "Frailty", "Geriatric"),
    ("geriatric_assessment", "Geriatric-Assessment", "CGA"),
]

FP_FNS = ["FallsRiskAssessment","TimedUpAndGo","BergBalance","MedicationFallRisk","HomeSafety","Footwear","VisionAndFalls","ExerciseForFalls","BoneHealth","FallsOutcome"]
FR_FNS = ["FrailtyIndex","ClinicalFrailtyScale","FriedFrailty","Sarcopenia","NutritionInFrail","FrailtyTrajectory","FrailtyAndSurgery","PolypharmacyInFrail","CognitiveFrailty","FrailtyOutcome"]
GA_FNS = ["ComprehensiveGeriatric","MiniCog","MoCA","ADL_IADL","GeriatricDepression","NutritionMNA","PolypharmacyGeri","ContinenceGeri","GeriatricPain","GoalsOfCare"]
FNS_MAP = dict(zip([m[0] for m in MODULES], [FP_FNS, FR_FNS, GA_FNS]))
...
def routes(mod, label, fns_list_str):
    return (
        ...
        f"  res.json({{ module: '{mod}', label: '{label}', functions: fns.length, version: '3.14.0' }});\n"
        f"}});\n\n"
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bb_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bb_up.sql"), "w", encoding="utf-8") as f:
