#!/usr/bin/env python3
"""P3-EF generator v3.96.0 — pcc_neuropsychology, pcc_pediatric_hematology, pcc_pediatric_nephrology"""
import os
...
MODULES = [
    ('pcc_neuropsychology', 'PCC Neuropsychology', 'pcc-neuropsychology'),
    ('pcc_pediatric_hematology', 'PCC Pediatric Hematology', 'pcc-pediatric-hematology'),
    ('pcc_pediatric_nephrology', 'PCC Pediatric Nephrology', 'pcc-pediatric-nephrology'),
]

ENGINE_FUNCS = {
    'pcc_neuropsychology': [
        'NeuropsychologicalAssessment', 'CognitiveRehabilitationPlan', 'DementiaDifferential',
        'TraumaticBrainInjuryEval', 'ADHDAdultAssessment', 'AutismSpectrumEval',
        'LearningDisorderEval', 'ExecutiveFunctionAssessment', 'MemoryDisorderEval',
        'NeuropsychiatricSyndrome',
    ],
    'pcc_pediatric_hematology': [
        'ChildhoodAnemiaWorkup', 'SickleCellDiseaseManagement', 'ThalassemiaSyndromes',
        'PediatricThrombocytopenia', 'HemophiliaManagement', 'VonWillebrandDisease',
        'PediatricLeukemiaSupport', 'BoneMarrowFailureSyndromes', 'IronDeficiencyAnemia',
        'NewbornHematologicScreening',
    ],
    'pcc_pediatric_nephrology': [
        'NephroticSyndromeChild', 'PediatricUTIWorkup', 'HemolyticUremicSyndrome',
        'ChronicKidneyDiseasePediatric', 'RenalTubularAcidosis', 'PolycysticKidneyDisease',
        'GlomerulonephritisPediatric', 'HypertensionPediatric', 'DialysisPediatric',
        'RenalTransplantPediatric',
    ],
}
...
def write_routes(module, label, funcs):
    lines = [f"// {PHASE} {module}_routes v{VER}", "// P3-EF: authenticate via requireAuth middleware (sandbox: helmet/CSP enforced at app level)", "'use strict';", "const express = require('express');", f"const Engine = require('./{module}_engine.js');", "const VER = '%s';" % VER, "const router = express.Router();", ""]
