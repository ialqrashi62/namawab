#!/usr/bin/env python3
"""P3-BV generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('gi_ext2', 'GI Extended 2', 'gi2'),
    ('ent_ext2', 'ENT Extended 2', 'ent2'),
    ('derma_ext2', 'Dermatology Extended 2', 'derm2'),
]
PKG = 'p3bv_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-BV package schema v{VER}\n',
             'CREATE SCHEMA IF NOT EXISTS p3bv;']
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-BV module schema for {mod} v{VER}\n'
...
def make_integ_test(mod):
    if mod == 'gi_ext2':
        cap = ['Dysphagia', 'GERD', 'PUD', 'IBD', 'IBS', 'Celiac', 'Pancreatitis', 'Cirrhosis', 'Jaundice', 'Bleed']
    elif mod == 'ent_ext2':
        cap = ['Hearing', 'Tinnitus', 'Vertigo', 'Sinus', 'OSA', 'Hoarseness', 'NeckMass', 'Epistaxis', 'Dysphagia', 'Allergic']
    else:
        cap = ['Eczema', 'Psoriasis', 'Acne', 'Melanoma', 'BCC', 'Rash', 'Urticaria', 'Autoimmune', 'Infxn', 'Burns']
    funcs = []
    for fn in cap:
        lname = fn[0].lower() + fn[1:]
        funcs.append(f"it('{lname}', () => {{ const r = Engine.{fn}({{}}); assert(r.plan); }});")
    body = '\n  '.join(funcs)
    return f"""// P3-BV {mod} integration test v{VER}
...
def make_routes(mod, label):
    if mod == 'gi_ext2':
        engine_funcs = ['Dysphagia', 'GERD', 'PUD', 'IBD', 'IBS', 'Celiac', 'Pancreatitis', 'Cirrhosis', 'Jaundice', 'Bleed']
    elif mod == 'ent_ext2':
        engine_funcs = ['Hearing', 'Tinnitus', 'Vertigo', 'Sinus', 'OSA', 'Hoarseness', 'NeckMass', 'Epistaxis', 'Dysphagia', 'Allergic']
    else:
        engine_funcs = ['Eczema', 'Psoriasis', 'Acne', 'Melanoma', 'BCC', 'Rash', 'Urticaria', 'Autoimmune', 'Infxn', 'Burns']
    fns = []
    ...
    body = '\n'.join(fns)
    return f"""// P3-BV {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3bv_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3bv_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-BV: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
