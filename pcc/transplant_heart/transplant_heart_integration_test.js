// P3-AP integration: transplant_heart
...
console.log('transplant_heart integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO transplant_heart (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); db.prepare('INSERT INTO transplant_heart (tenant_id, data) VALUES (?, ?)').run('t2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM transplant_heart WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO transplant_heart (tenant_id, data) VALUES (?, ?)').run('t1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM transplant_heart').get(); assert.strictEqual(r.c, 1); });
