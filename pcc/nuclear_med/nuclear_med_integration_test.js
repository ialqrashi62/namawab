// P3-AM integration: nuclear_med (self-running)
...
console.log('nuclear_med integration tests:');
it('multi-tenant isolation', () => {
  const db = makeDb();
  db.prepare('INSERT INTO nuclear_med (tenant_id, data) VALUES (?, ?)').run('t1', JSON.stringify({x:1}));
  db.prepare('INSERT INTO nuclear_med (tenant_id, data) VALUES (?, ?)').run('t2', JSON.stringify({x:2}));
  const r = db.prepare('SELECT COUNT(*) c FROM nuclear_med WHERE tenant_id=?').get('t1');
  assert.strictEqual(r.c, 1);
});
it('CRUD persistence', () => {
  const db = makeDb();
  db.prepare('INSERT INTO nuclear_med (tenant_id, data) VALUES (?, ?)').run('t1', '{}');
  const r = db.prepare('SELECT COUNT(*) c FROM nuclear_med').get();
  assert.strictEqual(r.c, 1);
});
