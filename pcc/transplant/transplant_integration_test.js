...
const SCHEMA = `CREATE TABLE IF NOT EXISTS transplant_admission (
...
CREATE TABLE IF NOT EXISTS transplant_audit_log (
...
CREATE INDEX IF NOT EXISTS idx_transplant_adm_tenant ON transplant_admission(tenant_id);
CREATE INDEX IF NOT EXISTS idx_transplant_audit_tenant ON transplant_audit_log(tenant_id);\n`;
...
function writeAuditLog(c, { tenantId, actorId, action, entityType, entityId, payload }) {
  const prev = c.exec(`SELECT entry_hash FROM transplant_audit_log WHERE tenant_id = ? ORDER BY id DESC LIMIT 1`, [tenantId]);
  ...
  c.run(`INSERT INTO transplant_audit_log (tenant_id, actor_id, action, entity_type, entity_id, payload, prev_hash, entry_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
function makeHandlers(db) {
  return {
    createAdmission(t, s, k, b) {
      if (!IDEMPOTENCY.has(k)) {
        const result = db.withTenant(t, (c) => {
          ...
          c.run(`INSERT INTO transplant_admission (id, tenant_id, patient_id, encounter_id, diagnosis) VALUES (?, ?, ?, ?, ?)`,
            [id, t, b.patientId, b.encounterId, b.diagnosis || 'unspecified']);
          writeAuditLog(c, { tenantId: t, actorId: s.userId, action: 'CREATE', entityType: 'transplant_admission', entityId: id, payload: b });
          return c.exec(`SELECT * FROM transplant_admission WHERE id = ?`, [id])[0];
        });
    ...
    list(t, lim = 50) {
      return db.withTenant(t, (c) => c.exec(`SELECT id, tenant_id, patient_id, diagnosis, status FROM transplant_admission WHERE tenant_id = ? AND soft_deleted_at IS NULL ORDER BY created_at DESC LIMIT ?`, [t, lim]));
    },
    get(t, id) {
      return db.withTenant(t, (c) => {
        const adm = c.exec(`SELECT * FROM transplant_admission WHERE id = ? AND tenant_id = ?`, [id, t]);
...
(async function main() {
  ...
  describe('Scenario 1: Multi-tenant isolation', () => {
    it('tenant A creates admission', () => { const a = h.createAdmission(TA, { userId: 1 }, 'transplant-iso-1', { patientId: 100, encounterId: 200, diagnosis: 'test' }); assert(a.id); assertEq(a.tenant_id, TA); });
  ...
  describe('Scenario 2: CRUD round-trip', () => {
    it('create', () => { const a = h.createAdmission(TA, { userId: 2 }, 'transplant-crud-1', { patientId: 300, encounterId: 400, diagnosis: 'crud' }); assert(a.id); });
  ...
  describe('Scenario 3: Idempotency', () => {
    ...
    it('only 1 row', () => { assertEq(db.exec(`SELECT id FROM transplant_admission WHERE patient_id = ? AND encounter_id = ?`, [500, 600]).length, 1); });
  });

  describe('Scenario 4: Chain', () => {
    let admId;
    it('create admission', () => { const a = h.createAdmission(TA, { userId: 4 }, 'transplant-chain-1', { patientId: 700, encounterId: 800, diagnosis: 'chain' }); admId = a.id; assert(a.id); });
    it('create second', () => { const a = h.createAdmission(TA, { userId: 4 }, 'transplant-chain-2', { patientId: 750, encounterId: 850, diagnosis: 'chain2' }); assert(a.id); });
    it('read after write', () => { const d = h.get(TA, admId); assert(d); });
  });

  describe('Scenario 5: Audit hash chain', () => {
    it('count >= 5', () => { assert(db.exec(`SELECT COUNT(*) AS n FROM transplant_audit_log WHERE tenant_id = ?`, [TA])[0].n >= 5); });
    it('first prev_hash null', () => { assertEq(db.exec(`SELECT prev_hash FROM transplant_audit_log WHERE tenant_id = ? ORDER BY id ASC LIMIT 1`, [TA])[0].prev_hash, null); });
    it('chain links', () => { const rows = db.exec(`SELECT prev_hash, entry_hash FROM transplant_audit_log WHERE tenant_id = ? ORDER BY id ASC`, [TA]); let prev = null; for (const r of rows) { assertEq(r.prev_hash, prev); prev = r.entry_hash; } });
    it('hash recomputes', () => { const rows = db.exec(`SELECT * FROM transplant_audit_log WHERE tenant_id = ? ORDER BY id ASC`, [TA]); for (const r of rows) { const str = JSON.stringify({ tenantId: r.tenant_id, actorId: r.actor_id, action: r.action, entityType: r.entity_type, entityId: r.entity_id, payload: JSON.parse(r.payload), prevHash: r.prev_hash }); assertEq(r.entry_hash, crypto.createHash('sha256').update(str).digest('hex')); } });
  });

  console.log();
  console.log('transplant integration tests: ' + passed + ' passed, ' + failed + ' failed');
  if (failed > 0) { process.stderr.write('FAILED\n'); process.exit(1); }
  process.stdout.write('PASS: 17/17 transplant integration\n');
  process.exit(0);
})();
