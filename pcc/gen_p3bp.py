"""
P3-BP generator: uro_ext, vasc_ext, ortho_ext
Same in-memory shim pattern as P3-BO.
"""
import os
...
VER = "3.28.0"
MODULES = [
    ("uro_ext", "Uro-Ext", "Urology"),
    ("vasc_ext", "Vasc-Ext", "Vascular-Surgery"),
    ("ortho_ext", "Ortho-Ext", "Orthopedics"),
]

UR_FNS = ["BPH","ProstateCancer","KidneyStone","UTI","Hematuria","ED","Incontinence","Testicular","Penile","BladderCancer"]
VA_FNS = ["AAA","Carotid","PAD","DVT","VaricoseVein","AorticDissect","MesentericIsch","ThoracicAortic","DialysisAccess","Lymphedema"]
OR_FNS = ["Osteoarthritis","RA","Fracture","Spine","Sports","Pediatric","Tumor","Hand","FootAnkle","Prosthetic"]
FNS_MAP = {"uro_ext": UR_FNS, "vasc_ext": VA_FNS, "ortho_ext": OR_FNS}
...
def main():
    ...
    for idx, (mod, label, parent) in enumerate(MODULES):
        ...
        per_path = os.path.join(ROOT, "migrations", f"p3bp_{mod}_up.sql")
        with open(per_path, "w", encoding="utf-8") as f:
            f.write(f"-- {P3} per-module: {mod}\n" + sql)
    with open(os.path.join(ROOT, "migrations", "p3bp_up.sql"), "w", encoding="utf-8") as f:
