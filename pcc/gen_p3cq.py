#!/usr/bin/env python3
"""P3-CQ generator: 3 modules = 1 SQL + 3 per-module SQL + 3 integration tests + 3 routes."""
import os
...
os.chdir(ROOT)
MODULES = [
    ('pcc_diet_nutr', 'PCC Diet Nutr', 'dnu'),
    ('pcc_social_work', 'PCC Social Work', 'sw'),
    ('pcc_case_mgmt', 'PCC Case Mgmt', 'cm'),
]
PKG = 'p3cq_'
COLUMNS = [
    ('encounter_id', 'TEXT'),
...
def make_pkg_sql():
    parts = [f'-- P3-CQ package schema v{VER}\n',
...
def make_module_sql(mod):
    cols = ',\n    '.join([f'{n} {t}' for n, t in COLUMNS])
    return (f'-- P3-CQ module schema for {mod} v{VER}\n'
...
ENGINE_FUNCS = {
    'pcc_diet_nutr': ['Bmi', 'Tpn', 'Diet', 'Tube', 'Supplement', 'Malnutrition', 'Intolerance', 'Aspiration', 'Refeeding', 'Allerg'],
    'pcc_social_work': ['Assessment', 'Placement', 'Psychosocial', 'Saf', 'Financial', 'Transport', 'Family', 'Abuse', 'Substance', 'Resources'],
    'pcc_case_mgmt': ['Intake', 'Coord', 'Dc', 'Transition', 'Followup', 'Barriers', 'Insurance', 'Uta', 'Readmission', 'Multidisc'],
}
...
def make_integ_test(mod):
    ...
    body = '\n  '.join(funcs)
    return f"""// P3-CQ {mod} integration test v{VER}
...
def make_routes(mod, label):
    ...
    body = '\n'.join(fns)
    return f"""// P3-CQ {mod} routes v{VER}
...
def main():
    ...
    pkg_path = f'migrations/p3cq_{VER.replace(".","")}_package_up.sql'
    with open(pkg_path, 'w', encoding='utf-8') as f:
        f.write(make_pkg_sql())
    for mod, label, _ in MODULES:
        os.makedirs(mod, exist_ok=True)
        mod_sql = f'migrations/p3cq_{VER.replace(".","")}_{mod}_up.sql'
    ...
    print(f'Generated P3-CQ: 1 SQL + 3 module SQL + 3 integ tests + 3 routes (v{VER})')
