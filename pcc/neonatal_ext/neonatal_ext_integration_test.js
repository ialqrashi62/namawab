// P3-BE integration: neonatal_ext (Neonatal-Ext)
...
console.log('neonatal_ext integration tests:');
it('multi-tenant isolation', () => { const db = makeDb(); db.prepare('INSERT INTO neonatal_ext (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); db.prepare('INSERT INTO neonatal_ext (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t2', 'p2', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM neonatal_ext WHERE tenant_id=?').get('t1'); assert.strictEqual(r.c, 1); });
it('CRUD persistence', () => { const db = makeDb(); db.prepare('INSERT INTO neonatal_ext (tenant_id, patient_id, data) VALUES (?, ?, ?)').run('t1', 'p1', '{}'); const r = db.prepare('SELECT COUNT(*) c FROM neonatal_ext').get(); assert.strictEqual(r.c, 1); });
...
it('engine pure function', () => { const r = Engine.APHARScore({}); assert.strictEqual(typeof r, 'object'); });
