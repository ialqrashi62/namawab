...
if ($healthy) {
    ...
    try { $r2 = Invoke-WebRequest -UseBasicParsing -Uri http://127.0.0.1:3000/api/health -TimeoutSec 5; L "post-recovery health=$($r2.StatusCode)" } catch { L "post-recovery health=ERR" }
}
