"""Generate SQL up + routes + integration for P3-AC modules"""
import os
...
extras = {
    'hematology': "router.post('/admissions/:id/bone-marrow-biopsy', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
    'oncology_ext': "router.post('/admissions/:id/recist-assessment', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
    'hepatology': "router.post('/admissions/:id/liver-biopsy', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
}

for m in ['hematology', 'oncology_ext', 'hepatology']:
