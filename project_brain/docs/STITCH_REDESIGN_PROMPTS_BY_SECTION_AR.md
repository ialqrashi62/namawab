...
> [!IMPORTANT]
> **توجيهات عامة هامة للأيقونات وثنائية اللغة وتجاوب الهواتف (Global Instructions for Google Stitch):**
> 1. **شعار وأيقونة الموقع (Favicon/App Icon)**: يجب دمج شعار "نما الطبي" المبتكر ثلاثي الأبعاد بتأثير زجاجي (Glassmorphic Shield/Heart) الذي يدمج حرف "ن" العربي مع رمز الرعاية الطبية/الهلال، باستخدام تدرجات الكحلي والتايل والذهبي في أعلى شريط التنقل.
> 2. **الأيقونات الموحدة (Unified Line Icons)**: استخدام أيقونات خطية بسيطة موحدة (Outline style, 1.5px stroke width) لجميع الأزرار والقوائم والمؤشرات السريرية.
> 3. **دعم التبديل اللغوي الفوري (RTL/LTR Toggle)**: يجب أن تدعم جميع تصاميم الشاشات المولدة التبديل التلقائي لاتجاه القراءة من اليمين لليسار (RTL) للعربية، ومن اليسار لليمين (LTR) للإنجليزية، مع تدوير وعكس اتجاه أسهم الملاحة والمؤشرات تلقائياً.
> 4. **تجاوب الجوال المتكامل (Responsive Mobile layouts)**:
>    - تخطيط الجوال يجب أن يعيد ترتيب الأعمدة والجداول رأسياً في عمود واحد مريح للمس.
>    - استخدام شريط تنقل سفلي مرن للجوالات (Bottom Navigation Bar) يحتوي على أيقونات: [الرئيسية، الاستقبال، الطبيب، الصيدلية، الطوارئ] بمساحة لمس لا تقل عن 48px * 48px.
