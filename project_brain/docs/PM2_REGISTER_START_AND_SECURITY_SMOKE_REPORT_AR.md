...
- هذه النسخة المحلية تستخدم `DB_USER=postgres` (إعداد تطوير) → فحوص **المستخدم محدود الصلاحيات (`nama_medical_app`) و RLS=115 جدولاً** هي خصائص **بيئة الإنتاج البعيدة** (تم التحقق منها سابقاً على `alfaisal-erp.com`)، وليست منطبقة على قاعدة dev المحلية.
