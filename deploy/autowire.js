...
function _doMount(target){
  ...
  // Factory routers register absolute paths (e.g. '/api/v4/pgx/pairs').
  // Build a cloned router that strips the leading base path so each route
  // becomes relative, then mount under _base so /api/v4/<base>/... matches.
  ...
  target.stack.forEach(function(layer){
    ...
    methods.forEach(function(m){
      if(m==='_all')return;
      // Express route layer has .handle as a single combined function
      // (when only middleware is used) or .route.stack[] when chain.
      var handler=layer.handle;
      if(layer.route.stack && layer.route.stack.length){
        // Recreate the chain of (mw, ..., final handler) by calling them in order
