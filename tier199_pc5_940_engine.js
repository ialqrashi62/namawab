// filepath: tier199_pc5_940_engine.js
...
function pc5_end_assessment(req) {
...
function pc5_end_screening(req) {
  ensureStr(req.tenant_id, 'tid'); ensureStr(req.patient_id, 'pid');
  ensureNum(req.age, 'ag'); ensureEnum(req.type, 'ty', ['type1','type2','type3','other','NA']);
  ensureNum(req.score_1, 's1'); ensureNum(req.score_2, 's2');
  ensureBool(req.flag_1, 'f1'); ensureBool(req.flag_2, 'f2');
...
function pc5_end_procedure(req) {
  ensureStr(req.tenant_id, 'tid'); ensureStr(req.patient_id, 'pid');
  ensureNum(req.age, 'ag'); ensureEnum(req.type, 'ty', ['type1','type2','type3','other','NA']);
  ensureNum(req.score_1, 's1'); ensureNum(req.score_2, 's2');
  ensureBool(req.flag_1, 'f1'); ensureBool(req.flag_2, 'f2');
...
function funcs() { return { pc5_end_assessment, pc5_end_screening, pc5_end_followup, pc5_end_procedure, pc5_end_outcome }; }
