...
CERT_DOMAINS: jumanasoft.com, www.jumanasoft.com
NGINX_TEST: PASS
NGINX_RELOAD: PASS
...
1. **التشخيص والقراءة فقط (Read-only Diagnosis)**:
   - تم التحقق من DNS لكل من `jumanasoft.com` و `www.jumanasoft.com` وتأكيد توجيههما بالكامل إلى عنوان خادم الاستضافة الفعلي `204.168.144.74`.
...
3. **إصدار/توسيع شهادة SSL المشتركة (Issue/Renew Certificate)**:
   - تم تنفيذ أداة Certbot مع تمرير اسمي النطاق لتحديث الشهادة الحالية لتصبح شهادة مشتركة تغطي النطاقين معاً:
     ```bash
     certbot --nginx --cert-name jumanasoft.com -d jumanasoft.com -d www.jumanasoft.com --non-interactive --agree-tos
     ```
...
4. **ضبط التحويل الكنسي (Canonical Redirect WWW to Non-WWW)**:
   ...
   - القواعد المعتمدة:
     - `https://www.jumanasoft.com/*` -> `301` -> `https://jumanasoft.com/*`
     - `http://www.jumanasoft.com/*` -> `301` -> `https://jumanasoft.com/*`
     - `http://jumanasoft.com/*` -> `301` -> `https://jumanasoft.com/*`
