...
1. **الفئة الأولى: جداول جاهزة للهجرة المباشرة (`MIGRATION_CANDIDATE_DIRECT_TENANT_ID`)**:
   * **[lab_samples](docs/sql/tenant_id_backfill_design_draft.sql)** (عينات المختبر):
...
2. **الفئة الثانية: جداول مهيأة مسبقاً (`ALREADY_CONFIGURED`)**:
   * **[beds](docs/sql/tenant_id_backfill_design_draft.sql)** (الأسرة) و **[bed_transfers](docs/sql/tenant_id_backfill_design_draft.sql)** (حركات الأسرة):
...
3. **الفئة الثالثة: كتالوجات مشتركة (`SHARED_CATALOG_READ_ONLY`)**:
   * **[medications](docs/sql/tenant_id_backfill_design_draft.sql)** (الأدوية)، **[lab_tests_catalog](docs/sql/tenant_id_backfill_design_draft.sql)** (فحوصات المختبر)، و **[radiology_catalog](docs/sql/tenant_id_backfill_design_draft.sql)** (الأشعة):
