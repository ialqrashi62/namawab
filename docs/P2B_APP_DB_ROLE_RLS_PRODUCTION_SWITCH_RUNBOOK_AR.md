...
> **نُفِّذ في P2D (2026-06-20):** أُنشئ الدور + المنح + finance RLS، وبُدِّل `.env` DB_USER إلى `nama_medical_app`؛ إنفاذ RLS مُثبت على طبقة DB. smoke عبر HTTP مؤجَّل (تبعية Redis قائمة). انظر docs/P2D_PRODUCTION_APP_DB_ROLE_SWITCH_REPORT_AR.md. ما يلي هو الدليل المرجعي.
>
> **تخطيط مرجعي.** كل خطوة تكتب على الإنتاج تتطلب موافقة صريحة منفصلة.
