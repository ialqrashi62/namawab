...
> **آخر تحديث:** 2026-06-26 — ✅ **الموجتان مكتملتان: E0–E18 (+E-X) = 20 فرعاً على origin.** التالي: DEPLOY_ALL (المالك).

## الموجة الثانية (التشغيلية/الإدارية) — E10→E18 ✅ كلها مدفوعة

| # | الإبيك | الفرع / commit | ملاحظات المراجعة (أُصلحت) |
|---|--------|----------------|----------------------------|
| E10 | المالية/GL/ZATCA | `feat/e10-finance-zatca` `d6a2da0` | C-1 renderZATCA معطّل (content) → UI ميت؛ I-1 qr_code/qr_tlv نفس $9؛ I-2 finance/summary fallback؛ posting OFF gated محفوظ |
| E11 | التأمين/NPHIES | `feat/e11-insurance-nphies` `4ba4225` | مبالغ التسوية موثوقة من العميل → ضوابط server؛ زر الطعن يلتفّ على سجل الإنكار؛ NPHIES gated |
| E12 | العمليات/OR | `feat/e12-surgery-or` `2c50451` | **C1 خصم مخزون مزدوج عند إعادة حفظ المذكّرة**؛ C2 Aldrete الافتراضي=2→خروج تلقائي كاذب؛ فرع tenant ميت؛ WHO checklist server-enforced |
| E13 | بنك الدم | `feat/e13-blood-bank` `960b641` | تفاعل غير معامِلاتي؛ ربط crossmatch متوافق؛ ABO المتبرّع؛ **توافق ABO/Rh fail-closed (incompatible→422)** |
| E14 | OB/الولادة | `feat/e14-ob-maternity` `c2f3822` | **F1 تحرير اتصال مزدوج (تلف pool)**؛ XSS؛ تدقيق NST؛ APGAR/EDD/GPAL server-side |
| E15 | علم الأمراض | `feat/e15-pathology` `3891a25` | سباق رقم accession → قفل advisory + 23505→409؛ تحقّق visit_id؛ سلسلة specimen→block→slide→report tenant-scoped |
| E16 | المخزون/CSSD | `feat/e16-inventory-cssd` `6e92226` | البناء مات→استؤنف؛ GRN فارغ؛ **بوابة BI fail-closed (لا تعقيم بلا BI pass)**؛ مخزون معامِلاتي بلا سالب؛ آلة حالة الدُفعات |
| E17 | الجودة/العدوى | `feat/e17-quality-infection` `3c4026e` | **BLOCK→مُصلَّح:** مسارات عدوى غير محروسة + زر يلتفّ على legacy + **بوابة سرّية الحوادث** (من المراجعة الأمنية الخلفية) |
| E18 | HR/القوى العاملة | `feat/e18-hr-workforce` `963c08e` | C1 payslip لموظّف خاطئ (id الراتب بدل الموظّف)؛ تسريب حضور عابر؛ صافي راتب وهمي؛ **payroll posting OFF gated**؛ PII محصورة بـHR |

> الموجة الأولى (السريرية E0–E9) في الجدول أدناه. النمط المتكرّر (زر واجهة يلتفّ على مسار غير محروس / تصادم اسم عام / حقل سلطة موثوق من العميل / تحرير اتصال مزدوج) تكرّر وعولج في كل موجة — يؤكّد قيمة المراجعة الخصامية.
...
| النشر على jumanasoft.com | **لا شيء منشور** — تحقّق مباشر: نقاط E5 الجديدة تُرجع 404، `/health`=200 |
...
| **E9** | **العناية المركّزة/ICU (Critical Care)** | `feat/e9-icu` `9d08964` | محرّك درجات (SOFA 6 أعضاء / GCS / APACHE-II) server-side مضادّ للانتحال (icu_scoring.js)، flowsheet/ventilator/infusions مربوطة بإقامة Active مملوكة للمستأجر، ICU board بالحدّة، فحص حساسية cds عند الـinfusion | 55+50+22 | APPROVE_WITH_FIXES (نظيف على كل الأنماط المتكرّرة): **C1 GCS=null خُزِّن 0 فأفسد ترتيب الحدّة** (والقراءة `Number(null)=0` نصف خفيّ ثانٍ) → `?? null` في التخزين والقراءة؛ I1 عدم تطابق lookup الجناح بين بوابة الكتابة واللوحة → سجلّات ICU يتيمة غير مرئية → توحيد `COALESCE(b.ward_id,a.ward_id)`؛ I2 فحوص FK ناقصة في validate |

---

## 4) ✅ مكتمل — كل الإبيكات

كل الإبيكات التسعة (+E-X) مبنية/مُراجَعة/مُصلَّحة/مُتحقَّق منها مستقلاً/مدفوعة على `origin/feat/*`. **لا شيء منشور** (live main `171b7c2`).

## 5) الخطوة التالية الوحيدة — DEPLOY_ALL (المالك)

انظر `DEPLOY_ALL_RUNBOOK_AR.md`: ترتيب الاعتماديات + بوابة DDL (المالك) + دمج تكامل (تحذير: الفروع تتعارض في server.js/app.js — دمج تسلسلي لا fast-forward) + نشر الكود + smoke + تراجع.
