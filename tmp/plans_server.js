...
// deps: { pool, getActor(req)->user, logAudit(...), idempotencyGuard?(req,res,next)->void }
//   - idempotencyGuard is OPTIONAL: when provided, it is applied to the new PUT/DELETE routes
//     (plans/:key/entitlements, tenants/:id/plan, DELETE plans/:key) so a client retry with the
//     same Idempotency-Key replays the stored response instead of writing a second row. The
//     existing POST/PUT routes keep their original semantics (no regression) and only the new
//     routes consume the guard — this keeps plans_test.js and the legacy UI working unchanged.
function makePlansRouter(deps = {}) {
    const express = require('express');
    const router = express.Router();
    const { pool, getActor, logAudit, idempotencyGuard } = deps;
    const actorOf = (req) => (getActor ? getActor(req) : (req.session && req.session.user)) || null;
    // Wrap an array of route handlers in the optional idempotency guard. The guard from
    // ./idempotency is opt-in: no Idempotency-Key => no-op, so unwired callers pay no cost.
    const withIdem = (...handlers) => (typeof idempotencyGuard === 'function' ? [idempotencyGuard, ...handlers] : handlers);
    ...
    // ---- Batch 3.1 additions: entitlements CRUD + tenants list + DELETE soft-disable + PUT alias ----
    // All of the new routes sit BEHIND the same requireSuperAdmin outer guard (mounted at /api/super-admin
    // in server.js) and never bypass the audit helper. The PUT/DELETE routes are wrapped in the
    // optional idempotency guard so a client retry with the same Idempotency-Key replays the stored
    // response — preventing duplicate assignments / entitlement writes from a double-click or
    // network race. AGENTS.md rail 4 is honored: DELETE is a SOFT-disable alias (active=false),
    // never a hard DELETE FROM plans.

    // GET plan entitlements (admin view). Includes the internal max_invoices_per_month which the
    // public surface intentionally omits.
    router.get('/plans/:key/entitlements', async (req, res) => {
        try {
            const cur = (await pool.query('SELECT * FROM plans WHERE plan_key=$1', [req.params.key])).rows[0];
            if (!cur) return res.status(404).json({ error: 'Plan not found' });
            res.json(await loadEnt(cur.id));
        } catch (e) { res.status(500).json({ error: 'Server error' }); }
    });

    // PUT plan entitlements (bulk replace). plan_key is immutable so the body is the entitlements
    // payload only (no plan fields here). Upserts into plan_entitlements. Audited + Idempotency-Key.
    router.put('/plans/:key/entitlements', withIdem(async (req, res) => {
        const ent = validateEntitlements(req.body || {});
        if (!ent.ok) return res.status(400).json({ error: 'Validation failed', details: ent.errors });
        try {
            const cur = (await pool.query('SELECT id FROM plans WHERE plan_key=$1', [req.params.key])).rows[0];
            if (!cur) return res.status(404).json({ error: 'Plan not found' });
            const e = ent.value;
            const row = (await pool.query(
                `INSERT INTO plan_entitlements (plan_id,max_users,max_branches,max_invoices_per_month,modules_enabled,support_level,api_access,custom_domain)
            ...
            audit(req, 'PLAN_ENTITLEMENTS_UPDATE', `plan ${req.params.key} entitlements replaced`);
            res.json(row);
        } catch (e) { res.status(500).json({ error: 'Server error' }); }
    }));

    // GET tenants list (Super Admin dropdown: id + name + current_plan_key). Admin view: returns
    // ALL tenants regardless of activation status, because the Super Admin needs to see disabled
    // tenants to re-enable / re-assign them. The UI is the only consumer; nothing in the public
    // surface exposes this data.
    router.get('/tenants', async (req, res) => {
        try {
            const rows = (await pool.query(
                `SELECT t.id, t.name, t.name_ar,
                        (SELECT plan_key FROM tenant_plan_assignments a
                          WHERE a.tenant_id = t.id AND a.effective_to IS NULL
                          ORDER BY a.assigned_at DESC LIMIT 1) AS current_plan_key
                   FROM tenants t ORDER BY t.id ASC`
            )).rows;
            res.json({ tenants: rows.map(r => ({ id: r.id, name: r.name, name_ar: r.name_ar || '', current_plan_key: r.current_plan_key || null })) });
        } catch (e) { res.status(500).json({ error: 'Server error' }); }
    });

    // PUT assign a plan to a tenant (idempotent alias for POST /tenants/:id/plan). Same body
    // shape, same audit, same 409/404/400 semantics. The HTTP method difference signals the
    // idempotent intent to clients and aligns with the Super Admin UI's PUT-based form.
    router.put('/tenants/:id/plan', withIdem(async (req, res) => {
        const id = posInt(req.params.id);
        if (!id) return res.status(400).json({ error: 'Invalid tenant id' });
        const planKey = typeof req.body.plan_key === 'string' ? req.body.plan_key.trim() : '';
        const source = typeof req.body.assignment_source === 'string' ? req.body.assignment_source.trim() : 'manual';
    ...
    // DELETE a plan — SOFT-DISABLE ONLY. Honors AGENTS.md §2.2 rail 4 (no DELETE FROM plans).
    // The row is preserved with active=false; a follow-up POST /:key/enable can re-enable it.
    router.delete('/plans/:key', withIdem((req, res) => setActive(req, res, false, 'PLAN_DISABLE')));
