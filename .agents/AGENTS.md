...
أي تعديل برمجي، ميزة جديدة، أو إصلاح لخلل يتم إنجازه **واجتياز كافة الاختبارات الآلية واليدوية المحلية بنجاح 100%** (بما يشمل اختبارات الوحدة الـ 86 واختبارات الـ E2E Local Smoke Tests)، **يجب نشره وتفعيله فوراً وتلقائياً على خادم الإنتاج الفعلي المستضيف للموقع `alfaisal-erp.com`** دون أي تأخير أو تجاهل.
...
4. **إعادة تشغيل الخدمة والتحقق (PM2 Restart & Health Check)**:
   ...
   - فحص رابط الصحة العام والمشفر عبر الإنترنت: `curl -s https://alfaisal-erp.com/api/health` والتأكد من استلام الاستجابة السليمة `{"status":"UP"}` برمز الحالة 200.
