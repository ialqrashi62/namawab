"""Generate SQL up + routes + integration for P3-AA modules"""
import os
...
extras = {
    'pmr': "router.post('/admissions/:id/rehab-plan', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
    'allergy': "router.post('/admissions/:id/allergy-test', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
    'pain': "router.post('/admissions/:id/pain-assessment', authenticate, (req, res) => { res.status(201).json({ id: require('crypto').randomUUID(), admissionId: req.params.id }); });",
}

for m in ['pmr', 'allergy', 'pain']:
